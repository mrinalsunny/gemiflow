...
The Hive Mind system represents the pinnacle of multi-agent coordination in GemiFlow, implementing a queen-led hierarchical architecture where a strategic queen coordinator directs specialized worker agents through collective decision-making and shared memory.
...
# Force reinitialize
npx gemiflow hive-mind init --force

# Custom configuration
npx gemiflow hive-mind init --config hive-config.json
```
...
npx gemiflow hive-mind spawn "Build microservices architecture"

# Strategic queen type
npx gemiflow hive-mind spawn "Research AI patterns" --queen-type strategic

# Tactical queen with max workers
npx gemiflow hive-mind spawn "Implement API" --queen-type tactical --max-workers 12
...
npx gemiflow hive-mind metrics

# Monitor collective memory
npx gemiflow hive-mind memory
```
...
npx gemiflow hive-mind sessions

# Pause a session
npx gemiflow hive-mind pause <session-id>

# Resume a paused session
npx gemiflow hive-mind resume <session-id>

# Stop a running session
npx gemiflow hive-mind stop <session-id>
```
...
npx gemiflow sparc tdd "User authentication" --hive-mind
...
npx gemiflow hive-mind spawn "Analyze repo quality" --objective "owner$repo"

# PR review coordination
npx gemiflow hive-mind spawn "Review PR #123" --queen-type tactical
```
...
Hive Mind integrates with GemiFlow hooks for automation:
...
```bash
npx gemiflow hive-mind spawn "Research ML frameworks" --queen-type strategic
```

**Tactical Queens** - For implementation and execution
```bash
npx gemiflow hive-mind spawn "Build authentication" --queen-type tactical
```

**Adaptive Queens** - For optimization and dynamic tasks
```bash
npx gemiflow hive-mind spawn "Optimize performance" --queen-type adaptive
```
...
npx gemiflow hive-mind memory --gc

# Optimize database
npx gemiflow hive-mind memory --optimize

# Export and clear
npx gemiflow hive-mind memory --export --clear
```
...
npx gemiflow hive-mind spawn "..." --consensus weighted

# Or use simple majority
npx gemiflow hive-mind spawn "..." --consensus majority
```
...
```bash
# Frontend hive
npx gemiflow hive-mind spawn "Build UI" --name frontend-hive

# Backend hive
npx gemiflow hive-mind spawn "Build API" --name backend-hive
...
npx gemiflow hive-mind export <session-id> --output backup.json

# Import session
npx gemiflow hive-mind import backup.json
```
...
npx gemiflow hive-mind spawn "Build e-commerce platform" \
...
```bash
# Spawn research hive
npx gemiflow hive-mind spawn "Research GraphQL vs REST" \
  --queen-type adaptive \
  --consensus byzantine
...
```bash
# Review coordination
npx gemiflow hive-mind spawn "Review PR #456" \
  --queen-type tactical \
  --max-workers 6
...
- [Hive Mind Documentation](https:/$github.com$mrinalsunny$gemiflow$docs$hive-mind)
- [Collective Intelligence Patterns](https:/$github.com$mrinalsunny$gemiflow$docs$patterns)
- [Byzantine Consensus](https:/$github.com$mrinalsunny$gemiflow$docs$consensus)
- [Memory Optimization](https:/$github.com$mrinalsunny$gemiflow$docs$memory)

---

**Skill Version**: 1.0.0
**Last Updated**: 2025-10-19
**Maintained By**: GemiFlow Team
**License**: MIT
