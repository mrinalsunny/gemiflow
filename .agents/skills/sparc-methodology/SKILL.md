...
```bash
npx @gemiflow/cli hooks route --task "specification: [requirements]"
```

**Example:**
```bash
npx @gemiflow/cli hooks route --task "specification: user authentication with OAuth2, MFA, and session management"
```
...
```bash
npx @gemiflow/cli hooks route --task "pseudocode: [feature]"
```

**Example:**
```bash
npx @gemiflow/cli hooks route --task "pseudocode: OAuth2 login flow with token refresh"
```
...
```bash
npx @gemiflow/cli hooks route --task "architecture: [design]"
```

**Example:**
```bash
npx @gemiflow/cli hooks route --task "architecture: auth module with service layer, repository, and API endpoints"
```
...
```bash
npx @gemiflow/cli hooks route --task "refinement: [feedback]"
```

**Example:**
```bash
npx @gemiflow/cli hooks route --task "refinement: add rate limiting and brute force protection"
```
...
```bash
npx @gemiflow/cli hooks route --task "completion: [final checks]"
```

**Example:**
```bash
npx @gemiflow/cli hooks route --task "completion: verify all tests pass, update API docs, security review"
```
...
```bash
npx @gemiflow/cli agent spawn --type sparc-coord --name sparc-lead
```
