...
### List Agents
```bash
npx gemiflow agent list --filter active
```

### Agent Status
```bash
npx gemiflow agent status --id agent-123
```

### Agent Metrics
```bash
npx gemiflow agent metrics --id agent-123
```

### Stop Agent
```bash
npx gemiflow agent stop --id agent-123
```

### Pool Management
```bash
npx gemiflow agent pool --size 5 --type coder
```
