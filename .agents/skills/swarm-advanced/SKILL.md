...
mcp__gemiflow__agent_spawn({ type: "researcher", name: "Agent 1" })

// 3. Orchestrate tasks
mcp__gemiflow__task_orchestrate({ task: "...", strategy: "parallel" })
```
...
// Cognitive analysis
mcp__gemiflow__cognitive_analyze({
...
mcp__gemiflow__neural_patterns({
  "action": "analyze",
...
npx gemiflow swarm "research AI trends in 2025" \
  --strategy research \
  --mode distributed \
  --max-agents 6 \
  --parallel \
...
// CI/CD pipeline
mcp__gemiflow__pipeline_create({
  "config": {
...
npx gemiflow swarm "build REST API with authentication" \
...
```bash
# Quick testing swarm
npx gemiflow swarm "test application comprehensively" \
  --strategy testing \
  --mode star \
  --parallel \
  --timeout 600
...
try {
  await mcp__gemiflow__task_orchestrate({
  ...
  // Check swarm health
  const status = await mcp__gemiflow__swarm_status({})

  // Analyze error patterns
  await mcp__gemiflow__error_analysis({
    "logs": [error.message]
  })
...
mcp__gemiflow__memory_namespace({
  "namespace": "research-swarm",
  "action": "create"
})

// Create state snapshot
mcp__gemiflow__state_snapshot({
...
mcp__gemiflow__memory_backup({
  "path": "$workspaces$gemiflow$backups$swarm-memory.json"
})
```
...
mcp__gemiflow__metrics_collect({
  "components": ["agents", "tasks", "memory", "performance"]
})

// Health monitoring
mcp__gemiflow__health_check({
  "components": ["swarm", "agents", "neural", "memory"]
})

// Usage statistics
mcp__gemiflow__usage_stats({
...
mcp__gemiflow__swarm_init({ topology: "mesh", maxAgents: 4 })
