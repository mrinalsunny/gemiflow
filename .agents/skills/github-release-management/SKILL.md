...
npx gemiflow github release-create \
  --version "2.0.0" \
  --build-artifacts \
...
npx gemiflow sparc pipeline "Release v2.0.0 with full validation"
```
...
npx gemiflow github version-suggest \
  --current v1.2.3 \
  --analyze-commits \
  --check-compatibility \
  --suggest-pre-release
...
```bash
# Multi-platform build coordination
npx gemiflow github release-build \
...
npx gemiflow github release-test \
...
npx gemiflow github release-deploy \
...
[Single Message - Multi-Package Release]:
  ...
  Task("Package A Manager", "Coordinate gemiflow package release v1.0.72", "coder")
  ...
  Write("packages$gemiflow$package.json", "[v1.0.72 content]")
  ...
  Bash("cd packages$gemiflow && npm install && npm test")
  Bash("cd packages$ruv-swarm && npm install && npm test")
  Bash("npm run test:integration")

  // Create unified release PR
  Bash(`gh pr create \
    --title "Release: gemiflow v1.0.72, ruv-swarm v1.0.12" \
...
npx gemiflow github multi-release \
...
[Single Message - Cross-Repo Release]:
  ...
  mcp__gemiflow__swarm_init { topology: "star", maxAgents: 6 }
  ...
  mcp__gemiflow__swarm_monitor { interval: 5, duration: 300 }
```
...
npx gemiflow github emergency-release \
  --issue 789 \
  --severity critical \
...
npx gemiflow github release-validate \
  --checks "
    version-conflicts,
...
npx gemiflow github compat-test \
...
npx gemiflow github performance-test \
  --baseline v1.9.0 \
  --candidate v2.0.0 \
...
npx gemiflow github release-monitor \
  --version v2.0.0 \
...
npx gemiflow github release-analytics \
  --version v2.0.0 \
  --compare-with v1.9.0 \
...
npx gemiflow github rollback-config \
  --triggers '{
    "error-rate": ">5%",
    "latency-p99": ">1000ms",
    "availability": "<99.9%",
...
npx gemiflow github release-security \
  --scan-dependencies \
  --check-secrets \
  --audit-permissions \
  --sign-artifacts \
...
npx gemiflow github release-compliance \
    ...
    steps:
      ...
      - name: Initialize Release Swarm
        run: |
          ...
          npx gemiflow@alpha swarm init --topology hierarchical
      ...
      - name: Generate Release Changelog
        run: |
          ...
          CHANGELOG=$(npx gemiflow@alpha github changelog \
      ...
      - name: Build Release Artifacts
        run: |
          ...
          npx gemiflow@alpha github release-build \
            --platforms "linux,macos,windows" \
            --architectures "x64,arm64" \
            --parallel
      ...
      - name: Security Scan
        run: |
          ...
          npx gemiflow@alpha github release-security \
            --scan-dependencies \
            --check-secrets \
            --sign-artifacts
      ...
      - name: Post-Release Validation
        run: |
          ...
          # Validate deployment
          npx gemiflow@alpha github release-validate \
      ...
      - name: Monitor Release
        run: |
          ...
          npx gemiflow@alpha github release-monitor \
            --version ${{ github.ref_name }} \
            --duration 1h \
            --alert-on-errors &
```
  ...
  emergency-hotfix:
    ...
    steps:
      ...
      - name: Emergency Release
        run: |
          npx gemiflow@alpha github emergency-release \
...
npx gemiflow@alpha diagnostic-run \
  --component build \
  --verbose
...
npx gemiflow@alpha github compat-test \
...
npx gemiflow@alpha github rollback \
  --to-version v1.9.9 \
...
npx gemiflow@alpha github release-analytics \
  --version v2.0.0 \
  --identify-issues
```
...
npx gemiflow@alpha github version-sync \
...
- [GemiFlow SPARC Guide](../..$docs$sparc-methodology.md)
...
- Documentation: https:/$gemiflow.dev$docs
