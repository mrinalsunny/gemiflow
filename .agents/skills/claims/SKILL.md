...
### Check Claim
```bash
npx gemiflow claims check --agent agent-123 --claim write
```

### Grant Claim
```bash
npx gemiflow claims grant --agent agent-123 --claim write --scope "/src/**"
```

### Revoke Claim
```bash
npx gemiflow claims revoke --agent agent-123 --claim write
```

### List Claims
```bash
npx gemiflow claims list --agent agent-123
```
