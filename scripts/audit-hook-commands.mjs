 ...
 *   2. If it invokes `npx` or `scripts/gemiflow-hook.sh`, it MUST be non-fatal:
...
const SHIM_RE = /gemiflow-hook\.sh/;
...
for (const file of files) {
  ...
  for (const { event, matcher, idx, command, continueOnError } of collectCommands(parsed)) {
    ...
    if (usesNpx && !PREFER_OFFLINE_RE.test(command)) {
      violations.push({
        file: rel, where,
        reason: 'invokes `npx` without `--prefer-offline` — re-resolves the dist-tag and re-installs from cold cache on every hook fire (#1921). Use scripts/gemiflow-hook.sh, or add `--prefer-offline`.',
        command,
      });
      continue;
    }
