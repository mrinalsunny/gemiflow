...
const CLI_BIN = join(REPO_ROOT, 'v3', '@gemiflow', 'cli', 'bin', 'cli.js');
...
// Run gemiflow init in a temporary directory
const tmpDir = mkdtempSync(join(tmpdir(), 'gemiflow-smoke-win-init-'));
console.log(`Running gemiflow init in: ${tmpDir}`);
...
const settingsPath = join(tmpDir, '.gemiflow', 'settings.json');

if (!existsSync(settingsPath)) {
  // Try alternate location (some init modes write to cwd/.gemiflow/)
  ...
  console.error('FAIL: settings.json not generated by gemiflow init');
  process.exit(1);
}
...
if (IS_WINDOWS) {
  ...
  const handlerPath = join(tmpDir, '.gemiflow', 'helpers', 'hook-handler.cjs');
  assert(existsSync(handlerPath), '.gemiflow/helpers/hook-handler.cjs exists');

  // Check that gemiflow-hook.cjs was deployed (new in #2132)
  const shimPath = join(tmpDir, '.gemiflow', 'helpers', 'gemiflow-hook.cjs');
  assert(existsSync(shimPath), '.gemiflow/helpers/gemiflow-hook.cjs exists (#2132)');
  ...
  // gemiflow-hook.cjs is always deployed now (cross-platform shim always available)
  const shimPath = join(tmpDir, '.gemiflow', 'helpers', 'gemiflow-hook.cjs');
  assert(existsSync(shimPath), '.gemiflow/helpers/gemiflow-hook.cjs exists (#2132)');
}
