#!/usr/bin/env node
/**
 * Smoke: #2105 — GEMIFLOW_DB_PATH env var + --path flag on memory subcommands.
 *
 * Verifies the three-tier path resolution in resolveDbPath():
 *   1. --path flag wins over everything
 *   2. GEMIFLOW_DB_PATH env var is honoured when --path is absent
...
try {
  const mod = await import(
    join(ROOT, 'v3/@gemiflow/cli/dist/src/memory/memory-initializer.js')
  );
  ...
  try {
    const mod = await import(
      join(ROOT, 'v3/@gemiflow/cli/src/memory/memory-initializer.js')
    );
    resolveDbPath = mod.resolveDbPath;
  } catch (e) {
...
try {
  if (resolveDbPath) {
    ...
    const origDbPath = process.env.GEMIFLOW_DB_PATH;
    const origMemPath = process.env.GEMIFLOW_MEMORY_PATH;

    // Test 1a: --path flag wins
    delete process.env.GEMIFLOW_DB_PATH;
    ...
    // Test 1b: GEMIFLOW_DB_PATH env var used when no --path
    process.env.GEMIFLOW_DB_PATH = join(tmpDir, 'env.db');
    delete process.env.GEMIFLOW_MEMORY_PATH;
    const envResult = resolveDbPath(undefined);
    check('GEMIFLOW_DB_PATH env var honoured without --path flag', () => {
      ok(envResult.endsWith('env.db'), `Expected env.db, got: ${envResult}`);
    });

    // Test 1c: --path wins over GEMIFLOW_DB_PATH
    process.env.GEMIFLOW_DB_PATH = join(tmpDir, 'env.db');
    const flagWinsResult = resolveDbPath(join(tmpDir, 'flag2.db'));
    check('--path flag wins over GEMIFLOW_DB_PATH env var', () => {
    ...
    // Test 1e: GEMIFLOW_MEMORY_PATH provides directory, memory.db appended
    process.env.GEMIFLOW_MEMORY_PATH = tmpDir;
    delete process.env.GEMIFLOW_DB_PATH;
    // Reset cache so GEMIFLOW_MEMORY_PATH takes effect
    const { _resetMemoryRootCache } = await import(
      join(ROOT, 'v3/@gemiflow/cli/dist/src/memory/memory-initializer.js')
    ).catch(() => import(join(ROOT, 'v3/@gemiflow/cli/src/memory/memory-initializer.js')));
    if (_resetMemoryRootCache) _resetMemoryRootCache();
    const memPathResult = resolveDbPath(undefined);
    check('GEMIFLOW_MEMORY_PATH provides directory, memory.db appended', () => {
      ok(
    ...
    // Restore env
    if (origDbPath !== undefined) process.env.GEMIFLOW_DB_PATH = origDbPath;
    else delete process.env.GEMIFLOW_DB_PATH;
    if (origMemPath !== undefined) process.env.GEMIFLOW_MEMORY_PATH = origMemPath;
    else delete process.env.GEMIFLOW_MEMORY_PATH;
    if (_resetMemoryRootCache) _resetMemoryRootCache();
  }
  ...
  try {
    const mod = await import(
      join(ROOT, 'v3/@gemiflow/cli/dist/src/commands/memory.js')
    );
    memoryCommand = mod.memoryCommand || mod.default;
  } catch {
    try {
      const mod = await import(
        join(ROOT, 'v3/@gemiflow/cli/src/commands/memory.js')
      );
      memoryCommand = mod.memoryCommand || mod.default;
    } catch (e) {
