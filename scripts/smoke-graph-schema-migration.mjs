...
async function testSchemaCreation() {
  ...
  try {
    ...
    const { initializeMemoryDatabase, MEMORY_SCHEMA_V3 } = await import(
      path.join(projectRoot, 'v3/@gemiflow/cli/dist/src/memory/memory-initializer.js')
    );
...
async function testEdgeInsert() {
  ...
  try {
    const { insertGraphEdge, countGraphEdges, _resetBridgeDb } = await import(
      path.join(projectRoot, 'v3/@gemiflow/cli/dist/src/memory/graph-edge-writer.js')
    );
    ...
    const { decodeEmbedding } = await import(
      path.join(projectRoot, 'v3/@gemiflow/cli/dist/src/memory/embedding-quantization.js')
    );
