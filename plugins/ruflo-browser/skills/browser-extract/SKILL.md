...
allowed-tools: mcp__gemiflow__browser_open mcp__gemiflow__browser_close mcp__gemiflow__browser_get-text mcp__gemiflow__browser_get-value mcp__gemiflow__browser_eval mcp__gemiflow__browser_snapshot mcp__gemiflow__browser_screenshot mcp__gemiflow__browser_scroll mcp__gemiflow__browser_wait mcp__gemiflow__browser_click mcp__gemiflow__aidefence_has_pii mcp__gemiflow__aidefence_is_safe mcp__gemiflow__aidefence_scan Bash Read Write
---
...
3. **Choose a path**:
   - **Template path** (`--template <name>`): retrieve from AgentDB and apply.
     ```bash
     npx -y @gemiflow/cli@latest memory retrieve --namespace browser-templates --key "<name>"
     ```
...
4. **AIDefence pre-storage**: every extracted string passes the PII gate.
   ```bash
   # Pseudocode — mcp__gemiflow__aidefence_has_pii returns true/false per string.
...
6. **Persist the template** if `--save-template <name>` was passed:
   ```bash
   npx -y @gemiflow/cli@latest memory store --namespace browser-templates \
