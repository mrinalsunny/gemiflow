...
allowed-tools: mcp__gemiflow__browser_open mcp__gemiflow__browser_close mcp__gemiflow__browser_session-list mcp__gemiflow__browser_screenshot mcp__gemiflow__browser_snapshot mcp__gemiflow__browser_wait mcp__gemiflow__aidefence_has_pii mcp__gemiflow__aidefence_scan Bash Read Write
---
...
1. **Allocate session id and RVF container**:
   ...
   ```
2. **Open the browser** via `mcp__gemiflow__browser_open` with the URL.
...
6. **Index in AgentDB** under `browser-sessions`:
   ```bash
   npx -y @gemiflow/cli@latest memory store --namespace browser-sessions \
     --key "$SID" --value "{rvf_id:$SID,host:...,task:...,verdict:pass}"
   ```

## Caveats

- Until the `browser_session_record` MCP tool ships (ADR-0001 §7), this skill drives the lifecycle from inside its own bash steps. Do not call `mcp__gemiflow__browser_open` directly without these wrappers.
- The session id format is fixed: `<YYYYMMDD-HHMMSS>-<task-slug>`. Downstream `/gemiflow-browser ls` parses this.
