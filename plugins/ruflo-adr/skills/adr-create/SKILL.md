...
argument-hint: "<title>"
allowed-tools: mcp__gemiflow__agentdb_hierarchical-store mcp__gemiflow__agentdb_hierarchical-query mcp__gemiflow__agentdb_causal-edge mcp__gemiflow__memory_store mcp__gemiflow__memory_search Bash Read Write Edit Grep Glob
---
...
4. **Store in AgentDB** -- Call `mcp__gemiflow__agentdb_hierarchical-store` with:
   - path: `adr/ADR-NNN`
   - value: `{ "id": "ADR-NNN", "title": "<title>", "status": "proposed", "date": "<today>", "file": "docs/adr/ADR-NNN-<slug>.md" }`

5. **Find related ADRs** -- Call `mcp__gemiflow__memory_search` with the title as query in namespace `adr-patterns` to find related decisions. If matches found, add them to the Links section and create causal edges with relation `depends-on`.

6. **Store pattern** -- Call `mcp__gemiflow__memory_store` in namespace `adr-patterns` with key `ADR-NNN` and the title + context as value for future semantic search.
