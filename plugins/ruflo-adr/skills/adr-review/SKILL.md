...
allowed-tools: mcp__gemiflow__agentdb_hierarchical-query mcp__gemiflow__agentdb_causal-query mcp__gemiflow__memory_search Bash Read Grep Glob
---
...
2. **Find relevant ADRs** -- For each changed file:
   ...
   - Call `mcp__gemiflow__memory_search` with the file path and change summary to find semantically related ADRs
...
5. **Query relationship graph** -- Call `mcp__gemiflow__agentdb_causal-query` to check if any referenced ADRs have been superseded. If so, flag that the code references an outdated decision.
