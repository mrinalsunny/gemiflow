...
3. Store in AgentDB: `mcp__gemiflow__agentdb_hierarchical-store` at path `adr/ADR-NNN`
...
3. Create causal edge: `mcp__gemiflow__agentdb_causal-edge` from old to new with relation "supersedes"
...
1. Query `mcp__gemiflow__agentdb_causal-query` for all ADR relationships
...
1. Call `mcp__gemiflow__memory_search` with the query in namespace `adr-patterns`
