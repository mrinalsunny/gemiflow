---
id: ADR-0001
title: gemiflow-goals plugin contract — pinning, namespace coordination + legacy mapping, GOAP/dossier workflow contract, smoke as contract
status: Accepted
date: 2026-05-04
updated: 2026-05-09
authors:
...
`gemiflow-goals` (v0.2.0) — the long-horizon planning + research + dossier plugin. Surface is rich:
...
|-----------|---------|----------------------|
| `adr` | `dossier-investigator`, `dossier-collect` | **Non-compliant** — should reference the canonical `adr-patterns` from [gemiflow-adr ADR-0001](../../gemiflow-adr/docs/adrs/0001-adr-plugin-pattern.md) |
| `dossier` | `dossier-investigator` writes here | **Documented exception** — base-name rule (cf. `federation` from gemiflow-federation) |
...
The six namespaces predate gemiflow-agentdb ADR-0001's namespace convention. Renaming them risks breaking projects that already have stored data in the legacy names. This ADR documents both:
...
- `plugins/gemiflow-agentdb/docs/adrs/0001-agentdb-optimization.md` — namespace convention (`<plugin-stem>-<intent>`)
- `plugins/gemiflow-adr/docs/adrs/0001-adr-plugin-pattern.md` — owns canonical `adr-patterns` namespace
- `plugins/gemiflow-federation/docs/adrs/0001-federation-contract.md` — base-name exception precedent (`federation` namespace)
- `v3/docs/adr/ADR-099-dossier-investigator-recursive-parallel-research.md` — dossier-collect spec

## Implementation status

Plugin version v0.2.0 shipped and listed in marketplace.json. Source exists at `plugins/gemiflow-goals/`. Contract elements implemented: 6 namespaces mapped to correct `memory_*` routing; GOAP A* planner, dossier recursive fan-out (ADR-099), and horizon-track cross-session agents shipped; smoke-as-contract gate defined in `scripts/smoke.sh`.
