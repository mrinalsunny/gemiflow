...
1. Call `mcp__gemiflow__memory_search` with namespace `horizons` and query `*` to list active horizons
2. For each horizon, show: objective, current milestone, progress %, target date, drift status
3. Call `mcp__gemiflow__memory_search` with namespace `research-synthesis` to list completed research reports
