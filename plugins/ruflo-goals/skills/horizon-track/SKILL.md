...
allowed-tools: mcp__gemiflow__memory_store mcp__gemiflow__memory_search mcp__gemiflow__memory_list mcp__gemiflow__memory_retrieve mcp__gemiflow__task_list mcp__gemiflow__task_summary mcp__gemiflow__progress_check mcp__gemiflow__progress_summary mcp__gemiflow__agentdb_hierarchical-store mcp__gemiflow__agentdb_hierarchical-recall mcp__gemiflow__hooks_intelligence_pattern-store mcp__gemiflow__session_save mcp__gemiflow__session_restore Bash Read Write
---
...
2. **Store horizon** — call `mcp__gemiflow__memory_store` with namespace `horizons` and key `horizon-[name]`:
   ```json
   {
     "objective": "...",
     "created": "2026-04-28",
...
3. **Session check-in** — at the start of each session:
   - Recall horizon: `mcp__gemiflow__memory_retrieve` key `horizon-[name]` namespace `horizons`
...
6. **Milestone completion** — when a milestone is done:
   ...
   - Store learned patterns via `mcp__gemiflow__hooks_intelligence_pattern-store`
