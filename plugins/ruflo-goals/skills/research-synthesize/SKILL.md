...
allowed-tools: mcp__gemiflow__memory_search mcp__gemiflow__memory_search_unified mcp__gemiflow__memory_list mcp__gemiflow__memory_retrieve mcp__gemiflow__memory_store mcp__gemiflow__agentdb_context-synthesize mcp__gemiflow__agentdb_pattern-search mcp__gemiflow__neural_predict Bash Read Write
---
...
1. **Gather findings** — search across research namespaces:
   - `mcp__gemiflow__memory_search` namespace `research` for raw findings
   - `mcp__gemiflow__memory_search` namespace `research-sources` for references
   - `mcp__gemiflow__agentdb_pattern-search` for discovered patterns
   - `mcp__gemiflow__agentdb_context-synthesize` for AI-assisted context building
...
4. **Predict relevance** — call `mcp__gemiflow__neural_predict` to score which findings are most relevant to the original goal
5. **Structure report**:
...
6. **Store synthesis** — call `mcp__gemiflow__memory_store` namespace `research-synthesis` with the full report
