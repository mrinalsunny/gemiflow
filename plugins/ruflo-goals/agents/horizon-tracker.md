...
1. **Horizon Initialization**:
   ...
   - Store in `horizons` namespace via `mcp__gemiflow__memory_store`

2. **Session Check-In** (start of every session):
   - Recall current horizon state via `mcp__gemiflow__memory_retrieve`
...
3. **Progress Recording** (during session):
   ...
   - Track learned patterns via `mcp__gemiflow__hooks_intelligence_pattern-store`
