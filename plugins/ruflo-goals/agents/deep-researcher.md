...
2. **Knowledge Retrieval**:
   - Search existing memory (`mcp__gemiflow__memory_search_unified`) for prior findings
   - Query pattern databases (`mcp__gemiflow__agentdb_pattern-search`) for known patterns
   - Check hierarchical memory (`mcp__gemiflow__agentdb_hierarchical-recall`) for related context
...
7. **Persistence**:
   - Store findings in `research` namespace via `mcp__gemiflow__memory_store`
   - Store reusable patterns via `mcp__gemiflow__agentdb_pattern-store`
