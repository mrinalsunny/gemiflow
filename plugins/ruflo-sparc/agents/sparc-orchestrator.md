...
- `mcp__gemiflow__memory_store` with namespace `sparc-state`, key `current-phase-{feature-slug}`
...
- `mcp__gemiflow__memory_search` with namespace `sparc-state` and query for the feature slug
...
- **gemiflow-goals**: Use horizon tracking to place SPARC features within long-term planning horizons. Query `horizons` namespace to align phase timelines with goal milestones.
- **gemiflow-workflows**: SPARC phases can be codified as workflow templates. Use `mcp__gemiflow__workflow_create` to create reusable phase workflows.
- **gemiflow-ddd**: Architecture phase (Phase 3) directly leverages DDD bounded context patterns. Query `ddd-contexts` namespace for existing domain models.

## Neural Learning

After completing a full SPARC cycle:
1. Record the trajectory: `mcp__gemiflow__hooks_intelligence_trajectory-start` through `trajectory-end`
2. Train patterns: `mcp__gemiflow__neural_train` with the successful phase sequence
3. Store the pattern: `mcp__gemiflow__memory_store` with namespace `patterns`, key `sparc-{feature-slug}`

Use learned patterns to predict phase durations and common blockers:
- `mcp__gemiflow__neural_predict` with the feature description to estimate phase effort
- `mcp__gemiflow__memory_search` with namespace `patterns` and query for similar features
...
- `mcp__gemiflow__memory_store` / `memory_search` / `memory_retrieve` — phase state and artifacts
- `mcp__gemiflow__task_create` / `task_update` / `task_complete` — track phase tasks
- `mcp__gemiflow__hooks_intelligence_trajectory-start` / `trajectory-step` / `trajectory-end` — record execution trajectories
- `mcp__gemiflow__neural_predict` / `neural_train` — predict and learn from SPARC cycles
- `mcp__gemiflow__workflow_create` / `workflow_execute` — automate repeatable phase workflows
