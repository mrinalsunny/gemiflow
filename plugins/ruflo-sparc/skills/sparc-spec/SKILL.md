...
allowed-tools: mcp__gemiflow__memory_store mcp__gemiflow__memory_search mcp__gemiflow__memory_retrieve mcp__gemiflow__task_create mcp__gemiflow__task_update mcp__gemiflow__task_complete mcp__gemiflow__hooks_intelligence_trajectory-start mcp__gemiflow__hooks_intelligence_trajectory-step mcp__gemiflow__neural_predict Bash Read Edit
---
...
1. **Initialize phase tracking** — call `mcp__gemiflow__hooks_intelligence_trajectory-start` with metadata `{ "phase": "specification", "feature": "$ARGUMENTS" }`

2. **Check for prior work** — call `mcp__gemiflow__memory_search` with namespace `sparc-state` and query for the feature to see if a SPARC workflow already exists. If it does, retrieve existing artifacts. If not, initialize state with phase 1.

3. **Search for similar patterns** — call `mcp__gemiflow__neural_predict` with the feature description to find relevant past specifications and learned patterns
...
8. **Store specification** — call `mcp__gemiflow__memory_store` with:
...
9. **Update phase state** — call `mcp__gemiflow__memory_store` with:
   - Namespace: `sparc-state`
   - Key: `current-phase-{feature-slug}`
   - Value: updated state with artifacts list including the spec key

10. **Record trajectory step** — call `mcp__gemiflow__hooks_intelligence_trajectory-step` with the specification summary
