...
argument-hint: ""
allowed-tools: mcp__gemiflow__memory_store mcp__gemiflow__memory_search mcp__gemiflow__memory_retrieve mcp__gemiflow__task_create mcp__gemiflow__task_update mcp__gemiflow__task_complete mcp__gemiflow__hooks_intelligence_trajectory-step mcp__gemiflow__hooks_intelligence_trajectory-end mcp__gemiflow__neural_train mcp__gemiflow__neural_predict Bash Read Write Edit
---
...
1. **Retrieve all prior artifacts** — call `mcp__gemiflow__memory_search` with namespace `sparc-phases` and query for the feature slug. Load spec (acceptance criteria), pseudocode, and architecture.

2. **Retrieve phase state** — call `mcp__gemiflow__memory_search` with namespace `sparc-state` to confirm we are in Phase 4.
...
7. **Store refinement artifact** — call `mcp__gemiflow__memory_store` with namespace `sparc-phases`, key `refine-{feature-slug}`, value: `{ status: "complete", reviewFindings: [...], coveragePercent: N, performanceResults: {...}, iterations: N }`

8. **Record trajectory step** — call `mcp__gemiflow__hooks_intelligence_trajectory-step` with refinement summary
...
13. **Store completion artifact** — call `mcp__gemiflow__memory_store` with namespace `sparc-phases`, key `complete-{feature-slug}`, value: `{ status: "complete", traceabilityMatrix: [...], documentationFiles: [...], deploymentChecklist: {...}, regressionResult: "pass" }`

14. **End trajectory** — call `mcp__gemiflow__hooks_intelligence_trajectory-end` with the full SPARC cycle summary

15. **Train neural patterns** — call `mcp__gemiflow__neural_train` with the successful SPARC cycle data to improve future predictions

16. **Store learned pattern** — call `mcp__gemiflow__memory_store` with namespace `patterns`, key `sparc-{feature-slug}`, value summarizing what worked, phase durations, and common blockers encountered
