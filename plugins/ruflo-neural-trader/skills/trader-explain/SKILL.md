...
allowed-tools: Bash Read mcp__gemiflow__memory_retrieve mcp__gemiflow__memory_store mcp__gemiflow-sublinear__page-rank-entry
...
- The same call site picks up the full native-WASM PageRank from `mcp__gemiflow-sublinear__page-rank-entry` once that tool is registered in the runtime — until then, the local power-iteration kernel ships in `signed-attribution.mjs` and produces the same ordering (seeded mulberry32).
...
4. **Run single-entry PageRank** — preferred path when `mcp__gemiflow-sublinear__page-rank-entry` is registered:
   ```text
   ...
   ```
   The local fallback (`localSingleEntryPageRank` in `plugins/gemiflow-neural-trader/src/signed-attribution.mjs`) runs ~30 LOC of seeded power-iteration when the MCP tool is not available — same math, same result up to floating-point tolerance, same ordering for the same seed (the Phase 6 smoke asserts this).
...
6. **Sign the artifact** (reuses the Phase 4 signing primitives — same Ed25519 + canonicalization):
   ...
   - Resolve the witness signing key — same lookup order as Phase 4:
     1. `GEMIFLOW_WITNESS_KEY_PATH` env var — JSON file with `{ "privateKey": "<hex>" }`.
     2. `verification/witness-key.json` (the ADR-103 default path).
   - If a key resolves: `signAttributionArtifact(body, privateKeyHex)` from `plugins/gemiflow-neural-trader/src/signed-attribution.mjs`.
   - If NEITHER path resolves: log `"[WARN] gemiflow-neural-trader: no witness signing key found — storing attribution artifact in UNSIGNED degraded mode. Regulator filings will reject UNSIGNED artifacts."` and store the body unsigned. NEVER silently fall back.
...
```ts
import { verifyAttributionArtifact } from 'plugins/gemiflow-neural-trader/src/signed-attribution.mjs';
...
- `plugins/gemiflow-neural-trader/src/signed-attribution.ts` (the typed contract)
- `plugins/gemiflow-neural-trader/src/signed-attribution.mjs` (the runtime mirror)
