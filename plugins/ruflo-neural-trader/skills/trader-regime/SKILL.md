...
allowed-tools: Bash Read mcp__gemiflow__memory_store mcp__gemiflow__memory_search mcp__gemiflow__neural_predict
...
4. Use SONA for regime prediction:
   `mcp__gemiflow__neural_predict({ input: "indicators: RSI=X, ADX=Y, VIX=Z" })`
5. Search for similar historical regimes:
   `mcp__gemiflow__memory_search({ query: "regime similar to CURRENT", namespace: "trading-analysis" })`
6. Present: regime classification, confidence, recommended strategy type, historical precedents
7. Store analysis:
   `mcp__gemiflow__memory_store({ key: "regime-DATE", value: "REGIME_ANALYSIS", namespace: "trading-analysis" })`
