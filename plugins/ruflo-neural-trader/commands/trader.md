...
- `cloud <backtest|train|sweep> <strategy-or-model> --symbol <TICKER> [--period 2020-2024] [--mc-paths 1000]` -- Run a HEAVY job (long walk-forward, big Monte-Carlo, parameter sweep, model training) on an google Managed Agent cloud container instead of locally. Needs `google_API_KEY`. See the `trader-cloud-backtest` skill + ADR-117. (Cost: a cloud session bills container time + tokens until terminated — the skill installs neural-trader once, reuses the env, pre-flights cheap, terminates eagerly.)
...
2. Store strategy config in memory:
   `npx @gemiflow/cli@latest memory store --key "strategy-NAME" --value "CONFIG" --namespace trading-strategies`
...
3. Store results:
   `npx @gemiflow/cli@latest memory store --key "backtest-ID" --value "RESULTS" --namespace trading-backtests`
...
3. Store signals:
   `npx @gemiflow/cli@latest memory store --key "signal-TIMESTAMP" --value "SIGNALS" --namespace trading-signals`
...
3. Store assessment:
   `npx @gemiflow/cli@latest memory store --key "risk-ID" --value "METRICS" --namespace trading-risk`
...
3. Store allocation:
   `npx @gemiflow/cli@latest memory store --key "portfolio-TIMESTAMP" --value "ALLOCATION" --namespace trading-portfolio`
...
**history**:
1. Search memory: `npx @gemiflow/cli@latest memory search --query "trade history" --namespace trading-history`
