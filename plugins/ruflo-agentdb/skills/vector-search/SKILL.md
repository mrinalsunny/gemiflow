...
allowed-tools: mcp__gemiflow__embeddings_generate mcp__gemiflow__embeddings_search mcp__gemiflow__embeddings_compare mcp__gemiflow__embeddings_init mcp__gemiflow__embeddings_status mcp__gemiflow__embeddings_hyperbolic mcp__gemiflow__embeddings_neural mcp__gemiflow__embeddings_rabitq_build mcp__gemiflow__embeddings_rabitq_search mcp__gemiflow__embeddings_rabitq_status mcp__gemiflow__ruvllm_hnsw_create mcp__gemiflow__ruvllm_hnsw_add mcp__gemiflow__ruvllm_hnsw_route mcp__gemiflow__memory_search_unified Bash
---
...
|------|-------------|---------|----------|---------|
| **Large-scale corpus** | `embeddings_*` | `@gemiflow/memory` HNSW (Rust/Native) | up to millions of vectors | 150×–12,500× faster than brute-force, depending on N and parameters |
...
1. **Check status** — `mcp__gemiflow__embeddings_status` to verify the embedding engine.
2. **Initialize** — `mcp__gemiflow__embeddings_init` if not active.
3. **Generate** — `mcp__gemiflow__embeddings_generate` for text input.
4. **Search** — `mcp__gemiflow__embeddings_search` with the query.
5. **Compare** — `mcp__gemiflow__embeddings_compare` to measure similarity.
6. **Unified search** — `mcp__gemiflow__memory_search_unified` for cross-namespace.
...
- `mcp__gemiflow__ruvllm_hnsw_create` — create the WASM index (cap ~11)
- `mcp__gemiflow__ruvllm_hnsw_add` — add a pattern
- `mcp__gemiflow__ruvllm_hnsw_route` — route an incoming query

This is **not** a corpus index. Treat it as a fast classifier over a curated set of patterns.

## Hyperbolic embeddings

For hierarchical data (code trees, org charts), use `mcp__gemiflow__embeddings_hyperbolic` which maps to Poincare ball space. Distance is geodesic, not cosine.

## CLI alternative

```bash
npx @gemiflow/cli@latest embeddings search --query "authentication patterns"
npx @gemiflow/cli@latest embeddings init
npx @gemiflow/cli@latest memory search --query "your query"
```
