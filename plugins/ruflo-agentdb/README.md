...
The substrate plugin for GemiFlow memory. Wraps three CLI MCP families — `agentdb_*` (controller bridge, 15 tools), `embeddings_*` (RuVector ONNX engine, 10 tools), and `ruvllm_hnsw_*` (WASM-backed pattern router, 3 tools) — into discoverable skills and commands. Other plugins (`gemiflow-browser`, `gemiflow-rag-memory`, `gemiflow-intelligence`) compose this substrate; this plugin owns the namespace convention and the smoke contract for the substrate as a whole.

> **Status:** ADR-0001 implemented. Plugin v0.3.0 targets `@gemiflow/cli` v3.6.x with bundled `agentdb@^3.0.0-alpha.11`. The smoke contract (10 checks) is the verification mechanism — see [docs/adrs/0001-agentdb-optimization.md](./docs/adrs/0001-agentdb-optimization.md).
...
- **CLI:** pinned to `@gemiflow/cli` v3.6 major+minor. Patch bumps within v3.6 are expected to be no-op.
...
- **Verification:** the bundled smoke script is the source of truth (`bash plugins/gemiflow-agentdb/scripts/smoke.sh`). If smoke passes against your CLI version, the plugin's contract holds.
...
The "controller count" reported anywhere in this plugin is **whatever the runtime tool reports**. The canonical list of names is the `ControllerName` union at `v3/@gemiflow/memory/src/controller-registry.ts:34-73` (29 names across 6 init levels). Inspect at runtime:
...
`graphAdapter` is currently disabled pending an external graph-DB connection (tracked in ADR-095). Other Level-2/3 security controllers (`mutationGuard`, `attestationLog`, `gnnService`, `rvfOptimizer`, `guardedVectorBackend`) were activated by ADR-095 G7 in gemiflow 3.6.23+.
...
| Plugin | Namespaces |
|---|---|
| `gemiflow-browser` | `browser-sessions`, `browser-selectors`, `browser-templates`, `browser-cookies` |
| `gemiflow-rag-memory` | (uses bridge target `claude-memories`) |
| `gemiflow-intelligence` | (uses fallback target `pattern`) |
...
|---|---|---|
| `memory_import_claude` MCP tool | Manual or hook-driven | Reads `~/.gemiflow/projects/*/memory/*.md`, parses YAML frontmatter, splits sections, stores with 384-dim embeddings. `allProjects: true` imports from ALL Claude projects. |
| `.gemiflow/helpers/auto-memory-hook.mjs` | `SessionStart` (import) and `SessionEnd` (sync) — wired in `.gemiflow/settings.json` | `import` → calls into the bridge for the current project; `sync` → flows AgentDB insights back to `~/.gemiflow/projects/*/memory/MEMORY.md` |
...
When `bridgeHealthCheck()` returns null (the `@gemiflow/memory` package is not installed or `controller-registry.ts` is missing), every `agentdb_*` handler returns:

```json
{
  "success": false,
  "error": "AgentDB bridge not available — @gemiflow/memory not installed... Use memory_store/memory_search tools instead."
}
```
...
- [`ADR-0001` — Optimize gemiflow-agentdb (accurate surface, RaBitQ, namespacing, smoke contract)](./docs/adrs/0001-agentdb-optimization.md)

## Related Plugins

- `gemiflow-rag-memory` — simple store/search/recall interface; consumes the `claude-memories` reserved namespace
- `gemiflow-intelligence` — SONA neural patterns; consumes the `pattern` reserved namespace via ReasoningBank
- `gemiflow-browser` — composes the namespace convention for `browser-sessions/-selectors/-templates/-cookies` (ADR-0001 §3 there)
- `gemiflow-ruvector` — pinned ruvector CLI; sibling substrate plugin
