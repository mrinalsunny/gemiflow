...
step "6. SONA cross-reference (gemiflow-intelligence canonical owner)"
F="$ROOT/README.md"
grep -q "gemiflow-intelligence" "$F" \
  && grep -qE "SONA|sona_create|sona_adapt" "$F" \
  && ok || bad "SONA cross-reference missing"

step "7. MicroLoRA cross-reference (gemiflow-intelligence DISTILL phase)"
F="$ROOT/README.md"
grep -qE "MicroLoRA|microlora" "$F" \
  && grep -qE "DISTILL|CONSOLIDATE|--consolidate" "$F" \
  && ok || bad "MicroLoRA / DISTILL cross-reference missing"

step "8. HNSW WASM router cross-reference (gemiflow-agentdb canonical owner)"
F="$ROOT/README.md"
grep -qE "ruvllm_hnsw|HNSW WASM router|11 patterns" "$F" \
  && grep -q "gemiflow-agentdb" "$F" \
