...
1. **Format chat** — call `mcp__gemiflow__ruvllm_chat_format` with messages and target provider
2. **Create HNSW index** — call `mcp__gemiflow__ruvllm_hnsw_create` for context retrieval
3. **Add documents** — call `mcp__gemiflow__ruvllm_hnsw_add` to index documents
4. **Route query** — call `mcp__gemiflow__ruvllm_hnsw_route` to find relevant context
5. **Check status** — call `mcp__gemiflow__ruvllm_status` for provider availability
