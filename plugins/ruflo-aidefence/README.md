...
## Defense-in-depth pairing (gemiflow 3.6.25+)
...
- **Encryption at rest** (opt-in via `GEMIFLOW_ENCRYPT_AT_REST=1`) — AES-256-GCM with magic-byte (`RFE1`) backward-compat sniff. Reports involving memory.db / sessions / terminal-history exfiltration should account for the gate state (`gemiflow doctor -c encryption`).
...
- **Verification:** `bash plugins/gemiflow-aidefence/scripts/smoke.sh` is the contract.
...
This is the pattern `gemiflow-browser` ADR-0001 §4 codified and `gemiflow-aidefence` ADR-0001 canonicalizes. Reference these gates by name in any new plugin that handles user-supplied content.
...
This plugin owns the `security-patterns` AgentDB namespace (kebab-case, follows the convention from [gemiflow-agentdb ADR-0001 §"Namespace convention"](../gemiflow-agentdb/docs/adrs/0001-agentdb-optimization.md)). Reserved namespaces (`pattern`, `claude-memories`, `default`) MUST NOT be shadowed.
...
- [`ADR-0001` — gemiflow-aidefence plugin contract (pinning, namespace coordination, 3-gate pattern, smoke as contract)](./docs/adrs/0001-aidefence-contract.md)
