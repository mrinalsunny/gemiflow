...
2. `npx -y -p @gemiflow/plugin-iot-cognitum@latest cognitum-iot register ENDPOINT`
3. If pairing token provided: `npx -y -p @gemiflow/plugin-iot-cognitum@latest cognitum-iot pair DEVICE_ID`
4. Show device status: `npx -y -p @gemiflow/plugin-iot-cognitum@latest cognitum-iot status DEVICE_ID`

Store registration event:
`mcp__gemiflow__memory_store({ key: "iot-register-DEVICEID", value: "Registered at ENDPOINT", namespace: "iot-devices" })`
