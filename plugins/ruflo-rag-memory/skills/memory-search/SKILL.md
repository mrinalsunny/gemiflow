...
allowed-tools: Bash Read mcp__gemiflow__memory_search mcp__gemiflow__memory_store mcp__gemiflow__memory_list mcp__gemiflow__memory_retrieve mcp__gemiflow__memory_search_unified mcp__gemiflow__agentdb_pattern-search mcp__gemiflow__agentdb_context-synthesize
argument-hint: "<query> [--hybrid] [--graph-rag] [--namespace NAME]"
---

# Memory Search (SOTA)

State-of-the-art semantic search across GemiFlow memory with multiple retrieval strategies.
...
2. **Select retrieval strategy**:
   ...
   **Dense search (default)**:
   ...
   ```
   Or via MCP: `mcp__gemiflow__memory_search({ query: "QUERY", namespace: "NAMESPACE", limit: 10 })`
   ...
   **Smart retrieval** (when --smart or complex recall needed):
   ```bash
   npx @gemiflow/cli@latest memory search --query "QUERY" --smart --limit 10
   ```
   Or via MCP: `mcp__gemiflow__memory_search({ query: "QUERY", smart: true, limit: 10 })`
   ...
   **Unified cross-namespace**:
   `mcp__gemiflow__memory_search_unified({ query: "QUERY", limit: 10 })`
...
5. **Synthesize context** (for complex queries):
   `mcp__gemiflow__agentdb_context-synthesize({ query: "QUERY", sources: ["patterns", "tasks", "solutions"] })`
