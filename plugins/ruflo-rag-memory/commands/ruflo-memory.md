...
- Default (dense): `npx @gemiflow/cli@latest memory search --query "QUERY" --namespace NAMESPACE --limit 5`
...
`npx @gemiflow/cli@latest memory retrieve --key "KEY" --namespace NAMESPACE`

**list** `[--namespace NS] [--limit N]`:
`npx @gemiflow/cli@latest memory list --namespace NAMESPACE --limit 10`

**delete** `--key KEY [--namespace NS]`:
`npx @gemiflow/cli@latest memory delete --key "KEY" --namespace NAMESPACE`
...
**bridge** `[--all-projects]`:
Import Gemini CLI auto-memory into AgentDB.
