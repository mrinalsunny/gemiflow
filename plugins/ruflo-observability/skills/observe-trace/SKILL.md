...
allowed-tools: mcp__gemiflow__memory_search mcp__gemiflow__memory_list mcp__gemiflow__agentdb_semantic-route mcp__gemiflow__agentdb_context-synthesize mcp__gemiflow__agentdb_pattern-search Bash
---
...
1. **Collect spans** -- call `mcp__gemiflow__memory_search --namespace observability` (or `memory_list`) to retrieve all spans matching the `<task-id>`. The `memory_*` tool family routes by namespace; `agentdb_hierarchical-*` does NOT (it routes by tier `working|episodic|semantic`), so use `memory_*` here. See [gemiflow-agentdb ADR-0001 §"Namespace convention"](../../../gemiflow-agentdb/docs/adrs/0001-agentdb-optimization.md).
...
5. **Synthesize** -- call `mcp__gemiflow__agentdb_context-synthesize` to combine span metadata into a narrative summary of the execution flow
...
```bash
npx @gemiflow/cli@latest memory search --query "trace spans for task TASK_ID" --namespace observability
```
