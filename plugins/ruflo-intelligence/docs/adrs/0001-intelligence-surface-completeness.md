---
id: ADR-0001
title: Optimize gemiflow-intelligence — surface completeness, 4-step pipeline, IPFS pattern transfer, namespace coordination with gemiflow-agentdb
status: Proposed
date: 2026-05-04
authors:
...
|--------|-----------------|------------|--------|
| `neural_*` | 5 of 6 | **6** (`neural_train`, `neural_predict`, `neural_patterns`, `neural_compress`, `neural_status`, `neural_optimize`) | `v3/@gemiflow/cli/src/mcp-tools/neural-tools.ts:195, 312, 413, 539, 651, 706` |
| `hooks_intelligence_*` (and dispatcher / reset) | 6 of 10 | **10** (`hooks_intelligence`, `hooks_intelligence-reset`, `hooks_intelligence_trajectory-start/step/end`, `hooks_intelligence_pattern-store`, `hooks_intelligence_pattern-search`, `hooks_intelligence_stats`, `hooks_intelligence_learn`, `hooks_intelligence_attention`) | `v3/@gemiflow/cli/src/mcp-tools/hooks-tools.ts:2093, 2226, 2296, 2355, 2404, 2556, 2634, 2741, 2952, 3027` |
...
| `ruvllm_sona_*` + `ruvllm_microlora_*` | 2 of 4 (only sona_create / sona_adapt) | **4** | `v3/@gemiflow/cli/src/mcp-tools/ruvllm-tools.ts:142, 169, 192, 222` |
| **Total** | ~14 of **29** | **29** | — |
...
- **`hooks_intelligence_attention`** — attention-mechanism dispatch (the same primitive `gemiflow-ruvector` surfaces via `attention list`). Plugin makes no mention.
...
`gemiflow-intelligence` is the **user-facing** surface for the self-learning system that the rest of the plugin family relies on. Documentation drift here means:
...
4. Downstream plugins (`gemiflow-browser` ADR-0001 references trajectory recording; `gemiflow-agentdb` references `pattern` namespace writes) lose their anchor doc.

We just fixed analogous drift in `gemiflow-ruvector` (ADR-0001) and `gemiflow-agentdb` (ADR-0001). Same pattern applies here.
...
`plugin.json` moves `0.1.0 → 0.3.0` (matches the gemiflow-agentdb cadence and reflects that this is a **substantial scope expansion**, not a patch). Keywords gain `microlora`, `ewc`, `attention`, `moe`, `pattern-transfer`, `model-routing`, `mcp`. Description rewrites to explicitly enumerate the three families and the IPFS transfer.
...
- **Hook integration** — table mapping `post-task --train-neural`, `pretrain`, `pre-task` to which intelligence tools fire. References gemiflow-agentdb's reserved namespaces (`pattern` for ReasoningBank, `patterns` for neural-train).
- **Namespace coordination** — explicit deferral to `gemiflow-agentdb` ADR-0001 §"Namespace convention"; reproduces the pluralization gotcha (`pattern` vs `patterns`).
...
- **Compatibility** — pinned to `@gemiflow/cli` v3.6 (matches gemiflow-agentdb).
...
10. Pluralization gotcha referenced (`pattern` vs `patterns`) — defers to gemiflow-agentdb.
11. No skill grants wildcard tool access.
12. ADR file exists with status Proposed.
13. Compatibility section pins to `@gemiflow/cli` v3.6.

Plus three doc invariants checked by single-line greps:

- `grep -q "4-step intelligence pipeline" plugins/gemiflow-intelligence/README.md`
- `grep -q "hooks_transfer" plugins/gemiflow-intelligence/skills/intelligence-transfer/SKILL.md`
- `grep -qE "RETRIEVE.+JUDGE.+DISTILL.+CONSOLIDATE" plugins/gemiflow-intelligence/README.md`
...
- Namespace coordination with `gemiflow-agentdb` is explicit; downstream plugins inherit one consistent story.
...
```bash
bash plugins/gemiflow-intelligence/scripts/smoke.sh
# Expected: "16 passed, 0 failed" (13 contract checks + 3 doc invariants)
```

## Related

- `plugins/gemiflow-ruvector/docs/adrs/0001-pin-ruvector-0.2.25.md` — pinning + smoke-as-contract precedent
- `plugins/gemiflow-agentdb/docs/adrs/0001-agentdb-optimization.md` — namespace convention + reserved namespaces (`pattern`/`claude-memories`/`default`); pluralization gotcha; hook integration table
- `plugins/gemiflow-browser/docs/adrs/0001-browser-skills-architecture.md` — uses ruvector trajectory hooks (a primary learning input for this plugin)
- `v3/@gemiflow/cli/src/mcp-tools/neural-tools.ts` — 6 neural tool definitions
- `v3/@gemiflow/cli/src/mcp-tools/hooks-tools.ts` — 19 hooks-family tools (intelligence, route, model, transfer, metrics, explain, pretrain, build-agents)
- `v3/@gemiflow/cli/src/mcp-tools/ruvllm-tools.ts` — 4 SONA + MicroLoRA tools
