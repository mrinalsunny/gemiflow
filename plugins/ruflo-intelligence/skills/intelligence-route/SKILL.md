...
allowed-tools: mcp__gemiflow__hooks_route mcp__gemiflow__hooks_explain mcp__gemiflow__hooks_model-route mcp__gemiflow__hooks_model-stats mcp__gemiflow__hooks_model-outcome mcp__gemiflow__hooks_intelligence_pattern-search mcp__gemiflow__hooks_intelligence_attention mcp__gemiflow__hooks_intelligence_stats mcp__gemiflow__neural_predict mcp__gemiflow__hooks_pre-task Bash
---
...
1. **Get an agent recommendation** — `mcp__gemiflow__hooks_route` with the task description. Returns `{ recommended, confidence, reasoning }`.
2. **Get a model tier recommendation** — `mcp__gemiflow__hooks_model-route` for Haiku/Sonnet/Opus selection.
3. **Search for similar past patterns** — `mcp__gemiflow__hooks_intelligence_pattern-search` to find prior successes.
4. **Predict outcome** — `mcp__gemiflow__neural_predict` with the task description for a confidence-scored prediction.
5. **Spawn the recommended agent** at the recommended model tier.
6. **(If `--why` was passed)** — call `mcp__gemiflow__hooks_explain` to surface the routing rationale to the user.
7. **After task completes** — call `mcp__gemiflow__hooks_model-outcome` with `success: true|false` to train the router.
...
```bash
npx @gemiflow/cli@latest hooks route --task "description"
npx @gemiflow/cli@latest hooks pre-task --description "description"
npx @gemiflow/cli@latest hooks explain --topic "routing decision"
```
