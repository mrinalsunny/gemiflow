...
allowed-tools: Bash(npx * npm *) mcp__gemiflow__memory_store Read
---
Check dependencies for CVEs and outdated packages:

```bash
npx @gemiflow/cli@latest security cve --check
npx @gemiflow/cli@latest security audit --include-dev
npm audit --json
```
...
Auto-fix: `npx @gemiflow/cli@latest security cve --fix`

For continuous monitoring, dispatch via MCP:
`mcp__gemiflow__hooks_worker-dispatch({ trigger: "audit" })`
