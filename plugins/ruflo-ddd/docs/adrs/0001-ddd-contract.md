---
id: ADR-0001
title: gemiflow-ddd plugin contract — pinning, namespace coordination, ADR cross-link, smoke as contract
status: Accepted
date: 2026-05-04
updated: 2026-05-09
authors:
...
`gemiflow-ddd` (v0.1.1) is a code-scaffolding plugin (no MCP tools of its own). Surface:
...
Stores the domain model as a navigable graph in AgentDB with hierarchical nodes + causal edges for context dependencies. Pairs naturally with `gemiflow-adr` (domain decisions) and `gemiflow-sparc` (Architecture phase).
...
2. README augment: Compatibility (pin v3.6), Namespace coordination (claims `ddd-patterns`; defers to gemiflow-agentdb ADR-0001), Verification + Architecture Decisions sections.
