...
allowed-tools: Bash(mkdir * find * ls *) Read Write Edit Grep Glob mcp__gemiflow__memory_store mcp__gemiflow__memory_search mcp__gemiflow__agentdb_hierarchical-store mcp__gemiflow__hooks_pre-task mcp__gemiflow__hooks_post-task
---
...
2. **Pre-task hook**: `npx @gemiflow/cli@latest hooks pre-task --description "DDD aggregate: <aggregate-name> in <context>"`
...
9. **Store in domain model graph**:
   ```
   mcp__gemiflow__agentdb_hierarchical-store --parent "context:<context>" --child "aggregate:<aggregate-name>" --relation "contains"
   mcp__gemiflow__memory_store --key "ddd-aggregate-<context>-<aggregate-name>" --value "AGGREGATE_SUMMARY" --namespace tasks
   ```

10. **Post-task hook**: `npx @gemiflow/cli@latest hooks post-task --task-id "ddd-aggregate-<aggregate-name>" --success true --train-neural true`
