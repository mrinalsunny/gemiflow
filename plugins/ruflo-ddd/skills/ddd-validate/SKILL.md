...
argument-hint: ""
allowed-tools: Bash(find * grep * npx *) Read Grep Glob mcp__gemiflow__memory_store mcp__gemiflow__memory_search mcp__gemiflow__hooks_pre-task mcp__gemiflow__hooks_post-task
---
...
7. **Store results**:
   ```bash
   npx @gemiflow/cli@latest memory store --key "ddd-validation-TIMESTAMP" --value "RESULTS_SUMMARY" --namespace tasks
   npx @gemiflow/cli@latest hooks post-task --task-id "ddd-validate" --success true --store-results true
   ```
