...
allowed-tools: Bash(mkdir * find * ls *) Read Write Edit Grep Glob mcp__gemiflow__memory_store mcp__gemiflow__memory_search mcp__gemiflow__agentdb_hierarchical-store
---
...
4. **Store in domain model graph**:
   ```
   mcp__gemiflow__agentdb_hierarchical-store --parent "domain" --child "context:<context-name>" --relation "contains"
   mcp__gemiflow__memory_store --key "ddd-context-<context-name>" --value "Created bounded context" --namespace tasks
   ```
