...
4. Store entities via `mcp__gemiflow__agentdb_hierarchical-store` in the `knowledge-graph` namespace
5. Create causal edges via `mcp__gemiflow__agentdb_causal-edge` for each relation
...
1. Look up the seed entity via `mcp__gemiflow__agentdb_hierarchical-recall`
...
1. Route the query via `mcp__gemiflow__agentdb_semantic-route` to find relevant entities
