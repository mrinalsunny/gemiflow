...
argument-hint: "<path>"
allowed-tools: Read Glob Grep mcp__gemiflow__agentdb_hierarchical-store mcp__gemiflow__agentdb_causal-edge mcp__gemiflow__agentdb_pattern-store mcp__gemiflow__embeddings_generate Bash
---
...
3. **Map relations** -- for each entity, determine its relations to other entities. **Critical: TypeScript `import type` and inline `type` specifiers (`import { type Foo, bar }`) are erased at compile time and MUST NOT be counted as value imports** -- they're a separate, weaker relation. Misclassifying them produces phantom runtime cycles (see mrinalsunny/gemiflow#2049).
   ...
   **Regex hint for classifying TS imports** (so a naive `from '...'` grep doesn't conflate the two):
   ...
   ```
4. **Store in AgentDB** -- call `mcp__gemiflow__agentdb_hierarchical-store` for each entity with metadata (name, type, file, line, description)
5. **Create edges** -- call `mcp__gemiflow__agentdb_causal-edge` for each relation with source, target, relation type, and weight
6. **Report** -- summarize: total entities by type, total relations by type, files scanned

## CLI alternative

```bash
npx @gemiflow/cli@latest memory store --namespace knowledge-graph --key "entity-NAME" --value "METADATA_JSON"
npx @gemiflow/cli@latest memory search --query "entities in auth module" --namespace knowledge-graph
```
