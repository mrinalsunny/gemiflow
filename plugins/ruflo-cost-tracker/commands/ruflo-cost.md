...
1. Run `node plugins/gemiflow-cost-tracker/scripts/budget.mjs set <amount>` to write the config to the cost-tracking namespace
...
1. Run `node plugins/gemiflow-cost-tracker/scripts/budget.mjs get`
...
1. Run `node plugins/gemiflow-cost-tracker/scripts/budget.mjs check`
...
**`cost track`** -- Auto-capture token usage for the active Gemini CLI session and persist to the `cost-tracking` namespace. Run after significant work or at session end so `cost report` has real data.
1. Invoke `node plugins/gemiflow-cost-tracker/scripts/track.mjs` (no flags = current cwd's most-recent session)
...
2. Runs `node plugins/gemiflow-cost-tracker/scripts/outcome.mjs "<task>" <model> <outcome>`
3. The script wraps `npx @gemiflow/cli hooks model-outcome -t ... -m ... -o ...` with explicit-argv spawnSync so quoting is safe
...
1. Run `node plugins/gemiflow-cost-tracker/scripts/summary.mjs --format json`
...
1. Run `node plugins/gemiflow-cost-tracker/scripts/federation.mjs`
...
1. Run `node plugins/gemiflow-cost-tracker/scripts/conversation.mjs`
...
1. Run `node plugins/gemiflow-cost-tracker/scripts/trend.mjs`
...
**`cost benchmark [--llm] [--google]`** -- Run the corpus benchmark to verify booster claims with measured numbers.
...
3. `--google`: also run Claude Sonnet 4.6 + Opus 4.7 (uses GCP `google_API_KEY` secret)
...
**`cost workers`** -- Inspect the `optimize` and `benchmark` background workers consumed from gemiflow-loop-workers.
1. Call `mcp__gemiflow__hooks_worker-status --worker optimize` -- report last-run timestamp, outcome, and any pending recommendations
2. Call `mcp__gemiflow__hooks_worker-status --worker benchmark` -- report last-run timestamp, outcome, and any pending benchmark deltas
3. Cross-link [gemiflow-loop-workers ADR-0001 §"12-worker trigger map"](../../gemiflow-loop-workers/docs/adrs/0001-loop-workers-contract.md) — the contract this command honors
