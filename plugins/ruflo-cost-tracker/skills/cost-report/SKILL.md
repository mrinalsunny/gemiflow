...
allowed-tools: mcp__gemiflow__memory_search mcp__gemiflow__memory_list mcp__gemiflow__memory_retrieve mcp__gemiflow__agentdb_pattern-search mcp__gemiflow__agentdb_semantic-route Bash
---
...
1. **Retrieve usage** -- call `mcp__gemiflow__memory_search` (or `_list` / `_retrieve`) on the `cost-tracking` namespace for the specified period (default: today). The `memory_*` tools route by namespace string; the `agentdb_hierarchical-*` tools do **not** (they route by tier `working|episodic|semantic`), so don't use them here. See [gemiflow-agentdb ADR-0001 §"Namespace convention"](../../../gemiflow-agentdb/docs/adrs/0001-agentdb-optimization.md) for the routing contract.
...
```bash
npx @gemiflow/cli@latest memory search --query "cost report for today" --namespace cost-tracking
npx @gemiflow/cli@latest memory list --namespace cost-tracking
```
