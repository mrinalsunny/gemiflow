...
- `mcp__gemiflow__memory_store` — store usage records, budget config, optimization patterns (the `memory_*` family is namespace-routed; prefer this over `agentdb_hierarchical-*`)
- `mcp__gemiflow__memory_search` / `memory_list` / `memory_retrieve` — read cost-tracking + cost-patterns namespaces
- `mcp__gemiflow__memory_delete` — clean up stale config (used by budget upsert-via-timestamp pattern)
- `mcp__gemiflow__hooks_route` — invoked by `cost-booster-route`
- `mcp__gemiflow__hooks_model-outcome` — invoked by `cost-optimize` step 8 (auto-emits via `outcome.mjs`)
- `mcp__gemiflow__hooks_worker-status` — `cost workers` subcommand (consumes `optimize` + `benchmark` worker outputs)
- `mcp__gemiflow__agentdb_pattern-store` / `_pattern-search` — ReasoningBank-routed (no namespace arg) for typed cost-optimization patterns
...
```bash
npx @gemiflow/cli@latest memory store --namespace cost-tracking --key "report-DATE" --value "REPORT_JSON"
npx @gemiflow/cli@latest memory store --namespace cost-patterns --key "optimization-OPT_NAME" --value "OPTIMIZATION_RESULT_JSON"
npx @gemiflow/cli@latest memory search --query "cost savings from model downgrades" --namespace cost-patterns
```

## Background workers

This plugin is the declared consumer of two `gemiflow-loop-workers` background workers (see [gemiflow-loop-workers ADR-0001 §"12-worker trigger map"](../../gemiflow-loop-workers/docs/adrs/0001-loop-workers-contract.md)):

- **`optimize`** — periodically scans recent cost data and produces optimization recommendations. Consumed by the `cost-optimize` skill and surfaced via `cost workers` (see `commands/gemiflow-cost.md`).
...
Use `mcp__gemiflow__hooks_worker-status --worker optimize` and `--worker benchmark` to inspect last-run timestamps and outcomes. The worker scheduling itself is owned by `gemiflow-loop-workers`; this plugin only consumes outputs.
...
npx @gemiflow/cli@latest neural train --pattern-type cost-optimization --epochs 5
```

## Related plugins

- **gemiflow-intelligence**: Model routing optimization data feeds cost analysis (3-tier routing reduces cost 75%).
- **gemiflow-autopilot**: Budget-aware autopilot mode uses cost data to throttle agent spawns.
- **gemiflow-observability**: Token usage metrics collected via observability instrumentation.
- **gemiflow-swarm**: Agent spawn/stop decisions informed by budget remaining.
- **gemiflow-federation**: Federation budget circuit breaker (ADR-097) — federation_send `maxTokens` / `maxUsd` enforcement complements local cost tracking.
