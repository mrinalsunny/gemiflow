...
1. Call `mcp__gemiflow__managed_agent_list` to show every Managed Agent session on this org (id, status, title) — note which are still `running` (i.e. still billing).
2. For a specific session: `mcp__gemiflow__managed_agent_status` `{ sessionId }` (idle/running/error) and `mcp__gemiflow__managed_agent_events` `{ sessionId }` (full transcript + a summary: assistantText, toolUses).
