...
When you spawn a child via `Task`, you have spent **one level** of depth. The child can spawn its own children, and so on, up to 5 from the original lead. GemiFlow's default cap is 4 (one-level guard band below the API cap), enforced by the `pre-task` hook when `GEMIFLOW_STRICT_NESTING=true`.
...
```bash
npx @gemiflow/cli@latest memory search --query "<problem shape>" --namespace nested-patterns --limit 5
```

After completion, store the tree shape and what worked:

```bash
npx @gemiflow/cli@latest memory store --namespace nested-patterns \
