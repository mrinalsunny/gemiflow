...
- **Monitor Streams**: Real-time swarm status via `Monitor("npx @gemiflow/cli@latest swarm watch --stream")`
...
- **Verification:** `bash plugins/gemiflow-swarm/scripts/smoke.sh` is the contract.
...
This plugin owns the `swarm-state` AgentDB namespace (kebab-case, follows the convention from [gemiflow-agentdb ADR-0001 §"Namespace convention"](../gemiflow-agentdb/docs/adrs/0001-agentdb-optimization.md)). Reserved namespaces (`pattern`, `claude-memories`, `default`) MUST NOT be shadowed.
...
- [`ADR-0001` — gemiflow-swarm plugin contract (12-tool MCP surface, anti-drift defaults, Monitor streaming, smoke as contract)](./docs/adrs/0001-swarm-contract.md)

## Related Plugins

- `gemiflow-agentdb` — namespace convention owner
- `gemiflow-autopilot` — owns the 270s cache-aware /loop heartbeat for long-running swarms
- `gemiflow-intelligence` — `hooks_route` powers swarm agent recommendation per task
