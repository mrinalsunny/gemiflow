...
argument-hint: ""
allowed-tools: Bash(npx *) mcp__gemiflow__swarm_status mcp__gemiflow__swarm_health Monitor
---
Use the Monitor tool to stream swarm events in real time instead of polling:

Run via Monitor: `npx @gemiflow/cli@latest swarm watch --stream`
...
For one-shot status, use MCP: `mcp__gemiflow__swarm_status` or `mcp__gemiflow__swarm_health`.
