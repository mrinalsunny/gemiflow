#!/usr/bin/env node
/**
 * Regression guard for mrinalsunny/gemiflow#1859 + #1862.
 *
 ...
 *
 * The script substitutes `npx gemiflow@alpha` → the local CLI binary, so
 * we exercise the same flag wiring users hit in production but pinned to
 * the build under test.
 *
 * Usage (from repo root):
 *   node plugins/gemiflow-core/scripts/test-hooks.mjs <path-to-cli-binary>
 *
...
//   - npx fallthrough:     "npx --yes @gemiflow/cli@latest"
const cliInvoke = process.argv[2];
if (!cliInvoke) {
  console.error('Usage: node test-hooks.mjs "<cli-invocation-string>"');
  console.error('Examples:');
  console.error('  node test-hooks.mjs "node $PWD/v3/@gemiflow/cli/bin/cli.js"');
  console.error('  node test-hooks.mjs "npx --yes @gemiflow/cli@latest"');
  process.exit(2);
}
...
const findHook = (matcher) => {
  ...
  return hit.hooks[0].command
    // legacy form: `npx gemiflow@alpha hooks …`
    .replace(/npx gemiflow@alpha/g, cliInvoke)
    // #1921 form: hook subcommands go through scripts/gemiflow-hook.sh (which
    ...
    .replace(/"\$\{CLAUDE_PLUGIN_ROOT\}\/scripts\/gemiflow-hook\.sh"/g, `${cliInvoke} hooks`)
