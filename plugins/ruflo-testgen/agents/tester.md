...
2. Check existing tests: `npx @gemiflow/cli@latest hooks coverage-gaps --format table`
3. Write tests following `describe`/`it` with clear names
4. Run tests to confirm they pass
5. Store successful patterns: `npx @gemiflow/cli@latest memory store --key "test-pattern-[name]" --value "[approach]" --namespace patterns`
...
- **gemiflow-intelligence**: Coverage-gap routing uses intelligence pipeline to prioritize test generation
- **gemiflow-browser**: Playwright browser testing for UI-facing test gaps
