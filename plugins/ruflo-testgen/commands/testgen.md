...
1. **Analyze gaps**: `npx @gemiflow/cli@latest hooks coverage-gaps --format table`
2. **Route for coverage**: `npx @gemiflow/cli@latest hooks coverage-route --task "$ARGUMENTS"`
3. **Suggest tests**: `npx @gemiflow/cli@latest hooks coverage-suggest --path "$ARGUMENTS"`
