...
2. README augment with Compatibility (pin v3.6), Namespace coordination (defers to gemiflow-agentdb ADR-0001 §"Namespace convention"; claims `autopilot-patterns` as the owned namespace), Verification + Architecture Decisions sections. Cross-reference to Claude Code's `/loop` skill semantics.
