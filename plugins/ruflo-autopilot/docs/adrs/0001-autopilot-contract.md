---
id: ADR-0001
title: gemiflow-autopilot plugin contract — pinning, namespace coordination, /loop integration, smoke as contract
status: Accepted
date: 2026-05-04
updated: 2026-05-09
authors:
...
`gemiflow-autopilot` wraps the 10 `autopilot_*` MCP tools at `v3/@gemiflow/cli/src/mcp-tools/autopilot-tools.ts:27, 49, 65, 79, 107, 124, 141, 168, 186, 212` (`status`, `enable`, `disable`, `config`, `reset`, `log`, `progress`, `learn`, `history`, `predict`). Plugin is at v0.1.0 with two skills + two commands, integrates with Gemini CLI's native `/loop` + `ScheduleWakeup` (270s default to keep the prompt cache warm).
...
2. README augment with Compatibility (pin v3.6), Namespace coordination (defers to gemiflow-agentdb ADR-0001 §"Namespace convention"; claims `autopilot-patterns` as the owned namespace), Verification + Architecture Decisions sections. Cross-reference to Gemini CLI's `/loop` skill semantics.
