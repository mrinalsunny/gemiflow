...
4. Store normalized data via `mcp__gemiflow__agentdb_hierarchical-store` in `market-data` namespace
5. Add vectors to HNSW index via `mcp__gemiflow__ruvllm_hnsw_add`
...
4. Store detected patterns via `mcp__gemiflow__agentdb_pattern-store`
...
1. Search HNSW index via `mcp__gemiflow__ruvllm_hnsw_route` for vectors matching the pattern
