---
id: ADR-0001
title: gemiflow-plugin-creator plugin contract — pinning, scaffold-the-canonical-contract, MCP-drift warnings, smoke as contract
status: Accepted
date: 2026-05-04
updated: 2026-05-09
authors:
...
The `create-plugin` skill mentioned "19 AgentDB controllers" — a stale count that [gemiflow-agentdb ADR-0001 §"Today's `gemiflow-agentdb`"](../../../gemiflow-agentdb/docs/adrs/0001-agentdb-optimization.md) declared mythical (real: 15 `agentdb_*` MCP tools, 29 `ControllerName` entries). Fixed in this pass.
...
- `plugins/gemiflow-agentdb/docs/adrs/0001-agentdb-optimization.md` — namespace convention + "19 controllers" myth fix
- `plugins/gemiflow-cost-tracker/docs/adrs/0001-cost-tracker-contract.md` — namespace-routing bug class
- `plugins/gemiflow-knowledge-graph/docs/adrs/0001-knowledge-graph-contract.md` — `embeddings_embed` rename
- `plugins/gemiflow-ruvector/docs/adrs/0001-pin-ruvector-0.2.25.md` — pinning + smoke-as-contract precedent

## Implementation status

Plugin version v0.2.0 shipped and listed in marketplace.json. Source exists at `plugins/gemiflow-plugin-creator/`. Contract elements implemented: scaffolding templates updated to include contract ADR, smoke test, Compatibility section, and namespace coordination block by default; MCP-drift warnings added to scaffolded skill templates; smoke-as-contract gate defined in `scripts/smoke.sh`.
