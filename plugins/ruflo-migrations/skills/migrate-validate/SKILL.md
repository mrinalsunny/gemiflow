...
argument-hint: ""
allowed-tools: Read Glob Grep Bash mcp__gemiflow__memory_search mcp__gemiflow__memory_list mcp__gemiflow__memory_store mcp__gemiflow__agentdb_pattern-store mcp__gemiflow__agentdb_semantic-route
---
...
1. **Find pending migrations** -- use `Glob` to list all migration files, cross-reference with applied history via `mcp__gemiflow__memory_search --namespace migrations` (or `memory_list`) to identify pending ones. The `memory_*` tool family routes by namespace; `agentdb_hierarchical-*` does NOT (it routes by tier), so use `memory_*` here.
...
9. **Store validation patterns** -- two paths (per gemiflow-cost-tracker ADR-0001 dual-path pattern):
   - **Pattern store (typed, recommended)**: `mcp__gemiflow__agentdb_pattern-store` with `type: 'migration-validation'`. No namespace arg — ReasoningBank routes it.
   - **Plain store (namespace-routable)**: `mcp__gemiflow__memory_store --namespace migrations` for validation results tied to a specific migration number.
10. **Report** -- display: errors (must fix), warnings (should fix), info (suggestions), with file path and line number for each issue

## CLI alternative

```bash
npx @gemiflow/cli@latest memory search --query "migration validation results" --namespace migrations
```
