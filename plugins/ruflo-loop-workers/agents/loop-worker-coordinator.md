...
1. **Dispatch workers** via `mcp__gemiflow__hooks_worker-dispatch` with the correct trigger name
2. **Monitor health** via `mcp__gemiflow__hooks_worker-status` and report failures
...
1. Check current worker status: `Bash("npx @gemiflow/cli@latest hooks worker status")`
2. Dispatch needed workers: `Bash("npx @gemiflow/cli@latest hooks worker dispatch --trigger WORKER_NAME")`
3. Schedule next check based on execution mode

## Tools

- `Bash(npx @gemiflow/cli@latest hooks worker *)` — worker management
- `ScheduleWakeup` — loop-mode scheduling
- `CronCreate` / `CronList` / `CronDelete` — persistent scheduling
- `mcp__gemiflow__hooks_worker-dispatch` — direct worker dispatch
- `mcp__gemiflow__hooks_worker-status` — worker health check
