...
allowed-tools: CronCreate CronList CronDelete mcp__gemiflow__hooks_worker-dispatch
---
Use `CronCreate` for workers that must survive session restarts:

`CronCreate({ schedule: "*/15 * * * *", prompt: "Run security audit worker via mcp__gemiflow__hooks_worker-dispatch" })`
