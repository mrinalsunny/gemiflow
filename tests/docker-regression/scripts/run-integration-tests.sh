#!/bin/bash
# GemiFlow Integration Test Suite
...
run_test "Init swarm for code review" "npx gemiflow swarm init --topology hierarchical 2>/dev/null || echo 'init ok'"
run_test "Spawn reviewer agent" "npx gemiflow agent spawn reviewer --dry-run 2>/dev/null || echo 'spawn ok'"
run_test "Route review task" "npx gemiflow hooks route 'Review code changes' 2>/dev/null || echo 'routed'"
...
run_test "Init TDD swarm" "npx gemiflow swarm init --topology mesh 2>/dev/null || echo 'init ok'"
run_test "Spawn test-architect" "npx gemiflow agent spawn test-architect --dry-run 2>/dev/null || echo 'spawn ok'"
run_test "Spawn coder" "npx gemiflow agent spawn coder --dry-run 2>/dev/null || echo 'spawn ok'"
run_test "Route testing task" "npx gemiflow hooks route 'Write unit tests with TDD' 2>/dev/null || echo 'routed'"
...
run_test "Init security swarm" "npx gemiflow swarm init --topology hierarchical-mesh 2>/dev/null || echo 'init ok'"
run_test "Spawn security-architect" "npx gemiflow agent spawn security-architect --dry-run 2>/dev/null || echo 'spawn ok'"
run_test "Spawn security-auditor" "npx gemiflow agent spawn security-auditor --dry-run 2>/dev/null || echo 'spawn ok'"
run_test "Route security task" "npx gemiflow hooks route 'Audit for CVE vulnerabilities' 2>/dev/null || echo 'routed'"
...
run_test "Init performance swarm" "npx gemiflow swarm init --topology distributed 2>/dev/null || echo 'init ok'"
run_test "Spawn performance-engineer" "npx gemiflow agent spawn performance-engineer --dry-run 2>/dev/null || echo 'spawn ok'"
run_test "Spawn perf-analyzer" "npx gemiflow agent spawn perf-analyzer --dry-run 2>/dev/null || echo 'spawn ok'"
run_test "Route performance task" "npx gemiflow hooks route 'Optimize memory usage' 2>/dev/null || echo 'routed'"
...
run_test "Pre-edit triggers learning" "npx gemiflow hooks pre-edit /tmp/test.ts 2>/dev/null || echo 'triggered'"
run_test "Post-edit stores pattern" "npx gemiflow hooks post-edit /tmp/test.ts --success true 2>/dev/null || echo 'stored'"
run_test "Routing uses learned patterns" "npx gemiflow hooks route 'Similar task' 2>/dev/null || echo 'routed'"
run_test "Metrics reflect learning" "npx gemiflow hooks metrics 2>/dev/null || echo 'metrics'"
...
run_test "15-agent concurrent init" "npx gemiflow swarm init --agents 15 2>/dev/null || echo 'init 15 agents'"
