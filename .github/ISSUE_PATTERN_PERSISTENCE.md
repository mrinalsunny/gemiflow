...
npx gemiflow hooks neural-train --pattern-type coordination --epochs 50
# ✅ Returns success with modelId

# 2. Try to retrieve the pattern
npx gemiflow hooks neural-patterns --action analyze --model-id <modelId>
# ❌ Pattern not found (not persisted)

# 3. Try to get statistics
npx gemiflow hooks neural-patterns --action stats --pattern-type coordination
...
npx gemiflow hooks neural-patterns --action analyze
