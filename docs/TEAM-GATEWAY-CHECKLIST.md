...
| Witness manifest | `npx gemiflow@latest verify` | Checksum mismatch |
...
```bash
npx gemiflow@latest sign --message "merge: <PR title>"
```
...
Each merge to main must include a signed witness manifest entry so `npx gemiflow@latest verify` can confirm the installed dist matches the audited fix footprint.

### Generating a manifest entry

```bash
# Sign the current dist with a descriptive message
npx gemiflow@latest sign --message "merge: <PR-number> — <one-line description>"
...
```bash
# After npm install / npx gemiflow@latest
npx gemiflow@latest verify
...
- Review `GEMIFLOW_LOG_LEVEL=debug` output before sharing logs — debug level may include request bodies.
