# BEIR Public-Benchmark Results — gemiflow

This page tracks gemiflow's measured retrieval performance on BEIR datasets.
...
| **gemiflow + BGE-base (direct dense, no rerank)** | **0.352** | **0.626** | **0.489** |
| **gemiflow + BM25+BGE-base RRF k=60 (3.10.27, did NOT improve)** | 0.328 | 0.569 | 0.449 |
...
| **gemiflow Lucene RRF + CE rerank (3.10.28)** | **110M** | **0.521** | **0.358** | **0.683** |
| gemiflow multi-field RRF + CE rerank | 110M | 0.520 | 0.355 | 0.685 |
| gemiflow Lucene BM25 alone | — | 0.505 | 0.328 | 0.681 |
...
| gemiflow dense alone (BGE-base) | 110M | 0.489 | 0.352 | 0.626 |
...
| Dataset | Best gemiflow | Pipeline | Rank | Best Listed |
|---|---:|---|---:|---:|
...
| Dataset | Best gemiflow | Pipeline | Rank |
|---|---:|---|---:|
...
> MMR + opt-in rerank, ADRs 078-083) is what gemiflow uses internally for
...
| **gemiflow + BGE-base-en-v1.5** | **110M** | **0.352** | **this repo** |
...
BEIR_DATA_DIR=/tmp/beir-scifact/scifact node /path/to/gemiflow/v3/@gemiflow/cli/scripts/run-beir-bge.mjs

# Paired bootstrap significance test (ADR-086)
node /path/to/gemiflow/v3/@gemiflow/cli/scripts/beir-bootstrap-significance.mjs \
  /path/to/gemiflow/docs/benchmarks/runs/beir-nfcorpus-bge-latest.json
```
...
- **Single-annotator labelled retrieval** for internal gemiflow bench
