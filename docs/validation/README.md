...
|---|---|---|---|
| `#1867` | `@gemiflow/memory` had `better-sqlite3` as a hard dep + static import | CI ran on Node 20 where prebuilds existed, so the static import evaluated fine | `npm install gemiflow@latest` failed on Node 26 with `node-gyp` errors |
| `#1862` | `gemiflow-core` plugin's `hooks.json` called `--format true` (not a real flag) | No CI test invoked the plugin's `hooks.json` against the CLI with realistic stdin | Every Write/Edit tool use printed `[ERROR] Invalid value for --format: true` |
...
|---|---|---|
| Install smoke | `v3/@gemiflow/memory/scripts/smoke-no-bsqlite.mjs` | `smoke-install-no-bsqlite` (`v3-ci.yml`) |
| Hook smoke | `plugins/gemiflow-core/scripts/test-hooks.mjs` | `plugin-hooks-smoke` (`v3-ci.yml`) |

**Install smoke** — packs `@gemiflow/memory`, installs the tarball into `/tmp/smoke` with `--omit=optional` (simulates "native better-sqlite3 build failed" on Node 26 without prebuilds), asserts the package loads, runtime auto-falls-back to RVF/sql.js, round-trip works. Catches *any* form of "install fails when an optional native dep can't build."

**Hook smoke** — reads each PostToolUse hook from `plugins/gemiflow-core/hooks/hooks.json`, pipes synthetic Claude-Code-style JSON to it, asserts both exit code 0 *and* that the recorded value matches the input. Negative assertions like `expect(stdout).not.toContain('Recording outcome for: true')` are critical — a naive `contains: 'true'` test would have spuriously passed against the broken CLI because the recorded value happened to be `"true"`.
...
SEED=$(echo -n "$GITSHA:gemiflow-witness/v1" | sha256sum | head -c 64)
...
All scripts live in `plugins/gemiflow-core/scripts/witness/`. Project-agnostic — the only runtime dep is `@noble/ed25519`. Adopt by copying the directory into your repo.
...
Plus surface area for Gemini CLI:

| File | Purpose |
|---|---|
| `plugins/gemiflow-core/skills/witness/SKILL.md` | Workflow guide + anti-patterns |
| `plugins/gemiflow-core/commands/witness.md` | Slash command |
| `plugins/gemiflow-core/agents/witness-curator.md` | Agent for adding fixes / interpreting regressions |
...
In gemiflow: `node scripts/regen-witness.mjs` is a thin wrapper that hard-codes gemiflow's paths.
...
These are the specific traps that hit gemiflow's GitHub Actions during the 2026-05-08 work and that adopters will hit too. The fixes are small once you know to look for them; the failure modes are subtle when you don't.
...
**Fix:** `verify.mjs` and `lib.mjs` probe paths now include the workspace packages that *do* declare `@noble/ed25519` directly. In gemiflow:

```js
const probes = [
  repoRoot,
  join(repoRoot, 'v3'),
  join(repoRoot, 'v3/@gemiflow/cli'),                         // declares ed25519
  join(repoRoot, 'v3/@gemiflow/plugin-agent-federation'),     // declares ed25519
];
```
...
There are two ways to invoke the verifier: the bundled CLI subcommand (`gemiflow verify`) and the standalone plugin script (`plugins/gemiflow-core/scripts/witness/verify.mjs`). They produce identical output.
...
- run: node plugins/gemiflow-core/scripts/witness/verify.mjs --manifest verification.md.json --json
```
...
- name: Pack memory tarball (pnpm rewrites workspace:* → versions)
  working-directory: v3/@gemiflow/memory
  run: |
...
- **No CLI required for adopters.** The standalone scripts depend only on `@noble/ed25519` (~15KB minified). Copy `plugins/gemiflow-core/scripts/witness/` into your project, install one package, run.
