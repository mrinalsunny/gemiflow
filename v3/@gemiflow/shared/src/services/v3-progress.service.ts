 ...
 *
 * @module @gemiflow/shared/services/v3-progress
 */
...
export class V3ProgressService extends EventEmitter {
  ...
  constructor(options: V3ProgressOptions = {}) {
    ...
    this.cliPath = join(this.v3Path, '@gemiflow', 'cli', 'src');
    this.metricsPath = options.outputPath || join(this.projectRoot, '.gemiflow', 'metrics', 'v3-progress.json');
  }
  ...
  private async countPackages(): Promise<{
    ...
    const packagesPath = join(this.v3Path, '@gemiflow');
    const list: string[] = [];
    let explicit = 0;
    let utility = 0;
