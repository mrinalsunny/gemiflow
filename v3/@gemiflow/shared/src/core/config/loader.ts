...
function loadEnvConfig(): Partial<SystemConfig> {
  ...
  if (process.env.GEMIFLOW_MAX_AGENTS) {
    config.orchestrator = {
      ...defaultSystemConfig.orchestrator,
      lifecycle: {
        ...defaultSystemConfig.orchestrator.lifecycle,
        maxConcurrentAgents: parseInt(process.env.GEMIFLOW_MAX_AGENTS, 10),
      },
    };
  }

  // Data directory
  if (process.env.GEMIFLOW_DATA_DIR) {
    config.orchestrator = {
      ...config.orchestrator,
      ...defaultSystemConfig.orchestrator,
      session: {
        ...defaultSystemConfig.orchestrator.session,
        dataDir: process.env.GEMIFLOW_DATA_DIR,
      },
    };
  }

  // Memory type
  if (process.env.GEMIFLOW_MEMORY_TYPE) {
    const memoryType = process.env.GEMIFLOW_MEMORY_TYPE as NonNullable<SystemConfig['memory']>['type'];
  ...
  // MCP transport
  const defaultMcp = defaultSystemConfig.mcp ?? { name: 'gemiflow', version: '3.0.0', transport: { type: 'stdio' as const } };
  if (process.env.GEMIFLOW_MCP_TRANSPORT) {
    const transport = process.env.GEMIFLOW_MCP_TRANSPORT as 'stdio' | 'http' | 'websocket';
  ...
  if (process.env.GEMIFLOW_MCP_PORT) {
    config.mcp = {
      ...config.mcp,
      ...defaultMcp,
      transport: {
        ...
        ...defaultMcp.transport,
        port: parseInt(process.env.GEMIFLOW_MCP_PORT, 10),
      },
    };
  }
  ...
  if (process.env.GEMIFLOW_SWARM_TOPOLOGY) {
    const topology = process.env.GEMIFLOW_SWARM_TOPOLOGY as NonNullable<SystemConfig['swarm']>['topology'];
...
export class ConfigLoader {
  ...
  constructor(additionalPaths?: string[]) {
    ...
    this.searchPaths = [
      process.cwd(),
      resolve(process.cwd(), '..'),
      resolve(process.env.HOME ?? '', '.gemiflow'),
    ];
