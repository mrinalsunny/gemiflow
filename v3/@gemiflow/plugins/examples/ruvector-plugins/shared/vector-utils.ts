...
export async function createVectorDB(dimensions: number): Promise<IVectorDB> {
  try {
    ...
    console.warn('[@gemiflow/plugins] @ruvector/wasm not available, using fallback');
...
export async function createLoRAEngine(): Promise<ILoRAEngine> {
  try {
    ...
    console.warn('[@gemiflow/plugins] @ruvector/learning-wasm not available, using fallback');
