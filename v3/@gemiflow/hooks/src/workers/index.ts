 ...
 *
 * Optimizes GemiFlow with non-blocking, scheduled workers.
...
export class WorkerManager extends EventEmitter {
  ...
  constructor(projectRoot?: string) {
    ...
    this.metricsDir = path.join(this.projectRoot, '.gemiflow', 'metrics');
...
export function createSwarmWorker(projectRoot: string): WorkerHandler {
  return async (): Promise<WorkerResult> => {
    ...
    const activityPath = path.join(projectRoot, '.gemiflow', 'metrics', 'swarm-activity.json');
    ...
    const queuePath = path.join(projectRoot, '.gemiflow', 'swarm', 'queue');
    let queueCount = 0;
    try {
...
export function createLearningWorker(projectRoot: string): WorkerHandler {
  return async (): Promise<WorkerResult> => {
    ...
    const patternsDbPath = path.join(projectRoot, '.gemiflow', 'learning', 'patterns.db');
    ...
    try {
      ...
      const metricsPath = path.join(projectRoot, '.gemiflow', 'metrics', 'learning.json');
      try {
...
export function createADRWorker(projectRoot: string): WorkerHandler {
  return async (): Promise<WorkerResult> => {
    ...
    // Save results
    try {
      const outputPath = path.join(projectRoot, '.gemiflow', 'metrics', 'adr-compliance.json');
...
export function createDDDWorker(projectRoot: string): WorkerHandler {
  return async (): Promise<WorkerResult> => {
    ...
    // Save metrics
    try {
      const outputPath = path.join(projectRoot, '.gemiflow', 'metrics', 'ddd-progress.json');
      await fs.writeFile(outputPath, JSON.stringify({
        timestamp: new Date().toISOString(),
...
export function createSecurityWorker(projectRoot: string): WorkerHandler {
  return async (): Promise<WorkerResult> => {
    ...
    // Save results
    try {
      const outputPath = path.join(projectRoot, '.gemiflow', 'security', 'scan-results.json');
...
export function createPatternsWorker(projectRoot: string): WorkerHandler {
  return async (): Promise<WorkerResult> => {
    ...
    const learningDir = path.join(projectRoot, '.gemiflow', 'learning');
    ...
    try {
      ...
      const metricsPath = path.join(projectRoot, '.gemiflow', 'metrics', 'patterns.json');
...
export function createCacheWorker(projectRoot: string): WorkerHandler {
  return async (): Promise<WorkerResult> => {
    ...
    // Only clean directories within .gemiflow (safe)
    const safeCleanDirs = [
      '.gemiflow/cache',
      '.gemiflow/temp',
    ];
...
export function createV3ProgressWorker(projectRoot: string): WorkerHandler {
  return async (): Promise<WorkerResult> => {
    const startTime = Date.now();
    const v3Path = path.join(projectRoot, 'v3');
    const cliPath = path.join(v3Path, '@gemiflow', 'cli', 'src');
    ...
    // Count @gemiflow packages (excluding hidden directories)
    let packages = 0;
    const packageDirs: string[] = [];
    try {
      const packagesPath = path.join(v3Path, '@gemiflow');
    ...
    for (const pkg of packageDirs) {
      ...
      try {
        const srcPath = path.join(v3Path, '@gemiflow', pkg, 'src');
    ...
    try {
      const metricsDir = path.join(projectRoot, '.gemiflow', 'metrics');
