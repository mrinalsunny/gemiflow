 ...
 *
 * Auto-starts workers when Gemini CLI session begins.
 */
...
/**
 * Generate session start output for Gemini CLI hooks
 *
 * Returns formatted output suitable for Gemini CLI SessionStart hook.
 */
...
/**
 * Generate a shell hook script for integration with .gemiflow/settings.json
 */
export function generateShellHook(projectRoot: string): string {
  const hookPath = path.join(projectRoot, 'v3', '@gemiflow', 'hooks');

  return `#!/bin/bash
# GemiFlow V3 Workers - Session Start Hook
