 ...
 *   npx @gemiflow/hooks user-prompt "Fix authentication bug"
 *   npx @gemiflow/hooks pre-edit "/path/to/file.ts"
 *   npx @gemiflow/hooks route "Implement caching layer"
 *
 * @module @gemiflow/hooks/cli/guidance-cli
 */
...
function printHelp(): void {
  console.log(`
@gemiflow/hooks - V3 Guidance System CLI

Usage: npx @gemiflow/hooks <command> [args]
...
Examples:
  ...
  npx @gemiflow/hooks user-prompt "Fix authentication security vulnerability"

  # Pre-edit security check
  npx @gemiflow/hooks pre-edit "src/auth/login.ts"

  # Agent routing
  npx @gemiflow/hooks route "Implement HNSW vector search"

  # Store a learned pattern
  npx @gemiflow/hooks store "Use dependency injection for testability" architecture

  # Broadcast pattern to swarm
  npx @gemiflow/hooks swarm-pattern-broadcast "Use HNSW for 150x faster search" memory

  # Initiate consensus
  npx @gemiflow/hooks swarm-consensus "Which auth method?" "JWT,OAuth2,Session"

  # Hand off task
  npx @gemiflow/hooks swarm-handoff security-auditor "Review auth implementation"
`);
}
