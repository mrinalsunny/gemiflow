 ...
 * - ONNX embeddings via @gemiflow/embeddings (MiniLM-L6 384-dim)
 * - AgentDB backend for persistence
 * - Pattern promotion from short-term to long-term memory
 *
 * @module @gemiflow/hooks/reasoningbank
 */
...
export class ReasoningBank extends EventEmitter {
  ...
  private async loadDependencies(): Promise<void> {
    ...
    const memoryModule = await dynamicImport('@gemiflow/memory');
    if (memoryModule) {
      AgentDBAdapter = memoryModule.AgentDBAdapter;
      HNSWIndex = memoryModule.HNSWIndex;
    }

    const embeddingsModule = await dynamicImport('@gemiflow/embeddings');
    if (embeddingsModule) {
...
/**
 * Real embedding service using @gemiflow/embeddings
 */
