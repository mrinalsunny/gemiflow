 ...
 * ▊ GemiFlow V3 ● mrinalsunny  │  ⎇ v3  │  Opus 4.5
 * ─────────────────────────────────────────────────────
...
export class StatuslineGenerator {
  ...
  /**
   * Generate formatted statusline string matching .gemiflow/statusline.sh format
   */
  ...
  private getV3Progress(): StatuslineData['v3Progress'] {
    ...
    const metricsPath = join(this.projectRoot, '.gemiflow', 'metrics', 'v3-progress.json');
    try {
  ...
  private getSecurityStatus(): StatuslineData['security'] {
    ...
    const auditPath = join(this.projectRoot, '.gemiflow', 'security', 'audit-status.json');
    try {
  ...
  private getSwarmActivity(): StatuslineData['swarm'] {
    ...
    const activityPath = join(this.projectRoot, '.gemiflow', 'metrics', 'swarm-activity.json');
    try {
  ...
  private getSystemMetrics(): ExtendedStatuslineData['system'] {
    ...
    const patternsPath = join(this.projectRoot, '.gemiflow', 'learning', 'patterns.db');
    try {
