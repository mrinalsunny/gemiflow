...
const TEST_PROJECT_ROOT = path.join(os.tmpdir(), 'gemiflow-test-' + Date.now());

async function setupTestDir(): Promise<void> {
  await fs.mkdir(path.join(TEST_PROJECT_ROOT, '.gemiflow', 'metrics'), { recursive: true });
  await fs.mkdir(path.join(TEST_PROJECT_ROOT, 'v3', '@gemiflow', 'hooks', 'src'), { recursive: true });
}
...
describe('Statusline Integration', () => {
  ...
  it('should export statusline to file', async () => {
    ...
    const statuslinePath = path.join(TEST_PROJECT_ROOT, '.gemiflow', 'metrics', 'statusline.json');
...
describe('Persistence', () => {
  ...
  it('should save state to disk', async () => {
    ...
    const statePath = path.join(TEST_PROJECT_ROOT, '.gemiflow', 'metrics', 'workers-state.json');
