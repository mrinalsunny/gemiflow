...
describe('generateClaudeMd', () => {
  ...
  it('includes guidance control plane section when enabled', () => {
    ...
    expect(md).toContain('@gemiflow/guidance');
  ...
  it('includes @imports', () => {
    const md = generateClaudeMd({
      ...minimal,
      imports: ['~/.gemiflow/my_instructions.md'],
    });
    expect(md).toContain('## Individual Preferences');
    expect(md).toContain('@~/.gemiflow/my_instructions.md');
  });
...
// ============================================================================
// Scaffold (Full .gemiflow/ Structure)
// ============================================================================
...
describe('scaffold', () => {
  ...
  it('generates custom agents in correct paths', () => {
    ...
    expect(result.files.has('.gemiflow/agents/custom/my-agent.md')).toBe(true);
    expect(result.files.has('.gemiflow/agents/index.yaml')).toBe(true);
  });
  ...
  it('generates custom skills in correct paths', () => {
    ...
    expect(result.files.has('.gemiflow/skills/my-skill/SKILL.md')).toBe(true);
  });
  ...
  it('includes default agents when requested', () => {
    ...
    expect(result.files.has('.gemiflow/agents/core/coordinator.md')).toBe(true);
    expect(result.files.has('.gemiflow/agents/core/coder.md')).toBe(true);
    expect(result.files.has('.gemiflow/agents/core/tester.md')).toBe(true);
    expect(result.files.has('.gemiflow/agents/core/reviewer.md')).toBe(true);
    expect(result.files.has('.gemiflow/agents/index.yaml')).toBe(true);
  });
  ...
  it('includes security agent when guidance control plane is enabled', () => {
    ...
    expect(result.files.has('.gemiflow/agents/security/security-auditor.md')).toBe(true);
  });
  ...
  it('includes default skills when requested', () => {
    ...
    expect(result.files.has('.gemiflow/skills/build-and-test/SKILL.md')).toBe(true);
    expect(result.files.has('.gemiflow/skills/code-review/SKILL.md')).toBe(true);
  });
  ...
  it('includes guidance skill when control plane is enabled', () => {
    ...
    expect(result.files.has('.gemiflow/skills/guidance-enforcement/SKILL.md')).toBe(true);
  });
  ...
  it('merges custom and default agents', () => {
    ...
    // Custom agent
    expect(result.files.has('.gemiflow/agents/core/custom-bot.md')).toBe(true);
    // Default agents
    expect(result.files.has('.gemiflow/agents/core/coordinator.md')).toBe(true);
    // Index includes all
    const index = result.files.get('.gemiflow/agents/index.yaml')!;
  ...
  it('generates a full project scaffold with all features', () => {
    ...
    const geminiMd = result.files.get('CLAUDE.md')!;
    expect(geminiMd).toContain('# enterprise-app');
    expect(geminiMd).toContain('SaaS application');
    expect(geminiMd).toContain('PEP 8');
    expect(geminiMd).toContain('No `any` types');
    expect(geminiMd).toContain('## React Conventions');
    expect(geminiMd).toContain('## Prisma Conventions');
    expect(geminiMd).toContain('## Domain Rules');
    expect(geminiMd).toContain('authentication');
    expect(geminiMd).toContain('NEVER: console.log');
    expect(geminiMd).toContain('ALWAYS: Error boundary');
    expect(geminiMd).toContain('## Guidance Control Plane');
    expect(geminiMd).toContain('WASM kernel');
    expect(geminiMd).toContain('## Swarm Configuration');
    expect(geminiMd).toContain('@~/.gemiflow/my_prefs.md');
