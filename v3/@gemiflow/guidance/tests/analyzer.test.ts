...
npx gemiflow agent spawn -t coder
npx gemiflow swarm init --v3-mode
npx gemiflow memory search -q "auth patterns"
npx gemiflow doctor --fix
npx gemiflow security scan --depth full
\`\`\`
...
class ABDifferentialExecutor implements IContentAwareExecutor {
  ...
  async execute(prompt: string): Promise<{ stdout: string; stderr: string; exitCode: number }> {
    ...
    if (lower.includes('publish') && lower.includes('npm')) {
      if (this.hasDeployInfo) {
        return { stdout: 'Set up npm publish workflow with dist-tag management. Publishes to alpha and latest tags. Both @gemiflow/cli and gemiflow umbrella package are updated. Tags verified after publish.', stderr: '', exitCode: 0 };
      }
