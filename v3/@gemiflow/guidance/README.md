...
| Capability | Plain CLAUDE.md | Prompt libraries | @gemiflow/guidance |
|-----------|:-:|:-:|:-:|
...
The most important gain: **GemiFlow can now say "no" to itself and survive.** Self-limiting behavior, self-correction, and self-preservation compound over time.
...
```typescript
import { getKernel } from '@gemiflow/guidance/wasm-kernel';
...
import { createCompiler } from '@gemiflow/guidance/compiler';
const compiler = createCompiler();
const bundle = compiler.compile(geminiMdContent);

// Retrieve task-relevant shards by intent
import { createRetriever } from '@gemiflow/guidance/retriever';
...
import { createGates } from '@gemiflow/guidance/gates';
...
```typescript
import { createContinueGate } from '@gemiflow/guidance/continue-gate';
const gate = createContinueGate({
...
```typescript
import { createProofChain } from '@gemiflow/guidance/proof';
...
import { createToolGateway } from '@gemiflow/guidance/gateway';
...
// Memory write gating
import { createMemoryWriteGate } from '@gemiflow/guidance/memory-gate';
...
import { TrustSystem } from '@gemiflow/guidance/trust';
...
import { createTruthAnchorStore, createTruthResolver } from '@gemiflow/guidance/truth-anchors';
...
import { UncertaintyLedger } from '@gemiflow/guidance/uncertainty';
...
import { TemporalStore, TemporalReasoner } from '@gemiflow/guidance/temporal';
...
```typescript
import { AuthorityGate, IrreversibilityClassifier } from '@gemiflow/guidance/authority';
...
import { createThreatDetector, createCollusionDetector, createMemoryQuorum }
  from '@gemiflow/guidance/adversarial';
...
```typescript
import { createMetaGovernor } from '@gemiflow/guidance/meta-governance';
...
```typescript
import { createConformanceRunner, createMemoryClerkCell } from '@gemiflow/guidance/conformance-kit';
...
```typescript
import { createEvolutionPipeline } from '@gemiflow/guidance/evolution';
...
import {
  ...
  generateAgentIndex,
  scaffold,
} from '@gemiflow/guidance/generators';

// Generate a CLAUDE.md from a project profile
const geminiMd = generateClaudeMd({
  name: 'my-api',
...
} from '@gemiflow/guidance/analyzer';

// 1. Score a CLAUDE.md file
const result = analyze(geminiMdContent);
...
```typescript
import type { IContentAwareExecutor } from '@gemiflow/guidance/analyzer';

class MyExecutor implements IContentAwareExecutor {
  private rules: string[] = [];

  setContext(geminiMdContent: string): void {
    // Parse loaded CLAUDE.md to determine how to behave
    this.rules = geminiMdContent.match(/\b(NEVER|ALWAYS|MUST)\b.+/g) || [];
  }
...
The final proof: does the control plane actually help? The `abBenchmark()` function implements the Measurement Plan: run 20 real tasks drawn from GemiFlow repo history under two configs — **A** (no control plane) vs **B** (with Phase 1 guidance) — and compute KPIs, composite scores, and category shift detection.

```typescript
import { abBenchmark, getDefaultABTasks } from '@gemiflow/guidance/analyzer';

// Run A/B benchmark with content-aware executor
const report = await abBenchmark(geminiMdContent, {
...
- **A**: Current GemiFlow without the wired control plane
...
Take 20 real GemiFlow tasks from repo history. Run A without control plane, run B with Phase 1 only. Success is B improves success rate and reduces tool calls per successful task, while producing replayable ledgers for every failure.
