 ...
 *
 * @module @gemiflow/guidance/generators
 */
...
export function generateClaudeMd(profile: ProjectProfile): string {
  ...
  if (profile.guidanceControlPlane) {
    ...
    sections.push('');
    sections.push('This project uses `@gemiflow/guidance` to enforce these rules programmatically.');
...
// ============================================================================
// Scaffold Generator (creates full .gemiflow/ directory structure)
// ============================================================================
...
export function scaffold(options: ScaffoldOptions): ScaffoldResult {
  ...
  for (const agent of agents) {
    ...
    const path = `.gemiflow/agents/${category}/${agent.name}.md`;
    files.set(path, generateAgentMd(agent));
  }

  // Generate agent index
  if (agents.length > 0) {
    files.set('.gemiflow/agents/index.yaml', generateAgentIndex(agents));
  }

  // Generate skill files
  for (const skill of skills) {
    const path = `.gemiflow/skills/${skill.name}/SKILL.md`;
  ...
  if (profile.guidanceControlPlane) {
    skills.push({
      ...
      instructions: [
        ...
        '',
        'This project uses `@gemiflow/guidance` to enforce CLAUDE.md rules programmatically.',
        '',
