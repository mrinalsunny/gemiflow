 ...
 *
 * @module @gemiflow/guidance/analyzer
 */
...
class DefaultHeadlessExecutor implements IContentAwareExecutor {
  ...
  setContext(geminiMdContent: string): void {
    this.contextContent = geminiMdContent;
  }
  ...
  async execute(prompt: string, workDir: string): Promise<{ stdout: string; stderr: string; exitCode: number }> {
    ...
    const geminiMdPath = join(workDir, 'CLAUDE.md');
    const backupPath = join(workDir, '.CLAUDE.md.ab-backup');
    let swapped = false;

    if (this.contextContent !== null) {
      try { await fs.copyFile(geminiMdPath, backupPath); } catch { /* no file to back up */ }

      if (this.contextContent.length > 0) {
        await fs.writeFile(geminiMdPath, this.contextContent, 'utf-8');
      } else {
        await fs.unlink(geminiMdPath).catch(() => {});
      }
      swapped = true;
    }
    ...
    try {
      ...
      if (swapped) {
        try {
          await fs.copyFile(backupPath, geminiMdPath);
...
/** A single benchmark task representing a real GemiFlow scenario */
...
// ── 20 Representative Tasks from GemiFlow History ───────────────────────
...
function getABTasks(): ABTask[] {
  ...
  return [
    ...
    // ── Deployment (2 tasks) ──────────────────────────────────────────
    {
      ...
      prompt: 'Create a multi-stage Dockerfile for the GemiFlow CLI. Include a build stage and a minimal runtime stage. Never include dev dependencies in production.',
      assertions: [
 ...
 * **Config B** (treatment): With guidance — executor gets setContext(geminiMd) +
 *   gate simulation on every output
 *
 * The 20 tasks span 7 task classes drawn from real GemiFlow repo history:
 ...
 * = "category shift"
 *
 * @param geminiMdContent - The CLAUDE.md content used for Config B
...
export async function abBenchmark(
  ...
  if (!contentAware) {
    throw new Error(
      'abBenchmark requires a content-aware executor. The provided IHeadlessExecutor lacks `setContext()`, so Config A and Config B will both read the same on-disk CLAUDE.md and the delta is guaranteed to be zero. Either use the DefaultHeadlessExecutor (content-aware as of @gemiflow/guidance@3.0.0-alpha.2) or implement IContentAwareExecutor on your custom executor.',
    );
  }
  ...
  if (contentAware) executor.setContext(geminiMdContent);
  ...
  if (proofKey) {
    ...
    const event: RunEvent = {
      ...
      intent: 'testing' as TaskIntent,
      guidanceHash: createHash('sha256').update(geminiMdContent).digest('hex').slice(0, 16),
      retrievedRuleIds: [],
