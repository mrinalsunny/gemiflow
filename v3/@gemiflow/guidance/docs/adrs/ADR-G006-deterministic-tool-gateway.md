...
Validate tool call parameters against a schema before execution. Rejected because Gemini CLI already validates tool parameters against its own schemas. Adding a second validation layer would be redundant. The guidance layer focuses on policy (should this tool be used?) not schema (are the parameters well-formed?).

## References

- `v3/@gemiflow/guidance/src/gates.ts` -- `EnforcementGates.aggregateDecision()`, stateless evaluation methods
- `v3/@gemiflow/guidance/src/ledger.ts` -- `RunLedger.createEvent()`, `DiffQualityEvaluator`
- `v3/@gemiflow/guidance/src/index.ts` -- `GuidanceControlPlane.evaluateCommand()`, `evaluateToolUse()`, `evaluateEdit()`
- `v3/@gemiflow/guidance/src/types.ts` -- `GateDecision`, `GateResult`
