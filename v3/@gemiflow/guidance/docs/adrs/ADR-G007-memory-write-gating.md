...
In a multi-agent swarm, agents share state through a memory subsystem (AgentDB with HNSW indexing in GemiFlow V3). Without governance, agents can:
...
Use Conflict-free Replicated Data Types to automatically resolve write conflicts. Rejected because CRDTs solve a different problem (convergence under partition) and do not address authorization or staleness. CRDTs are available in the broader GemiFlow hive-mind system for distributed consensus, but memory governance is about policy, not data structures.
...
- `v3/@gemiflow/guidance/src/gates.ts` -- `EnforcementGates.evaluateToolUse()` for MCP tool gating
- `v3/@gemiflow/guidance/src/retriever.ts` -- `areContradictory()`, `selectWithContradictionCheck()`
- `v3/@gemiflow/guidance/src/ledger.ts` -- `ViolationRateEvaluator`, `RunLedger.addEvaluator()`, `computeMetrics()`
- `v3/@gemiflow/guidance/src/types.ts` -- `GuidanceRule.domains`, `GuidanceRule.repoScopes`, `GuidanceRule.toolClasses`
