...
All exports from `@gemiflow/guidance`. Each module is also available as a standalone import.

## Import Map

| Import Path | Key Exports |
|-------------|-------------|
| `@gemiflow/guidance` | `GuidanceControlPlane`, `createGuidanceControlPlane` |
| `@gemiflow/guidance/compiler` | `GuidanceCompiler`, `createCompiler` |
| `@gemiflow/guidance/retriever` | `ShardRetriever`, `createRetriever`, `HashEmbeddingProvider` |
| `@gemiflow/guidance/gates` | `EnforcementGates`, `createGates` |
| `@gemiflow/guidance/hooks` | `GuidanceHookProvider`, `createGuidanceHooks` |
| `@gemiflow/guidance/ledger` | `RunLedger`, `createLedger`, `TestsPassEvaluator`, `ForbiddenCommandEvaluator`, `ForbiddenDependencyEvaluator`, `ViolationRateEvaluator`, `DiffQualityEvaluator` |
| `@gemiflow/guidance/optimizer` | `OptimizerLoop`, `createOptimizer` |
| `@gemiflow/guidance/persistence` | `PersistentLedger`, `EventStore`, `createPersistentLedger`, `createEventStore` |
| `@gemiflow/guidance/headless` | `HeadlessRunner`, `createHeadlessRunner`, `createComplianceSuite` |
| `@gemiflow/guidance/gateway` | `DeterministicToolGateway`, `createToolGateway` |
| `@gemiflow/guidance/artifacts` | `ArtifactLedger`, `createArtifactLedger` |
| `@gemiflow/guidance/evolution` | `EvolutionPipeline`, `createEvolutionPipeline` |
| `@gemiflow/guidance/manifest-validator` | `ManifestValidator`, `ConformanceSuite`, `createManifestValidator`, `createConformanceSuite` |
| `@gemiflow/guidance/proof` | `ProofChain`, `createProofChain` |
| `@gemiflow/guidance/memory-gate` | `MemoryWriteGate`, `createMemoryWriteGate`, `createMemoryEntry` |
| `@gemiflow/guidance/coherence` | `CoherenceScheduler`, `EconomicGovernor`, `createCoherenceScheduler`, `createEconomicGovernor` |
| `@gemiflow/guidance/capabilities` | `CapabilityAlgebra`, `createCapabilityAlgebra` |
| `@gemiflow/guidance/conformance-kit` | `SimulatedRuntime`, `MemoryClerkCell`, `ConformanceRunner`, `createMemoryClerkCell`, `createConformanceRunner` |
| `@gemiflow/guidance/ruvbot-integration` | `RuvBotGuidanceBridge`, `AIDefenceGate`, `RuvBotMemoryAdapter`, `createRuvBotBridge`, `createAIDefenceGate`, `createRuvBotMemoryAdapter` |
| `@gemiflow/guidance/meta-governance` | `MetaGovernor`, `createMetaGovernor` |
| `@gemiflow/guidance/adversarial` | `ThreatDetector`, `CollusionDetector`, `MemoryQuorum`, `createThreatDetector`, `createCollusionDetector`, `createMemoryQuorum` |
| `@gemiflow/guidance/trust` | `TrustAccumulator`, `TrustScoreLedger`, `TrustSystem`, `getTrustBasedRateLimit`, `createTrustAccumulator`, `createTrustSystem` |
| `@gemiflow/guidance/truth-anchors` | `TruthAnchorStore`, `TruthResolver`, `createTruthAnchorStore`, `createTruthResolver` |
| `@gemiflow/guidance/uncertainty` | `UncertaintyLedger`, `UncertaintyAggregator`, `createUncertaintyLedger`, `createUncertaintyAggregator` |
| `@gemiflow/guidance/temporal` | `TemporalStore`, `TemporalReasoner`, `createTemporalStore`, `createTemporalReasoner` |
| `@gemiflow/guidance/authority` | `AuthorityGate`, `IrreversibilityClassifier`, `createAuthorityGate`, `createIrreversibilityClassifier`, `isHigherAuthority`, `getAuthorityHierarchy` |
| `@gemiflow/guidance/continue-gate` | `ContinueGate`, `createContinueGate` |
| `@gemiflow/guidance/wasm-kernel` | `getKernel`, `isWasmAvailable`, `resetKernel` |
| `@gemiflow/guidance/generators` | `generateClaudeMd`, `generateClaudeLocalMd`, `generateSkillMd`, `generateAgentMd`, `generateAgentIndex`, `scaffold` |
| `@gemiflow/guidance/analyzer` | `analyze`, `benchmark`, `autoOptimize`, `optimizeForSize`, `headlessBenchmark`, `validateEffect`, `abBenchmark`, `getDefaultABTasks`, `formatReport`, `formatBenchmark` |
...
```ts
import { generateClaudeMd, scaffold } from '@gemiflow/guidance/generators';
```
...
```ts
import { analyze, validateEffect } from '@gemiflow/guidance/analyzer';
```
...
**IContentAwareExecutor**: Extends `IHeadlessExecutor` with `setContext(geminiMdContent)` — called before each phase so the executor can vary behavior based on guidance quality.
...
| Function | Returns | Description |
|----------|---------|-------------|
| `abBenchmark(geminiMd, options?)` | `Promise<ABReport>` | Run 20 tasks under Config A (no guidance) vs Config B (with guidance) |
