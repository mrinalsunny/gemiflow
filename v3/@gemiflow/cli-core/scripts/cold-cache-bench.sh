#!/bin/bash
# Cold-cache benchmark — @gemiflow/cli@latest vs @gemiflow/cli-core@alpha
#
...
#
# Usage:  bash v3/@gemiflow/cli-core/scripts/cold-cache-bench.sh
...
for i in $(seq 1 $RUNS); do
  rm -rf "$BENCH_CACHE"
  v=$(time_pkg "@gemiflow/cli@latest");      echo "cli  cold #$i: ${v}s";  CC+=("$v")
  v=$(time_pkg "@gemiflow/cli@latest");      echo "cli  warm #$i: ${v}s";  CW+=("$v")
done
for i in $(seq 1 $RUNS); do
  rm -rf "$BENCH_CACHE"
  v=$(time_pkg "@gemiflow/cli-core@alpha");  echo "core cold #$i: ${v}s";  KC+=("$v")
  v=$(time_pkg "@gemiflow/cli-core@alpha");  echo "core warm #$i: ${v}s";  KW+=("$v")
done
