 ...
 *
 * Scans a plugin (or any .mjs/.js) directory for `npx @gemiflow/cli`
...
for (const f of files) {
  ...
  // Pattern 1: spawnSync('npx', ['@gemiflow/cli@latest', 'memory', 'store', ...])
  ...
  for (const op of SAFE_MEM_OPS) {
    const pattern = new RegExp(
      `(['"\`])@gemiflow/cli@latest\\1(\\s*,\\s*['"\`]${op}\\b)`,
      'g',
    );
    if (pattern.test(updated)) {
      updated = updated.replace(
        pattern,
        `$1\${process.env.CLI_CORE === '1' ? '@gemiflow/cli-core@alpha' : '@gemiflow/cli@latest'}\$1$2`,
      );
      ...
      changed = false;
      needsReview++;
      console.error(`[review] ${f}: contains @gemiflow/cli@latest with ${op} — apply MIGRATION.md env-flag pattern manually`);
      break;
    }
  }

  // Pattern 2: bare 'npx @gemiflow/cli@latest <safe-op>' in shell-style.
  ...
  for (const op of SAFE_MEM_OPS) {
    const re = new RegExp(`npx @gemiflow/cli@latest ${op}\\b`, 'g');
    if (re.test(updated)) {
      // Check next-token-is-hooks etc. excluded by SAFE list — ok to rewrite.
      const before = updated;
      updated = updated.replace(re, `npx \${CLI_CORE:-@gemiflow/cli@latest=@gemiflow/cli-core@alpha} ${op}`);
  ...
  for (const op of NEEDS_REVIEW) {
    const re = new RegExp(`@gemiflow/cli@latest['"\`]?\\s*,?\\s*['"\`]?${op}\\b`, 'g');
    if (re.test(text)) {
