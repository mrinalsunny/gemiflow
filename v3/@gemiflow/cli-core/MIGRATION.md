# Migration Guide — switching plugin scripts to `@gemiflow/cli-core`
...
> **alpha.5 adds two subpath exports** (`./mcp-tools/types`, `./mcp-tools/validate-input`) so `@gemiflow/cli` can re-export those foundation modules from cli-core in a follow-up PR (eliminating ~1,229 LOC of byte-identical duplication). No behavior change for existing plugin scripts; this is groundwork for the cli-side metapackage refactor.

## TL;DR

Each `npx @gemiflow/cli@latest memory <subcommand> ...` in a plugin script can be swapped for `npx @gemiflow/cli-core@alpha memory <subcommand> ...`. Cold-cache wall-time drops from ~25s to ~2s.
...
const r = spawnSync('npx', [
  '@gemiflow/cli@latest', 'memory', 'store',
...
const cliPkg = process.env.CLI_CORE === '1'
  ? '@gemiflow/cli-core@alpha'  // lite path — JSON backend, fast cold cache
  : '@gemiflow/cli@latest';     // heavy path — SQLite/HNSW, slow cold cache
const r = spawnSync('npx', [
  cliPkg, 'memory', 'store',
  '--namespace', 'cost-tracking',
  '--key', `session-${id}`,
...
3. **If yes**, fall back to `cli@latest` for the affected call sites and document the gap; ping us on issue [#1760](https://github.com/mrinalsunny/gemiflow/issues/1760) so we can prioritize.
...
Or replace the conditional with the bare `'@gemiflow/cli@latest'` and re-run. cli-core left no permanent footprint other than the JSON file (which you can delete: `rm .swarm/memory.json`).

## Reporting issues

cli-core is alpha — please file feedback at https://github.com/mrinalsunny/gemiflow/issues with the label `cli-core-alpha`. Specifically helpful:

- Cold-cache wall-time on your network (run `rm -rf ~/.npm/_npx; time npx -y @gemiflow/cli-core@alpha --version`)
...
## Planned: `@gemiflow/cli` foundation re-export (post-alpha.5)

The following 4 foundation modules are byte-identical between `@gemiflow/cli` and `@gemiflow/cli-core` (verified via `diff -q`):

| File | Lines | cli-core export |
|---|---:|---|
| `cli/src/types.ts` | 287 | `@gemiflow/cli-core/types` |
| `cli/src/output.ts` | 640 | `@gemiflow/cli-core/output` |
| `cli/src/mcp-tools/types.ts` | 46 | `@gemiflow/cli-core/mcp-tools/types` |
| `cli/src/mcp-tools/validate-input.ts` | 256 | `@gemiflow/cli-core/mcp-tools/validate-input` |
| **Total** | **1,229** | |

The follow-up PR will:

1. Add `@gemiflow/cli-core: ^3.7.0-alpha.5` as a runtime dep in `cli/package.json`.
...
- [Issue #1748 #3](https://github.com/mrinalsunny/gemiflow/issues/1748) — the cold-cache problem this addresses
- [Issue #1760](https://github.com/mrinalsunny/gemiflow/issues/1760) — alpha tracking
- Branch [`feat/cli-core-split`](https://github.com/mrinalsunny/gemiflow/tree/feat/cli-core-split) — work in progress
