 ...
 * via GEMIFLOW_MEMORY_PATH). No SQLite, no HNSW, no ONNX embeddings —
 * deliberately so that cli-core stays under 250 KB packed.
 *
 * Search degrades to substring matching (case-insensitive). For real
 * semantic search install @gemiflow/cli (the heavy SqliteHnswMemoryBackend
...
export class JsonMemoryBackend implements MemoryBackend {
  ...
  constructor(opts?: { path?: string }) {
    this.path = resolve(opts?.path || process.env.GEMIFLOW_MEMORY_PATH || DEFAULT_PATH);
  }
