#!/usr/bin/env node
/**
 * @gemiflow/cli-core entry point — alpha.1 surface.
 *
 ...
 *                       stay in @gemiflow/cli, dynamic-imported)
...
// Bin entry — runs when invoked as `gemiflow-core <command>`.
...
const isMain = (() => {
  ...
  try {
    ...
    // npm bin-shim case: argv[1] is the .bin/gemiflow-core symlink.
...
if (isMain) {
  ...
  if (args[0] === '--help' || args[0] === '-h' || args.length === 0) {
    console.log(`@gemiflow/cli-core — alpha.1 (ADR-100)
...
Storage:
  ...
  GEMIFLOW_MEMORY_PATH or --path. Search is substring-only — for semantic
  vector search, install the heavy @gemiflow/cli@alpha.

Programmatic use:
  import { JsonMemoryBackend, runMemoryCommand } from '@gemiflow/cli-core';

Track progress: https://github.com/mrinalsunny/gemiflow/issues/1760`);
    process.exit(0);
  }

  console.error(`@gemiflow/cli-core: command "${args[0]}" not yet wired into cli-core.
For now, use the full CLI:  npx @gemiflow/cli@alpha ${args.join(' ')}`);
  process.exit(1);
}
