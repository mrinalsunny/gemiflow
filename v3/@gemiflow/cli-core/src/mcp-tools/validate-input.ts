 ...
 *
 * Loads @gemiflow/security validators when available
...
// Try to load the full @gemiflow/security module for enhanced validation
...
async function getSecurityModule(): Promise<Record<string, any> | null> {
  ...
  try {
    // Dynamic import — @gemiflow/security is an optional dependency
    _securityModule = await (Function('return import("@gemiflow/security")')() as Promise<Record<string, any>>);
  } catch {
    // @gemiflow/security is optional — fallback to inline validation above
  }
  return _securityModule;
}

/**
 * Enhanced validation using @gemiflow/security Zod schemas when available.
...
export async function validateAgentSpawn(input: Record<string, unknown>): Promise<{ valid: boolean; errors: string[] }> {
  ...
  // Fix for #1567: @gemiflow/security's SpawnAgentSchema expects `type` and
