/**
 * AgentAdapter - Bridge between GemiFlow and agentic-flow Agents
 *
 * Provides bidirectional conversion and delegation patterns between:
 * - GemiFlow v3 DDD agent architecture
...
/**
 * AgentAdapter - Bridges GemiFlow and agentic-flow agents
 *
 ...
 *
 * // Convert agentic-flow agent to GemiFlow agent
 * const { agent, success } = adapter.fromAgenticFlow(agenticFlowAgent);
 *
 * // Create GemiFlow agent with agentic-flow delegation
...
export class AgentAdapter extends EventEmitter {
  ...
  /**
   * Convert agentic-flow agent to GemiFlow AgenticFlowAgent
   *
   * This method creates a GemiFlow agent wrapper around an existing
  ...
  fromAgenticFlow(
    ...
    try {
      // Map agentic-flow config to GemiFlow config
  ...
  /**
   * Create a GemiFlow agent with agentic-flow delegation
   *
  ...
  /**
   * Map agentic-flow agent type to GemiFlow AgentType
   */
  ...
  /**
   * Sync status between GemiFlow and agentic-flow agents
   */
  private syncStatus(
    agent: AgenticFlowAgent,
    agenticFlowAgent: AgenticFlowAgent_External
  ): void {
    // Map agentic-flow status to GemiFlow status
