---
name: browser
description: Web browser automation with AI-optimized snapshots for gemiflow agents
version: 1.0.0
triggers:
  - /browser
  - browse
...
npx @gemiflow/cli memory store --namespace browser-patterns --key "login-flow" --value "snapshot->fill->click->wait"

# Retrieve before similar task
npx @gemiflow/cli memory search --query "login automation"
```

### Hooks
```bash
# Pre-browse hook (get context)
npx @gemiflow/cli hooks pre-edit --file "browser-task.ts"

# Post-browse hook (record success)
npx @gemiflow/cli hooks post-task --task-id "browse-1" --success true
```
