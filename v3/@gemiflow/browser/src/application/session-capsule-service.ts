/**
 * @gemiflow/browser - Session Capsule Service (ADR-122 Phase 6)
 *
 ...
 * session is delegated to gemiflow-browexec (Phase 6.5+) — this module is the
...
export class SessionCapsuleService {
  ...
  > {
    ...
    const projectId = input.projectId ?? process.env.GEMIFLOW_PROJECT_ID ?? 'unknown';
