...
export class CredentialGenerator {
  ...
  createEnvScript(credentials: GeneratedCredentials): string {
    return `# GemiFlow V3 - Generated Credentials
# Generated: ${credentials.generatedAt.toISOString()}
# IMPORTANT: Store these securely and delete this file after use

export GEMIFLOW_ADMIN_PASSWORD="${credentials.adminPassword}"
export GEMIFLOW_SERVICE_PASSWORD="${credentials.servicePassword}"
export GEMIFLOW_JWT_SECRET="${credentials.jwtSecret}"
export GEMIFLOW_SESSION_SECRET="${credentials.sessionSecret}"
export GEMIFLOW_ENCRYPTION_KEY="${credentials.encryptionKey}"
`;
  }
  ...
  createJsonConfig(credentials: GeneratedCredentials): string {
    return JSON.stringify({
      'gemiflow/admin-password': credentials.adminPassword,
      'gemiflow/service-password': credentials.servicePassword,
      'gemiflow/jwt-secret': credentials.jwtSecret,
      'gemiflow/session-secret': credentials.sessionSecret,
      'gemiflow/encryption-key': credentials.encryptionKey,
      'gemiflow/generated-at': credentials.generatedAt.toISOString(),
      'gemiflow/expires-at': credentials.expiresAt?.toISOString() ?? null,
    }, null, 2);
  }
}
