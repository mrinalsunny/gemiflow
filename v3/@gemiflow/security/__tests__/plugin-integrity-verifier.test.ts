...
describe('findAnchor — trust resolution', () => {
  ...
  it('matches by exact key + matching exact scope', () => {
    const anchors: TrustAnchor[] = [
      { publicKey: 'PK', owner: 'o', scope: '@gemiflow/memory' },
    ];
    expect(findAnchor(anchors, '@gemiflow/memory', 'PK', NOW)).not.toBeNull();
  });

  it('matches by exact key + wildcard scope', () => {
    const anchors: TrustAnchor[] = [{ publicKey: 'PK', owner: 'o', scope: '@gemiflow/*' }];
    expect(findAnchor(anchors, '@gemiflow/anything', 'PK', NOW)).not.toBeNull();
  });

  it('rejects exact-key with out-of-scope plugin id', () => {
    const anchors: TrustAnchor[] = [
      { publicKey: 'PK', owner: 'o', scope: '@gemiflow/*' },
    ];
