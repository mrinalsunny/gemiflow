...
describe('CredentialGenerator', () => {
  ...
  describe('Environment Script Generation', () => {
    it('should generate valid shell script', () => {
      ...
      expect(script).toContain('export GEMIFLOW_ADMIN_PASSWORD=');
      expect(script).toContain('export GEMIFLOW_SERVICE_PASSWORD=');
      expect(script).toContain('export GEMIFLOW_JWT_SECRET=');
      expect(script).toContain('export GEMIFLOW_SESSION_SECRET=');
      expect(script).toContain('export GEMIFLOW_ENCRYPTION_KEY=');
    });
  ...
  describe('JSON Config Generation', () => {
    ...
    it('should include all credentials', () => {
      ...
      expect(parsed['gemiflow/admin-password']).toBe(credentials.adminPassword);
      expect(parsed['gemiflow/service-password']).toBe(credentials.servicePassword);
      expect(parsed['gemiflow/jwt-secret']).toBe(credentials.jwtSecret);
      expect(parsed['gemiflow/session-secret']).toBe(credentials.sessionSecret);
      expect(parsed['gemiflow/encryption-key']).toBe(credentials.encryptionKey);
    });
  });
