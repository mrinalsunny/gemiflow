 ...
 *
 * @gemiflow/swarm has zero runtime dependencies and shouldn't take a
 * hard dep on @gemiflow/plugin-agent-federation (the wiring would also
...
export interface FederationTransportOptions {
  ...
  /** Stream id for consensus traffic (ADR-104 stream-mux). Defaults to 'gemiflow-consensus'. */
...
export class FederationTransport implements ConsensusTransport {
  ...
  constructor(wire: AgenticFlowTransportLike, opts: FederationTransportOptions) {
    ...
    this.streamId = opts.streamId ?? 'gemiflow-consensus';
