import type { CLICommandDefinition, PluginContext } from '@gemiflow/shared/src/plugin-interface.js';
  ...
  return [
    ...
    },
    {
      ...
      options: [
        ...
        { name: 'name', short: 'n', description: 'Client name for pairing', type: 'string', default: 'gemiflow' },
      ],
      handler: async (args) => {
        const coordinator = requireCoordinator(getCoordinator);
        const device = await coordinator.pairDevice(
          args['device-id'] as string,
          (args['name'] as string) ?? 'gemiflow',
        );
