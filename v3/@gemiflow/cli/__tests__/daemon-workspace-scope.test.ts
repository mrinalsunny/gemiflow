/**
 * Regression guard for mrinalsunny/gemiflow#1914 — `killStaleDaemons` must only ever
...
const psLine = (root: string, pid = 4242, extra = '') =>
  `${pid} node /usr/local/lib/node_modules/@gemiflow/cli/bin/cli.js daemon start --foreground --quiet${extra} --workspace ${root}`;
...
describe('#1914 — daemon workspace scoping', () => {
  describe('daemonCommandLineBelongsToWorkspace', () => {
    ...
    it('does NOT match a pre-#1914 daemon line (no --workspace stamp)', () => {
      const legacy = '4242 node /usr/local/lib/node_modules/@gemiflow/cli/bin/cli.js daemon start --foreground --quiet';
