...
describe('#2250 — escalation does not override learned opus suppression', () => {
  ...
  it('respects GEMIFLOW_MAX_UNCERTAINTY env override', async () => {
    ...
    const old = process.env.GEMIFLOW_MAX_UNCERTAINTY;
    process.env.GEMIFLOW_MAX_UNCERTAINTY = '0.99';
    try {
      ...
      if (old === undefined) delete process.env.GEMIFLOW_MAX_UNCERTAINTY;
      else process.env.GEMIFLOW_MAX_UNCERTAINTY = old;
    }
  });
});
