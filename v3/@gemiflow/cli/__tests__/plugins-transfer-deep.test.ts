...
describe('PluginManager', () => {
  ...
  it('should return correct plugins dir and manifest path', () => {
    expect(manager.getPluginsDir()).toContain('.gemiflow/plugins');
...
describe('Config Adapter', () => {
  ...
  it('should convert V3Config back to SystemConfig', () => {
    ...
    expect(sys.mcp?.name).toBe('gemiflow');
  });
...
describe('Update Validator', () => {
  it('should validate compatible update', () => {
    const result = validateUpdate(
      '@gemiflow/cli', '3.0.0-alpha.50', '3.0.0-alpha.55', {}
    );
    expect(result.valid).toBe(true);
  });
  ...
  it('validateBulkUpdate checks all updates', () => {
    const result = validateBulkUpdate(
      [
        { package: '@gemiflow/cli', from: '3.0.0-alpha.50', to: '3.0.0-alpha.55' },
        { package: '@gemiflow/embeddings', from: '3.0.0-alpha.1', to: '3.0.0-alpha.5' },
      ],
      { '@gemiflow/cli': '3.0.0-alpha.50', '@gemiflow/embeddings': '3.0.0-alpha.1' }
    );
    expect(result.valid).toBe(true);
  });
});
...
describe('Update Rate Limiter', () => {
  ...
  it('should block when auto-update disabled', () => {
    const origCI = process.env.CI;
    const origAutoUpdate = process.env.GEMIFLOW_AUTO_UPDATE;
    delete process.env.CI;
    delete process.env.CONTINUOUS_INTEGRATION;
    process.env.GEMIFLOW_AUTO_UPDATE = 'false';
    const result = shouldCheckForUpdates();
    expect(result.allowed).toBe(false);
    expect(result.reason).toContain('disabled');
    if (origCI) process.env.CI = origCI;
    if (origAutoUpdate) process.env.GEMIFLOW_AUTO_UPDATE = origAutoUpdate;
    else delete process.env.GEMIFLOW_AUTO_UPDATE;
  });

  it('should allow when force update requested', () => {
    const origCI = process.env.CI;
    const origForce = process.env.GEMIFLOW_FORCE_UPDATE;
    delete process.env.CI;
    delete process.env.CONTINUOUS_INTEGRATION;
    delete process.env.GEMIFLOW_AUTO_UPDATE;
    process.env.GEMIFLOW_FORCE_UPDATE = 'true';
    const result = shouldCheckForUpdates();
    expect(result.allowed).toBe(true);
    if (origCI) process.env.CI = origCI;
    if (origForce) process.env.GEMIFLOW_FORCE_UPDATE = origForce;
    else delete process.env.GEMIFLOW_FORCE_UPDATE;
  });
});
