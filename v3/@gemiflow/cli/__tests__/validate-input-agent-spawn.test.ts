 ...
 * `{ agentType, name }` to `@gemiflow/security`'s SpawnAgentSchema, which
...
describe('validateAgentSpawn (#1567)', () => {
  ...
  it('accepts a custom agent type not in the SpawnAgentSchema enum', async () => {
    ...
    // @gemiflow/security's AgentTypeSchema enumerates, but it's a valid
    // GemiFlow agent type and should NOT be rejected.
