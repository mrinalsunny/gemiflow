...
class HeadlessWorkerExecutor extends EventEmitter {
  ...
  async isAvailable(): Promise<boolean> {
    try {
      ...
      return result.includes('claude') || result.includes('Claude') || result.includes('gemini') || result.includes('Gemini');
    } catch {
      // Try alternative path
      try {
        const altResult = (execSync as Mock)('npx gemini-cli --version', { encoding: 'utf-8' });
        this.claudePath = 'npx gemini-cli';
        return altResult.includes('claude') || altResult.includes('Claude') || altResult.includes('gemini') || altResult.includes('Gemini');
      } catch {
        return false;
      }
    }
...
describe('HeadlessWorkerExecutor', () => {
  ...
  describe('isAvailable', () => {
    ...
    it('should try npx gemini-cli when claude fails', async () => {
      (execSync as Mock)
        .mockImplementationOnce(() => {
          throw new Error('Command not found');
        })
        .mockReturnValue('gemini-cli version 1.0.0');
    ...
    it('should return false when both claude and npx gemini-cli fail', async () => {
