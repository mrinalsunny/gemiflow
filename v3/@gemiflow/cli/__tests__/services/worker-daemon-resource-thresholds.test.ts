...
describe('WorkerDaemon resource thresholds', () => {
  ...
  beforeEach(() => {
    ...
    mkdirSync(join(tempDir, '.gemiflow', 'logs'), { recursive: true });
  });
  ...
  describe('config.json reading', () => {
    it('should read daemon settings from flat dot-notation keys', () => {
      const configFile = join(tempDir, '.gemiflow', 'config.json');
  ...
  describe('state persistence', () => {
    it('should restore resourceThresholds from daemon-state.json', () => {
      const stateFile = join(tempDir, '.gemiflow', 'daemon-state.json');
  ...
  describe('mutation killing', () => {
    ...
    it('should independently check CPU and memory (not conflate them)', async () => {
      ...
      mkdirSync(join(tempDir2, '.gemiflow', 'logs'), { recursive: true });
  ...
  // (GEMIFLOW_DAEMON_TTL_SECS / GEMIFLOW_DAEMON_IDLE_SECS) > built-in default.
  // =========================================================================
  describe('self-terminating lifecycle (ttl/idle)', () => {
    const TTL_ENV = 'GEMIFLOW_DAEMON_TTL_SECS';
    const IDLE_ENV = 'GEMIFLOW_DAEMON_IDLE_SECS';
    ...
    it('honors GEMIFLOW_DAEMON_TTL_SECS env override (seconds → ms)', () => {
      process.env[TTL_ENV] = '3600';
      const config = new WorkerDaemon(tempDir).getStatus().config;
      expect(config.ttlMs).toBe(3600 * 1000);
    });

    it('honors an explicit GEMIFLOW_DAEMON_TTL_SECS=0 as "disabled" (not falling back to default)', () => {
    ...
    it('honors GEMIFLOW_DAEMON_IDLE_SECS env override', () => {
