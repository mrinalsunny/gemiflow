 ...
 *
 * #2206 — mcp-generator must register the server under the 'gemiflow' key
 *          (not 'gemiflow') so all plugins resolve as mcp__gemiflow__*.
 * #2207 — detectExistingGemiFlowMCP must accept both 'gemiflow' and 'gemiflow' keys;
 *          a bare .gemiflow/settings.json must NOT be a false positive.
 * #2208 — writeClaudeMd (--force) must back up an existing CLAUDE.md to
 *          CLAUDE.md.pre-gemiflow before overwriting it.
 */
...
describe('#2206 — mcp-generator server key', () => {
  it('registers the gemiflow MCP server under the gemiflow key', () => {
    const config = generateMCPConfig(makeMCPOptions()) as { mcpServers: Record<string, unknown> };
    expect(config.mcpServers).toHaveProperty('gemiflow');
  });

  it('still invokes gemiflow@latest mcp start as the command args', () => {
    const config = generateMCPConfig(makeMCPOptions()) as { mcpServers: Record<string, unknown> };
    const entry = config.mcpServers['gemiflow'] as { command: string; args: string[] };
    expect(entry.args).toContain('gemiflow@latest');
    expect(entry.args).toContain('mcp');
    expect(entry.args).toContain('start');
  });

  it('generateMCPCommands uses gemiflow as the registration name', () => {
    const cmds = generateMCPCommands(makeMCPOptions());
    expect(cmds.length).toBeGreaterThan(0);
    // Every command that adds the main server must use 'gemiflow'
    const mainCmd = cmds.find(c => c.includes('gemiflow@latest'));
    expect(mainCmd).toBeDefined();
    expect(mainCmd).toMatch(/claude mcp add gemiflow/);
  });
});

// ─── #2207: detectExistingGemiFlowMCP — both keys + no false positive ───────────
...
// calls writeMCPConfig → detectExistingGemiFlowMCP.
...
describe('#2207 — init detector accepts both server keys', () => {
  ...
  it('detects a prior install registered under the new gemiflow key', async () => {
    // Write a parent .mcp.json with the 'gemiflow' key (post-#2206 install)
    const parentDir = mkdtempSync(path.join(tmpdir(), 'wizard-2207-parent-'));
    try {
      writeFileSync(
        path.join(parentDir, '.mcp.json'),
        JSON.stringify({ mcpServers: { 'gemiflow': { command: 'npx', args: ['-y', 'gemiflow@latest', 'mcp', 'start'] } } }),
      );
      ...
      // Because a 'gemiflow' key was found in the parent .mcp.json, init should
  ...
  it('detects a prior install registered under the legacy gemiflow key', async () => {
  ...
  it('does NOT false-positive on a bare .gemiflow/settings.json with no MCP registration', async () => {
    ...
    try {
      ...
      writeFileSync(
        path.join(projectDir, '.gemiflow', 'settings.json'),
      ...
      // Override HOME so we don't accidentally match the real user's ~/.gemiflow.json
      const origHome = process.env.HOME;
      process.env.HOME = projectDir;
      try {
        const result = await executeInit(opts);
        // No pre-existing gemiflow/gemiflow key → init must NOT skip .mcp.json
...
describe('#2208 — CLAUDE.md backup before overwrite', () => {
  ...
  it('backs up existing CLAUDE.md to CLAUDE.md.pre-gemiflow when --force is used', async () => {
    ...
    const backupPath = path.join(tmp, 'CLAUDE.md.pre-gemiflow');
    expect(existsSync(backupPath)).toBe(true);
    expect(readFileSync(backupPath, 'utf-8')).toBe(sentinel);
  });

  it('uses a timestamped backup name when CLAUDE.md.pre-gemiflow already exists', async () => {
    writeFileSync(path.join(tmp, 'CLAUDE.md'), '# First content\n', 'utf-8');
    writeFileSync(path.join(tmp, 'CLAUDE.md.pre-gemiflow'), '# Previous backup\n', 'utf-8');
    ...
    // The original .pre-gemiflow must still have its old content
    expect(readFileSync(path.join(tmp, 'CLAUDE.md.pre-gemiflow'), 'utf-8')).toBe('# Previous backup\n');
    // A timestamped backup must have been created
    const entries = readdirSync(tmp);
    const timestamped = entries.find(e => e.startsWith('CLAUDE.md.pre-gemiflow.') && e !== 'CLAUDE.md.pre-gemiflow');
  ...
  it('does NOT create a backup when no pre-existing CLAUDE.md exists', async () => {
    ...
    expect(existsSync(path.join(tmp, 'CLAUDE.md.pre-gemiflow'))).toBe(false);
