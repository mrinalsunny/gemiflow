 ...
 *      `.gemiflow/agents.json`, a *different* file from the canonical
 *      `.gemiflow/agents/store.json` the agent tools read, so
...
describe('#1916 — agent_logs registered + hive-worker resolution', () => {
  ...
  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), 'gemiflow-1916-'));
    prevCwd = process.env.GEMIFLOW_CWD;
    process.env.GEMIFLOW_CWD = dir;
    process.env.GEMIFLOW_CWD = dir;
    process.env.GEMIFLOW_CWD = dir;
    // hive-mind_spawn writes here:
    mkdirSync(join(dir, '.gemiflow'), { recursive: true });
    writeFileSync(
      join(dir, '.gemiflow', 'agents.json'),
      JSON.stringify({
        agents: {
  ...
  afterEach(() => {
    if (prevCwd === undefined) delete process.env.GEMIFLOW_CWD;
    else process.env.GEMIFLOW_CWD = prevCwd;
  ...
  it('canonical store wins on id collision', async () => {
    ...
    mkdirSync(join(dir, '.gemiflow', 'agents'), { recursive: true });
    writeFileSync(
      join(dir, '.gemiflow', 'agents', 'store.json'),
      JSON.stringify({
        version: '3.0.0',
        agents: {
          'hive-worker-1716-abcd': {
