...
describe('isSafePackageSpec (audit_1776853149979)', () => {
  ...
  describe('shell injection in version', () => {
    it.each([
      ...
      expect(isSafePackageSpec('gemiflow', version)).toBe(false);
    });
  });
  ...
  describe('edge cases', () => {
    ...
    it('rejects an empty version', () => {
      expect(isSafePackageSpec('gemiflow', '')).toBe(false);
    });
    ...
    it('rejects a version longer than 64 chars', () => {
      expect(isSafePackageSpec('gemiflow', '1.'.repeat(40))).toBe(false);
    });
