...
//   3. env GEMIFLOW_MEMORY_SEARCH_NAMESPACES
...
function snapshotEnv() {
  return process.env.GEMIFLOW_MEMORY_SEARCH_NAMESPACES;
}

function restoreEnv(v: string | undefined) {
  if (v === undefined) delete process.env.GEMIFLOW_MEMORY_SEARCH_NAMESPACES;
  else process.env.GEMIFLOW_MEMORY_SEARCH_NAMESPACES = v;
}
...
describe('memory_search_unified namespace fan-out (#2246)', () => {
  ...
  it('with no params, reports `namespaceSource` (not the legacy hardcode silently)', async () => {
    delete process.env.GEMIFLOW_MEMORY_SEARCH_NAMESPACES;
  ...
  it('with env GEMIFLOW_MEMORY_SEARCH_NAMESPACES, reports namespaceSource=env', async () => {
    process.env.GEMIFLOW_MEMORY_SEARCH_NAMESPACES = 'foo,bar,baz';
  ...
  it('explicit `namespaces` array beats env (priority 2 > priority 3)', async () => {
    process.env.GEMIFLOW_MEMORY_SEARCH_NAMESPACES = 'env-a,env-b';
