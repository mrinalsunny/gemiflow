...
describe('Memory Initializer', () => {
  ...
  describe('generateEmbedding backend field', () => {
    const prevDisableBridge = process.env.GEMIFLOW_DISABLE_BRIDGE;
    ...
    afterEach(() => {
      ...
      if (prevDisableBridge === undefined) delete process.env.GEMIFLOW_DISABLE_BRIDGE;
      else process.env.GEMIFLOW_DISABLE_BRIDGE = prevDisableBridge;
    });
    ...
    it("reports backend='mock' when no real embedding model is available", async () => {
      ...
      process.env.GEMIFLOW_DISABLE_BRIDGE = '1';
      vi.resetModules();
...
describe('Registry API', () => {
  ...
  it('should reject invalid ratings', async () => {
    const api = await import('../src/services/registry-api.js');
    await expect(api.rateItem('@gemiflow/test', 0)).rejects.toThrow('Rating must be integer 1-5');
    await expect(api.rateItem('@gemiflow/test', 6)).rejects.toThrow('Rating must be integer 1-5');
  });

  it('should reject non-integer ratings', async () => {
    const api = await import('../src/services/registry-api.js');
    await expect(api.rateItem('@gemiflow/test', 3.5)).rejects.toThrow('Rating must be integer 1-5');
  });
  ...
  it('should accept valid item IDs with scopes', async () => {
    ...
    try {
      await api.getRating('@gemiflow/test-plugin', 'plugin');
    } catch (e: any) {
