 ...
 *  - Strict mode (GEMIFLOW_STRICT_GUARDRAIL=true): a known indirect-
...
describe('callMCPTool — ADR-146 P2 content-boundary guardrail', () => {
  const ORIG_STRICT = process.env.GEMIFLOW_STRICT_GUARDRAIL;
  ...
  afterEach(() => {
    if (ORIG_STRICT === undefined) delete process.env.GEMIFLOW_STRICT_GUARDRAIL;
    else process.env.GEMIFLOW_STRICT_GUARDRAIL = ORIG_STRICT;
  });

  it('legacy mode (default): returns the tool result unchanged', async () => {
    delete process.env.GEMIFLOW_STRICT_GUARDRAIL;
    const payload = {
  ...
  it('strict mode: a known injection in a string field is replaced with a typed marker', async () => {
    process.env.GEMIFLOW_STRICT_GUARDRAIL = 'true';
