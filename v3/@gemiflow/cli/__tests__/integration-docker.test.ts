 ...
 *
 * Validates the GemiFlow Docker-based deployment stack without running Docker.
...
const ROOT = resolve(__dirname, '..', '..', '..', '..');            // /workspaces/gemiflow
const CLI_DIR = resolve(__dirname, '..');                             // v3/@gemiflow/cli
const GEMIFLOW_DIR = join(ROOT, 'gemiflow');
const COMPOSE_PATH = join(GEMIFLOW_DIR, 'docker-compose.yml');
const NGINX_CONF_PATH = join(GEMIFLOW_DIR, 'src', 'nginx', 'nginx.conf');
const NGINX_DOCKERFILE = join(GEMIFLOW_DIR, 'src', 'nginx', 'Dockerfile');
const MCP_BRIDGE_INDEX = join(GEMIFLOW_DIR, 'src', 'mcp-bridge', 'index.js');
const MCP_BRIDGE_DOCKERFILE = join(GEMIFLOW_DIR, 'src', 'mcp-bridge', 'Dockerfile');
const CLI_DOCKERFILE = join(CLI_DIR, 'docker', 'Dockerfile');
const ENV_EXAMPLE = join(GEMIFLOW_DIR, '.env.example');
...
describe('Nginx Configuration', () => {
  ...
  it('injects GemiFlow welcome.js script via sub_filter', () => {
    expect(nginxContent).toMatch(/sub_filter\s+'<\/head>'\s+'<script src="\/gemiflow\/welcome\.js"/);
  });
  ...
  it('serves /gemiflow/ static assets from /etc/nginx/static/', () => {
    expect(nginxContent).toMatch(/location\s+\/gemiflow\//);
  });
...
describe('MCP Bridge Dockerfile', () => {
  ...
  it('creates writable .gemiflow directories', () => {
    expect(dockerContent).toContain('/app/.gemiflow/tasks');
    expect(dockerContent).toContain('/app/.gemiflow/memory');
    expect(dockerContent).toContain('/app/.gemiflow/sessions');
  });
...
describe('CLI Dockerfile (gemiflow:lite)', () => {
  let dockerContent: string;
  ...
  it('installs gemiflow globally in the build stage', () => {
    expect(dockerContent).toContain('npm install -g gemiflow@latest');
  });
  ...
  it('creates a non-root user (gemiflow)', () => {
    expect(dockerContent).toContain('adduser');
    expect(dockerContent).toContain('gemiflow');
    expect(dockerContent).toContain('USER gemiflow');
  });
  ...
  it('has a HEALTHCHECK using gemiflow doctor', () => {
    expect(dockerContent).toContain('HEALTHCHECK');
    expect(dockerContent).toContain('gemiflow doctor');
  });

  it('default CMD starts MCP server', () => {
    expect(dockerContent).toContain('CMD ["gemiflow", "mcp", "start"]');
  });
});
...
describe('CLI Build', () => {
  it('package.json exists and has correct name', () => {
    ...
    expect(pkg.name).toBe('@gemiflow/cli');
  });
...
describe('Security Checks', () => {
  ...
  it('CLI Dockerfile runs as non-root', () => {
    ...
    expect(dockerfile).toContain('USER gemiflow');
  });
