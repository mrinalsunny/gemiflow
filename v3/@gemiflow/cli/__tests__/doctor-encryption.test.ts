/**
 * Test for `gemiflow doctor` encryption-at-rest check (ADR-096 Phase 5).
 *
...
describe('gemiflow doctor encryption-at-rest check (ADR-096 Phase 5)', () => {
  let workdir: string;
  let cwdSpy: ReturnType<typeof vi.spyOn> | null = null;

  beforeEach(() => {
    saveEnv('GEMIFLOW_ENCRYPT_AT_REST', 'GEMIFLOW_ENCRYPTION_KEY');
  ...
  it('command runs cleanly with the encryption component', async () => {
    delete process.env.GEMIFLOW_ENCRYPT_AT_REST;
  ...
  it('command runs cleanly when the env var is set + key valid', async () => {
    process.env.GEMIFLOW_ENCRYPT_AT_REST = '1';
    process.env.GEMIFLOW_ENCRYPTION_KEY = randomBytes(32).toString('hex');
    const result = await runViaComponent();
    expect(result).toBeDefined();
  });
  ...
  it('command runs cleanly when the env var is set but key missing (fail-closed)', async () => {
    process.env.GEMIFLOW_ENCRYPT_AT_REST = '1';
    delete process.env.GEMIFLOW_ENCRYPTION_KEY;
    const result = await runViaComponent();
    expect(result).toBeDefined();
  ...
  it('command runs cleanly when the env var is set but key malformed', async () => {
    process.env.GEMIFLOW_ENCRYPT_AT_REST = '1';
    process.env.GEMIFLOW_ENCRYPTION_KEY = 'not-a-real-key';
    const result = await runViaComponent();
    expect(result).toBeDefined();
  });
  ...
  it('command sees encrypted store on disk when one exists', async () => {
    ...
    process.env.GEMIFLOW_ENCRYPTION_KEY = hexKey;

    // Plant a real encrypted blob at .gemiflow/terminals/store.json
    const dir = join(workdir, '.gemiflow', 'terminals');
