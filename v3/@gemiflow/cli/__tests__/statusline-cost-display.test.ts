 ...
 *
 *   GEMIFLOW_STATUSLINE_COST_SYMBOL  override the leading '$' ('' => number alone)
 *   GEMIFLOW_STATUSLINE_HIDE_COST    1/true/yes/on => omit the segment
 *
...
function renderHeader(env: Record<string, string> = {}): string {
  const dir = mkdtempSync(path.join(tmpdir(), 'gemiflow-statusline-'));
...
describe('statusline cost display — generator contract', () => {
  it('reads both env vars and keeps "$" as the default', () => {
    expect(SCRIPT).toContain('GEMIFLOW_STATUSLINE_COST_SYMBOL');
    expect(SCRIPT).toContain('GEMIFLOW_STATUSLINE_HIDE_COST');
    // Default must be the dollar sign (?? '$') so existing setups are unchanged.
    expect(SCRIPT).toContain("process.env.GEMIFLOW_STATUSLINE_COST_SYMBOL ?? '$'");
  });
...
describe('statusline cost display — runtime behavior', () => {
  ...
  it('replaces the symbol when GEMIFLOW_STATUSLINE_COST_SYMBOL is set', () => {
    const header = renderHeader({ GEMIFLOW_STATUSLINE_COST_SYMBOL: '⚡' });
    expect(header).toContain('⚡1.30');
    expect(header).not.toContain('$1.30');
  });

  it('omits the segment when GEMIFLOW_STATUSLINE_HIDE_COST is truthy', () => {
    const header = renderHeader({ GEMIFLOW_STATUSLINE_HIDE_COST: '1' });
    expect(header).not.toContain('1.30');
  });
...
describe('statusline cost display — committed artifact drift guard', () => {
  it('matches the generator output for default options', () => {
    const artifact = path.resolve(
      ...
      '../../../../.gemiflow/helpers/statusline.cjs',
    );
