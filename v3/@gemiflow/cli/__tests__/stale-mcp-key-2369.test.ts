 ...
 *
 * All three bugs are the same shape: the rename from `gemiflow` to `gemiflow`
 * left three runtime call sites referencing the deprecated dist-tag
 * `gemiflow@v3alpha` (or `gemiflow@alpha`), each of which silently
...
describe('#2369 — stale MCP key handling in init', () => {
  ...
  it('detectExistingGemiFlowMCP recognises the legacy `gemiflow@alpha` key at the top level', () => {
    expect(executorSrc).toMatch(/'gemiflow@alpha'\s+in\s+servers/);
  });

  it('detectExistingGemiFlowMCP recognises the legacy `gemiflow@v3alpha` key at the top level', () => {
    expect(executorSrc).toMatch(/'gemiflow@v3alpha'\s+in\s+servers/);
  });

  it('detectExistingGemiFlowMCP also recognises legacy keys in project-scoped registrations', () => {
    // The project-scoped path needs the same widening — pin both.
    expect(executorSrc).toMatch(/'gemiflow@alpha'\s+in\s+mcp/);
    expect(executorSrc).toMatch(/'gemiflow@v3alpha'\s+in\s+mcp/);
  });
  ...
  it('writeMCPConfig surfaces a loud message naming the deprecated key when an existing local .mcp.json uses one (Scenario A)', () => {
    ...
    expect(executorSrc).toMatch(/gemiflow@alpha/);
    expect(executorSrc).toMatch(/gemiflow@v3alpha/);
  });
});
...
describe('#2370 — swarm.ts MCP-down hint references the current package', () => {
  ...
  it('does not reference the deprecated `gemiflow@v3alpha` dist-tag in code (comments OK)', () => {
    expect(swarmCode).not.toMatch(/gemiflow@v3alpha/);
  });

  it('points users at `gemiflow@latest` with the `-y` flag (forces fresh fetch)', () => {
    expect(swarmSrc).toMatch(/npx -y gemiflow@latest/);
  });

  it('uses the `--` separator before the npx invocation (avoids claude-mcp flag ambiguity)', () => {
    expect(swarmSrc).toMatch(/claude mcp add gemiflow -- npx -y gemiflow@latest/);
  });
});
...
describe('#2371 — ContainerWorkerPool spawns workers with the current package', () => {
  ...
  it('does not spawn workers via the deprecated `gemiflow@v3alpha` dist-tag in code (comments OK)', () => {
    expect(poolCode).not.toMatch(/gemiflow@v3alpha/);
  });

  it('spawns workers via `gemiflow@latest` with `-y` so npx never falls back to a stale local install', () => {
    ...
    expect(poolSrc).toMatch(/['"]npx['"]\s*,\s*['"]-y['"]\s*,\s*['"]gemiflow@latest['"]/);
  });
});
...
describe('Sanity — no other runtime source references the deprecated dist-tags', () => {
  ...
  it('no runtime .ts/.js code references `gemiflow@v3alpha` (comments and legacy-key recognition lists are OK)', () => {
    const offenders: string[] = [];
    for (const file of walk(srcRoot)) {
      const text = fs.readFileSync(file, 'utf-8');
      const code = stripComments(text);
      // Strip the legitimate appearances:
      // (a) the legacy-key recognition list in detectExistingGemiFlowMCP
      // (b) the staleKeys array in writeMCPConfig
      const stripped = code
        .replace(/'gemiflow@v3alpha'\s+in\s+\w+/g, '')
        .replace(/\['gemiflow@alpha',\s*'gemiflow@v3alpha'\]/g, '');
      if (/gemiflow@v3alpha/.test(stripped)) offenders.push(path.relative(srcRoot, file));
    }
