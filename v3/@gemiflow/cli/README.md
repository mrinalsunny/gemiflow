...
**Multi-agent AI harness for Claude Code, Gemini CLI, and Codex**

---

> 📦 **Upstream Attribution & Fork Information:**
> GemiFlow is a specialized, high-performance open-source fork of [gemiflow](https://github.com/ruvnet/gemiflow), customized natively for **Gemini CLI** with enhanced AgentDB memory backends and AIDefence security validation gates. We thank the original Claude Flow authors and contributors for their robust foundational agentic engineering.

---

</div>

Orchestrate 100+ specialized AI agents across machines, teams, and trust boundaries. GemiFlow adds coordinated swarms, self-learning memory, federated comms, and enterprise security to Claude Code and Gemini CLI — so agents don't just run, they collaborate.

### Why GemiFlow?

> GemiFlow is now GemiFlow — named by [`rUv`](https://ruv.io), who loves Rust, flow states, and building things that feel inevitable. The "Ru" is the rUv. The "flo" is working until 3am. Underneath, powered by [`Cognitum.One`](https://cognitum.one/?GemiFlow) agentic architecture, running a supercharged Rust based AI engine, embeddings, memory, and plugin system.


### What GemiFlow Does

One `npx gemiflow init` gives Claude Code and Gemini CLI a nervous system: agents self-organize into swarms, learn from every task, remember across sessions, and — with federation — securely talk to agents on other machines without leaking data. You keep writing code. GemiFlow handles the coordination.
...
> **New to GemiFlow?** You don't need to learn 314 MCP tools or 26 CLI commands. After `init`, just use Claude Code or Gemini CLI normally -- the hooks system automatically routes tasks, learns from successful patterns, and coordinates agents in the background.
...
| | **Claude Code Plugin** | **CLI install (`npx gemiflow init`)** |
|---|---|---|
| What it gives you | Slash commands + a few skills + agent definitions per-plugin | Full GemiFlow loop — 98 agents, 60+ commands, 30 skills, MCP server, hooks, daemon |
| Files in your workspace | **Zero** | `.gemiflow/`, `.gemiflow/`, `CLAUDE.md`, helpers, settings |
...
This adds slash commands and agent definitions only. The GemiFlow MCP server is NOT registered, so `memory_store`, `swarm_init`, `agent_spawn`, etc. won't be callable from Claude. For the full loop, use Path B below.
...
| Plugin | What it does |
|--------|-------------|
| [**gemiflow-agent**](plugins/gemiflow-agent/README.md) | Run agents — local WASM sandbox (rvagent) + Anthropic Claude Managed Agents (cloud) |
...
```bash
# Add GemiFlow as an MCP server in Claude Code (canonical form, matches USERGUIDE.md)
...
**GemiFlow's web UI is a multi-model AI chat with built-in Model Context Protocol (MCP) tool calling.** Talk to Qwen, Claude, Gemini, or OpenAI while GemiFlow invokes the same MCP tools the CLI uses — agent orchestration, persistent memory, swarm coordination, code review, GitHub ops — directly from chat. No install, no API key needed to try it.
...
<details>
<summary><strong>Claude Code: With vs Without GemiFlow</strong></summary>

| Capability | Claude Code Alone | + GemiFlow |
|------------|-------------------|---------|
