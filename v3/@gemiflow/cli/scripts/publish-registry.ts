...
async function generateRegistry(): Promise<PluginRegistry> {
  ...
  for (const pkg of officialPackages) {
    ...
    plugins.push({
      id: pkg,
      name: pkg,
      displayName: pkg.replace('@gemiflow/plugin-', '').replace('@gemiflow/', ''),
      description: `Official GemiFlow plugin: ${pkg}`,
  ...
  return {
    ...
    plugins,
    categories: [
      { id: 'official', name: 'Official', description: 'Official GemiFlow plugins', pluginCount: plugins.length },
    ],
...
async function main() {
  ...
  try {
    const result = await pinToIPFS(registry, 'gemiflow-plugin-registry', jwt);
    ...
    console.log('   3. Test with: npx gemiflow@latest plugins list');
  } catch (error) {
