...
```bash
# Create new project
gcloud projects create gemiflow-registry --name="GemiFlow Registry"

# Set as active project
gcloud config set project gemiflow-registry

# Enable billing (required for Cloud Functions)
gcloud beta billing projects link gemiflow-registry \
...
gcloud storage buckets create gs://gemiflow-plugin-registry \
  --location=US \
  --uniform-bucket-level-access

# Enable versioning for rollback
gsutil versioning set on gs://gemiflow-plugin-registry

# Make registry.json publicly readable (optional)
gsutil iam ch allUsers:objectViewer gs://gemiflow-plugin-registry
```
...
# Grant permissions
gcloud projects add-iam-policy-binding gemiflow-registry \
  --member="serviceAccount:plugin-registry-publisher@gemiflow-registry.iam.gserviceaccount.com" \
...
async function getSecret(name) {
  const [version] = await secretManager.accessSecretVersion({
    name: `projects/gemiflow-registry/secrets/${name}/versions/latest`,
  });
...
export async function publishRegistry(req, res) {
  try {
    ...
    const bucket = storage.bucket('gemiflow-plugin-registry');
...
gcloud functions deploy publish-registry \
  ...
  --service-account=plugin-registry-publisher@gemiflow-registry.iam.gserviceaccount.com \
  --set-env-vars=GCP_PROJECT=gemiflow-registry
```
...
# Upload to GCS
gsutil cp registry.json gs://gemiflow-plugin-registry/registry.json
```
...
steps:
  ...
  - name: 'gcr.io/cloud-builders/gsutil'
    args: ['cp', 'registry.json', 'gs://gemiflow-plugin-registry/registry.json']
...
substitutions:
  _FUNCTION_URL: 'https://us-central1-gemiflow-registry.cloudfunctions.net/publish-registry'
...
gcloud scheduler jobs create http publish-registry-daily \
  ...
  --schedule="0 2 * * *" \
  --uri="https://us-central1-gemiflow-registry.cloudfunctions.net/publish-registry" \
  --http-method=POST
```

## Step 6: Update GemiFlow CLI
