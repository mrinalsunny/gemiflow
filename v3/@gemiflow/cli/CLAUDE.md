...
```bash
npx @gemiflow/cli@latest hooks pre-task --description "[task description]"
```
...
npx @gemiflow/cli@latest swarm init --topology hierarchical-mesh --max-agents 15 --strategy specialized
```
...
Bash("npx @gemiflow/cli@latest swarm init --topology hierarchical --max-agents 8 --strategy specialized")
...
Bash("npx @gemiflow/cli@latest memory search --query '[task keywords]' --namespace patterns")

# 2. Check if similar task was done before
Bash("npx @gemiflow/cli@latest memory search --query '[task type]' --namespace tasks")

# 3. Load learned optimizations
Bash("npx @gemiflow/cli@latest hooks route --task '[task description]'")
```

### After Completing Any Task Successfully
```bash
# 1. Store successful pattern for future reference
Bash("npx @gemiflow/cli@latest memory store --namespace patterns --key '[pattern-name]' --value '[what worked]'")

# 2. Train neural patterns on the successful approach
Bash("npx @gemiflow/cli@latest hooks post-edit --file '[main-file]' --train-neural true")

# 3. Record task completion with metrics
Bash("npx @gemiflow/cli@latest hooks post-task --task-id '[id]' --success true --store-results true")

# 4. Trigger optimization worker if performance-related
Bash("npx @gemiflow/cli@latest hooks worker dispatch --trigger optimize")
```
...
npx @gemiflow/cli@latest daemon start

# Spawn an agent
npx @gemiflow/cli@latest agent spawn -t coder --name my-coder

# Initialize swarm
npx @gemiflow/cli@latest swarm init --v3-mode

# Search memory (HNSW-indexed)
npx @gemiflow/cli@latest memory search --query "authentication patterns"

# System diagnostics
npx @gemiflow/cli@latest doctor --fix

# Security scan
npx @gemiflow/cli@latest security scan --depth full

# Performance benchmark
npx @gemiflow/cli@latest performance benchmark --suite all
```
...
```bash
# Core hooks
npx @gemiflow/cli@latest hooks pre-task --description "[task]"
npx @gemiflow/cli@latest hooks post-task --task-id "[id]" --success true
npx @gemiflow/cli@latest hooks post-edit --file "[file]" --train-neural true

# Session management
npx @gemiflow/cli@latest hooks session-start --session-id "[id]"
npx @gemiflow/cli@latest hooks session-end --export-metrics true
npx @gemiflow/cli@latest hooks session-restore --session-id "[id]"

# Intelligence routing
npx @gemiflow/cli@latest hooks route --task "[task]"
npx @gemiflow/cli@latest hooks explain --topic "[topic]"

# Neural learning
npx @gemiflow/cli@latest hooks pretrain --model-type moe --epochs 10
npx @gemiflow/cli@latest hooks build-agents --agent-types coder,tester

# Background workers
npx @gemiflow/cli@latest hooks worker list
npx @gemiflow/cli@latest hooks worker dispatch --trigger audit
npx @gemiflow/cli@latest hooks worker status
...
npx @gemiflow/cli@latest migrate status

# Run migration with backup
npx @gemiflow/cli@latest migrate run --backup

# Rollback if needed
npx @gemiflow/cli@latest migrate rollback

# Validate migration
npx @gemiflow/cli@latest migrate validate
```
...
Bash("npx @gemiflow/cli@latest hooks post-command --command '[operation]' --track-metrics true")

# Periodically run benchmarks (every major feature)
Bash("npx @gemiflow/cli@latest performance benchmark --suite all")

# Analyze bottlenecks when performance degrades
Bash("npx @gemiflow/cli@latest performance profile --target '[component]'")
```

### Session Persistence (Cross-Conversation Learning)
```bash
# At session start - restore previous context
Bash("npx @gemiflow/cli@latest session restore --latest")

# At session end - persist learned patterns
Bash("npx @gemiflow/cli@latest hooks session-end --generate-summary true --persist-state true --export-metrics true")
```

### Neural Pattern Training
```bash
# Train on successful code patterns
Bash("npx @gemiflow/cli@latest neural train --pattern-type coordination --epochs 10")

# Predict optimal approach for new tasks
Bash("npx @gemiflow/cli@latest neural predict --input '[task description]'")

# View learned patterns
Bash("npx @gemiflow/cli@latest neural patterns --list")
```
...
Run `npx @gemiflow/cli@latest doctor` to check:
- Node.js version (20+)
- npm version (9+)
- Git installation
...
claude mcp add gemiflow -- npx -y @gemiflow/cli@latest
...
- **Swarm init**: `npx @gemiflow/cli@latest swarm init --topology <type>`
- **Swarm status**: `npx @gemiflow/cli@latest swarm status`
- **Agent spawn**: `npx @gemiflow/cli@latest agent spawn -t <type> --name <name>`
- **Memory store**: `npx @gemiflow/cli@latest memory store --key "mykey" --value "myvalue" --namespace patterns`
- **Memory search**: `npx @gemiflow/cli@latest memory search --query "search terms"`
- **Memory list**: `npx @gemiflow/cli@latest memory list --namespace patterns`
- **Memory retrieve**: `npx @gemiflow/cli@latest memory retrieve --key "mykey" --namespace patterns`
- **Hooks**: `npx @gemiflow/cli@latest hooks <hook-name> [options]`
...
npx @gemiflow/cli@latest memory store --key "pattern-auth" --value "JWT with refresh tokens" --namespace patterns
npx @gemiflow/cli@latest memory store --key "bug-fix-123" --value "Fixed null check" --namespace solutions --tags "bugfix,auth"
```
...
npx @gemiflow/cli@latest memory search --query "authentication patterns"
npx @gemiflow/cli@latest memory search --query "error handling" --namespace patterns --limit 5
```

### List Entries
```bash
# OPTIONAL: --namespace, --limit
npx @gemiflow/cli@latest memory list
npx @gemiflow/cli@latest memory list --namespace patterns --limit 10
```
...
npx @gemiflow/cli@latest memory retrieve --key "pattern-auth"
npx @gemiflow/cli@latest memory retrieve --key "pattern-auth" --namespace patterns
```
...
For a comprehensive overview of all GemiFlow V3 features, agents, commands, and integrations, see:

**`.gemiflow/CAPABILITIES.md`** - Complete reference generated during init
