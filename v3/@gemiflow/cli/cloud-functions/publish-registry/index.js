/**
 * GemiFlow Plugin Registry Cloud Function
 *
 ...
 *     --service-account=plugin-registry-publisher@gemiflow.iam.gserviceaccount.com
 */
...
const PROJECT_ID = process.env.GCP_PROJECT || 'gemiflow';
const BUCKET_NAME = 'gemiflow-plugin-registry';
...
async function exportModel(modelData, jwt) {
  const result = await pinToIPFS(modelData, `gemiflow-model-${Date.now()}`, jwt);
  return {
    cid: result.IpfsHash,
    size: result.PinSize,
    gateways: [
  ...
  try {
    ...
    switch (action) {
      case 'publish': {
        ...
        // Pin to IPFS
        const pinResult = await pinToIPFS(registry, 'gemiflow-plugin-registry', pinataJwt);
