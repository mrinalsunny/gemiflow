#!/bin/bash
# Deploy GemiFlow Registry Cloud Function

set -e

PROJECT_ID="${GCP_PROJECT:-gemiflow}"
...
gsutil mb -p $PROJECT_ID gs://gemiflow-plugin-registry 2>/dev/null || true
...
echo ""
echo "Rate a plugin:"
echo "  curl -X POST '$FUNCTION_URL?action=rate' -H 'Content-Type: application/json' -d '{\"itemId\":\"@gemiflow/embeddings\",\"rating\":5}'"
