...
1. **`src/config-adapter.ts`** (NEW)
   - Converts between `SystemConfig` (from `@gemiflow/shared`) and `V3Config` (CLI-specific format)
...
- `GEMIFLOW_MAX_AGENTS` - Maximum concurrent agents
- `GEMIFLOW_DATA_DIR` - Data directory path
- `GEMIFLOW_MEMORY_TYPE` - Memory backend type
- `GEMIFLOW_MCP_TRANSPORT` - MCP transport type
- `GEMIFLOW_MCP_PORT` - MCP server port
- `GEMIFLOW_SWARM_TOPOLOGY` - Swarm topology type
...
gemiflow agent spawn -t coder

# Use specific config file
gemiflow agent spawn -t coder --config ./custom-config.json

# Override with environment variables
GEMIFLOW_MAX_AGENTS=20 gemiflow swarm init
```
...
3. **Validation** - Uses existing Zod schemas from `@gemiflow/shared`
...
- [ ] Config validation command (`gemiflow config validate`)
