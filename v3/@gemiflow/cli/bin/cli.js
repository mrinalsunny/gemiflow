#!/usr/bin/env node
/**
 * @gemiflow/cli - CLI Entry Point
 *
 * GemiFlow V3 Command Line Interface
 *
 * Auto-detects MCP mode when stdin is piped and no args provided.
 * This allows: echo '{"jsonrpc":"2.0",...}' | npx @gemiflow/cli
 */
...
// version directly from package.json and exit before any heavy import.
{
  ...
  if (_argv.length === 1 && (_argv[0] === '--version' || _argv[0] === '-V')) {
    try {
      ...
      process.stdout.write(`gemiflow v${pkg.version || '0.0.0'}\n`);
    } catch {
      process.stdout.write('gemiflow v0.0.0\n');
    }
    process.exit(0);
  }
...
//   2. stdin is being piped AND args are "mcp start" (explicit, e.g. npx gemiflow@alpha mcp start)
