#!/usr/bin/env node
/**
 * @gemiflow/cli - MCP Server Entry Point
 *
 * Direct stdio MCP server for Gemini CLI integration.
  ...
  for (const line of lines) {
    if (line.trim()) {
      try {
        ...
        console.error(
          `[${new Date().toISOString()}] ERROR [gemiflow-mcp] Failed to parse:`,
...
process.on('SIGINT', () => {
  console.error(`[${new Date().toISOString()}] INFO [gemiflow-mcp] Received SIGINT`);
  process.exit(0);
});

process.on('SIGTERM', () => {
  console.error(`[${new Date().toISOString()}] INFO [gemiflow-mcp] Received SIGTERM`);
  process.exit(0);
});
  ...
  try {
    switch (message.method) {
      case 'initialize':
        return {
          ...
          result: {
            ...
            serverInfo: { name: 'gemiflow', version: VERSION },
            capabilities: {
      ...
      case 'notifications/initialized':
        console.error(`[${new Date().toISOString()}] INFO [gemiflow-mcp] Client initialized`);
    ...
    console.error(`[${new Date().toISOString()}] ERROR [gemiflow-mcp] ${message.method}:`, error);
    return {
      jsonrpc: '2.0',
      id: message.id,
      error: {
