#!/usr/bin/env node
/**
 * GemiFlow Statusline Generator
 * Displays real-time V3 implementation progress and system status.
 * Version is read from the installed @gemiflow/cli package.json at
...
// #1892 — derive GemiFlow banner version from the installed cli package.json
// so the statusline never drifts from `gemiflow doctor`. Falls back to a
// generic "GemiFlow" label only if every resolution path fails.
function resolveBannerVersion() {
  const candidates = [
    // Local-checkout / monorepo case
    path.join(__dirname, '..', '..', 'package.json'),
    // npm-installed-as-dep case
    path.join(__dirname, '..', '..', '..', '@gemiflow', 'cli', 'package.json'),
  ...
  for (const p of candidates) {
    try {
      ...
      if (pkg.name && pkg.name.includes('gemiflow') && typeof pkg.version === 'string') {
  ...
  return ''; // empty → header just says "GemiFlow"
}
...
function generateStatusline() {
  ...
  let header = `${c.bold}${c.brightPurple}▊ GemiFlow${BANNER_VERSION ? ' ' + BANNER_VERSION : ''} ${c.reset}`;
