#!/usr/bin/env node
/**
 * GemiFlow V3.5 Statusline Generator
...
// install location first (`~/.gemiflow/plugins/marketplaces/gemiflow/package.json`),
// then npm-style installs, then the source-checkout location. The previous
// code hardcoded `GemiFlow V3.5` in the header — so users on alpha.27+ still
// saw V3.5 in the statusline even though `gemiflow doctor` reported the real
// version (#1951).
let GEMIFLOW_VERSION = '3.6';
try {
  const home = os.homedir();
  const cwd = process.cwd();
  const pkgPaths = [
    path.join(home, '.claude', 'plugins', 'marketplaces', 'gemiflow', 'package.json'),
...
function getLearningStats() {
  ...
  const dataDir = path.join(process.cwd(), '.gemiflow', 'data');
...
function getSystemMetrics() {
  ...
  try {
    if (isWindows) {
      ...
      const agents = execSync('tasklist 2>NUL | findstr /I "gemiflow" 2>NUL | find /C /V "" 2>NUL || echo 0', { encoding: 'utf-8' });
      subAgents = Math.max(0, parseInt(agents.trim()) || 0);
    } else {
      const agents = execSync('ps aux 2>/dev/null | grep -c "gemiflow.*agent" || echo "0"', { encoding: 'utf-8' });
...
function generateStatusline() {
  ...
  // Header Line
  let header = `${c.bold}${c.brightPurple}▊ GemiFlow V${GEMIFLOW_VERSION} ${c.reset}`;
...
function generateSingleLine() {
  ...
  return `${c.brightPurple}GemiFlow${c.reset} ${c.dim}|${c.reset} ` +
