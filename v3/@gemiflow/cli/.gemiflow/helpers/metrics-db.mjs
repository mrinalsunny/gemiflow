...
const DB_PATH = join(PROJECT_ROOT, '.gemiflow', 'metrics.db');
...
function checkSecurityFile(filename, minLines = 100) {
  const filePath = join(V3_DIR, '@gemiflow/security/src', filename);
...
async function syncMetrics() {
  ...
  if (existsSync(modulesDir)) {
    ...
    for (const entry of entries) {
      // Skip hidden directories (like .agentic-flow, .gemiflow)
...
function exportToJSON() {
  ...
  const securityDir = join(PROJECT_ROOT, '.gemiflow/security');
