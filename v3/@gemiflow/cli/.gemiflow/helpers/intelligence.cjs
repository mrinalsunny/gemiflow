 ...
 * the hook system. Pure CJS — no ESM imports of @gemiflow/memory.
 *
 * Data files (all under .gemiflow/data/):
 ...
 * on @gemiflow/memory for the initial seed.
 */
function bootstrapFromMemoryFiles() {
  ...
  const candidates = [
    // Gemini CLI auto-memory (project-scoped)
    path.join(require('os').homedir(), '.claude', 'projects'),
    // Local project memory
    path.join(cwd, '.gemiflow', 'memory'),
