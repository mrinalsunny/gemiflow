...
async function loadMemoryPackage() {
  ...
  const localDist = join(PROJECT_ROOT, 'v3/@gemiflow/memory/dist/index.js');
  ...
  // Strategy 3: ESM import (works when @gemiflow/memory is a direct dependency)
  try {
    return await import('@gemiflow/memory');
  } catch { /* fall through */ }

  // Strategy 4: Walk up from PROJECT_ROOT looking for @gemiflow/memory in any node_modules
...
// ============================================================================
// Read config from .gemiflow/config.yaml
// ============================================================================

function readConfig() {
  const configPath = join(PROJECT_ROOT, '.gemiflow', 'config.yaml');
  const defaults = {
