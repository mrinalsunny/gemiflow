...
pre_edit_checkpoint() {
    ...
    if [ -n "$file" ]; then
        ...
        # Store metadata
        mkdir -p .gemiflow/checkpoints
        cat > ".gemiflow/checkpoints/$(date +%s).json" <<EOF
{
...
task_checkpoint() {
    ...
    if [ -n "$task" ]; then
        ...
        # Store metadata
        mkdir -p .gemiflow/checkpoints
        cat > ".gemiflow/checkpoints/task-$(date +%s).json" <<EOF
{
...
session_end_checkpoint() {
    ...
    local summary_file=".gemiflow/checkpoints/summary-$session_id.md"
...
## Checkpoints Created
$(find .gemiflow/checkpoints -name '*.json' -mtime -1 -exec basename {} \; | sort)
