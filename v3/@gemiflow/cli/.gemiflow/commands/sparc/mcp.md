...
npx gemiflow sparc run mcp "integrate with external API"

# For alpha features
npx gemiflow@alpha sparc run mcp "integrate with external API"

# With namespace
npx gemiflow sparc run mcp "your task" --namespace mcp

# Non-interactive mode
npx gemiflow sparc run mcp "your task" --non-interactive
```

### Option 3: Local Installation
```bash
# If gemiflow is installed locally
./gemiflow sparc run mcp "integrate with external API"
```
...
```bash
# Store mode-specific context
npx gemiflow memory store "mcp_context" "important decisions" --namespace mcp

# Query previous work
npx gemiflow memory query "mcp" --limit 5
```
