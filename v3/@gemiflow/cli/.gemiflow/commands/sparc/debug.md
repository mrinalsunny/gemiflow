...
npx gemiflow sparc run debug "fix memory leak in service"

# For alpha features
npx gemiflow@alpha sparc run debug "fix memory leak in service"

# With namespace
npx gemiflow sparc run debug "your task" --namespace debug

# Non-interactive mode
npx gemiflow sparc run debug "your task" --non-interactive
```

### Option 3: Local Installation
```bash
# If gemiflow is installed locally
./gemiflow sparc run debug "fix memory leak in service"
```
...
```bash
# Store mode-specific context
npx gemiflow memory store "debug_context" "important decisions" --namespace debug

# Query previous work
npx gemiflow memory query "debug" --limit 5
```
