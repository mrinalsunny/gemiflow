...
gemiflow truth --period 7d

# Export to HTML report
gemiflow truth --export report.html --format html
```

### Advanced Analysis
```bash
# Monitor real-time scores
gemiflow truth --watch

# Find problematic files
gemiflow truth --threshold 0.8

# Agent-specific metrics
gemiflow truth --agent coder --period 24h

# JSON for processing
gemiflow truth --format json | jq '.overall_score'
```
...
- name: Check Truth Scores
  run: |
    gemiflow truth --format json > truth.json
...
Set truth display preferences in `.gemiflow/config.json`:
