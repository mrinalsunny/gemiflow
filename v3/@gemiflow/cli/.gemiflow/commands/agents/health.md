...
```bash
npx gemiflow agent health [agent-id] [options]
```
...
npx gemiflow agent health coder-lx7m9k2

# Continuous monitoring
npx gemiflow agent health --watch

# Custom interval
npx gemiflow agent health -w -i 10

# JSON output
npx gemiflow agent health --format json
```
...
- `npx gemiflow agent status` - Detailed agent info
- `npx gemiflow agent metrics` - Performance metrics
- `npx gemiflow doctor` - System-wide health
