...
List all active agents in the GemiFlow system with filtering options.

## Usage

```bash
npx gemiflow agent list [options]
npx gemiflow agent ls [options]  # Alias
```
...
npx gemiflow agent list

# List all agents including inactive
npx gemiflow agent list --all

# Filter by type
npx gemiflow agent list -t coder

# Filter by status
npx gemiflow agent list -s idle

# JSON output for scripting
npx gemiflow agent list --format json

# Combined filters
npx gemiflow agent list -t researcher -s active
```
