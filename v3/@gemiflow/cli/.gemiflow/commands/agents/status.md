...
```bash
npx gemiflow agent status <agent-id>
npx gemiflow agent status --id <agent-id>
```
...
```bash
# Get status by ID
npx gemiflow agent status coder-lx7m9k2

# Using --id flag
npx gemiflow agent status --id researcher-abc123

# JSON output
npx gemiflow agent status coder-lx7m9k2 --format json
```
...
- `npx gemiflow agent metrics` - Aggregate metrics
- `npx gemiflow agent health` - Health check
- `npx gemiflow agent logs <id>` - Activity logs
