...
```bash
npx gemiflow agent metrics [agent-id] [options]
```
...
npx gemiflow agent metrics coder-lx7m9k2

# Last hour
npx gemiflow agent metrics -p 1h

# Last 7 days
npx gemiflow agent metrics --period 7d

# JSON output
npx gemiflow agent metrics --format json
```
...
- `npx gemiflow agent status` - Individual agent metrics
- `npx gemiflow performance benchmark` - Full performance suite
- `npx gemiflow status` - System-wide status
