...
npx gemiflow agent pool

# Set pool size
npx gemiflow agent pool --size 5

# Configure auto-scaling
npx gemiflow agent pool --min 2 --max 15 --auto-scale

# Pre-warm agents for fast response
npx gemiflow agent pool --warmup

# Disable auto-scaling
npx gemiflow agent pool --auto-scale false
```
...
Configure in `.gemiflow/config.yaml`:
...
- `npx gemiflow agent spawn` - Manual agent spawning
- `npx gemiflow agent list` - View active agents
- `npx gemiflow swarm scale` - Swarm-level scaling
