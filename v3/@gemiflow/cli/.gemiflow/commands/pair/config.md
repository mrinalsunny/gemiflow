...
Main configuration file: `.gemiflow/pair-config.json`
...
gemiflow pair config reset verification
```
...
### Project-Specific
`.gemiflow/pair-config.json` in project root

### User-Specific
`~/.gemiflow/pair-config.json`

### Global
`/etc/gemiflow/pair-config.json`
...
gemiflow pair config validate

# Test configuration
gemiflow pair config test
```
...
### From Version 1.x
```bash
gemiflow pair config migrate --from 1.x
```

### Export/Import
```bash
# Export current config
gemiflow pair config export > my-config.json

# Import config
gemiflow pair config import my-config.json
```
