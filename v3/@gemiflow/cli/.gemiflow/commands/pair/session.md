...
```bash
gemiflow pair --switch <session-id>
```

### Merge Sessions
```bash
gemiflow pair --merge <session-1> <session-2>
```
...
### Manual Recovery
```bash
gemiflow pair --recover [--point <timestamp>]
```

### Crash Recovery
```bash
gemiflow pair --crash-recovery
```
...
### Share with Team
```bash
gemiflow pair --share <session-id> \
  --team <team-id>
```

### Export for Review
```bash
gemiflow pair --export-review <session-id>
```

### Create Learning Material
```bash
gemiflow pair --create-tutorial <session-id>
```
