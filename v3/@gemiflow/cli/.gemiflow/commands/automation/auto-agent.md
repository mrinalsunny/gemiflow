...
```bash
npx gemiflow auto agent [options]
```
...
```bash
npx gemiflow auto agent --task "Build a REST API with authentication"
```

### Constrained spawning

```bash
npx gemiflow auto agent -t "Debug performance issue" --max-agents 3
```

### Analysis only

```bash
npx gemiflow auto agent -t "Refactor codebase" --no-spawn
```

### Minimal strategy

```bash
npx gemiflow auto agent -t "Fix bug in login" -s minimal
```
