---
name: sparc
description: Execute SPARC methodology workflows with GemiFlow
---
...
```bash
# Run SPARC orchestrator (default)
npx gemiflow sparc "build complete authentication system"

# Run a specific mode
npx gemiflow sparc run architect "design API structure"
npx gemiflow sparc run tdd "implement user service"

# Execute full TDD workflow
npx gemiflow sparc tdd "implement user authentication"

# List all modes with details
npx gemiflow sparc modes --verbose

# For alpha features
npx gemiflow@alpha sparc run <mode> "your task"
```

### Option 3: Local Installation
```bash
# If gemiflow is installed locally
./gemiflow sparc "build complete authentication system"
./gemiflow sparc run architect "design API structure"
```
...
```bash
# Store specifications
npx gemiflow memory store "spec_auth" "OAuth2 + JWT requirements" --namespace spec

# Store architectural decisions
./gemiflow memory store "arch_api" "RESTful microservices design" --namespace arch

# Query previous work
./gemiflow memory query "authentication" --limit 10

# Export project memory
./gemiflow memory export sparc-project-backup.json
```
...
./gemiflow swarm "Build e-commerce platform" --strategy development --monitor --review

# Background optimization swarm
./gemiflow swarm "Optimize system performance" --strategy optimization --background

# Distributed research swarm
./gemiflow swarm "Analyze market trends" --strategy research --distributed --ui
```
...
```bash
./gemiflow sparc run code "implement API" --non-interactive
./gemiflow sparc tdd "user tests" --non-interactive --enable-permissions
```
...
See `/gemiflow-help` for all available commands.
