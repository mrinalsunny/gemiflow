...
```bash
gemiflow stream-chain pipeline analysis
```
...
```bash
gemiflow stream-chain run <prompt1> <prompt2> [...] [options]
```
...
```bash
gemiflow stream-chain pipeline <type> [options]
```
...
```bash
gemiflow stream-chain pipeline refactor
```
...
```bash
gemiflow stream-chain pipeline test
```
...
```bash
gemiflow stream-chain pipeline optimize
```
...
```bash
gemiflow stream-chain pipeline refactor --timeout 60 --verbose
```

#### Debug Test Generation

```bash
gemiflow stream-chain pipeline test --debug
```

#### Comprehensive Optimization

```bash
gemiflow stream-chain pipeline optimize --timeout 90 --verbose
```
...
Define reusable pipelines in `.gemiflow/config.json`:
...
```bash
gemiflow stream-chain pipeline security
gemiflow stream-chain pipeline documentation
```
...
gemiflow swarm init --topology mesh
...
# Results stored in .gemiflow/memory/stream-chain/
```
...
gemiflow stream-chain pipeline optimize --debug
...
```bash
gemiflow stream-chain run "complex task" --timeout 120
```
...
```bash
gemiflow stream-chain run "step 1" "step 2" --debug
```
...
cat .gemiflow/config.json | grep -A 10 "streamChain"
```
