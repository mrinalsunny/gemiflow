...
```bash
# Check status
gemiflow pair --status

# View history
gemiflow pair --history
...
```bash
gemiflow pair --start --mode driver
```
...
```bash
gemiflow pair --start --mode navigator
```
...
gemiflow pair --start --mode switch

# 5-minute intervals (rapid)
gemiflow pair --start --mode switch --interval 5m

# 15-minute intervals (deep focus)
gemiflow pair --start --mode switch --interval 15m
```
...
#### Basic Configuration
Create `.gemiflow/pair-config.json`:
...
```bash
# Set configuration
gemiflow pair config set defaultMode switch
gemiflow pair config set verification.threshold 0.98

# Get configuration
gemiflow pair config get
gemiflow pair config get defaultMode

# Export/Import
gemiflow pair config export > config.json
gemiflow pair config import config.json

# Reset
gemiflow pair config reset
```
...
```bash
# Create profile
gemiflow pair profile create refactoring \
  --mode driver \
  --verify true \
  --threshold 0.98 \
  --focus refactor
...
# Use profile
gemiflow pair --start --profile refactoring

# List profiles
gemiflow pair profile list
```
...
gemiflow pair --template refactor
...
# Feature template
gemiflow pair --template feature
...
```bash
# Save session
gemiflow pair --save [--name <name>]

# Load session
gemiflow pair --load <session-id>

# Export session
gemiflow pair --export <session-id> [--format json|md]

# Generate report
gemiflow pair --report <session-id>
```
...
```bash
# Start in background
gemiflow pair --start --background

# Monitor background session
gemiflow pair --monitor

# Attach to background session
gemiflow pair --attach <session-id>

# End background session
gemiflow pair --end <session-id>
```
...
gemiflow pair --start --record

# Replay session
gemiflow pair --replay <session-id>

# Session analytics
gemiflow pair --analytics <session-id>
```
...
**With Git:**
```bash
gemiflow pair --start --git --auto-commit
```

**With CI/CD:**
```bash
gemiflow pair --start --ci --non-interactive
```

**With IDE:**
```bash
gemiflow pair --start --ide vscode
```
...
- Run `gemiflow pair config validate`
...
- `gemiflow pair --help` - Show help
- `gemiflow pair config` - Manage configuration
- `gemiflow pair profile` - Manage profiles
- `gemiflow pair templates` - List templates
- `gemiflow pair agents` - List available agents
