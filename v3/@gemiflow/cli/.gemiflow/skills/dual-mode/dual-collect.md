...
```bash
# List all results
npx gemiflow@v3alpha memory list --namespace {{namespace}}

# Search for specific patterns
npx gemiflow@v3alpha memory search -q "{{filter}}" -n {{namespace}}

# Get detailed entries
{{#each results}}
npx gemiflow@v3alpha memory get -k "{{this.key}}" -n {{namespace}}
{{/each}}
```
