...
node plugins/gemiflow-core/scripts/test-hooks.mjs "node $PWD/v3/@gemiflow/cli/bin/cli.js"
...
npx gemiflow@alpha truth --export .gemiflow/metrics/truth-$(date +%Y%m%d).json
```
...
Set verification preferences in `.gemiflow/config.json`:
...
```json
{
  ...
  "truth": {
    ...
    "autoExport": {
      "enabled": true,
      "path": ".gemiflow/metrics/truth-daily.json"
    }
  }
}
```
...
npx gemiflow@alpha truth --format json | \
  curl -X POST https://pushgateway.example.com/metrics/job/gemiflow \
  -d @-
