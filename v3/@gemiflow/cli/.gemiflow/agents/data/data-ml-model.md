...
hooks:
  pre_execution: |
    ...
    SIMILAR_MODELS=$(npx gemiflow@alpha memory search-patterns "ML training: $TASK" --k=5 --min-reward=0.8 2>/dev/null || echo "")
    if [ -n "$SIMILAR_MODELS" ]; then
      echo "📚 Found similar successful model training patterns"
      npx gemiflow@alpha memory get-pattern-stats "ML training" --k=5 2>/dev/null || true
    fi
