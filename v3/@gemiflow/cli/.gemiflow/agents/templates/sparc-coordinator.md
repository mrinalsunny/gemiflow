...
hooks:
  pre: |
    ...
    PAST_CYCLES=$(npx gemiflow@alpha memory search-patterns "sparc-cycle: $TASK" --k=5 --min-reward=0.85 2>/dev/null || echo "")
    if [ -n "$PAST_CYCLES" ]; then
      echo "📚 Found ${PAST_CYCLES} successful SPARC cycles - applying learned patterns"
      npx gemiflow@alpha memory get-pattern-stats "sparc-cycle: $TASK" --k=5 2>/dev/null || true
    fi
