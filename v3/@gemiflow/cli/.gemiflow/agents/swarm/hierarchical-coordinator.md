...
hooks:
  pre: |
    ...
    mcp__gemiflow__memory_usage store "swarm:hierarchy:${TASK_ID}" "$(date): Hierarchical coordination started" --namespace=swarm
    # Set up monitoring
  ...
  post: |
    ...
    mcp__gemiflow__memory_usage store "swarm:hierarchy:${TASK_ID}:complete" "$(date): Task completed with $(mcp__gemiflow__swarm_status | jq '.agents.total') agents"
    # Cleanup resources
