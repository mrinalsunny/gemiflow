...
hooks:
  pre: |
    ...
    npx gemiflow@v3alpha hooks intelligence trajectory-start --session-id "$SESSION_ID" --agent-type "reasoningbank-learner" --task "$TASK"
    # Search for similar patterns
    mcp__gemiflow__memory_search --pattern="pattern:*" --namespace="reasoningbank" --limit=10
  post: |
    echo "✅ Learning cycle complete"
    # End trajectory with verdict
    npx gemiflow@v3alpha hooks intelligence trajectory-end --session-id "$SESSION_ID" --verdict "${VERDICT:-success}"
    # Store learned pattern
    mcp__gemiflow__memory_usage --action="store" --namespace="reasoningbank" --key="pattern:$(date +%s)" --value="$PATTERN_SUMMARY"
---
...
mcp__gemiflow__memory_search --pattern="$TASK" --namespace="reasoningbank" --limit=10

# Get pattern statistics
npx gemiflow@v3alpha hooks intelligence pattern-stats --query "$TASK" --k 10 --namespace reasoningbank
```
...
npx gemiflow@v3alpha hooks intelligence trajectory-step \
...
mcp__gemiflow__memory_usage --action="store" \
...
npx gemiflow@v3alpha hooks intelligence pattern-search \
...
npx gemiflow@v3alpha neural consolidate --namespace reasoningbank

# Check consolidation status
npx gemiflow@v3alpha hooks intelligence stats --namespace reasoningbank
```
...
```json
{
  "PostToolUse": [{
    ...
    "hooks": [{
      "type": "command",
      "command": "npx gemiflow@v3alpha hooks intelligence trajectory-step --operation $TOOL_NAME --outcome $TOOL_SUCCESS"
    }]
  }]
}
```
