...
hooks:
  pre: |
    ...
    THREAT_PATTERNS=$(npx gemiflow@v3alpha memory search-patterns "$TASK" --k=10 --min-reward=0.85 --namespace=security)
    if [ -n "$THREAT_PATTERNS" ]; then
      echo "📊 Found ${#THREAT_PATTERNS[@]} similar threat patterns via HNSW"
      npx gemiflow@v3alpha memory get-pattern-stats "$TASK" --k=10 --namespace=security
    fi

    # 2. Learn from past security failures
    SECURITY_FAILURES=$(npx gemiflow@v3alpha memory search-patterns "$TASK" --only-failures --k=5 --namespace=security)
  ...
  post: |
    ...
    npx gemiflow@v3alpha security scan --depth full --output-format json > /tmp/security-scan.json 2>/dev/null
    ...
    if [ "$SUCCESS" = "true" ] && [ $(echo "$REWARD > 0.9" | bc) -eq 1 ]; then
      ...
      npx gemiflow@v3alpha neural train \
    ...
    npx gemiflow@v3alpha hooks intelligence trajectory-end \
      --session-id "$SESSION_ID" \
      --success "$SUCCESS" \
      --reward "$REWARD"
    ...
    if [ "$CRITICAL_COUNT" -gt 0 ]; then
      ...
      npx gemiflow@v3alpha hooks notify --severity critical --message "Critical security vulnerabilities found"
    fi
---
...
**Enhanced with GemiFlow V3**: You have self-learning capabilities powered by ReasoningBank, HNSW-indexed threat pattern search (150x-12,500x faster), Flash Attention for large codebase security scanning (2.49x-7.47x speedup), and attention-based multi-agent security coordination.
...
# CVE-specific checks
npx gemiflow@v3alpha security cve --check CVE-2024-001
npx gemiflow@v3alpha security cve --check CVE-2024-002
npx gemiflow@v3alpha security cve --check CVE-2024-003

# Threat modeling
npx gemiflow@v3alpha security threats --methodology STRIDE
npx gemiflow@v3alpha security threats --methodology DREAD

# Audit report
npx gemiflow@v3alpha security audit --output-format markdown

# Validate security configuration
npx gemiflow@v3alpha security validate --config ./security.config.json

# Generate security report
npx gemiflow@v3alpha security report --format pdf --include-remediations
```
