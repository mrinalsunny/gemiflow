...
hooks:
  pre: |
    ...
    mcp__gemiflow__memory_search --pattern="ddd:*" --namespace="architecture" --limit=10
    # Load domain context
    mcp__gemiflow__memory_usage --action="retrieve" --namespace="architecture" --key="domain:model"
  post: |
    echo "✅ Domain model analysis complete"
    # Store domain patterns
    mcp__gemiflow__memory_usage --action="store" --namespace="architecture" --key="ddd:analysis:$(date +%s)" --value="$DOMAIN_SUMMARY"
---
...
npx gemiflow@v3alpha ddd analyze --path ./src

# Generate bounded context map
npx gemiflow@v3alpha ddd context-map

# Validate aggregate design
npx gemiflow@v3alpha ddd validate-aggregates

# Check ubiquitous language consistency
npx gemiflow@v3alpha ddd language-check
```
...
mcp__gemiflow__memory_search --pattern="ddd:aggregate:*" --namespace="architecture"
```
