...
hooks:
  pre_task: |
    ...
    npx gemiflow@v3alpha hook pre-task --mode release-swarm --init-swarm
  post_edit: |
    echo "🔄 Synchronizing release swarm state and validating changes..."
    npx gemiflow@v3alpha hook post-edit --mode release-swarm --sync-swarm
  post_task: |
    echo "🎯 Release swarm task completed. Coordinating final deployment..."
    npx gemiflow@v3alpha hook post-task --mode release-swarm --finalize-release
  notification: |
    echo "📡 Broadcasting release completion across all swarm agents..."
    npx gemiflow@v3alpha hook notification --mode release-swarm --broadcast
---
...
npx gemiflow@v3alpha github release-plan \
  --commits "$COMMITS" \
...
npx gemiflow@v3alpha github release-version \
  --strategy "semantic" \
  --analyze-changes \
  --check-breaking \
  --update-files
...
CHANGELOG=$(gh api repos/:owner/:repo/compare/${LAST_TAG}...HEAD \
  ...
  npx gemiflow@v3alpha github generate-changelog)
...
npx gemiflow@v3alpha github release-create \
  --version "2.0.0" \
  --changelog "$CHANGELOG" \
  --build-artifacts \
...
CHANGELOG=$(npx gemiflow@v3alpha github changelog \
  --prs "$PRS" \
  --commits "$COMMITS" \
...
npx gemiflow@v3alpha github version-suggest \
  --current v1.2.3 \
  --analyze-commits \
  --check-compatibility \
  --suggest-pre-release
...
npx gemiflow@v3alpha github release-build \
...
```bash
# Pre-release testing
npx gemiflow@v3alpha github release-test \
...
npx gemiflow@v3alpha github release-deploy \
...
npx gemiflow@v3alpha github multi-release \
...
npx gemiflow@v3alpha github hotfix \
  --issue 789 \
    ...
    steps:
      ...
      - name: Initialize Release Swarm
        run: |
          ...
          npx gemiflow@v3alpha github release-init \
            --tag $RELEASE_TAG \
      ...
      - name: Generate Release Assets
        run: |
          ...
          CHANGELOG=$(npx gemiflow@v3alpha github release-changelog \
            --format markdown)
          ...
          npx gemiflow@v3alpha github release-assets \
            --changelog \
            --binaries \
            --documentation
      ...
      - name: Publish Release
        run: |
          ...
          npx gemiflow@v3alpha github release-publish \
            --platforms all
...
npx gemiflow@v3alpha github cd-pipeline \
...
npx gemiflow@v3alpha github release-validate \
  --checks "
    version-conflicts,
...
npx gemiflow@v3alpha github compat-test \
...
```bash
# Security validation
npx gemiflow@v3alpha github release-security \
  --scan-dependencies \
  --check-secrets \
  --audit-permissions \
  --sign-artifacts
...
npx gemiflow@v3alpha github release-monitor \
  --version v2.0.0 \
...
npx gemiflow@v3alpha github rollback-config \
  --triggers '{
    "error-rate": ">5%",
    "latency-p99": ">1000ms",
    "availability": "<99.9%"
...
npx gemiflow@v3alpha github release-analytics \
  --version v2.0.0 \
  --compare-with v1.9.0 \
...
npx gemiflow@v3alpha github release-docs \
  --api-changes \
  --migration-guide \
  --example-updates \
...
```bash
# NPM package release
npx gemiflow@v3alpha github npm-release \
  --version patch \
  --test-all \
  --publish-beta \
...
npx gemiflow@v3alpha github docker-release \
...
npx gemiflow@v3alpha github mobile-release \
...
```bash
# Emergency hotfix
npx gemiflow@v3alpha github emergency-release \
  --severity critical \
...
```bash
# Immediate rollback
npx gemiflow@v3alpha github rollback \
  --to-version v1.9.9 \
