...
npx gemiflow@v3alpha github issue-to-swarm 456 \
...
npx gemiflow@v3alpha github issues-batch \
  --issues "$ISSUES" \
  --parallel
...
npx gemiflow@v3alpha github issue-analyze 456 \
  --suggest-agents \
  --estimate-complexity \
  --create-subtasks
```
...
# Initialize swarm
npx gemiflow@v3alpha github issue-init 456 \
  --issue-data "$ISSUE" \
...
SUBTASKS=$(npx gemiflow@v3alpha github issue-decompose 456 \
  --body "$ISSUE_BODY" \
  --max-subtasks 10 \
  --assign-priorities)
...
# Get swarm progress
PROGRESS=$(npx gemiflow@v3alpha github issue-progress 456)

# Update checklist in issue body
UPDATED_BODY=$(echo "$CURRENT" | jq -r '.body' | \
  npx gemiflow@v3alpha github update-checklist --progress "$PROGRESS")
...
npx gemiflow@v3alpha github issue-deps 456 \
  --resolve-order \
  --parallel-safe \
  --update-blocking
```
...
npx gemiflow@v3alpha github epic-swarm \
  --epic 123 \
...
npx gemiflow@v3alpha github create-issues \
  --from-analysis \
  --template "bug-report" \
  --auto-assign
```
        ...
        with:
          command: |
            if [[ "${{ github.event.label.name }}" == "swarm-ready" ]]; then
              npx gemiflow@v3alpha github issue-init ${{ github.event.issue.number }}
            fi
```
...
npx gemiflow@v3alpha github issue-board-sync \
  --project "Development" \
  --column-mapping '{
    "To Do": "pending",
    "In Progress": "active",
...
npx gemiflow@v3alpha github bug-swarm 456 \
  --reproduce \
  --isolate \
  --fix \
  --test
...
npx gemiflow@v3alpha github feature-swarm 456 \
  --design \
  --implement \
  --document \
  --demo
...
```bash
# Refactoring swarm
npx gemiflow@v3alpha github debt-swarm 456 \
  --analyze-impact \
  --plan-migration \
  --execute \
  --validate
...
echo "$STALE_ISSUES" | jq -r '.number' | while read -r num; do
  ...
  # Analyze with swarm
  ACTION=$(npx gemiflow@v3alpha github analyze-stale \
    --issue "$ISSUE" \
    --suggest-action)
...
npx gemiflow@v3alpha github triage \
  --unlabeled \
  --analyze-content \
  --suggest-labels \
  --assign-priority
...
npx gemiflow@v3alpha github find-duplicates \
  --threshold 0.8 \
  --link-related \
  --close-duplicates
```
...
npx gemiflow@v3alpha github link-pr \
  --issue 456 \
  --pr 789 \
  --update-both
```
...
npx gemiflow@v3alpha github milestone-swarm \
  --milestone "v2.0" \
  --parallel-issues \
  --track-progress
```
...
npx gemiflow@v3alpha github cross-repo \
  --issue "org/repo#456" \
...
npx gemiflow@v3alpha github issue-metrics \
  --issue 456 \
...
npx gemiflow@v3alpha github effectiveness \
...
npx gemiflow@v3alpha github issue-init 789 \
...
npx gemiflow@v3alpha github issue-init 234 \
  --topology mesh \
...
npx gemiflow@v3alpha github issue-init 567 \
  --topology ring \
