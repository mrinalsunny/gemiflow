...
tools:
  ...
  - mcp__gemiflow__github_sync_coord
...
npx gemiflow@v3alpha github multi-repo-init \
...
npx gemiflow@v3alpha github discover-repos \
  --repos "$REPOS" \
  --dependencies "$DEPS" \
...
echo "$MATCHING_REPOS" | while read -r repo; do
  ...
  # Execute task
  cd /tmp/$repo
  npx gemiflow@v3alpha github task-execute \
...
npx gemiflow@v3alpha github link-prs --urls "$PR_URLS"
```
...
npx gemiflow@v3alpha github multi-repo-refactor \
...
npx gemiflow@v3alpha github multi-repo-security \
  --scan-all \
...
npx gemiflow@v3alpha github multi-repo-queue \
  --backend redis \
  --workers 10 \
  --priority-routing \
  --dead-letter-queue
...
npx gemiflow@v3alpha github multi-repo-test \
  --setup-test-env \
  --link-services \
  --run-e2e \
  --tear-down
...
npx gemiflow@v3alpha github to-monorepo \
  --analyze-repos \
  --suggest-structure \
  --preserve-history \
...
npx gemiflow@v3alpha github multi-repo-dashboard \
  --port 3000 \
...
npx gemiflow@v3alpha github dep-graph \
  --format mermaid \
  --include-agents \
  --show-data-flow
```
...
npx gemiflow@v3alpha github health-check \
  --repos "org/*" \
...
npx gemiflow@v3alpha github microservices \
...
npx gemiflow@v3alpha github lib-update \
...
npx gemiflow@v3alpha github org-policy \
...
npx gemiflow@v3alpha github cache-strategy \
  --analyze-patterns \
...
npx gemiflow@v3alpha github parallel-optimize \
  --analyze-dependencies \
...
npx gemiflow@v3alpha github resource-pool \
  --share-agents \
  --distribute-load \
  --monitor-usage
```
...
npx gemiflow@v3alpha github diagnose-connectivity \
  --test-all-repos \
  --check-permissions \
  --verify-webhooks
```
...
npx gemiflow@v3alpha github debug-memory \
  --check-consistency \
  --identify-conflicts \
  --repair-state
```
...
npx gemiflow@v3alpha github perf-analysis \
  --profile-operations \
...
npx gemiflow@v3alpha github fullstack-update \
...
npx gemiflow@v3alpha github cross-team \
