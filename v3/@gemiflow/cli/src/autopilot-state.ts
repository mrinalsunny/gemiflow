...
export const STATE_DIR = '.gemiflow/data';
...
export function discoverTasks(sources: string[]): TaskInfo[] {
  ...
  for (const source of validSources) {
    if (source === 'team-tasks') {
      const tasksDir = join(homedir(), '.gemiflow', 'tasks');
      try {
    ...
    if (source === 'swarm-tasks') {
      const swarmFile = resolve('.gemiflow/swarm-tasks.json');
      try {
    ...
    if (source === 'file-checklist') {
      const checklistFile = resolve('.gemiflow/data/checklist.json');
      try {
