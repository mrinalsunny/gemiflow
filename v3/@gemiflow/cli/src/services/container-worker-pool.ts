...
const DEFAULT_CONFIG: ContainerPoolConfig = {
  ...
  statePath: '.gemiflow/container-pool',
...
export class ContainerWorkerPool extends EventEmitter {
  ...
  private async createContainer(): Promise<ContainerInfo | null> {
    ...
    try {
      ...
      const args = [
        ...
        '-v', `${join(this.projectRoot, this.config.statePath)}:/root/.gemiflow`,
   ...
   *
   * #2371: the old command spawned `npx gemiflow@v3alpha daemon trigger`,
   ...
   * locally-installed stale `gemiflow` without fetching the published
   * version. Workers were running pre-autopilot / pre-browser builds.
   * Use the current `gemiflow@latest` and force a fresh resolution with `-y`.
   */
  private buildWorkerCommand(options: ContainerExecutionOptions): string[] {
    return [
      'npx', '-y', 'gemiflow@latest',
      'daemon', 'trigger',
