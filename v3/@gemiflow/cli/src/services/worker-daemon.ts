...
export interface DaemonConfig {
  ...
  // #2356 (carry-forward from pacphi/gemiflow-machine-ref token-leak findings):
...
// pacphi/gemiflow-machine-ref kit's proven value) heals a forgotten daemon within
// half a day; set GEMIFLOW_DAEMON_TTL_SECS=0 (or `--ttl 0`) to opt out. Idle
...
export class WorkerDaemon extends EventEmitter {
  ...
  // path, `gemiflow daemon trigger -w <worker>` runs before headlessAvailable
  ...
  constructor(projectRoot: string, config?: Partial<DaemonConfig>) {
    ...
    const gemiflowDir = join(projectRoot, '.gemiflow');

    // Read daemon config from .gemiflow/config.json (Layer B)
    const fileConfig = this.readDaemonConfigFromFile(gemiflowDir);
    ...
    this.config = {
      ...
      logDir: config?.logDir ?? join(gemiflowDir, 'logs'),
      stateFile: config?.stateFile ?? join(gemiflowDir, 'daemon-state.json'),
      ...
      // env (GEMIFLOW_DAEMON_TTL_SECS) > built-in default. readEnvSecsAsMs folds
      // env-or-default and honors an explicit 0 (disable).
      ttlMs: config?.ttlMs ?? fileConfig.ttlMs ?? readEnvSecsAsMs('GEMIFLOW_DAEMON_TTL_SECS', DEFAULT_DAEMON_TTL_MS),
      idleShutdownMs: config?.idleShutdownMs ?? fileConfig.idleShutdownMs ?? readEnvSecsAsMs('GEMIFLOW_DAEMON_IDLE_SECS', DEFAULT_DAEMON_IDLE_SHUTDOWN_MS),
    ...
    if (!existsSync(gemiflowDir)) {
      mkdirSync(gemiflowDir, { recursive: true });
    }
  ...
  /**
   * Read daemon-specific config from .gemiflow/config.{json,yaml,yml}.
   ...
   */
  private readDaemonConfigFromFile(gemiflowDir: string): {
    autoStart?: boolean;
    maxConcurrent?: number;
    workerTimeoutMs?: number;
    maxCpuLoad?: number;
    ...
    const jsonPath = join(gemiflowDir, 'config.json');
    const yamlPath = join(gemiflowDir, 'config.yaml');
    const ymlPath = join(gemiflowDir, 'config.yml');
  ...
  /**
   * Append a structured crash record to .gemiflow/logs/crash.log.
   * Inspectable by hand or via `gemiflow daemon status` follow-ups.
   */
  ...
  private get childrenFile(): string {
    return join(this.projectRoot, '.gemiflow', 'daemon-children.json');
  }
  ...
  private writeChildrenSnapshot(): void {
    ...
    try {
      ...
      const dir = join(this.projectRoot, '.gemiflow');
   ...
   * previous crashed lifetime. Reads `.gemiflow/daemon-children.json`,
  ...
  private get pidFile(): string {
    return join(this.projectRoot, '.gemiflow', 'daemon.pid');
  }
   ...
   * Each entry is a JSON file at `.gemiflow/daemon-queue/<id>.json`
   * with `{ workerId, trigger, context, enqueuedAt }`. We move processed
   * files to `.gemiflow/daemon-queue/.processed/` so the daemon never
   * re-runs the same dispatch and operators can inspect history.
   */
  private async processDispatchQueue(): Promise<void> {
    if (!this.running) return;
    const queueDir = join(this.projectRoot, '.gemiflow', 'daemon-queue');
    ...
    if (isHeadlessWorker(workerConfig.type) && this.headlessAvailable && this.headlessExecutor) {
      try {
        ...
        if (!ok) {
          ...
          // parsedOutput that lived only in `.gemiflow/logs/headless/*` and
          // never reached `.gemiflow/metrics/<name>.json` — `memory stats`
   ...
   * that lived only in `.gemiflow/logs/headless/*_result.log` and was
   * invisible to `npx gemiflow memory stats` or the metrics consumers.
   *
  ...
  private persistHeadlessResult(
    ...
    const metricsDir = join(this.projectRoot, '.gemiflow', 'metrics');
  ...
  private async runMapWorker(): Promise<unknown> {
    ...
    const metricsFile = join(this.projectRoot, '.gemiflow', 'metrics', 'codebase-map.json');
    const metricsDir = join(this.projectRoot, '.gemiflow', 'metrics');
  ...
  private async runAuditWorkerLocal(): Promise<unknown> {
    ...
    const auditFile = join(this.projectRoot, '.gemiflow', 'metrics', 'security-audit.json');
    const metricsDir = join(this.projectRoot, '.gemiflow', 'metrics');
  ...
  private async runOptimizeWorkerLocal(): Promise<unknown> {
    ...
    const optimizeFile = join(this.projectRoot, '.gemiflow', 'metrics', 'performance.json');
    const metricsDir = join(this.projectRoot, '.gemiflow', 'metrics');
  ...
  private async runConsolidateWorker(): Promise<unknown> {
    ...
    const consolidateFile = join(this.projectRoot, '.gemiflow', 'metrics', 'consolidation.json');
    const metricsDir = join(this.projectRoot, '.gemiflow', 'metrics');
  ...
  private async runTestGapsWorkerLocal(): Promise<unknown> {
    ...
    const testGapsFile = join(this.projectRoot, '.gemiflow', 'metrics', 'test-gaps.json');
    const metricsDir = join(this.projectRoot, '.gemiflow', 'metrics');
  ...
  private async runBenchmarkWorkerLocal(): Promise<unknown> {
    const benchmarkFile = join(this.projectRoot, '.gemiflow', 'metrics', 'benchmark.json');
    const metricsDir = join(this.projectRoot, '.gemiflow', 'metrics');
