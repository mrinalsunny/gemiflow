...
export class HeadlessWorkerExecutor extends EventEmitter {
  ...
  constructor(projectRoot: string, options?: HeadlessExecutorConfig) {
    ...
    this.config = {
      ...
      logDir: options?.logDir ?? join(projectRoot, '.gemiflow', 'logs', 'headless'),
  ...
  async isAvailable(): Promise<boolean> {
    ...
    try {
      ...
      try {
        const output = execSync('npx gemini-cli --version', {
          encoding: 'utf-8',
          stdio: 'pipe',
          timeout: timeoutMs,
          windowsHide: true,
        ...
        const reason =
          errFallback instanceof Error
            ? `${errFallback.name}: ${errFallback.message}`.slice(0, 200)
            : String(errFallback).slice(0, 200);
