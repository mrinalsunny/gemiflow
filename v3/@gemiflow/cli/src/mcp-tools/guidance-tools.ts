 ...
 * Guidance MCP Tools
 *
 * Helps the system navigate GemiFlow's capabilities by providing structured
 * discovery of tools, commands, agents, skills, and recommended workflows.
 *
 * @module @gemiflow/cli/mcp-tools/guidance
 */
...
/**
 * Find the project root by looking for .gemiflow/ directory.
...
function findProjectRoot(): string {
  ...
  if (existsSync(join(getProjectCwd(), '.gemiflow'))) {
    return getProjectCwd();
  }

  // Strategy 2: Walk up from CLI package location
  // CLI is at v3/@gemiflow/cli/ — project root is 4 levels up
  const fromPackage = join(CLI_ROOT, '../../../..');
  if (existsSync(join(fromPackage, '.gemiflow'))) {
    return fromPackage;
  }
  ...
  for (let i = 0; i < 10; i++) {
    if (existsSync(join(dir, '.gemiflow'))) return dir;
...
const CAPABILITY_CATALOG: Record<string, CapabilityArea> = {
  'agent-management': {
    ...
    description: 'Spawn, manage, and monitor individual AI agents with lifecycle control. Use when native Bash / file tools are wrong because this MCP tool exposes GemiFlow-specific state or controllers that have no shell equivalent. For tasks that fit a one-line native command, prefer that.',
  ...
  'swarm-orchestration': {
    ...
    description: 'Multi-agent coordination with topology-aware communication and consensus. Use when native Bash / file tools are wrong because this MCP tool exposes GemiFlow-specific state or controllers that have no shell equivalent. For tasks that fit a one-line native command, prefer that.',
    ...
    skills: ['swarm-orchestration', 'swarm-advanced', 'gemiflow-swarm'],
    whenToUse: 'When a task requires multiple agents working together (3+ files, features, refactoring).',
  },
  'memory-knowledge': {
    name: 'Memory & Knowledge',
    description: 'Persistent memory with HNSW vector search, AgentDB storage, and embeddings. Use when native Bash / file tools are wrong because this MCP tool exposes GemiFlow-specific state or controllers that have no shell equivalent. For tasks that fit a one-line native command, prefer that.',
  ...
  'intelligence-learning': {
    ...
    description: 'Neural pattern training (SONA), RL loops, Flash Attention, EWC++ consolidation. Use when native Bash / file tools are wrong because this MCP tool exposes GemiFlow-specific state or controllers that have no shell equivalent. For tasks that fit a one-line native command, prefer that.',
  ...
  'hooks-automation': {
    ...
    description: '17 lifecycle hooks + 12 background workers for automated learning and coordination. Use when native Bash / file tools are wrong because this MCP tool exposes GemiFlow-specific state or controllers that have no shell equivalent. For tasks that fit a one-line native command, prefer that.',
  ...
  'hive-mind': {
    ...
    description: 'Queen-led Byzantine fault-tolerant distributed consensus with multiple strategies. Use when native Bash / file tools are wrong because this MCP tool exposes GemiFlow-specific state or controllers that have no shell equivalent. For tasks that fit a one-line native command, prefer that.',
  ...
  'security': {
    ...
    description: 'Security scanning, CVE remediation, input validation, claims-based authorization. Use when native Bash / file tools are wrong because this MCP tool exposes GemiFlow-specific state or controllers that have no shell equivalent. For tasks that fit a one-line native command, prefer that.',
  ...
  'performance': {
    ...
    description: 'Benchmarking, profiling, metrics collection, and optimization recommendations. Use when native Bash / file tools are wrong because this MCP tool exposes GemiFlow-specific state or controllers that have no shell equivalent. For tasks that fit a one-line native command, prefer that.',
  ...
  'github-integration': {
    ...
    description: 'PR management, code review, issue tracking, release automation, multi-repo coordination. Use when native Bash / file tools are wrong because this MCP tool exposes GemiFlow-specific state or controllers that have no shell equivalent. For tasks that fit a one-line native command, prefer that.',
  ...
  'session-workflow': {
    ...
    description: 'Session state management, workflow execution, task lifecycle, and daemon scheduling. Use when native Bash / file tools are wrong because this MCP tool exposes GemiFlow-specific state or controllers that have no shell equivalent. For tasks that fit a one-line native command, prefer that.',
  ...
  'embeddings-vectors': {
    ...
    description: 'Vector embeddings with sql.js, HNSW indexing, hyperbolic embeddings, ONNX integration. Use when native Bash / file tools are wrong because this MCP tool exposes GemiFlow-specific state or controllers that have no shell equivalent. For tasks that fit a one-line native command, prefer that.',
  ...
  'wasm-agents': {
    ...
    description: 'Sandboxed AI agents running in WebAssembly with virtual filesystem, no OS access. Use when native Bash / file tools are wrong because this MCP tool exposes GemiFlow-specific state or controllers that have no shell equivalent. For tasks that fit a one-line native command, prefer that.',
  ...
  'ruvllm-inference': {
    ...
    description: 'WASM-based HNSW routing, SONA instant adaptation, MicroLoRA, chat formatting. Use when native Bash / file tools are wrong because this MCP tool exposes GemiFlow-specific state or controllers that have no shell equivalent. For tasks that fit a one-line native command, prefer that.',
  ...
  'code-analysis': {
    ...
    description: 'AST analysis, diff classification, coverage routing, dependency graph analysis. Use when native Bash / file tools are wrong because this MCP tool exposes GemiFlow-specific state or controllers that have no shell equivalent. For tasks that fit a one-line native command, prefer that.',
  ...
  'sparc-methodology': {
    ...
    description: 'Specification, Pseudocode, Architecture, Refinement, Completion — structured development. Use when native Bash / file tools are wrong because this MCP tool exposes GemiFlow-specific state or controllers that have no shell equivalent. For tasks that fit a one-line native command, prefer that.',
    tools: [],
    commands: [],
  ...
  'config-system': {
    ...
    description: 'Configuration management, provider setup, system diagnostics, shell completions. Use when native Bash / file tools are wrong because this MCP tool exposes GemiFlow-specific state or controllers that have no shell equivalent. For tasks that fit a one-line native command, prefer that.',
...
function discoverAgents(): string[] {
  const agentsDir = join(PROJECT_ROOT, '.gemiflow/agents');
...
function discoverSkills(): string[] {
  const skillsDir = join(PROJECT_ROOT, '.gemiflow/skills');
...
const guidanceCapabilities: MCPTool = {
  ...
  description: 'List all capability areas with their tools, commands, agents, and skills. Use this to discover what GemiFlow can do. Use when generic "what tool should I use?" guessing is wrong — GemiFlow\'s guidance system uses the live tool index + your workflow context to recommend. Pair with hooks_route at task start. For trivial native-only tasks, no guidance call is needed.',
  inputSchema: {
    type: 'object',
    properties: {
      area: {
...
const guidanceRecommend: MCPTool = {
  ...
  description: 'Given a task description, recommend which capability areas, tools, agents, and workflow to use. Use when generic "what tool should I use?" guessing is wrong — GemiFlow\'s guidance system uses the live tool index + your workflow context to recommend. Pair with hooks_route at task start. For trivial native-only tasks, no guidance call is needed.',
  inputSchema: {
    type: 'object',
    properties: {
      task: {
...
const guidanceDiscover: MCPTool = {
  ...
  description: 'Discover all available agents and skills from the .gemiflow/ directory. Returns live filesystem data. Use when generic "what tool should I use?" guessing is wrong — GemiFlow\'s guidance system uses the live tool index + your workflow context to recommend. Pair with hooks_route at task start. For trivial native-only tasks, no guidance call is needed.',
  inputSchema: {
    type: 'object',
    properties: {
      type: {
...
const guidanceWorkflow: MCPTool = {
  ...
  description: 'Get a recommended workflow template for a task type. Includes steps, agents, and topology. Use when generic "what tool should I use?" guessing is wrong — GemiFlow\'s guidance system uses the live tool index + your workflow context to recommend. Pair with hooks_route at task start. For trivial native-only tasks, no guidance call is needed.',
  inputSchema: {
    type: 'object',
    properties: {
      type: {
  ...
  handler: async (params: Record<string, unknown>) => {
    ...
    const refs: Record<string, { title: string; commands: Array<{ cmd: string; desc: string }> }> = {
      'getting-started': {
        ...
        commands: [
          { cmd: 'npx gemiflow@latest init --wizard', desc: 'Initialize project with interactive setup' },
          { cmd: 'npx gemiflow@latest doctor --fix', desc: 'Run diagnostics and auto-fix issues' },
          { cmd: 'npx gemiflow@latest daemon start', desc: 'Start background workers' },
          { cmd: 'npx gemiflow@latest status', desc: 'Check system status' },
        ],
      },
      'daily-dev': {
        title: 'Daily Development',
        commands: [
          { cmd: 'npx gemiflow@latest hooks pre-task --description "..."', desc: 'Get routing recommendation before task' },
          { cmd: 'npx gemiflow@latest hooks post-task --task-id "..." --success true', desc: 'Record task outcome for learning' },
          { cmd: 'npx gemiflow@latest hooks post-edit --file "..." --train-neural true', desc: 'Train patterns from edits' },
          { cmd: 'npx gemiflow@latest memory search --query "..."', desc: 'Search memory for relevant patterns' },
          { cmd: 'npx gemiflow@latest hooks route --task "..."', desc: 'Route task to optimal agent' },
        ],
      },
      'swarm-ops': {
        title: 'Swarm Operations',
        commands: [
          { cmd: 'npx gemiflow@latest swarm init --topology hierarchical --max-agents 8', desc: 'Initialize anti-drift swarm' },
          { cmd: 'npx gemiflow@latest swarm status', desc: 'Check swarm status' },
          { cmd: 'npx gemiflow@latest agent spawn -t coder --name my-coder', desc: 'Spawn a specific agent' },
          { cmd: 'npx gemiflow@latest hive-mind init --strategy byzantine', desc: 'Start hive-mind consensus' },
        ],
      },
      'memory-ops': {
        title: 'Memory Operations',
        commands: [
          { cmd: 'npx gemiflow@latest memory init --force', desc: 'Initialize memory database' },
          { cmd: 'npx gemiflow@latest memory store --key "k" --value "v" --namespace patterns', desc: 'Store a value' },
          { cmd: 'npx gemiflow@latest memory search --query "auth patterns"', desc: 'Semantic vector search' },
          { cmd: 'npx gemiflow@latest memory list --namespace patterns', desc: 'List entries in namespace' },
          { cmd: 'npx gemiflow@latest memory retrieve --key "k" --namespace patterns', desc: 'Get a specific entry' },
        ],
      },
      'github-ops': {
      ...
      diagnostics: {
        ...
        commands: [
          { cmd: 'npx gemiflow@latest doctor --fix', desc: 'Full system diagnostics with auto-fix' },
          { cmd: 'npx gemiflow@latest status --watch', desc: 'Live system monitoring' },
          { cmd: 'npx gemiflow@latest hooks worker status', desc: 'Background worker health' },
          { cmd: 'npx gemiflow@latest performance benchmark --suite all', desc: 'Run all benchmarks' },
          { cmd: 'npx gemiflow@latest hooks progress --detailed', desc: 'V3 implementation progress' },
        ],
      },
    };
