...
export async function autoInstallPackage(
  ...
  if (!validPackageName.test(packageName)) {
    if (!silent) {
      console.error(`[gemiflow] Invalid package name: ${packageName}`);
    }
    return false;
  }
  ...
  try {
    if (!silent) {
      console.error(`[gemiflow] Auto-installing ${packageName}...`);
    }
    ...
    if (!silent) {
      console.error(`[gemiflow] Successfully installed ${packageName}`);
    }
    return true;
  } catch (error) {
    if (!silent) {
      console.error(`[gemiflow] Failed to auto-install ${packageName}: ${error}`);
    }
    return false;
  }
}
  ...
  try {
    ...
    if (installed) {
      try {
        ...
        console.error(`[gemiflow] ${packageName} installed but failed to load. Restart MCP server.`);
        return null;
      }
    }
    return null;
