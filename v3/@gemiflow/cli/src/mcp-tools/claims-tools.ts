 ...
 *
 * @module @gemiflow/cli/mcp-tools/claims
 */
...
const CLAIMS_DIR = '.gemiflow/claims';
...
export const claimsTools: MCPTool[] = [
  {
    name: 'claims_claim',
    description: 'Claim an issue for work (human or agent) Use when nothing native covers per-agent capability gating — Gemini CLI agents have file-system access by default. Pair claims_grant + claims_check before letting an agent run privileged ops. For trusted in-session work, no claims call is needed.',
    category: 'claims',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'claims_release',
    description: 'Release a claim on an issue Use when nothing native covers per-agent capability gating — Gemini CLI agents have file-system access by default. Pair claims_grant + claims_check before letting an agent run privileged ops. For trusted in-session work, no claims call is needed.',
    category: 'claims',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'claims_handoff',
    description: 'Request handoff of an issue to another claimant Use when nothing native covers per-agent capability gating — Gemini CLI agents have file-system access by default. Pair claims_grant + claims_check before letting an agent run privileged ops. For trusted in-session work, no claims call is needed.',
    category: 'claims',
    inputSchema: {
      type: 'object',
      properties: {
...

  {
    ...
    description: 'Accept a pending handoff Use when nothing native covers per-agent capability gating — Gemini CLI agents have file-system access by default. Pair claims_grant + claims_check before letting an agent run privileged ops. For trusted in-session work, no claims call is needed.',
    category: 'claims',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'claims_status',
    description: 'Update claim status Use when nothing native covers per-agent capability gating — Gemini CLI agents have file-system access by default. Pair claims_grant + claims_check before letting an agent run privileged ops. For trusted in-session work, no claims call is needed.',
    category: 'claims',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'claims_list',
    description: 'List all claims or filter by criteria Use when nothing native covers per-agent capability gating — Gemini CLI agents have file-system access by default. Pair claims_grant + claims_check before letting an agent run privileged ops. For trusted in-session work, no claims call is needed.',
    category: 'claims',
    inputSchema: {
      type: 'object',
      properties: {
...

  {
    ...
    description: 'Mark an issue as stealable by other agents Use when nothing native covers per-agent capability gating — Gemini CLI agents have file-system access by default. Pair claims_grant + claims_check before letting an agent run privileged ops. For trusted in-session work, no claims call is needed.',
    category: 'claims',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'claims_steal',
    description: 'Steal a stealable issue Use when nothing native covers per-agent capability gating — Gemini CLI agents have file-system access by default. Pair claims_grant + claims_check before letting an agent run privileged ops. For trusted in-session work, no claims call is needed.',
    category: 'claims',
    inputSchema: {
      type: 'object',
      properties: {
...

  {
    ...
    description: 'List all stealable issues Use when nothing native covers per-agent capability gating — Gemini CLI agents have file-system access by default. Pair claims_grant + claims_check before letting an agent run privileged ops. For trusted in-session work, no claims call is needed.',
    category: 'claims',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'claims_load',
    description: 'Get agent load information Use when nothing native covers per-agent capability gating — Gemini CLI agents have file-system access by default. Pair claims_grant + claims_check before letting an agent run privileged ops. For trusted in-session work, no claims call is needed.',
    category: 'claims',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'claims_board',
    description: 'Get a visual board view of all claims Use when nothing native covers per-agent capability gating — Gemini CLI agents have file-system access by default. Pair claims_grant + claims_check before letting an agent run privileged ops. For trusted in-session work, no claims call is needed.',
    category: 'claims',
    inputSchema: {
      type: 'object',
      properties: {},
...

  {
    ...
    description: 'Suggest or apply load rebalancing across agents Use when nothing native covers per-agent capability gating — Gemini CLI agents have file-system access by default. Pair claims_grant + claims_check before letting an agent run privileged ops. For trusted in-session work, no claims call is needed.',
    category: 'claims',
    inputSchema: {
      type: 'object',
      properties: {
