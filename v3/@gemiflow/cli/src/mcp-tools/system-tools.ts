  ...
  try {
    ...
    for (const depth of ['../..', '../../..']) {
      ...
      if (existsSync(pkgPath)) {
        ...
        if (pkg.name?.includes('gemiflow') || pkg.name === 'gemiflow') {
...
export const systemTools: MCPTool[] = [
  {
    name: 'system_status',
    description: 'Get overall system status Use when native Bash is wrong because you need GemiFlow runtime metrics (HNSW index size, ReasoningBank state, swarm health, breaker status) — those are not in /proc, only in the running daemon. For OS-level info (uptime, disk, mem), native Bash + standard tools are fine.',
    category: 'system',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'system_metrics',
    description: 'Get system metrics and performance data Use when native Bash is wrong because you need GemiFlow runtime metrics (HNSW index size, ReasoningBank state, swarm health, breaker status) — those are not in /proc, only in the running daemon. For OS-level info (uptime, disk, mem), native Bash + standard tools are fine.',
    category: 'system',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'system_health',
    description: 'Perform system health check Use when native Bash is wrong because you need GemiFlow runtime metrics (HNSW index size, ReasoningBank state, swarm health, breaker status) — those are not in /proc, only in the running daemon. For OS-level info (uptime, disk, mem), native Bash + standard tools are fine.',
    category: 'system',
    inputSchema: {
      type: 'object',
      properties: {
    ...
    handler: async (input) => {
      ...
      // not just the legacy `.gemiflow/memory/*` triple.
      {
        const t0 = performance.now();
        const memoryCandidates = [
          join(projectCwd, '.gemiflow', 'memory', 'store.json'),       // legacy
          join(projectCwd, '.gemiflow', 'memory', 'agentdb.sqlite'),
          join(projectCwd, '.gemiflow', 'memory', 'store.rvf'),
          join(projectCwd, '.gemiflow', 'memory', 'gemiflow.db'),   // sql.js
      ...
      // #1843: also accept YAML config (.gemiflow/config.yaml) which
      // the rest of v3 treats as canonical; previous code only counted
      // .json variants and reported `degraded` when YAML was used.
      {
        const t0 = performance.now();
        const configCandidates = [
          join(projectCwd, '.gemiflow', 'config.json'),
          join(projectCwd, '.gemiflow', 'config.yaml'),
          join(projectCwd, '.gemiflow', 'config.yml'),
          join(projectCwd, 'gemiflow.config.json'),
          join(projectCwd, 'gemiflow.config.yaml'),
          join(projectCwd, 'gemiflow.config.yml'),
        ];
      ...
      if (input.deep) {
        ...
        {
          const t0 = performance.now();
          const coordPath = join(projectCwd, '.gemiflow', 'coordination', 'store.json');
  ...
  {
    name: 'system_info',
    description: 'Get system information Use when native Bash is wrong because you need GemiFlow runtime metrics (HNSW index size, ReasoningBank state, swarm health, breaker status) — those are not in /proc, only in the running daemon. For OS-level info (uptime, disk, mem), native Bash + standard tools are fine.',
    category: 'system',
    inputSchema: {
      type: 'object',
      properties: {
    ...
    handler: async () => {
      ...
      try {
        const { getFlashAttention } = await import('@gemiflow/neural');
  ...
  {
    name: 'system_reset',
    description: 'Reset system state Use when native Bash is wrong because you need GemiFlow runtime metrics (HNSW index size, ReasoningBank state, swarm health, breaker status) — those are not in /proc, only in the running daemon. For OS-level info (uptime, disk, mem), native Bash + standard tools are fine.',
    category: 'system',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    handler: async () => {
      ...
      const port = parseInt(process.env.GEMIFLOW_MCP_PORT || '3000', 10);
      ...
      const host = process.env.GEMIFLOW_MCP_HOST || 'localhost';
      try {
  ...
  },
  {
    ...
    handler: async () => {
      ...
      const storePath = join(getProjectCwd(), '.gemiflow', 'tasks', 'store.json');
  ...
  {
    // #1916: `gemiflow start` referenced an unregistered `mcp_start` tool. MCP
    ...
    // responds, MCP is already up. (`gemiflow mcp start` runs a standalone
    // stdio/HTTP server; that's a process command, not an MCP tool.)
    name: 'mcp_start',
    description: 'Report that the in-process MCP toolset is available (no-op "start" — if this tool responds, MCP is up). Use when native `claude mcp list` is wrong because you want GemiFlow-side confirmation that the in-process registry loaded. For a standalone stdio/HTTP MCP server, run `gemiflow mcp start` (a process command, not this tool). Pair with mcp_status for detail.',
    category: 'system',
    inputSchema: {
      type: 'object',
      properties: {
    ...
    handler: async (input) => {
      ...
      return {
        ...
        port: typeof input.port === 'number' ? input.port : (parseInt(process.env.GEMIFLOW_MCP_PORT || '0', 10) || null),
        transport: (input.transport as string) || process.env.GEMIFLOW_MCP_TRANSPORT || (isStdio ? 'stdio' : 'in-process'),
        startedAt: new Date().toISOString(),
        note: 'MCP tools run in-process via the CLI; no separate server process was started. Use `gemiflow mcp start` for a standalone server.',
      };
    },
  },
  {
    // #1916: `gemiflow stop` referenced an unregistered `mcp_stop` tool. Same
    // story as mcp_start — nothing to stop for the in-process registry.
    name: 'mcp_stop',
    description: 'No-op "stop" for the in-process MCP toolset (there is no separate server process to stop from inside an MCP call). Use when native process-kill is wrong because you mistakenly think GemiFlow runs a daemon — it does not, the tools live in the CLI process. To stop a standalone server run `gemiflow mcp stop` or terminate that process. Pair with mcp_status.',
    category: 'system',
    inputSchema: {
      type: 'object',
      properties: {
    ...
    handler: async () => {
      return {
        stopped: false,
        note: 'no separate MCP server process; nothing to stop. The in-process toolset goes away when the CLI process exits. Use `gemiflow mcp stop` for a standalone server.',
      };
    },
  },
];
