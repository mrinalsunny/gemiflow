...
type AIDefenceInstance = ReturnType<typeof import('@gemiflow/aidefence').createAIDefence>;
...
async function getAIDefence(): Promise<AIDefenceInstance> {
  ...
  const packageName = '@gemiflow/aidefence';
  ...
  if (installAttempted) {
    throw new Error('AIDefence package not available. Install with: npm install @gemiflow/aidefence');
  }
  installAttempted = true;

  // Second attempt - auto-install and retry
  console.error(`[gemiflow] ${packageName} not found, attempting auto-install...`);
  ...
  try {
    ...
    if (instance) {
      aidefenceInstance = instance;
      console.error(`[gemiflow] ${packageName} loaded after install (resolver path)`);
      return instance;
    }
  ...
  try {
    ...
    aidefenceInstance = instance;
    console.error(`[gemiflow] ${packageName} loaded after install (file:// path)`);
    return instance;
  } catch (retryError) {
    throw new Error(
      ...
      `(common with global installs of \`gemiflow\`). Recovery options:\n` +
      `  1. Run \`npm install --save @gemiflow/aidefence\` in your project's working directory.\n` +
      `  2. Or run \`npx gemiflow@latest mcp start\` from a directory whose node_modules contains the package.\n` +
...
const aidefenceScanTool: MCPTool = {
  name: 'aidefence_scan',
  description: 'Scan input text for AI manipulation threats (prompt injection, jailbreaks, PII). Returns threat assessment with <10ms latency. Use when nothing native exists — Gemini CLI does not have a PII / prompt-injection / adversarial-text scanner. Pair with any tool that ingests untrusted input (browser scrape, federation envelope, memory_import_claude).',
  inputSchema: {
    type: 'object',
    properties: {
      input: {
...
const aidefenceAnalyzeTool: MCPTool = {
  ...
  description: 'Deep analysis of input for specific threat types with similar pattern search and mitigation recommendations. Use when nothing native exists — Gemini CLI does not have a PII / prompt-injection / adversarial-text scanner. Pair with any tool that ingests untrusted input (browser scrape, federation envelope, memory_import_claude).',
  inputSchema: {
    type: 'object',
    properties: {
      input: {
...
const aidefenceStatsTool: MCPTool = {
  ...
  description: 'Get AIDefence detection and learning statistics. Use when nothing native exists — Gemini CLI does not have a PII / prompt-injection / adversarial-text scanner. Pair with any tool that ingests untrusted input (browser scrape, federation envelope, memory_import_claude).',
  inputSchema: {
    type: 'object',
    properties: {},
  },
...
const aidefenceLearnTool: MCPTool = {
  ...
  description: 'Record detection feedback for pattern learning. Improves future detection accuracy. Use when nothing native exists — Gemini CLI does not have a PII / prompt-injection / adversarial-text scanner. Pair with any tool that ingests untrusted input (browser scrape, federation envelope, memory_import_claude).',
  inputSchema: {
    type: 'object',
    properties: {
      input: {
...
const aidefenceIsSafeTool: MCPTool = {
  ...
  description: 'Quick boolean check if input is safe. Fastest option for simple validation. Use when nothing native exists — Gemini CLI does not have a PII / prompt-injection / adversarial-text scanner. Pair with any tool that ingests untrusted input (browser scrape, federation envelope, memory_import_claude).',
  inputSchema: {
    type: 'object',
    properties: {
      input: {
  ...
  handler: async (args: Record<string, unknown>): Promise<MCPToolResult> => {
    ...
    try {
      ...
      const defender = await getAIDefence();
      const { isSafe } = await import('@gemiflow/aidefence');
...
const aidefenceHasPIITool: MCPTool = {
  ...
  description: 'Check if input contains PII (emails, SSNs, API keys, passwords, etc.). Use when nothing native exists — Gemini CLI does not have a PII / prompt-injection / adversarial-text scanner. Pair with any tool that ingests untrusted input (browser scrape, federation envelope, memory_import_claude).',
  inputSchema: {
    type: 'object',
    properties: {
      input: {
