...
function loadAgentStore(): { agents: Record<string, unknown> } {
  const storePath = join(getProjectCwd(), '.gemiflow', 'agents.json');
  try {
...
function saveAgentStore(store: { agents: Record<string, unknown> }): void {
  const storeDir = join(getProjectCwd(), '.gemiflow');
...
export const hiveMindTools: MCPTool[] = [
  ...
  },
  {
    ...
    handler: async (input) => {
      ...
      const taskStorePath = join(getProjectCwd(), '.gemiflow', 'tasks', 'store.json');
  ...
  {
    // #1916: `gemiflow hive-mind optimize-memory` referenced an unregistered
