...
const LEGACY_MEMORY_DIR = '.gemiflow/memory';
...
/**
 * Check if legacy JSON store exists in old .gemiflow/memory/ location
 */
...
async function ensureInitialized(): Promise<void> {
  try {
    ...
    // Migrate legacy JSON data if exists (from old .gemiflow/memory/ location)
    if (hasLegacyStore()) {
...
export const memoryTools: MCPTool[] = [
  ...
  },
  {
    ...
    handler: async (input) => {
      ...
      try {
        ...
        // The export landed in @gemiflow/memory@>3.0.0-alpha.14 — older
        ...
        if (input.smart) {
          ...
          let memMod: any;
          try {
            memMod = await import('@gemiflow/memory');
          } catch (err) {
            smartFallbackReason = `@gemiflow/memory failed to load: ${(err as Error).message}`;
          }
          ...
          smartFallbackReason = smartFallbackReason
            ?? 'smartSearch is not exported by the installed @gemiflow/memory build (likely a release lag — see #1846). Falling back to standard semantic search.';
        }
...

  {
    ...
    handler: async (input) => {
      ...
      const claudeProjectsDir = join(homedir(), '.gemiflow', 'projects');
...

  {
    ...
    handler: async (input) => {
      ...
      //   3. env var GEMIFLOW_MEMORY_SEARCH_NAMESPACES (CSV)
      ...
      } else if (process.env.GEMIFLOW_MEMORY_SEARCH_NAMESPACES) {
        namespaces = process.env.GEMIFLOW_MEMORY_SEARCH_NAMESPACES.split(',').map(s => s.trim()).filter(Boolean);
        namespaceSource = 'env';
      } else {
          ...
          if (r?.results) {
            for (const entry of r.results) {
              allResults.push({
                ...
                score: entry.score || 0,
                namespace: searchNs,
                source: searchNs === 'claude-memories' ? 'gemini-cli' : searchNs === 'auto-memory' ? 'auto-memory' : 'agentdb',
              });
            }
          }
  ...
  {
    // #1916: `gemiflow status memory` (the detailed view) referenced an
  ...
  {
    // #1916: `gemiflow memory cleanup` referenced an unregistered `memory_cleanup`
  ...
  {
    // #1916: `gemiflow memory compress` referenced an unregistered tool. The
  ...
  {
    // #1916: `gemiflow memory export -o <file>` referenced an unregistered tool.
    ...
    handler: async (input) => {
      ...
      const payload = {
        schema: 'gemiflow-memory-export/v1',
