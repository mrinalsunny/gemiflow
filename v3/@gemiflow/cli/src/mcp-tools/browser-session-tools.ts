/**
 * Browser Session Lifecycle MCP Tools (ADR-0001 gemiflow-browser §7).
 *
 ...
 *   - Pinned to ruvector@0.2.25 to match `gemiflow-ruvector` ADR-0001.
...
const RVF_DIR_DEFAULT = '.gemiflow/browser-sessions';
...
export const browserSessionTools: MCPTool[] = [
  ...
  // ==========================================================================
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        rvf_dir: { type: 'string', description: 'Override the default .gemiflow/browser-sessions directory' },
      },
      required: ['url', 'task'],
    },
    handler: async (input) => {
  ...
  // ==========================================================================
  {
    ...
    handler: async (input) => {
      ...
      // 3. AgentDB index — best-effort via memory store (gemiflow bridges)
      ...
      const idx = await shell('npx', ['-y', '@gemiflow/cli@latest', 'memory', 'store',
  ...
  // ==========================================================================
  {
    ...
    handler: async (input) => {
      ...
      const r = await shell('npx', ['-y', '@gemiflow/cli@latest', 'memory', 'retrieve',
