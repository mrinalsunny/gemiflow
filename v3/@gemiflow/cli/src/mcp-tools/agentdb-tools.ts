...
export const agentdbControllers: MCPTool = {
  ...
  handler: async () => {
    try {
      ...
      if (!controllers) return { available: false, controllers: [], error: 'AgentDB bridge not available — @gemiflow/memory not installed or missing controller-registry. Use memory_store/memory_search tools instead.' };
      return {
        available: true,
        controllers,
    ...
    try {
      ...
      if (mode === 'semantic') {
        try {
          ...
          if (!db) return { success: false, error: 'graph_edges DB unavailable (sql.js could not load)', hint: 'Check Node version + try `gemiflow memory init` to initialize manually.', mode, nodeId };
 ...
 * Used as fallback when gemiflow-graph-intelligence is unavailable.
...
export const agentdbGraphPathfinder: MCPTool = {
  ...
  description: 'Multi-algorithm native graph pathfinder (ADR-130 Phase 5). Use when agentdb_graph-query k-hop is not enough — pathfinder supports personalized-pagerank, dynamic-mincut, spectral-sparsify, temporal-centrality, connected-component-churn, and witness-chain-divergence. Prefer over prompt-level graph loops in gemiflow-knowledge-graph graph-navigator when you need ranked paths with formal complexityBudget enforcement.',
  inputSchema: {
    type: 'object',
    properties: {
  ...
  handler: async (params: Record<string, unknown>) => {
    ...
    try {
      ...
      const db = await getBridgeDb(undefined, { createIfMissing: true });
      if (!db) return { success: false, error: 'graph_edges DB unavailable (sql.js could not load)', hint: 'Check Node version + try `gemiflow memory init` to initialize manually.', seedNodeId };
