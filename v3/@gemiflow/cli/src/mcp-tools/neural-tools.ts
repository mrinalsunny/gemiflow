 ...
 * - Uses @gemiflow/embeddings for REAL ML embeddings when available
 ...
 *
 * Note: For production neural features, use @gemiflow/neural module
 */
...
//   silent-fallback bug at source; closes the chain described in mrinalsunny/ruvector#523).
// Tier 1: agentic-flow v3 ReasoningBank (was Tier 1 — broken on darwin-arm64 without sharp)
// Tier 2-3: @gemiflow/embeddings
...
try {
  ...
  // Tier 2: @gemiflow/embeddings with agentic-flow provider
  if (!realEmbeddings) {
  ...
  // Tier 3: @gemiflow/embeddings with ONNX provider
  if (!realEmbeddings) {
    const embeddingsModule = await import('@gemiflow/embeddings').catch(() => null);
    if (embeddingsModule?.createEmbeddingService) {
      try {
...
export function getNeuralStoreStats(): {
  ...
  return {
    ...
    source: '.gemiflow/neural/patterns.json (loadNeuralStore)',
  };
}
    ...
    handler: async (input) => {
      ...
      return {
        ...
        ...(embeddingServiceName === 'hash-fallback' || embeddingServiceName === 'none' ? {
          platformNote: 'ONNX embeddings not available — using hash-based fallback. Install @gemiflow/embeddings and run "embeddings init --download" for semantic search.',
        } : {}),
      };
    },
  },
  ...
  },
  {
    ...
    handler: async (input) => {
      ...
      return {
        ...
        embeddingProvider: realEmbeddings ? `@gemiflow/embeddings (${embeddingServiceName})` : 'hash-based (deterministic)',
        models: {
          total: models.length,
        ...
        features: {
          ...
          flashAttention: await (async () => {
            try {
              // #1773 item 4 — flash-attention now lives in @gemiflow/neural
