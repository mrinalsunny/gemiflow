 ...
 * @module @gemiflow/cli/mcp-tools/autopilot
 */
...
const autopilotStatus: MCPTool = {
  ...
  description: 'Get autopilot state including enabled status, iteration count, task progress, and learning metrics. Use when running long-horizon goals that should resume automatically across sessions — Gemini CLI has no native autonomous-loop scheduler. Pair with autopilot_enable + a goal description, then let cron fires advance the work. For interactive single-task sessions, native Task is fine.',
  category: 'autopilot',
