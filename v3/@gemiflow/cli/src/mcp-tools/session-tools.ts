...
function saveSession(session: SessionRecord): void {
  ...
  // only when GEMIFLOW_ENCRYPT_AT_REST is set; otherwise the legacy
...
export const sessionTools: MCPTool[] = [
  ...
  },
  {
    ...
    handler: async (input) => {
      ...
      // while the auto-session writer (gemiflow daemon) writes
  ...
  {
    // #1916: `gemiflow session current` referenced an unregistered
  ...
  {
    // #1916: `gemiflow session export <id> -o <file>` referenced an unregistered
  ...
  {
    // #1916: `gemiflow session import <file>` referenced an unregistered
