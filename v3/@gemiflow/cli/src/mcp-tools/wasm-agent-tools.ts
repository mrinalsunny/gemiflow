...
function loadPluginManifest(pluginName: string): PluginManifest | null {
  const candidateDirs = [
    ...
    resolve(process.cwd(), 'plugins', `gemiflow-${pluginName}`, '.claude-plugin', 'plugin.json'),
    ...
    handler: async (args: Record<string, unknown>) => {
      try {
        ...
        const mcpToolDescriptors = requestedTools.map(name => ({
          name,
          description: SAFE_MCP_TOOLS.has(name) ? `GemiFlow MCP tool: ${name}` : `MCP tool: ${name}`,
          input_schema: {},
          group: 'gemiflow',
        }));
  ...
  },
  {
    ...
    handler: async (args: Record<string, unknown>) => {
      ...
      try {
        ...
        // ADR-118 pattern: lazy import @gemiflow/aidefence; warn and continue if unavailable.
        let aiDefenceWarning: string | undefined;
        try {
          const aidefenceMod = await import('@gemiflow/aidefence');
