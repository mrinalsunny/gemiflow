...
// #1916: hive-mind_spawn writes its workers to `.gemiflow/agents.json`
// (a *different* file from the canonical `.gemiflow/agents/store.json`
...
// #1916: read hive-mind-spawned workers from `.gemiflow/agents.json`.
...
export const agentTools: MCPTool[] = [
  ...
  {
    // #1916 — the `gemiflow agent logs <id>` CLI subcommand and the guidance
    ...
    handler: async (input) => {
      ...
      return {
        ...
        note: 'per-agent activity logging is not yet wired; entries are synthetic (mrinalsunny/gemiflow#1916)',
      };
    },
  },
];
