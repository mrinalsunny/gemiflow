 ...
 * .gemiflow/sessions/*.json files keep working unchanged.
 */
...
/** ASCII "RFE1" — GemiFlow File Encrypted v1. 4 bytes. */
...
const ENV_ENABLE_FLAG = 'GEMIFLOW_ENCRYPT_AT_REST';
const ENV_KEY_VAR = 'GEMIFLOW_ENCRYPTION_KEY';
...
/**
 * Resolve a 32-byte encryption key from GEMIFLOW_ENCRYPTION_KEY.
 *
...
export function decryptBuffer(blob: Buffer, key: Buffer): Buffer {
  ...
  if (!timingSafeEqual(magic, MAGIC)) {
    throw new Error(
      'decryptBuffer: bad magic — blob is not GemiFlow-encrypted (RFE1)',
    );
  }
