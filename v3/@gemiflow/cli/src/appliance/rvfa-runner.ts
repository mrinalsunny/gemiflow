...
export class RvfaRunner {
  ...
  /**
   * Run natively via Node.js: extract GEMIFLOW section to a temp dir,
  ...
  async runNative(options: RunOptions): Promise<RunResult> {
    ...
    try {
      ...
      const gemiflow = tryExtract(this.reader, 'gemiflow');
      if (!gemiflow) return fail('RVFA appliance does not contain a "gemiflow" section');

      const entryFile = join(workDir, 'gemiflow-bundle.js');
      await writeFile(entryFile, gemiflow);
  ...
  async runContainer(options: RunOptions): Promise<RunResult> {
    ...
    try {
      ...
      const gemiflow = tryExtract(this.reader, 'gemiflow');
      if (!gemiflow) return fail('RVFA appliance does not contain a "gemiflow" section');
      await writeFile(join(workDir, 'gemiflow-bundle.js'), gemiflow);
      ...
      const dockerfile = [
        `FROM ${baseImage}`, 'WORKDIR /app', 'COPY gemiflow-bundle.js .',
        data ? 'COPY data.bin .' : '', `CMD ["node", "gemiflow-bundle.js"${cmdArgs}]`,
      ].filter(Boolean).join('\n');
