 ...
 *
 * Creates a single binary containing kernel, runtime, GemiFlow CLI, models/keys,
...
const GEMIFLOW_COMMANDS = 'init agent swarm memory mcp task session config status start workflow hooks hive-mind daemon neural security performance providers plugins deployment embeddings claims migrate process doctor completions'.split(' ');
...
type SectionId = 'kernel' | 'runtime' | 'gemiflow' | 'models' | 'data' | 'verify';
...
export class RvfaBuilder {
  ...
  constructor(options: BuildOptions) {
    this.opts = {
      ...
      gemiflowVersion: options.gemiflowVersion || detectGemiFlowVersion(),
  ...
  private buildKernelSection(): Buffer {
    return jsonBuf({
      ...
      init: '/sbin/init -> gemiflow-init (PID 1)',
  ...
  private buildRuntimeSection(): Buffer {
    ...
    return jsonBuf({
      ...
      claudeCode: { name: 'gemini-cli-cli', entrypoint: '/usr/local/bin/claude' },
      paths: { node: '/usr/local/bin/node', claude: '/usr/local/bin/claude' },
    });
  }

  private buildGemiFlowSection(): Buffer {
    let packageMeta: Record<string, unknown> | null = null;
    try {
      const raw = execSync('npm pack gemiflow@latest --dry-run --json 2>/dev/null', { encoding: 'utf-8', timeout: 15_000 });
      const parsed = JSON.parse(raw);
      if (Array.isArray(parsed) && parsed.length > 0) packageMeta = parsed[0];
    } catch { /* manifest-only fallback */ }

    return jsonBuf({
      type: 'gemiflow', version: this.opts.gemiflowVersion,
      package: packageMeta ?? { name: 'gemiflow', version: this.opts.gemiflowVersion },
      commands: GEMIFLOW_COMMANDS, commandCount: GEMIFLOW_COMMANDS.length,
  ...
  private buildVerifySection(): Buffer {
    ...
    if (existsSync(scriptPath)) {
      ...
      script = Buffer.from([
        '#!/bin/sh', 'set -e', 'GEMIFLOW_CMD="${GEMIFLOW_CMD:-gemiflow}"',
        'echo "Running basic verification..."',
        '$GEMIFLOW_CMD --version && echo "  OK: CLI" || echo "  FAIL: CLI"',
        '$GEMIFLOW_CMD doctor && echo "  OK: Doctor" || echo "  FAIL: Doctor"',
  ...
  private buildHeaderPartial(): Partial<RvfaHeader> {
    ...
    const boot: RvfaBootConfig = {
      entrypoint: '/opt/gemiflow/bin/cli.js',
      args: ['--profile', this.opts.profile],
      env: { NODE_ENV: 'production', GEMIFLOW_MEMORY_BACKEND: 'hybrid', GEMIFLOW_LOG_LEVEL: 'info' },
    ...
    return {
      magic: 'RVFA', version: 1, name: 'gemiflow-appliance', appVersion: this.opts.gemiflowVersion,
...
function detectGemiFlowVersion(): string {
  try {
