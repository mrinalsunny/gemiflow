 ...
 *
 * This is one of gemiflow's 6 architectural primitives distinguishing it
 * from HAL: HAL is stateless across runs; gemiflow accumulates causal memory.
 *
 * Storage: JSONL file at ~/.cache/gemiflow/gaia/causal-edges.jsonl
 *   - Simple, portable, no runtime dependency on AgentDB
 *   - Production upgrade path: switch to AgentDB causal-edge MCP controller
 *     (`mcp__gemiflow__agentdb_causal-edge`) for cross-session persistence
...
export interface CausalMemoryOptions {
   ...
   * Default: ~/.cache/gemiflow/gaia/causal-edges.jsonl
   */
  storePath?: string;
  /**
...
const DEFAULT_STORE_SUFFIX = path.join('.cache', 'gemiflow', 'gaia', 'causal-edges.jsonl');
const DEFAULT_MAX_EDGES = 5;

/** Resolve the JSONL store path from options, defaulting to ~/.cache/gemiflow/…  */
