...
const listCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow providers list', description: 'List all providers' },
    { command: 'gemiflow providers list -t embedding', description: 'List embedding providers' },
  ],
...
const testCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow providers test -p openai', description: 'Test OpenAI connection' },
    { command: 'gemiflow providers test --all', description: 'Test all providers' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    try {
      const provider = (ctx.flags.provider as string) || (ctx.args && ctx.args[0]) || '';
...
const usageCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow providers usage', description: 'View all usage' },
    { command: 'gemiflow providers usage -t 30d', description: 'View 30-day usage' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
export const providersCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow providers list', description: 'List all providers' },
    { command: 'gemiflow providers configure -p openai', description: 'Configure OpenAI' },
    { command: 'gemiflow providers test --all', description: 'Test all providers' },
  ],
  action: async (): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('GemiFlow Provider Management'));
