...
const daemonCommand: Command = {
  ...
  options: [
    ...
    },
    {
      ...
      default: '.gemiflow/daemon.pid',
    },
    {
      name: 'log-file',
      type: 'string',
      description: 'Log file location',
      default: '.gemiflow/daemon.log',
    },
    {
      name: 'detach',
      type: 'boolean',
  ...
  examples: [
    { command: 'gemiflow process daemon --action start', description: 'Start the daemon' },
    { command: 'gemiflow process daemon --action stop', description: 'Stop the daemon' },
    { command: 'gemiflow process daemon --action restart --port 3850', description: 'Restart on different port' },
    { command: 'gemiflow process daemon --action status', description: 'Check daemon status' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const action = (ctx.flags?.action as string) || 'status';
    const port = (ctx.flags?.port as number) || 3847;
    const pidFile = (ctx.flags?.['pid-file'] as string) || '.gemiflow/daemon.pid';
    const logFile = (ctx.flags?.['log-file'] as string) || '.gemiflow/daemon.log';
    ...
    switch (action) {
      ...
      case 'status':
        ...
        console.log('  └─────────────────────────────────────────┘');
        if (!existingDaemon) {
          console.log('\n  To start: gemiflow process daemon --action start');
        }
        break;
    }
...
const monitorCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow process monitor', description: 'Show process dashboard' },
    { command: 'gemiflow process monitor --watch --interval 5', description: 'Watch mode' },
    { command: 'gemiflow process monitor --components agents,memory,tasks', description: 'Monitor specific components' },
    { command: 'gemiflow process monitor --format json', description: 'JSON output' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      const agentStorePath = resolve('.gemiflow/agents/store.json');
    ...
    try {
      const taskStorePath = resolve('.gemiflow/tasks/store.json');
    ...
    const metrics = {
      ...
      agents: {
        total: agentCount,
        _note: agentCount === 0 ? 'No agent store found at .gemiflow/agents/store.json' : null,
      },
      tasks: {
        ...taskCounts,
        _note: (taskCounts.running + taskCounts.queued + taskCounts.completed + taskCounts.failed) === 0
          ? 'No task store found at .gemiflow/tasks/store.json' : null,
      },
      memory: {
    ...
    // Dashboard format
    console.log('\n╔══════════════════════════════════════════════════════════════╗');
    console.log('║            🖥️  gemiflow PROCESS MONITOR                    ║');
    console.log('╠══════════════════════════════════════════════════════════════╣');
...
const workersCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow process workers --action list', description: 'List all workers' },
    { command: 'gemiflow process workers --action spawn --type task --count 3', description: 'Spawn task workers' },
    { command: 'gemiflow process workers --action kill --id worker-123', description: 'Kill specific worker' },
    { command: 'gemiflow process workers --action scale --type memory --count 5', description: 'Scale memory workers' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const signalsCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow process signals --target daemon --signal graceful-shutdown', description: 'Graceful shutdown' },
    { command: 'gemiflow process signals --target workers --signal pause', description: 'Pause workers' },
    { command: 'gemiflow process signals --target all --signal reload-config', description: 'Reload all configs' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const logsCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow process logs', description: 'Show recent logs' },
    { command: 'gemiflow process logs --source daemon --tail 100', description: 'Daemon logs' },
    { command: 'gemiflow process logs --follow --level error', description: 'Follow error logs' },
    { command: 'gemiflow process logs --since 1h --grep "error"', description: 'Search logs' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    // Read actual log files from .gemiflow/logs/ if they exist
    const logsDir = resolve('.gemiflow/logs');
...
export const processCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow process daemon --action start', description: 'Start daemon' },
    { command: 'gemiflow process monitor --watch', description: 'Watch processes' },
    { command: 'gemiflow process workers --action list', description: 'List workers' },
    { command: 'gemiflow process logs --follow', description: 'Follow logs' },
  ],
  action: async (_ctx: CommandContext): Promise<CommandResult> => {
    ...
    console.log('\nExamples:');
    console.log('  gemiflow process daemon --action start');
    console.log('  gemiflow process monitor --watch');
    console.log('  gemiflow process workers --action spawn --type task --count 3');
    console.log('  gemiflow process logs --follow --level error');
