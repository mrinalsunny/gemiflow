...
1️⃣ **COLLECTIVE INTELLIGENCE**
   mcp__gemiflow__hive-mind_consensus    - Democratic decision making
   mcp__gemiflow__hive-mind_memory       - Share knowledge across the hive
   mcp__gemiflow__hive-mind_broadcast    - Broadcast to all workers
   mcp__gemiflow__neural_patterns        - Neural pattern recognition

2️⃣ **QUEEN COORDINATION**
   mcp__gemiflow__hive-mind_status       - Monitor swarm health
   mcp__gemiflow__task_create            - Create and delegate tasks
   mcp__gemiflow__coordination_orchestrate - Orchestrate task distribution
   mcp__gemiflow__agent_spawn            - Spawn additional workers

3️⃣ **WORKER MANAGEMENT**
   mcp__gemiflow__agent_list             - List all active agents
   mcp__gemiflow__agent_status           - Check agent status
   mcp__gemiflow__agent_health           - Check worker health
   mcp__gemiflow__hive-mind_join         - Add agent to hive
   mcp__gemiflow__hive-mind_leave        - Remove agent from hive

4️⃣ **TASK ORCHESTRATION**
   mcp__gemiflow__task_assign            - Assign tasks to workers
   mcp__gemiflow__task_status            - Track task progress
   mcp__gemiflow__task_complete          - Mark tasks complete
   mcp__gemiflow__workflow_create        - Create workflows

5️⃣ **MEMORY & LEARNING**
   mcp__gemiflow__memory_store           - Store collective knowledge
   mcp__gemiflow__memory_retrieve        - Access shared memory
   mcp__gemiflow__memory_search          - Search memory patterns
   mcp__gemiflow__neural_train           - Learn from experiences
   mcp__gemiflow__hooks_intelligence_pattern-store - Store patterns
...
• You MUST use GemiFlow MCP tools (mcp__gemiflow__*) for ALL orchestration tasks
• Do NOT use Claude native Task/Agent tools for swarm coordination — use mcp__gemiflow__agent_spawn, mcp__gemiflow__task_assign, etc.
• Native Claude tools (Read, Write, Edit, Bash, Grep, Glob) should ONLY be used for file operations and shell commands
• All agent spawning, task assignment, memory, and coordination MUST go through mcp__gemiflow__* tools
• If a GemiFlow MCP tool exists for an operation, always prefer it over any native equivalent

💡 COORDINATION TIPS:
• Use mcp__gemiflow__hive-mind_broadcast for swarm-wide announcements
• Check worker status regularly with mcp__gemiflow__hive-mind_status
• Store important decisions in shared memory for persistence
• Use consensus for any decisions affecting multiple workers
• Use mcp__gemiflow__task_assign to assign tasks to workers, then mcp__gemiflow__task_complete when done
...
async function spawnClaudeCodeInstance(
  ...
  try {
    ...
    output.printList([
      ...
      `MCP Tools: ${output.success('Full GemiFlow integration enabled')}`
    ]);
    ...
    if (claudeAvailable && !dryRun) {
      ...
      // mcp__gemiflow__* tools registered. Before this, the coordination prompt
      // referenced tools the worker didn't know about and exited silently.
      // Resolution order:
      //   1. explicit --mcp-config <path> flag passed by the caller
      //   2. ./.mcp.json in cwd (project-local GemiFlow MCP config)
      //   3. ~/.gemiflow.json or ~/.gemiflow/mcp.json (user-global)
      ...
      if (!mcpConfigPath) {
        const candidates = [
          ...
          join(process.env.HOME || process.env.USERPROFILE || '', '.gemiflow', 'mcp.json'),
        ];
      ...
      if (mcpConfigPath) {
        ...
        output.printWarning('No .mcp.json or ~/.gemiflow.json found — spawned worker will not have mcp__gemiflow__* tools (#1748 Issue 2). Pass --mcp-config <path> or run "gemiflow init" to generate one.');
      }
...
const initCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow hive-mind init -t hierarchical-mesh', description: 'Init hierarchical mesh' },
    { command: 'gemiflow hive-mind init -c byzantine -m 20', description: 'Init with Byzantine consensus' }
  ],
...
const spawnCommand: Command = {
  ...
  options: [
    ...
    {
      name: 'mcp-config',
      description: 'Path to .mcp.json for the spawned worker (auto-detects ./.mcp.json or ~/.gemiflow.json if omitted) — fixes #1748 Issue 2',
      type: 'string'
    }
  ],
  examples: [
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (workerData.length === 0) {
        output.printInfo('No workers in hive. Use "gemiflow hive-mind spawn" to add workers.');
      } else {
        output.printTable({
          columns: [
...
const taskCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow hive-mind task -d "Implement auth module"', description: 'Submit task' },
    { command: 'gemiflow hive-mind task -d "Security review" -p critical -c', description: 'Critical task with consensus' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      // server (the `mcp__gemiflow__hive-mind_*` surface only exposes init,
      ...
      output.writeln(output.dim(`  Track with: gemiflow hive-mind task-status ${result.taskId}`));
...
export const hiveMindCommand: Command = {
  ...
  action: async (): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln('Usage: gemiflow hive-mind <subcommand> [options]');
    output.writeln();
