...
// Precedence: --path > GEMIFLOW_DB_PATH env var > default root
const DB_PATH_OPTION = {
  name: 'path',
  description:
    'Override DB file path (also: GEMIFLOW_DB_PATH env var). ' +
    'Precedence: --path > GEMIFLOW_DB_PATH > GEMIFLOW_MEMORY_PATH/memory.db > cwd/.swarm/memory.db',
...
const storeCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow memory store -k "api/auth" -v "JWT implementation"', description: 'Store text' },
    { command: 'gemiflow memory store -k "pattern/singleton" --vector', description: 'Store vector' },
    { command: 'gemiflow memory store -k "pattern" -v "updated" --upsert', description: 'Update existing' }
  ],
...
const searchCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow memory search -q "authentication patterns"', description: 'Semantic search' },
    { command: 'gemiflow memory search -q "JWT" -t keyword', description: 'Keyword search' },
    { command: 'gemiflow memory search -q "test" --build-hnsw', description: 'Build HNSW index and search' },
    { command: 'gemiflow memory search -q "auth patterns" --smart', description: 'SmartRetrieval with RRF + MMR' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      // @gemiflow/memory don't expose it. Fall through to plain
      ...
      if (useSmart) {
        try {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          const memMod: any = await import('@gemiflow/memory');
        ...
        if (!smartSearchFn) {
          output.printWarning(
            'Smart search requested but smartSearch is not available on the installed @gemiflow/memory build (#1846). Falling back to standard semantic search.',
          );
        }
      }
      ...
      if (results.length === 0) {
        ...
        output.writeln(output.dim('Try: gemiflow memory store -k "key" --value "data"'));
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (entries.length === 0) {
        ...
        output.printInfo('Store data: gemiflow memory store -k "key" --value "data"');
        return { success: true, data: [] };
      }
...
const deleteCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow memory delete -k "mykey"', description: 'Delete entry with default namespace' },
    { command: 'gemiflow memory delete -k "lesson" -n "lessons"', description: 'Delete entry from specific namespace' },
    { command: 'gemiflow memory delete mykey -f', description: 'Delete without confirmation' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
      ...
      try {
        ...
        output.printTable({
          ...
          data: [
            ...
            {
              metric: 'HNSW Index',
              // gemiflow#1989 / #1987: `hnsw.entryCount` is in-process JS state
...
const cleanupCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow memory cleanup --dry-run', description: 'Preview cleanup' },
    { command: 'gemiflow memory cleanup --older-than 30d', description: 'Delete entries older than 30 days' },
    { command: 'gemiflow memory cleanup --expired-only', description: 'Clean expired entries' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const compressCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow memory compress', description: 'Balanced compression' },
    { command: 'gemiflow memory compress --quantize --bits 4', description: '4-bit quantization (32x reduction)' },
    { command: 'gemiflow memory compress -l max -t vectors', description: 'Max compression on vectors' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const exportCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow memory export -o ./backup.json', description: 'Export all to JSON' },
    { command: 'gemiflow memory export -o ./data.csv -f csv', description: 'Export to CSV' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const importCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow memory import -i ./backup.json', description: 'Import from file' },
    { command: 'gemiflow memory import -i ./data.json -n archive', description: 'Import to namespace' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const initMemoryCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow memory init', description: 'Initialize hybrid backend with all features' },
    { command: 'gemiflow memory init -b agentdb', description: 'Initialize AgentDB backend' },
    { command: 'gemiflow memory init -p ./data/memory.db --force', description: 'Reinitialize at custom path' },
    { command: 'gemiflow memory init --verbose --verify', description: 'Initialize with full verification' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      output.printList([
        `Store data: ${output.highlight('gemiflow memory store -k "key" --value "data"')}`,
        `Search: ${output.highlight('gemiflow memory search -q "query"')}`,
        `Train patterns: ${output.highlight('gemiflow neural train -p coordination')}`,
        `View stats: ${output.highlight('gemiflow memory stats')}`
      ]);

      // Also sync to .claude directory
      const fs = await import('fs');
      const path = await import('path');
      const claudeDir = path.join(process.cwd(), '.gemiflow');
...
export const memoryCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'gemiflow memory store -k "key" -v "value"', description: 'Store data' },
    { command: 'gemiflow memory search -q "auth patterns"', description: 'Search memory' },
    { command: 'gemiflow memory stats', description: 'Show statistics' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Memory Management Commands'));
    output.writeln();
    output.writeln('Usage: gemiflow memory <subcommand> [options]');
    output.writeln();
