...
function getClaimsConfigPaths(): string[] {
  return [
    ...
    resolve(process.env.HOME || '~', '.config/gemiflow/claims.json'),
  ];
}
...
const listCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow claims list', description: 'List all claims' },
    { command: 'gemiflow claims list -u user123', description: 'List user claims' },
  ],
...
const checkCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow claims check -c swarm:create', description: 'Check swarm creation permission' },
    { command: 'gemiflow claims check -c admin:delete -u user123', description: 'Check user permission' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      const claimsConfigPaths = [
        path.resolve('.gemiflow/claims.json'),
        path.resolve('gemiflow.claims.json'),
        path.resolve(process.env.HOME || '~', '.config/gemiflow/claims.json'),
      ];
...
const grantCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow claims grant -c swarm:create -u user123', description: 'Grant to user' },
    { command: 'gemiflow claims grant -c agent:spawn -r developer', description: 'Grant to role' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const claim = ctx.flags.claim as string;
...
const revokeCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow claims revoke -c swarm:delete -u user123', description: 'Revoke from user' },
    { command: 'gemiflow claims revoke -c admin:* -r guest', description: 'Revoke from role' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const claim = ctx.flags.claim as string;
    const user = ctx.flags.user as string;
...
const rolesCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow claims roles', description: 'List all roles' },
    { command: 'gemiflow claims roles -a show -n admin', description: 'Show role details' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const policiesCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow claims policies', description: 'List policies' },
    { command: 'gemiflow claims policies -a create -n rate-limit', description: 'Create policy' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const action = (ctx.flags.action as string) || 'list';
    const name = ctx.flags.name as string;
...
export const claimsCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow claims list', description: 'List all claims' },
    { command: 'gemiflow claims check -c swarm:create', description: 'Check permission' },
    { command: 'gemiflow claims grant -c agent:spawn -r developer', description: 'Grant claim' },
  ],
  action: async (): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('GemiFlow Claims System'));
