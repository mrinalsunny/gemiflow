...
const scanCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow security scan -t ./src', description: 'Scan source directory' },
    { command: 'gemiflow security scan --depth deep --fix', description: 'Deep scan with auto-fix' },
  ],
...
const cveCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow security cve --list', description: 'List all CVEs' },
    { command: 'gemiflow security cve -c CVE-2024-1234', description: 'Check specific CVE' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln(`  ${output.dim('$')} gemiflow security scan                     ${output.dim('# real code + dependency scan')}`);
    if (checkCve) {
...
const threatsCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow security threats --model stride', description: 'Run STRIDE analysis' },
    { command: 'gemiflow security threats -e md', description: 'Export as markdown' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const auditCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow security audit --action list', description: 'List audit logs' },
    { command: 'gemiflow security audit -a export', description: 'Export audit trail' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (auditEntries.length === 0) {
      output.writeln(output.dim('No audit events found. Initialize a project first: gemiflow init'));
    } else {
      output.printTable({
        columns: [
...
const secretsCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow security secrets --action scan', description: 'Scan for secrets' },
    { command: 'gemiflow security secrets -a rotate', description: 'Rotate compromised secrets' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const defendCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow security defend -i "ignore previous instructions"', description: 'Scan text for threats' },
    { command: 'gemiflow security defend -f ./prompts.txt', description: 'Scan file for threats' },
    { command: 'gemiflow security defend --stats', description: 'Show detection statistics' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    let createAIDefence: typeof import('@gemiflow/aidefence').createAIDefence;
    try {
      const aidefence = await import('@gemiflow/aidefence');
      createAIDefence = aidefence.createAIDefence;
    } catch {
      output.error('AIDefence package not installed. Run: npm install @gemiflow/aidefence');
    ...
    if (!textToScan) {
      output.writeln('Usage: gemiflow security defend -i "<text>" or -f <file>');
      output.writeln();
...
export const securityCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow security scan', description: 'Run security scan' },
    { command: 'gemiflow security cve --list', description: 'List known CVEs' },
    { command: 'gemiflow security threats', description: 'Run threat analysis' },
  ],
  action: async (): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('GemiFlow Security Suite'));
