...
/** Task intent categories matching @gemiflow/guidance TaskIntent */
type TaskIntent = 'bug-fix' | 'feature' | 'refactor' | 'security' | 'performance' | 'testing' | 'docs' | 'deployment' | 'architecture' | 'debug' | 'general';

/** Gate evaluation result matching @gemiflow/guidance GateResult */
...
const compileCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow guidance compile', description: 'Compile default CLAUDE.md' },
    { command: 'gemiflow guidance compile -r ./CLAUDE.md -l ./CLAUDE.local.md', description: 'Compile with local overlay' },
    { command: 'gemiflow guidance compile --json', description: 'Output compiled bundle as JSON' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      const { GuidanceCompiler } = await import('@gemiflow/guidance/compiler');
...
const retrieveCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow guidance retrieve -t "Fix SQL injection in user search"', description: 'Retrieve guidance for a security task' },
    { command: 'gemiflow guidance retrieve -t "Add unit tests" -n 3', description: 'Retrieve top 3 shards for testing' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      const { readFile } = await import('node:fs/promises');
      const { GuidanceCompiler } = await import('@gemiflow/guidance/compiler');
      const { ShardRetriever, HashEmbeddingProvider } = await import('@gemiflow/guidance/retriever');
...
const gatesCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow guidance gates -c "rm -rf /tmp"', description: 'Check if a command is destructive' },
    { command: 'gemiflow guidance gates --content "api_key=sk-abc123..."', description: 'Check content for secrets' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      const { EnforcementGates } = await import('@gemiflow/guidance/gates');
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      const statusData = {
        ...
        dataDir: existsSync('./.gemiflow/guidance') ? 'exists' : 'not created',
      };
...
const optimizeCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow guidance optimize', description: 'Analyze current CLAUDE.md and show suggestions' },
    { command: 'gemiflow guidance optimize --apply', description: 'Apply optimizations to CLAUDE.md' },
    { command: 'gemiflow guidance optimize -s compact --apply', description: 'Optimize for compact context window' },
    { command: 'gemiflow guidance optimize --target-score 95', description: 'Optimize until score reaches 95' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const rootPath = ctx.flags.root as string || './CLAUDE.md';
    const localPath = ctx.flags.local as string | undefined;
    ...
    try {
      ...
      const { analyze, formatReport, optimizeForSize, formatBenchmark } = await import('@gemiflow/guidance/analyzer');
...
const abTestCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow guidance ab-test', description: 'Run default A/B test (no guidance vs ./CLAUDE.md)' },
    { command: 'gemiflow guidance ab-test -a old.md -b new.md', description: 'Compare two CLAUDE.md versions' },
    { command: 'gemiflow guidance ab-test --tasks custom-tasks.json', description: 'Run with custom test tasks' },
    { command: 'gemiflow guidance ab-test --json', description: 'Output full report as JSON' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      const { readFile } = await import('node:fs/promises');
      const { abBenchmark, getDefaultABTasks } = await import('@gemiflow/guidance/analyzer');
...
export const guidanceCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'gemiflow guidance compile', description: 'Compile CLAUDE.md into policy bundle' },
    { command: 'gemiflow guidance retrieve -t "Fix auth bug"', description: 'Retrieve relevant guidance' },
    { command: 'gemiflow guidance gates -c "rm -rf /"', description: 'Check enforcement gates' },
    { command: 'gemiflow guidance status', description: 'Show control plane status' },
    { command: 'gemiflow guidance optimize', description: 'Analyze and optimize CLAUDE.md' },
    { command: 'gemiflow guidance ab-test', description: 'Run A/B behavioral comparison' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
    ...
    output.writeln();
    output.writeln(output.dim('Use gemiflow guidance <subcommand> --help for details'));
