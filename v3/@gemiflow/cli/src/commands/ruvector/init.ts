...
export const initCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow ruvector init -d mydb', description: 'Initialize with database name' },
    { command: 'gemiflow ruvector init -d mydb -h db.example.com --ssl', description: 'Remote with SSL' },
    { command: 'gemiflow ruvector init -d mydb --force', description: 'Force re-initialization' },
    { command: 'gemiflow ruvector init -d mydb --dimensions 768', description: 'Custom vector dimensions' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      output.printBox([
        ...
        '',
        'Next steps:',
        '  1. Run migrations: gemiflow ruvector migrate --up',
        '  2. Check status: gemiflow ruvector status --verbose',
        '  3. Run benchmark: gemiflow ruvector benchmark',
