 ...
 *   npx gemiflow ruvector setup --output /path/to/dir
 *   npx gemiflow ruvector setup --print      # Print to stdout only
 *
...
services:
  postgres:
    ...
    environment:
      ...
      POSTGRES_PASSWORD: gemiflow-test
      POSTGRES_DB: gemiflow
    ports:
      - "5432:5432"
    volumes:
    ...
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U claude -d gemiflow"]
      interval: 5s
      timeout: 5s
      retries: 10
    command: >
  ...
  pgadmin:
    ...
    environment:
      PGADMIN_DEFAULT_EMAIL: admin@gemiflow.local
...
-- Create the gemiflow schema
CREATE SCHEMA IF NOT EXISTS gemiflow;

-- Grant permissions
GRANT ALL ON SCHEMA gemiflow TO claude;

-- Set search path
SET search_path TO gemiflow, public;
...
CREATE TABLE IF NOT EXISTS gemiflow.embeddings (
...
CREATE TABLE IF NOT EXISTS gemiflow.patterns (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS gemiflow.agents (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS gemiflow.trajectories (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
-- Memory entries table (main storage for GemiFlow memory)
CREATE TABLE IF NOT EXISTS gemiflow.memory_entries (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS gemiflow.hyperbolic_embeddings (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    content TEXT NOT NULL,
    ...
    parent_id UUID REFERENCES gemiflow.hyperbolic_embeddings(id),
    metadata JSONB DEFAULT '{}',
    created_at TIMESTAMPTZ DEFAULT NOW()
);

-- Graph nodes for GNN operations
CREATE TABLE IF NOT EXISTS gemiflow.graph_nodes (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS gemiflow.graph_edges (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    source_id UUID REFERENCES gemiflow.graph_nodes(id),
    target_id UUID REFERENCES gemiflow.graph_nodes(id),
...
ON gemiflow.graph_nodes
USING hnsw (embedding ruvector_cosine_ops)
WITH (m = 16, ef_construction = 64);

-- Additional indices for common queries
CREATE INDEX IF NOT EXISTS idx_embeddings_namespace ON gemiflow.embeddings(namespace);
CREATE INDEX IF NOT EXISTS idx_memory_entries_namespace ON gemiflow.memory_entries(namespace);
CREATE INDEX IF NOT EXISTS idx_memory_entries_key ON gemiflow.memory_entries(key);
...
CREATE OR REPLACE FUNCTION gemiflow.search_similar(
...
CREATE OR REPLACE FUNCTION gemiflow.search_memory(
    query_embedding ruvector(384),
...
BEGIN
    ...
    SELECT
        ...
        m.metadata
    FROM gemiflow.memory_entries m
...
CREATE OR REPLACE FUNCTION gemiflow.search_patterns(
    query_embedding ruvector(384),
...
CREATE OR REPLACE FUNCTION gemiflow.find_agents(
    query_embedding ruvector(384),
...
CREATE OR REPLACE FUNCTION gemiflow.to_poincare(
    euclidean real[],
    curvature FLOAT DEFAULT -1.0
)
RETURNS real[] AS $$
...
CREATE OR REPLACE FUNCTION gemiflow.poincare_distance(
    x real[],
    y real[],
    curvature FLOAT DEFAULT -1.0
)
...
CREATE OR REPLACE FUNCTION gemiflow.hyperbolic_search(
    query ruvector(384),
    limit_count INT DEFAULT 10,
    curvature FLOAT DEFAULT -1.0
)
...
BEGIN
    ...
    query_poincare := gemiflow.to_poincare(query_arr, curvature);
    ...
    SELECT
        ...
        he.hierarchy_level,
        he.metadata
    FROM gemiflow.hyperbolic_embeddings he
...
CREATE OR REPLACE FUNCTION gemiflow.ruvector_info()
RETURNS TABLE (
    version TEXT,
    simd_info TEXT
) AS $$
...
CREATE OR REPLACE FUNCTION gemiflow.cosine_similarity(
    a ruvector,
    b ruvector
)
RETURNS FLOAT AS $$
...
-- L2 distance helper
CREATE OR REPLACE FUNCTION gemiflow.l2_distance(
    a ruvector,
    b ruvector
)
RETURNS FLOAT AS $$
...
-- Upsert memory entry
CREATE OR REPLACE FUNCTION gemiflow.upsert_memory(
    p_key VARCHAR(255),
    p_value TEXT,
...
DECLARE
    v_id UUID;
BEGIN
    INSERT INTO gemiflow.memory_entries (key, value, embedding, namespace, metadata, ttl, updated_at)
    VALUES (p_key, p_value, p_embedding, p_namespace, p_metadata, p_ttl, NOW())
    ON CONFLICT (key, namespace) DO UPDATE SET
        value = EXCLUDED.value,
        embedding = COALESCE(EXCLUDED.embedding, gemiflow.memory_entries.embedding),
...
BEGIN
    ...
    RAISE NOTICE '  - gemiflow.search_similar(embedding, limit, min_sim)';
    RAISE NOTICE '  - gemiflow.search_memory(embedding, namespace, limit)';
    RAISE NOTICE '  - gemiflow.search_patterns(embedding, type, limit)';
    RAISE NOTICE '  - gemiflow.find_agents(embedding, type, limit)';
    RAISE NOTICE '  - gemiflow.hyperbolic_search(embedding, limit, curvature)';
    RAISE NOTICE '  - gemiflow.upsert_memory(key, value, embedding, namespace)';
    RAISE NOTICE '';
...
This directory contains the Docker configuration for RuVector PostgreSQL with GemiFlow V3.
...
docker exec ruvector-postgres psql -U claude -d gemiflow -c "SELECT ruvector_version();"
\`\`\`
...
\`\`\`bash
# Export current GemiFlow memory
npx gemiflow memory list --format json > memory-export.json
...
export const setupCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow ruvector setup', description: 'Output files to ./ruvector-postgres/' },
    { command: 'gemiflow ruvector setup --output /path/to/dir', description: 'Output to custom directory' },
    { command: 'gemiflow ruvector setup --print', description: 'Print files to stdout' },
    { command: 'gemiflow ruvector setup --force', description: 'Overwrite existing files' },
  ],
