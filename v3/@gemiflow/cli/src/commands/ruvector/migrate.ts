...
export const migrateCommand: Command = {
  ...
  examples: [
    ...
    { command: 'gemiflow ruvector migrate --down', description: 'Rollback last migration' },
    { command: 'gemiflow ruvector migrate --to 1.2.0', description: 'Migrate to version 1.2.0' },
    { command: 'gemiflow ruvector migrate --dry-run', description: 'Preview migration SQL' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (schemaExists.rows.length === 0) {
        ...
        output.printError('Run `gemiflow ruvector init` first');
        await client.end();
        return { success: false, exitCode: 1 };
      }
