...
export const ruvectorCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow ruvector setup', description: 'Output Docker files and SQL for setup' },
    { command: 'gemiflow ruvector import --input memory.json', description: 'Import from sql.js/JSON export' },
    { command: 'gemiflow ruvector init --database mydb', description: 'Initialize RuVector in PostgreSQL' },
    { command: 'gemiflow ruvector status --verbose', description: 'Check connection and schema status' },
    { command: 'gemiflow ruvector migrate --up', description: 'Run pending migrations' },
    { command: 'gemiflow ruvector benchmark --vectors 10000', description: 'Run performance benchmark' },
    { command: 'gemiflow ruvector optimize --analyze', description: 'Analyze and suggest optimizations' },
    { command: 'gemiflow ruvector backup --output backup.sql', description: 'Backup RuVector data' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.printBox([
      'RuVector provides PostgreSQL integration for GemiFlow with:',
      '',
    ...
    output.writeln();
    output.printInfo('Run `gemiflow ruvector <command> --help` for details');
    output.writeln();
