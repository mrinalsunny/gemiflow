 ...
 *   npx gemiflow ruvector import --from-memory
 *   npx gemiflow ruvector import --input data.json --batch-size 100
 *
...
function generateInsertSQL(entry: MemoryEntry): string {
  ...
  return `INSERT INTO gemiflow.memory_entries (key, value, embedding, namespace, metadata, created_at, updated_at)
VALUES (
  '${key}',
  '${value}',
  ${embeddingClause},
...
export const importCommand: Command = {
  ...
  options: [
    ...
    {
      name: 'from-memory',
      description: 'Export from current GemiFlow memory and import',
      type: 'boolean',
      default: false,
    },
    {
  ...
  examples: [
    { command: 'gemiflow ruvector import --input memory-export.json', description: 'Import from JSON file' },
    { command: 'gemiflow ruvector import --input data.json --output import.sql', description: 'Generate SQL file (dry-run)' },
    { command: 'gemiflow ruvector import --from-memory', description: 'Export current memory and import' },
    { command: 'gemiflow ruvector import --input data.json --container my-postgres', description: 'Import using custom container' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (!inputFile && !fromMemory) {
      ...
      output.writeln('  gemiflow ruvector import --input memory-export.json');
      output.writeln('  gemiflow ruvector import --from-memory');
    ...
    if (fromMemory) {
      output.printInfo('Exporting from current GemiFlow memory...');
      output.printWarning('Note: Run "npx gemiflow memory list --format json > memory-export.json" first');
      output.printInfo('Then use: npx gemiflow ruvector import --input memory-export.json');
    ...
    if (outputFile) {
      try {
        ...
        output.writeln(`  docker exec -i ${containerName} psql -U claude -d gemiflow < ${outputFile}`);
      } catch (error) {
        const errorMessage = error instanceof Error ? error.message : String(error);
      ...
      try {
        ...
        output.writeln(output.dim(`  docker exec -i ${containerName} psql -U claude -d gemiflow < ${tempFile}`));
        output.writeln();
        ...
        try {
          ...
          output.writeln(`  docker exec -i ${containerName} psql -U claude -d gemiflow < ${tempFile}`);
    ...
    output.writeln(`  docker exec ${containerName} psql -U claude -d gemiflow -c "SELECT COUNT(*) FROM gemiflow.memory_entries;"`);
    output.writeln();
