...
function updateSwarmActivityMetrics(agentCountDelta: number): void {
  try {
    const metricsDir = path.join(process.cwd(), '.gemiflow', 'metrics');
...
const spawnCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow agent spawn --type coder --name bot-1', description: 'Spawn a coder agent' },
    { command: 'gemiflow agent spawn -t researcher --task "Research React 19"', description: 'Spawn researcher with task' }
  ],
...
const poolCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow agent pool --size 5', description: 'Set pool size' },
    { command: 'gemiflow agent pool --min 2 --max 15', description: 'Configure auto-scaling' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    try {
      const result = await callMCPTool<{
...
const healthCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow agent health', description: 'Show all agents health' },
    { command: 'gemiflow agent health -i agent-001 -d', description: 'Detailed health for specific agent' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const logsCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow agent logs -i agent-001', description: 'Show agent logs' },
    { command: 'gemiflow agent logs -i agent-001 -f', description: 'Follow agent logs' },
    { command: 'gemiflow agent logs -l error --since 1h', description: 'Show errors from last hour' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const agentId = ctx.args[0] || ctx.flags.id as string;
...
export const agentCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'gemiflow agent spawn -t coder', description: 'Spawn a coder agent' },
    { command: 'gemiflow agent list', description: 'List all agents' },
    { command: 'gemiflow agent status agent-001', description: 'Show agent status' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln('Usage: gemiflow agent <subcommand> [options]');
    output.writeln();
    ...
    output.writeln();
    output.writeln('Run "gemiflow agent <subcommand> --help" for subcommand help');
