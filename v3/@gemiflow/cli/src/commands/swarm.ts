...
function getSwarmStatus(swarmId?: string) {
  ...
  const sessionDir = path.join(process.cwd(), '.gemiflow', 'sessions');
  const memoryPaths = [
...
const startCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow swarm start -o "Build REST API" -s development', description: 'Start development swarm' },
    { command: 'gemiflow swarm start -o "Analyze codebase" --parallel', description: 'Parallel analysis' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      // #2370: the old hint referenced the deprecated `gemiflow@v3alpha`
      // dist-tag which now resolves to a pre-rename package. Use the current
      // `gemiflow@latest` and force a fresh fetch with `-y` so npx doesn't pick
      // a stale local install.
      output.writeln(output.dim('  The MCP server may not be running. Start it with: claude mcp add gemiflow -- npx -y gemiflow@latest mcp start'));
    }
    ...
    output.writeln(output.dim('  - Gemini CLI Agent tool (interactive)'));
    output.writeln(output.dim('  - claude -p (headless background)'));
    output.writeln(output.dim('  - hive-mind spawn --claude (autonomous)'));
    output.writeln(output.dim(`  Monitor: gemiflow swarm status ${swarmId}`));
...
const statusCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (!status.hasActiveSwarm) {
      ...
      output.writeln(output.dim('  npx @gemiflow/cli@latest swarm init'));
      output.writeln(output.dim('  npx @gemiflow/cli@latest swarm start'));
      output.writeln();
      return { success: true, data: status };
    }
...
const coordinateCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln(output.dim('Note: Use Gemini CLI Task tool or hive-mind spawn --claude to'));
...
export const swarmCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'gemiflow swarm init --v3-mode', description: 'Initialize V3 swarm' },
    { command: 'gemiflow swarm start -o "Build API" -s development', description: 'Start development swarm' },
    { command: 'gemiflow swarm coordinate --agents 15', description: 'V3 coordination' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Swarm Coordination Commands'));
    output.writeln();
    output.writeln('Usage: gemiflow swarm <subcommand> [options]');
    output.writeln();
