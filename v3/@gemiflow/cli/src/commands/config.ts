...
const getCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow config get swarm.topology', description: 'Get swarm topology' },
    { command: 'gemiflow config get -k memory.backend', description: 'Get memory backend' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const setCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow config set swarm.maxAgents 20', description: 'Set max agents' },
    { command: 'gemiflow config set -k memory.backend -v agentdb', description: 'Set memory backend' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const key = ctx.flags.key as string || ctx.args[0];
...
const exportCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    try {
      const exportPath = (ctx.flags.output as string) || ctx.args[0] || 'gemiflow.config.export.json';
...
export const configCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'gemiflow config init --v3', description: 'Initialize V3 config' },
    { command: 'gemiflow config get swarm.topology', description: 'Get config value' },
    { command: 'gemiflow config set swarm.maxAgents 20', description: 'Set config value' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Configuration Management'));
    output.writeln();
    output.writeln('Usage: gemiflow config <subcommand> [options]');
    output.writeln();
