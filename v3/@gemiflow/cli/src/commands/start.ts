 ...
 * System startup for GemiFlow orchestration
 */
...
function isInitialized(cwd: string): boolean {
  const configPath = path.join(cwd, '.gemiflow', 'config.yaml');
...
const startAction = async (ctx: CommandContext): Promise<CommandResult> => {
  ...
  if (!isInitialized(cwd)) {
    output.printError('GemiFlow is not initialized in this directory');
    output.printInfo('Run "gemiflow init" first to initialize');
  ...
  output.writeln();
  output.writeln(output.bold('Starting GemiFlow V3'));
  output.writeln();
  ...
  try {
    ...
    // Success output
    output.writeln();
    output.printSuccess('GemiFlow V3 is running!');
    output.writeln();
    ...
    output.printList([
      `${output.highlight('gemiflow status')} - View system status`,
      `${output.highlight('gemiflow agent spawn -t coder')} - Spawn an agent`,
      `${output.highlight('gemiflow swarm status')} - View swarm details`,
      `${output.highlight('gemiflow stop')} - Stop the system`
    ]);

    // Daemon mode
    if (daemon) {
      output.writeln();
      output.printInfo('Running in daemon mode. Use "gemiflow stop" to stop.');

      // Store PID for daemon management
      const daemonPidPath = path.join(cwd, '.gemiflow', 'daemon.pid');
...
const stopCommand: Command = {
  name: 'stop',
  description: 'Stop the GemiFlow system',
  options: [
    {
      name: 'force',
      short: 'f',
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln(output.bold('Stopping GemiFlow'));
    output.writeln();

    if (!force && ctx.interactive) {
      const confirmed = await confirm({
        message: 'Are you sure you want to stop GemiFlow?',
        default: false
      });
    ...
    try {
      ...
      // Clean up daemon PID
      const daemonPidPath = path.join(ctx.cwd, '.gemiflow', 'daemon.pid');
      if (fs.existsSync(daemonPidPath)) {
        fs.unlinkSync(daemonPidPath);
      }

      output.writeln();
      output.printSuccess('GemiFlow stopped successfully');
...
const restartCommand: Command = {
  name: 'restart',
  description: 'Restart the GemiFlow system',
  options: [
    {
      name: 'force',
      short: 'f',
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Restarting GemiFlow'));
    output.writeln();
...
export const startCommand: Command = {
  name: 'start',
  description: 'Start the GemiFlow orchestration system',
  ...
  examples: [
    { command: 'gemiflow start', description: 'Start with configuration defaults' },
    { command: 'gemiflow start --daemon', description: 'Start as background daemon' },
    { command: 'gemiflow start --port 3001', description: 'Start MCP on custom port' },
    { command: 'gemiflow start --topology mesh', description: 'Start with mesh topology' },
    { command: 'gemiflow start --skip-mcp', description: 'Start without MCP server' },
    { command: 'gemiflow start quick', description: 'Quick start with defaults' },
    { command: 'gemiflow start stop', description: 'Stop the running system' }
  ],
  action: startAction
};
