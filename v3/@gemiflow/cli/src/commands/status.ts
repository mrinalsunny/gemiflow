 ...
 * System status display for GemiFlow
 */
...
//
// #2120 — the old check required `.gemiflow/config.yaml`, which
// missed projects that were initialized via `gemiflow memory init` (writes
...
// and a running MCP, yet `gemiflow status` reported "not initialized".
//
...
function isInitialized(cwd: string): boolean {
  const candidates = [
    ...
    path.join(cwd, '.gemiflow', 'config.json'),
    path.join(cwd, '.swarm', 'memory.db'),
    path.join(cwd, '.gemiflow', 'settings.json'),
  ];
...
function displayStatus(status: Awaited<ReturnType<typeof getSystemStatus>>): void {
  ...
  output.writeln(`${output.bold('GemiFlow V3')} ${statusIcon}`);
  output.writeln();
...
export const statusCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow status', description: 'Show current system status' },
    { command: 'gemiflow status --watch', description: 'Watch mode with live updates' },
    { command: 'gemiflow status --watch -i 5', description: 'Watch mode updating every 5 seconds' },
    { command: 'gemiflow status --health-check', description: 'Run health checks' },
    { command: 'gemiflow status --json', description: 'Output status as JSON' },
    { command: 'gemiflow status agents', description: 'Show detailed agent status' },
    { command: 'gemiflow status tasks', description: 'Show detailed task status' },
    { command: 'gemiflow status memory', description: 'Show detailed memory status' }
  ],
  action: statusAction
};
