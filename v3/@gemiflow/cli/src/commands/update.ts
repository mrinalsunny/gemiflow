...
const checkCommand: Command = {
  name: 'check',
  description: 'Check for available @gemiflow package updates',
  options: [
  ...
  async action(ctx: CommandContext): Promise<CommandResult> {
    ...
    try {
      ...
      if (results.length === 0) {
        output.printSuccess('All @gemiflow packages are up to date!');
        return { success: true };
      }
      ...
      if (manualUpdates.length > 0) {
        ...
        output.writeln('  gemiflow update all');
      }

      return { success: true };
    } finally {
      delete process.env.GEMIFLOW_FORCE_UPDATE;
    }
  },
};
...
const allCommand: Command = {
  name: 'all',
  description: 'Update all @gemiflow packages',
  options: [
...
const updateCommand: Command = {
  name: 'update',
  description: 'Manage @gemiflow package updates (ADR-025)',
  ...
  async action(): Promise<CommandResult> {
    ...
    output.writeln();
    output.writeln('Manage @gemiflow package updates with auto-update support.');
    output.writeln();
    ...
    output.printList([
      `${output.dim('GEMIFLOW_AUTO_UPDATE=false')}  - Disable auto-update`,
      `${output.dim('GEMIFLOW_FORCE_UPDATE=true')} - Force update check`,
    ]);
    output.writeln();
    output.writeln('Run "gemiflow update <subcommand> --help" for subcommand help');
