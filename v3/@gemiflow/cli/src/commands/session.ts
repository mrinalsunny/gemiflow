 ...
 * Session management for GemiFlow
 */
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (result.sessions.length === 0) {
        ...
        output.printInfo('Run "gemiflow session save" to create a session');
        return { success: true, data: result };
      }
...
const saveCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      output.printInfo(`Restore with: gemiflow session restore ${result.sessionId}`);
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    try {
      ...
      if (error instanceof MCPClientError) {
        ...
        output.printInfo('Start a session with "gemiflow start"');
...
export const sessionCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'gemiflow session list', description: 'List all sessions' },
    { command: 'gemiflow session save -n "checkpoint-1"', description: 'Save current session' },
    { command: 'gemiflow session restore session-123', description: 'Restore a session' },
    { command: 'gemiflow session delete session-123', description: 'Delete a session' },
    { command: 'gemiflow session export -o backup.json', description: 'Export session to file' },
    { command: 'gemiflow session import backup.json', description: 'Import session from file' },
    { command: 'gemiflow session current', description: 'Show current session' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln('Usage: gemiflow session <subcommand> [options]');
    output.writeln();
    ...
    output.writeln();
    output.writeln('Run "gemiflow session <subcommand> --help" for subcommand help');
