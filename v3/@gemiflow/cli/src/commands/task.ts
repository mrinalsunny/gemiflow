...
export const taskCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'gemiflow task create -t implementation -d "Add user auth"', description: 'Create a task' },
    { command: 'gemiflow task list', description: 'List pending/running tasks' },
    { command: 'gemiflow task list --all', description: 'List all tasks' },
    { command: 'gemiflow task status task-123', description: 'Get task details' },
    { command: 'gemiflow task cancel task-123', description: 'Cancel a task' },
    { command: 'gemiflow task assign task-123 --agent coder-1', description: 'Assign task to agent' },
    { command: 'gemiflow task retry task-123', description: 'Retry a failed task' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln('Usage: gemiflow task <subcommand> [options]');
    output.writeln();
    ...
    output.writeln();
    output.writeln('Run "gemiflow task <subcommand> --help" for subcommand help');
