...
async function getEmbeddings() {
  try {
    return await import('@gemiflow/embeddings');
  } catch {
    return null;
  }
}
...
const generateCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow embeddings generate -t "Hello world"', description: 'Generate embedding' },
    { command: 'gemiflow embeddings generate -t "Test" -o json', description: 'Output as JSON' },
  ],
...
const searchCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow embeddings search -q "error handling"', description: 'Search for similar' },
    { command: 'gemiflow embeddings search -q "test" -l 5', description: 'Limit results' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (!fs.existsSync(fullDbPath)) {
        ...
        output.printInfo('Run: gemiflow memory init');
        return { success: false, exitCode: 1 };
      }
      ...
      if (topResults.length === 0) {
        ...
        output.printInfo(`Try: gemiflow memory store -k "key" --value "your data"`);
...
const compareCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow embeddings compare --text1 "Hello" --text2 "Hi there"', description: 'Compare texts' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const collectionsCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow embeddings collections', description: 'List collections' },
    { command: 'gemiflow embeddings collections -a stats', description: 'Show detailed stats' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (collections.length === 0) {
        ...
        output.writeln(output.highlight('  gemiflow memory store -k "key" --value "data"'));
        return { success: true, data: [] };
      }
...
const indexCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow embeddings index', description: 'Show index status' },
    { command: 'gemiflow embeddings index -a build', description: 'Build index from all namespaces' },
    { command: 'gemiflow embeddings index -a rebuild -c project', description: 'Rebuild (label as `project`)' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (action === 'status') {
        ...
        if (status.available && status.entryCount > 0) {
          ...
          output.printInfo('Run: gemiflow memory store -k "key" --value "text"');
        } else {
          output.writeln();
      ...
      if (action === 'build' || action === 'rebuild') {
        ...
        if (!collection && newStatus.entryCount === 0) {
          ...
          output.printInfo('  gemiflow memory store -k "key" --value "text" --namespace <ns>');
        }
...
const initCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow embeddings init', description: 'Initialize with defaults' },
    { command: 'gemiflow embeddings init --model all-mpnet-base-v2', description: 'Use higher quality model' },
    { command: 'gemiflow embeddings init --no-hyperbolic', description: 'Euclidean only' },
    { command: 'gemiflow embeddings init --curvature=-0.5', description: 'Custom curvature (use = for negative)' },
    { command: 'gemiflow embeddings init --force', description: 'Overwrite existing config' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      // Create directories
      const configDir = path.join(process.cwd(), '.gemiflow');
      ...
      if (download) {
        ...
        if (embeddings) {
          ...
          output.writeln(output.dim('  (Skipped — @gemiflow/embeddings not installed)'));
        }
      }
...
const providersCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'gemiflow embeddings providers', description: 'List providers' },
  ],
...
const chunkCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow embeddings chunk -t "Long text..." -s 256', description: 'Chunk with 256 char limit' },
    { command: 'gemiflow embeddings chunk -f doc.txt --strategy paragraph', description: 'Chunk file by paragraph' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const embeddings = await getEmbeddings();
    const text = ctx.flags.text as string || '';
    ...
    if (!embeddings) {
      output.printWarning('@gemiflow/embeddings not installed, showing preview');
      output.writeln();
      output.printBox([
        `Strategy: ${strategy}`,
...
const normalizeCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow embeddings normalize -i "[0.5, 0.3, 0.8]" -t l2', description: 'L2 normalize' },
    { command: 'gemiflow embeddings normalize --check -i "[...]"', description: 'Check if normalized' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const hyperbolicCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow embeddings hyperbolic -a convert -i "[0.5, 0.3]"', description: 'Convert to Poincaré' },
    { command: 'gemiflow embeddings hyperbolic -a distance', description: 'Compute hyperbolic distance' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      const hyperbolic = await import('@gemiflow/embeddings').then(m => m).catch(() => null);

      if (!hyperbolic || !hyperbolic.euclideanToPoincare) {
        output.printWarning('@gemiflow/embeddings hyperbolic module not available');
        output.printInfo('Install with: npm install @gemiflow/embeddings');
        return { success: false, exitCode: 1 };
      }
...
const neuralCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow embeddings neural --init', description: 'Initialize RuVector substrate' },
    { command: 'gemiflow embeddings neural -f drift', description: 'Semantic drift detection' },
    { command: 'gemiflow embeddings neural -f memory', description: 'Memory physics (hippocampal)' },
    { command: 'gemiflow embeddings neural -f coherence', description: 'Safety & alignment monitoring' },
    { command: 'gemiflow embeddings neural --drift-threshold=0.2', description: 'Custom drift threshold' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    const fs = await import('fs');
    const path = await import('path');
    const configPath = path.join(process.cwd(), '.gemiflow', 'embeddings.json');
    ...
    if (!neuralConfig.enabled) {
      ...
      output.writeln(output.dim('Configuration: .gemiflow/embeddings.json'));
...
const modelsCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow embeddings models', description: 'List models' },
    { command: 'gemiflow embeddings models -d all-MiniLM-L6-v2', description: 'Download model' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (download) {
      ...
      if (embeddings) {
        ...
        await new Promise(r => setTimeout(r, 500));
        spinner.succeed(`Download skipped — @gemiflow/embeddings not installed`);
      }
      return { success: true };
    }
...
const cacheCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow embeddings cache', description: 'Show cache stats' },
    { command: 'gemiflow embeddings cache -a clear', description: 'Clear cache' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const warmupCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow embeddings warmup', description: 'Preload model with test' },
    { command: 'gemiflow embeddings warmup -b', description: 'Background warmup' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const benchmarkCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow embeddings benchmark', description: 'Quick benchmark' },
    { command: 'gemiflow embeddings benchmark -n 50 -f', description: 'Full benchmark' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
export const embeddingsCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow embeddings init', description: 'Initialize ONNX embedding system' },
    { command: 'gemiflow embeddings init --model all-mpnet-base-v2', description: 'Init with larger model' },
    { command: 'gemiflow embeddings generate -t "Hello"', description: 'Generate embedding' },
    { command: 'gemiflow embeddings search -q "error handling"', description: 'Semantic search' },
    { command: 'gemiflow embeddings chunk -t "Long doc..."', description: 'Chunk document' },
    { command: 'gemiflow embeddings hyperbolic -a convert', description: 'Hyperbolic space' },
    { command: 'gemiflow embed neural -f drift', description: 'Neural substrate' },
  ],
  action: async (): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('GemiFlow Embeddings'));
