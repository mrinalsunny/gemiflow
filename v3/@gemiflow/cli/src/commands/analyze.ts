...
const diffCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow analyze diff --risk', description: 'Analyze current diff with risk assessment' },
    { command: 'gemiflow analyze diff HEAD~1 --classify', description: 'Classify changes from last commit' },
    { command: 'gemiflow analyze diff main..feature --format json', description: 'Compare branches with JSON output' },
    { command: 'gemiflow analyze diff --reviewers', description: 'Get recommended reviewers for changes' },
  ],
...
const codeCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow analyze code -p ./src', description: 'Analyze source directory' },
    { command: 'gemiflow analyze code --type complexity', description: 'Run complexity analysis' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const astCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow analyze ast src/', description: 'Analyze all files in src/' },
    { command: 'gemiflow analyze ast src/index.ts --complexity', description: 'Analyze with complexity' },
    { command: 'gemiflow analyze ast . --format json', description: 'JSON output' },
    { command: 'gemiflow analyze ast src/ --symbols', description: 'Extract symbols' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const complexityAstCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow analyze complexity src/', description: 'Analyze complexity' },
    { command: 'gemiflow analyze complexity src/ --threshold 15', description: 'Flag high complexity' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const targetPath = ctx.args[0] || ctx.cwd;
...
const symbolsCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow analyze symbols src/', description: 'Extract all symbols' },
    { command: 'gemiflow analyze symbols src/ --type function', description: 'Only functions' },
    { command: 'gemiflow analyze symbols src/ --format json', description: 'JSON output' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const targetPath = ctx.args[0] || ctx.cwd;
...
const importsCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow analyze imports src/', description: 'Analyze all imports' },
    { command: 'gemiflow analyze imports src/ --external', description: 'Only npm packages' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const targetPath = ctx.args[0] || ctx.cwd;
    const formatType = (ctx.flags.format as string) || 'text';
...
const depsCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow analyze deps --outdated', description: 'Show outdated dependencies' },
    { command: 'gemiflow analyze deps --security', description: 'Check for vulnerabilities' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const boundariesCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow analyze boundaries src/', description: 'Find code boundaries in src/' },
    { command: 'gemiflow analyze boundaries -p 3 src/', description: 'Find 3 partitions' },
    { command: 'gemiflow analyze boundaries -f dot -o graph.dot src/', description: 'Export to DOT format' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const modulesCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow analyze modules src/', description: 'Detect module communities' },
    { command: 'gemiflow analyze modules -f dot -o modules.dot src/', description: 'Export colored DOT graph' },
    { command: 'gemiflow analyze modules -m 3 src/', description: 'Only show communities with 3+ files' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const targetDir = ctx.args[0] || ctx.cwd;
    const outputFile = ctx.flags.output as string | undefined;
...
const dependenciesCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow analyze dependencies src/', description: 'Build dependency graph' },
    { command: 'gemiflow analyze dependencies -f dot -o deps.dot src/', description: 'Export to DOT' },
    { command: 'gemiflow analyze dependencies -i .ts,.tsx src/', description: 'Only TypeScript files' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const targetDir = ctx.args[0] || ctx.cwd;
    const outputFile = ctx.flags.output as string | undefined;
...
const circularCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow analyze circular src/', description: 'Find circular dependencies' },
    { command: 'gemiflow analyze circular -s high src/', description: 'Only high severity cycles' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const targetDir = ctx.args[0] || ctx.cwd;
    const outputFile = ctx.flags.output as string | undefined;
...
export const analyzeCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow analyze ast src/', description: 'Analyze code with AST parsing' },
    { command: 'gemiflow analyze complexity src/ --threshold 15', description: 'Find high-complexity files' },
    { command: 'gemiflow analyze symbols src/ --type function', description: 'Extract all functions' },
    { command: 'gemiflow analyze imports src/ --external', description: 'List npm dependencies' },
    { command: 'gemiflow analyze diff --risk', description: 'Analyze diff with risk assessment' },
    { command: 'gemiflow analyze boundaries src/', description: 'Find code boundaries using MinCut' },
    { command: 'gemiflow analyze modules src/', description: 'Detect module communities with Louvain' },
    { command: 'gemiflow analyze dependencies src/ --format dot', description: 'Export dependency graph as DOT' },
    { command: 'gemiflow analyze circular src/', description: 'Find circular dependencies' },
    { command: 'gemiflow analyze deps --security', description: 'Check dependency vulnerabilities' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln(`  ${output.dim('gemiflow analyze ast src/')}                  # Full AST analysis`);
    output.writeln(`  ${output.dim('gemiflow analyze ast src/index.ts -c')}       # Include complexity`);
    output.writeln(`  ${output.dim('gemiflow analyze complexity src/ -t 15')}     # Flag high complexity`);
    output.writeln(`  ${output.dim('gemiflow analyze symbols src/ --type fn')}    # Extract functions`);
    output.writeln(`  ${output.dim('gemiflow analyze imports src/ --external')}   # Only npm imports`);
    output.writeln();

    output.writeln(output.bold('Graph Analysis Examples:'));
    output.writeln();
    output.writeln(`  ${output.dim('gemiflow analyze boundaries src/')}            # Find natural code boundaries`);
    output.writeln(`  ${output.dim('gemiflow analyze modules src/')}               # Detect module communities`);
    output.writeln(`  ${output.dim('gemiflow analyze dependencies -f dot src/')}   # Export to DOT format`);
    output.writeln(`  ${output.dim('gemiflow analyze circular src/')}              # Find circular deps`);
    output.writeln();

    output.writeln(output.bold('Diff Analysis Examples:'));
    output.writeln();
    output.writeln(`  ${output.dim('gemiflow analyze diff --risk')}              # Risk assessment`);
    output.writeln(`  ${output.dim('gemiflow analyze diff HEAD~1 --classify')}   # Classify changes`);
    output.writeln(`  ${output.dim('gemiflow analyze diff main..feature')}       # Compare branches`);
    output.writeln();
