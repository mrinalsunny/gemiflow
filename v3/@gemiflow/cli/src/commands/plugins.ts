...
const listCommand: Command = {
  ...
  options: [
    ...
    { name: 'registry', short: 'r', type: 'string', description: 'Registry to use (default: gemiflow-official)' },
  ],
  examples: [
    { command: 'gemiflow plugins list', description: 'List all plugins from registry' },
    { command: 'gemiflow plugins list --installed', description: 'List installed only' },
    { command: 'gemiflow plugins list --official', description: 'List official plugins' },
    { command: 'gemiflow plugins list --category security', description: 'List security plugins' },
  ],
    ...
    if (installedOnly) {
      ...
      try {
        ...
        if (installed.length === 0) {
          ...
          output.writeln();
          output.writeln(output.dim('Run "gemiflow plugins list" to see available plugins'));
          output.writeln(output.dim('Run "gemiflow plugins install -n <plugin>" to install'));
          return { success: true };
        }
...
const installCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow plugins install -n community-analytics', description: 'Install plugin from IPFS' },
    { command: 'gemiflow plugins install -n ./my-plugin --dev', description: 'Install local plugin' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (existingPlugin) {
        ...
        output.writeln();
        output.writeln(output.dim('Use "gemiflow plugins upgrade -n ' + name + '" to update'));
        return { success: false, exitCode: 1 };
      }
...
const uninstallCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow plugins uninstall -n community-analytics', description: 'Uninstall plugin' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const name = ctx.flags.name as string;
...
const toggleCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow plugins toggle -n analytics --enable', description: 'Enable plugin' },
    { command: 'gemiflow plugins toggle -n analytics --disable', description: 'Disable plugin' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const name = ctx.flags.name as string;
...
const infoCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow plugins info -n @gemiflow/neural', description: 'Show plugin info' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const name = ctx.flags.name as string;
    const registryName = ctx.flags.registry as string;
...
const createCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow plugins create -n my-plugin', description: 'Create basic plugin' },
    { command: 'gemiflow plugins create -n my-plugin -t hooks', description: 'Create hooks plugin' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const name = ctx.flags.name as string;
...
const upgradeCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow plugins upgrade -n @gemiflow/neural', description: 'Upgrade to latest' },
    { command: 'gemiflow plugins upgrade -n @gemiflow/neural -v 3.1.0', description: 'Upgrade to specific version' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const name = ctx.flags.name as string;
...
const searchCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow plugins search -q neural', description: 'Search for neural plugins' },
    { command: 'gemiflow plugins search -q security --verified', description: 'Search verified security plugins' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const rateCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow plugins rate -n @gemiflow/embeddings -r 5', description: 'Rate 5 stars' },
    { command: 'gemiflow plugins rate -n my-plugin -r 4', description: 'Rate 4 stars' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
export const pluginsCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow plugins list', description: 'List plugins from IPFS registry' },
    { command: 'gemiflow plugins search -q neural', description: 'Search for plugins' },
    { command: 'gemiflow plugins install -n community-analytics', description: 'Install from IPFS' },
    { command: 'gemiflow plugins create -n my-plugin', description: 'Create new plugin' },
  ],
  action: async (): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('GemiFlow Plugin System'));
    ...
    output.printList([
      '@gemiflow/neural              - Neural patterns and inference (WASM SIMD)',
      '@gemiflow/security            - Security scanning and CVE detection',
      '@gemiflow/embeddings          - Vector embeddings with hyperbolic support',
      '@gemiflow/claims              - Claims-based authorization',
      '@gemiflow/performance         - Performance profiling and benchmarks',
      '@gemiflow/plugin-gastown-bridge - Gas Town orchestrator integration (WASM-accelerated)',
    ]);
    output.writeln();
    output.writeln(output.dim('Run "gemiflow plugins list --official" to see all official plugins'));
