...
const benchmarkCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow performance benchmark -s neural', description: 'Benchmark neural operations' },
    { command: 'gemiflow performance benchmark -i 1000', description: 'Run with 1000 iterations' },
  ],
...
const profileCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow performance profile -t cpu', description: 'Profile CPU usage' },
    { command: 'gemiflow performance profile -d 60', description: 'Profile for 60 seconds' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const metricsCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow performance metrics -t 7d', description: 'Show 7-day metrics' },
    { command: 'gemiflow performance metrics -f prometheus', description: 'Export as Prometheus format' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (format === 'prometheus') {
      output.writeln(`# HELP gemiflow_heap_used_bytes Heap memory used`);
      output.writeln(`gemiflow_heap_used_bytes ${memUsage.heapUsed}`);
      output.writeln(`# HELP gemiflow_heap_total_bytes Total heap memory`);
      output.writeln(`gemiflow_heap_total_bytes ${memUsage.heapTotal}`);
      output.writeln(`# HELP gemiflow_rss_bytes Resident set size`);
      output.writeln(`gemiflow_rss_bytes ${memUsage.rss}`);
      output.writeln(`# HELP gemiflow_cpu_user_microseconds CPU user time`);
      output.writeln(`gemiflow_cpu_user_microseconds ${cpuUsage.user}`);
      output.writeln(`# HELP gemiflow_cpu_system_microseconds CPU system time`);
      output.writeln(`gemiflow_cpu_system_microseconds ${cpuUsage.system}`);
      output.writeln(`# HELP gemiflow_cache_entries Embedding cache entries`);
      output.writeln(`gemiflow_cache_entries ${cacheEntries}`);
      output.writeln(`# HELP gemiflow_hnsw_entries HNSW index entries`);
      output.writeln(`gemiflow_hnsw_entries ${hnswEntries}`);
      output.writeln(`# HELP gemiflow_uptime_seconds Process uptime`);
      output.writeln(`gemiflow_uptime_seconds ${uptime}`);
      return { success: true };
    }
...
const optimizeCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow performance optimize -t memory', description: 'Optimize memory usage' },
    { command: 'gemiflow performance optimize --apply', description: 'Apply all optimizations' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const bottleneckCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow performance bottleneck', description: 'Find bottlenecks' },
    { command: 'gemiflow performance bottleneck -d full', description: 'Full analysis' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
...
export const performanceCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow performance benchmark', description: 'Run benchmarks' },
    { command: 'gemiflow performance profile', description: 'Profile application' },
    { command: 'gemiflow perf metrics', description: 'View metrics (alias)' },
  ],
  action: async (): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('GemiFlow Performance Suite'));
