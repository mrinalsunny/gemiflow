...
async function loadModule<T>(path: string, exportName: string, label: string): Promise<T | null> {
  try {
    ...
    output.printError(`RVFA ${label} module not found`, 'Install with: npm install @gemiflow/appliance');
    return null;
  }
}
...
const buildCommand: Command = {
  name: 'build',
  description: 'Build a self-contained gemiflow.rvf appliance',
  options: [
    { name: 'profile', short: 'p', type: 'string', description: 'Build profile: cloud, hybrid, offline', default: 'cloud' },
    { name: 'output', short: 'o', type: 'string', description: 'Output file path', default: 'gemiflow.rvf' },
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    const outputPath = ctx.flags.output as string || 'gemiflow.rvf';
    ...
    const steps = [
      ...
      'Packaging gemiflow CLI + MCP tools', 'Compressing sections',
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      for (const [label, value] of [
        ['Name', hdr.name || 'gemiflow'], ['Version', hdr.version || 'unknown'],
...
const extractCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      for (const d of ['kernel', 'runtime', 'gemiflow', 'models', 'data', 'verify']) {
...
export const applianceCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow appliance build -p cloud', description: 'Build a cloud appliance' },
    { command: 'gemiflow appliance inspect -f gemiflow.rvf', description: 'Inspect appliance contents' },
    { command: 'gemiflow appliance verify -f gemiflow.rvf', description: 'Verify integrity' },
    { command: 'gemiflow appliance extract -f gemiflow.rvf', description: 'Extract sections' },
    { command: 'gemiflow appliance run -f gemiflow.rvf', description: 'Boot and run appliance' },
    { command: 'gemiflow appliance sign -f gemiflow.rvf --generate-keys', description: 'Generate keys and sign' },
    { command: 'gemiflow appliance publish -f gemiflow.rvf', description: 'Publish to IPFS via Pinata' },
    { command: 'gemiflow appliance update -f gemiflow.rvf -s gemiflow -d ./new-gemiflow.bin', description: 'Hot-patch a section' },
  ],
  action: async (): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('GemiFlow Appliance (RVFA)'));
    output.writeln(output.dim('Self-contained deployment format for the full GemiFlow platform.'));
    output.writeln();
    output.writeln('Subcommands:');
    output.printList([
      'build     - Build a self-contained gemiflow.rvf appliance',
    ...
    output.writeln();
    output.writeln(output.dim('Use "gemiflow appliance <subcommand> --help" for details.'));
    return { success: true };
  },
};
