...
async function checkConfigFile(): Promise<HealthCheck> {
  ...
  // pre-v3 init flows; v3 init writes only `.gemiflow/config.yaml`.
  const jsonPaths = [
  ...
  return { name: 'Config File', status: 'warn', message: 'No config file (using defaults)', fix: 'gemiflow config init' };
}

// Check daemon status
async function checkDaemonStatus(): Promise<HealthCheck> {
  try {
    const pidFile = '.gemiflow/daemon.pid';
    if (existsSync(pidFile)) {
      ...
      try {
        ...
        return { name: 'Daemon Status', status: 'warn', message: 'Stale PID file', fix: 'rm .gemiflow/daemon.pid && gemiflow daemon start' };
      }
    }
    return { name: 'Daemon Status', status: 'warn', message: 'Not running', fix: 'gemiflow daemon start' };
  } catch {
    return { name: 'Daemon Status', status: 'warn', message: 'Unable to check', fix: 'gemiflow daemon status' };
  }
}
...
async function checkMemoryDatabase(): Promise<HealthCheck> {
  ...
  // `GEMIFLOW_MEMORY_PATH`, gemiflow.config.json's `memory.persistPath`,
  ...
  candidates.push(
    '.swarm/memory.db',
    '.gemiflow/memory.db',
    'data/memory/memory.db', // matches `GEMIFLOW_MEMORY_PATH=data/memory`
    'data/memory.db',
  );
  ...
  return { name: 'Memory Database', status: 'warn', message: 'Not initialized', fix: 'gemiflow memory configure --backend hybrid' };
}
...
// `@gemiflow/aidefence` to be installed and loadable. The package is an
// optional dependency — present in some installs (project-local) but
// missing in others (npm-global of `gemiflow`). Without it, every
...
async function checkAIDefence(): Promise<HealthCheck> {
  try {
    ...
    await import('@gemiflow/aidefence');
    return {
      name: 'AIDefence',
      status: 'pass',
      message: '@gemiflow/aidefence loadable — aidefence_* MCP tools functional',
    };
  } catch {
    return {
      name: 'AIDefence',
      status: 'warn',
      message: '@gemiflow/aidefence not loadable — aidefence_* MCP tools will fail (optional package)',
      fix: 'npm install --save @gemiflow/aidefence  (in your project), or run `gemiflow mcp start` from a directory that has it installed',
    };
  }
}
...
async function checkFederationBreaker(): Promise<HealthCheck> {
  try {
    // Optional plugin — not a hard dep of @gemiflow/cli. Build the
    ...
    const specifier = ['@gemiflow', 'plugin-agent-federation'].join('/');
    ...
    if (!mod.FederationNodeState) {
      return {
        ...
        status: 'warn',
        message:
          '@gemiflow/plugin-agent-federation loaded but FederationNodeState export missing — version older than ADR-097 Phase 2',
        fix: 'Upgrade: npm install @gemiflow/plugin-agent-federation@alpha',
      };
    }
    return {
      name: 'Federation Breaker',
    ...
    return {
      ...
      fix: 'npm install --save @gemiflow/plugin-agent-federation@alpha',
    };
  }
}
...
async function checkMcpServers(): Promise<HealthCheck> {
  ...
  // #1842: ~/.gemiflow.json holds project-scoped registrations under
  // parsed.projects[<projectPath>].mcpServers.gemiflow, in addition to any
  // top-level mcpServers. Check both shapes plus the legacy desktop and
  // local .mcp.json paths.
  const mcpConfigPaths = [
    join(home, '.claude.json'),
    join(home, '.gemiflow/claude_desktop_config.json'),
    join(home, '.config/claude/mcp.json'),
    '.mcp.json',
  ];

  const isGemiFlowKey = (k: string) =>
    k === 'gemiflow' || k === 'gemiflow_alpha' || k === 'gemiflow' || k === 'gemiflow_alpha';
      ...
      if (content.projects && typeof content.projects === 'object') {
        for (const projectVal of Object.values(content.projects)) {
          ...
          if (pm && typeof pm === 'object') {
            ...
            if (keys.some(isGemiFlowKey)) projectHits += 1;
          }
        }
      }

      const totalServers = topServerKeys.length + projectScannedServers;
      if (topHasGemiFlow || projectHits > 0) {
        const where = topHasGemiFlow
          ? 'top-level'
          : `${projectHits} project-scoped`;
        return {
          name: 'MCP Servers',
          status: 'pass',
          message: `${totalServers} servers (gemiflow configured: ${where})`,
        };
      }
      if (totalServers > 0) {
        return {
          name: 'MCP Servers',
          status: 'warn',
          message: `${totalServers} servers (gemiflow not found)`,
          fix: 'claude mcp add gemiflow -- npx -y gemiflow@latest mcp start',
        };
      }
    } catch {
...
async function checkVersionFreshness(): Promise<HealthCheck> {
  try {
    ...
    try {
      ...
      // package.json that belongs to @gemiflow/cli (or gemiflow/cli).
        ...
        try {
          if (existsSync(candidate)) {
            ...
            if (
              ...
              (pkg.name === '@gemiflow/cli' || pkg.name === 'gemiflow' || pkg.name === 'gemiflow')
            ) {
    ...
    try {
      const npmInfo = await runCommand('npm view @gemiflow/cli@alpha version', 5000);
    ...
    if (isOutdated) {
      const fix = isNpx
        ? 'rm -rf ~/.npm/_npx/* && npx -y @gemiflow/cli@latest'
        : 'npm update @gemiflow/cli';
...
//   1. Gate status — is GEMIFLOW_ENCRYPT_AT_REST set?
//   2. Key resolution — does GEMIFLOW_ENCRYPTION_KEY resolve to a valid
...
async function checkEncryptionAtRest(): Promise<HealthCheck> {
  if (!isEncryptionEnabled()) {
    return {
      ...
      fix: 'export GEMIFLOW_ENCRYPT_AT_REST=1 && export GEMIFLOW_ENCRYPTION_KEY=<64-char-hex>',
    };
  }

  // Gate is on — try to resolve the key. Fail-closed if missing or malformed.
  const rawKey = process.env.GEMIFLOW_ENCRYPTION_KEY;
  if (!rawKey) {
    return {
      name: 'Encryption at Rest',
      status: 'fail',
      message: 'Gate is on but GEMIFLOW_ENCRYPTION_KEY is unset (fail-closed)',
      fix: 'Generate a key: openssl rand -hex 32 → export GEMIFLOW_ENCRYPTION_KEY=<value>',
    };
  }
  ...
  try {
    ...
    return {
      name: 'Encryption at Rest',
      status: 'fail',
      message: `GEMIFLOW_ENCRYPTION_KEY invalid: ${err instanceof Error ? err.message : String(err)}`,
  ...
  const stores: Array<{ label: string; path: string }> = [
    { label: 'sessions/', path: join(cwd, '.gemiflow', 'sessions') },
    { label: 'terminals', path: join(cwd, '.gemiflow', 'terminals', 'store.json') },
...
export const doctorCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow doctor', description: 'Run full health check' },
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln(output.bold('GemiFlow Doctor'));
