...
function generateBashCompletion(): string {
  return `# gemiflow bash completion
# Generated by gemiflow completions bash
# Add this to ~/.bashrc or ~/.bash_completion

_gemiflow_completions() {
...
complete -F _gemiflow_completions gemiflow
complete -F _gemiflow_completions npx\\ @gemiflow/cli@v3alpha
`;
}

// Generate zsh completion script
function generateZshCompletion(): string {
  return `#compdef gemiflow
# gemiflow zsh completion
# Generated by gemiflow completions zsh
# Add to ~/.zfunc/_gemiflow or ~/.zsh/completions/_gemiflow
...
_gemiflow() {
    ...
    case $state in
        command)
            _describe -t commands 'gemiflow commands' commands
            ;;
        subcommand)
            case $words[2] in
                swarm)
...
function generateFishCompletion(): string {
  return `# gemiflow fish completion
# Generated by gemiflow completions fish
# Save to ~/.config/fish/completions/gemiflow.fish

# Disable file completion by default
complete -c gemiflow -f

# Top-level commands
${TOP_LEVEL_COMMANDS.map(cmd => `complete -c gemiflow -n "__fish_use_subcommand" -a "${cmd}"`).join('\n')}

# Swarm subcommands
${SWARM_SUBCOMMANDS.map(sub => `complete -c gemiflow -n "__fish_seen_subcommand_from swarm" -a "${sub}"`).join('\n')}

# Agent subcommands
${AGENT_SUBCOMMANDS.map(sub => `complete -c gemiflow -n "__fish_seen_subcommand_from agent" -a "${sub}"`).join('\n')}

# Task subcommands
${TASK_SUBCOMMANDS.map(sub => `complete -c gemiflow -n "__fish_seen_subcommand_from task" -a "${sub}"`).join('\n')}

# Memory subcommands
${MEMORY_SUBCOMMANDS.map(sub => `complete -c gemiflow -n "__fish_seen_subcommand_from memory" -a "${sub}"`).join('\n')}

# Hive-mind subcommands
${HIVE_MIND_SUBCOMMANDS.map(sub => `complete -c gemiflow -n "__fish_seen_subcommand_from hive-mind hive" -a "${sub}"`).join('\n')}

# Hooks subcommands
${HOOKS_SUBCOMMANDS.map(sub => `complete -c gemiflow -n "__fish_seen_subcommand_from hooks" -a "${sub}"`).join('\n')}

# Neural subcommands
complete -c gemiflow -n "__fish_seen_subcommand_from neural" -a "train status patterns predict optimize"

# Security subcommands
complete -c gemiflow -n "__fish_seen_subcommand_from security" -a "scan cve threats audit secrets"

# Performance subcommands
complete -c gemiflow -n "__fish_seen_subcommand_from performance" -a "benchmark profile metrics optimize bottleneck"

# Plugins subcommands
complete -c gemiflow -n "__fish_seen_subcommand_from plugins" -a "list install uninstall toggle info create"

# Deployment subcommands
complete -c gemiflow -n "__fish_seen_subcommand_from deployment deploy" -a "deploy status rollback history environments logs"

# Claims subcommands
complete -c gemiflow -n "__fish_seen_subcommand_from claims" -a "list check grant revoke roles policies"

# Embeddings subcommands
complete -c gemiflow -n "__fish_seen_subcommand_from embeddings" -a "generate search compare collections index providers"

# Daemon subcommands
complete -c gemiflow -n "__fish_seen_subcommand_from daemon" -a "start stop status trigger enable"

# Providers subcommands
complete -c gemiflow -n "__fish_seen_subcommand_from providers" -a "list configure test models usage"
`;
}

// Generate PowerShell completion script
function generatePowerShellCompletion(): string {
  return `# gemiflow PowerShell completion
# Generated by gemiflow completions powershell
# Add to $PROFILE or save to a separate file and dot-source it

$script:gemiflowCommands = @(
...
Register-ArgumentCompleter -Native -CommandName gemiflow -ScriptBlock {
    ...
    if ($tokens.Count -eq 2) {
        ...
        $script:gemiflowCommands | Where-Object { $_ -like "$wordToComplete*" } | ForEach-Object {
...
export const completionsCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'gemiflow completions bash > ~/.bash_completion.d/gemiflow', description: 'Install bash completions' },
    { command: 'gemiflow completions zsh > ~/.zfunc/_gemiflow', description: 'Install zsh completions' },
    { command: 'gemiflow completions fish > ~/.config/fish/completions/gemiflow.fish', description: 'Install fish completions' },
    { command: 'gemiflow completions powershell >> $PROFILE', description: 'Install PowerShell completions' }
  ],
  action: async (): Promise<CommandResult> => {
    ...
    output.writeln(output.dim('  gemiflow completions bash > ~/.bash_completion.d/gemiflow'));
    output.writeln(output.dim('  source ~/.bash_completion.d/gemiflow'));
    output.writeln();
    output.writeln(output.bold('Zsh:'));
    output.writeln(output.dim('  mkdir -p ~/.zfunc'));
    output.writeln(output.dim('  gemiflow completions zsh > ~/.zfunc/_gemiflow'));
    output.writeln(output.dim('  # Add to ~/.zshrc: fpath=(~/.zfunc $fpath); autoload -Uz compinit && compinit'));
    output.writeln();
    output.writeln(output.bold('Fish:'));
    output.writeln(output.dim('  gemiflow completions fish > ~/.config/fish/completions/gemiflow.fish'));
    output.writeln();
    output.writeln(output.bold('PowerShell:'));
    output.writeln(output.dim('  gemiflow completions powershell >> $PROFILE'));
