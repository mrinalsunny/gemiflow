...
export const wasmStatusCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'gemiflow agent wasm-status', description: 'Check WASM agent runtime status' },
  ],
...
export const wasmPromptCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow agent wasm-prompt -a wasm-agent-1-abc -i "Write a hello world"', description: 'Send prompt to WASM agent' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
export const wasmGalleryCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow agent wasm-gallery', description: 'List all gallery templates' },
    { command: 'gemiflow agent wasm-gallery -s coder', description: 'Search gallery templates' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    try {
      const wasm = await loadWasm();
