...
const statusCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    // Check v2 config: gemiflow.config.json with version "2" or missing version
    const v2ConfigPath = path.join(cwd, 'gemiflow.config.json');
    const v3ConfigDir = path.join(cwd, '.gemiflow');
    ...
    const migrationStatePath = path.join(cwd, '.gemiflow', 'migration-state.json');
    ...
    output.writeln();
    if (needsMigration) {
      output.printInfo('V2 artifacts detected. Run "gemiflow migrate run" to migrate.');
    } else {
...
const runCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    const v3Dir = path.join(cwd, '.gemiflow');
    ...
    // Ensure .gemiflow directory exists
    if (!dryRun) {
      ...
      if (fs.existsSync(v2ConfigPath)) {
        ...
        if (parsed.version === '2' || parsed.version === 2 || !parsed.version) {
          if (dryRun) {
            ...
            // Backup
            if (!skipBackup) {
              fs.copyFileSync(v2ConfigPath, path.join(backupDir, 'gemiflow.config.json'));
            }
      ...
      if (fs.existsSync(v2MemoryDir)) {
        ...
        if (jsonFiles.length > 0 || hasDb) {
          if (dryRun) {
            ...
            output.printInfo('Run "gemiflow memory init --force" to import v2 memory into v3 AgentDB.');
          }
    ...
    if (dryRun) {
      ...
      output.printInfo('Run "gemiflow migrate verify" to validate the migration.');
    } else {
...
const verifyCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (allPassed) {
      ...
      output.printInfo('Run "gemiflow migrate run" to re-run the migration, or "migrate rollback" to restore from backup.');
    }
...
const rollbackCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      // Restore config
      const backupConfig = path.join(backupPath, 'gemiflow.config.json');
      if (fs.existsSync(backupConfig)) {
        const destConfig = path.join(cwd, 'gemiflow.config.json');
...
const breakingCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const changes = [
      ...
      },
      {
        ...
        changes: [
          ...
          { change: 'Storage path changed', from: '.gemiflow/memory', to: 'data/memory' }
        ]
      },
      {
        category: 'CLI',
    ...
    output.printInfo('Run "gemiflow migrate run" to automatically handle these changes');
...
export const migrateCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'gemiflow migrate status', description: 'Check migration status' },
    { command: 'gemiflow migrate run --dry-run', description: 'Preview migration' },
    { command: 'gemiflow migrate run -t all', description: 'Run full migration' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('V2 to V3 Migration Tools'));
    output.writeln();
    output.writeln('Usage: gemiflow migrate <subcommand> [options]');
    output.writeln();
...
function getMigrationSteps(target: string): Array<{ name: string; description: string; source: string; dest: string }> {
  const allSteps = [
    { name: 'Configuration Files', description: 'Migrate config schema to V3 format', source: './gemiflow.json', dest: './gemiflow.config.json' },
    { name: 'Memory Backend', description: 'Upgrade to hybrid backend with AgentDB', source: './.gemiflow/memory', dest: './data/memory' },
    { name: 'Agent Definitions', description: 'Convert agent configs to V3 format', source: './.gemiflow/agents', dest: './v3/agents' },
    { name: 'Hook Registry', description: 'Migrate hooks to V3 hook system', source: './src/hooks', dest: './v3/hooks' },
    { name: 'Workflow Definitions', description: 'Convert workflows to event-sourced format', source: './.gemiflow/workflows', dest: './data/workflows' },
    { name: 'Embeddings System', description: 'Migrate to ONNX with hyperbolic (Poincaré ball)', source: 'OpenAI/TF.js embeddings', dest: '.gemiflow/embeddings.json' }
  ];
