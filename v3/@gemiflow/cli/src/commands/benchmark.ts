...
const pretrainCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow benchmark pretrain', description: 'Run pre-training benchmarks' },
    { command: 'gemiflow benchmark pretrain -i 500 --save results.json', description: 'Extended benchmark with results saved' },
    { command: 'gemiflow benchmark pretrain -o json', description: 'Output results as JSON' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (saveFile) {
        const resultsDir = join(process.cwd(), '.gemiflow', 'benchmarks');
...
const neuralCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow benchmark neural', description: 'Run neural benchmarks' },
    { command: 'gemiflow benchmark neural -d 768 -n 5000', description: 'Higher dimension, more vectors' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const iterations = parseInt(ctx.flags.iterations as string || '100', 10);
...
const memoryCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow benchmark memory', description: 'Run memory benchmarks' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const iterations = parseInt(ctx.flags.iterations as string || '100', 10);
    const outputFormat = ctx.flags.output as string || 'text';
    ...
    try {
      ...
      try {
        ...
        // @gemiflow/memory not available — return null metrics instead of fake numbers
...
const allCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow benchmark all', description: 'Run all benchmarks' },
    { command: 'gemiflow benchmark all --save full-results.json', description: 'Run all and save results' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold(output.highlight('═'.repeat(65))));
    output.writeln(output.bold('  GemiFlow V3 - Full Benchmark Suite'));
    output.writeln(output.bold(output.highlight('═'.repeat(65))));
...
export const benchmarkCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow benchmark pretrain', description: 'Benchmark pre-training system' },
    { command: 'gemiflow benchmark neural', description: 'Benchmark neural operations' },
    { command: 'gemiflow benchmark memory', description: 'Benchmark memory operations' },
    { command: 'gemiflow benchmark all', description: 'Run all benchmarks' },
  ],
  action: async (_ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('GemiFlow V3 Benchmark Suite'));
    ...
    output.writeln('  gemiflow benchmark pretrain -i 200');
    output.writeln('  gemiflow benchmark all --save results.json');
    output.writeln();
