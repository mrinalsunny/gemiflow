...
const trainCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow neural train -p coordination -e 100', description: 'Train coordination patterns' },
    { command: 'gemiflow neural train -d ./training-data.json --flash', description: 'Train from file with Flash Attention' },
    { command: 'gemiflow neural train -p security --wasm --contrastive', description: 'Security patterns with contrastive learning' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      try {
        ...
        const cpDir = path.join(process.cwd(), '.gemiflow', 'neural');
...
const statusCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow neural status', description: 'Show all neural status' },
    { command: 'gemiflow neural status -m model-123', description: 'Check specific model' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const patternsCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow neural patterns --action list', description: 'List all patterns' },
    { command: 'gemiflow neural patterns -a analyze -q "error handling"', description: 'Analyze patterns' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      output.writeln(output.dim('Run: gemiflow neural train --pattern-type general'));
...
const predictCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow neural predict -i "implement authentication"', description: 'Predict routing for task' },
    { command: 'gemiflow neural predict -i "fix bug in login" -k 3', description: 'Get top 3 predictions' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (matches.length === 0) {
        output.writeln(output.warning('No similar patterns found. Try training first: gemiflow neural train'));
...
const optimizeCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow neural optimize --method quantize', description: 'Quantize patterns to Int8' },
    { command: 'gemiflow neural optimize --method analyze -v', description: 'Analyze memory usage' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      const patternDir = path.join(process.cwd(), '.gemiflow', 'neural');
      let beforeSize = 0;
      try {
...
const exportCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow neural export -m security-patterns --ipfs', description: 'Export and pin to IPFS' },
    { command: 'gemiflow neural export -m code-review -o ./export.json', description: 'Export to file' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      const exportData = {
        ...
        version: '1.0.0',
        name: customName || `gemiflow-model-${Date.now()}`,
      ...
      const memoryDir = path.join(process.cwd(), '.gemiflow', 'memory');
      ...
      if (pinToIpfs) {
        ...
        output.writeln(output.dim(`Import command: gemiflow neural import --cid ${result.IpfsHash}`));
      }
...
const listCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow neural list', description: 'List all available models' },
    { command: 'gemiflow neural list --category security', description: 'List only security models' },
    { command: 'gemiflow neural list -f json', description: 'Output as JSON' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (format === 'json') {
        ...
        output.writeln(output.dim('  All models:      ') + `gemiflow neural import --cid ${registryCid}`);
        if (category) {
          output.writeln(output.dim(`  ${category} only: `) + `gemiflow neural import --cid ${registryCid} --category ${category}`);
        } else {
          output.writeln(output.dim('  By category:     ') + `gemiflow neural import --cid ${registryCid} --category <category>`);
        }
      }
...
const importCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow neural import --cid QmXxx...', description: 'Import from IPFS' },
    { command: 'gemiflow neural import -f ./patterns.json --verify', description: 'Import from file' },
    { command: 'gemiflow neural import --cid QmNr1yYMK... --category security', description: 'Import only security patterns' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      output.writeln(output.dim('Run "gemiflow neural patterns --action list" to see imported patterns'));
...
const benchmarkCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow neural benchmark', description: 'Run default benchmark' },
    { command: 'gemiflow neural benchmark -d 128 -i 5000', description: 'Custom benchmark' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const dim = Math.min(parseInt(ctx.flags.dim as string || '256', 10), 256);
...
export const neuralCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow neural status', description: 'Check neural system status' },
    { command: 'gemiflow neural train -p coordination', description: 'Train coordination patterns' },
    { command: 'gemiflow neural patterns --action list', description: 'List learned patterns' },
  ],
  action: async (): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('GemiFlow Neural System'));
