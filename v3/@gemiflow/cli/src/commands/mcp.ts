 ...
 *
 * @module @gemiflow/cli/commands/mcp
 * @version 3.0.0
 */
...
const startCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow mcp start', description: 'Start with defaults (stdio)' },
    { command: 'gemiflow mcp start -p 8080 -t http', description: 'Start HTTP server' },
    { command: 'gemiflow mcp start -d', description: 'Start as daemon' },
    { command: 'gemiflow mcp start -f', description: 'Force restart (kill existing)' }
  ],
    ...
    if (existingStatus.running && !isSelfDetected) {
      ...
      if (!shouldForceRestart) {
        ...
        if (health.healthy) {
          ...
          output.writeln(output.dim('Use "gemiflow mcp stop" to stop the server first, or use --force'));
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    try {
      ...
      if (!status.running) {
        ...
        output.writeln();
        output.writeln(output.dim('Run "gemiflow mcp start" to start the server'));
        return { success: true, data: status };
      }
...
const execCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow mcp exec -t swarm_init -p \'{"topology":"mesh"}\'', description: 'Execute tool' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
export const mcpCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'gemiflow mcp start', description: 'Start MCP server' },
    { command: 'gemiflow mcp start -t http -p 8080', description: 'Start HTTP server on port 8080' },
    { command: 'gemiflow mcp status', description: 'Show server status' },
    { command: 'gemiflow mcp tools', description: 'List tools' },
    { command: 'gemiflow mcp stop', description: 'Stop the server' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('MCP Server Management'));
    output.writeln();
    output.writeln('Usage: gemiflow mcp <subcommand> [options]');
    output.writeln();
