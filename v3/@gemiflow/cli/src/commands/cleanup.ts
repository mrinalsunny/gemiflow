 ...
 * Removes project artifacts created by gemiflow/gemiflow
 *
 * Created with ruv.io
 */
...
/**
 * Artifact directories and files that gemiflow/gemiflow may create
 */
const ARTIFACT_DIRS = [
  { path: '.gemiflow', description: 'Capabilities and configuration' },
...
const ARTIFACT_FILES = [
  { path: 'gemiflow.config.json', description: 'GemiFlow configuration' },
];
...
export const cleanupCommand: Command = {
  name: 'cleanup',
  description: 'Remove project artifacts created by gemiflow/gemiflow',
  aliases: ['clean'],
  options: [
    {
      name: 'dry-run',
    ...
    },
    {
      name: 'keep-config',
      short: 'k',
      description: 'Preserve gemiflow.config.json and .gemiflow/settings.json',
      type: 'boolean',
      default: false,
    },
  ],
  examples: [
    ...
    },
    {
      ...
      description: 'Remove all gemiflow artifacts',
    },
    {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    // Check if .gemiflow/settings.json has gemiflow hooks/gemiflow blocks to clean
    const settingsPath = join(cwd, '.gemiflow', 'settings.json');
    if (existsSync(settingsPath)) {
      found.push({ path: join('.gemiflow', 'settings.json'), description: 'Remove gemiflow hooks/gemiflow blocks (preserves rest)', size: 0, type: 'file' });
    }
    ...
    if (found.length === 0) {
      output.writeln(output.info('No gemiflow artifacts found in the current directory.'));
    ...
    for (const item of found) {
      ...
      if (dryRun) {
        ...
        try {
          ...
          if (item.path === join('.gemiflow', 'settings.json')) {
            try {
