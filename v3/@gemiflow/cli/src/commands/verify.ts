 ...
 *
 * Run via: gemiflow verify [--branch <branch>] [--manifest <local-path>]
 *
...
const DEFAULT_MANIFEST_URL = 'https://raw.githubusercontent.com/mrinalsunny/gemiflow/{branch}/verification.md.json';
...
export const verifyCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow verify', description: 'Fetch latest manifest from main branch + verify' },
    { command: 'gemiflow verify --branch main', description: 'Verify against a specific branch' },
    { command: 'gemiflow verify --manifest ./verification.md.json', description: 'Use a local manifest copy' },
    { command: 'gemiflow verify --json', description: 'Machine-readable output for CI' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (!asJson) {
      output.writeln();
      output.writeln(output.bold('GemiFlow Verification'));
