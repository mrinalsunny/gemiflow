...
const runCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow workflow run -t development --task "Build auth system"', description: 'Run development workflow' },
    { command: 'gemiflow workflow run -f ./workflow.yaml', description: 'Run from file' },
    { command: 'gemiflow workflow run -t sparc --dry-run', description: 'Validate SPARC workflow' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (!dryRun) {
        output.writeln();
        output.printInfo(`Track progress: gemiflow workflow status ${result.workflowId}`);
      }
...
const validateCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow workflow validate -f ./workflow.yaml', description: 'Validate workflow file' },
    { command: 'gemiflow workflow validate -f ./workflow.json --strict', description: 'Strict validation' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const templateCommand: Command = {
  ...
  subcommands: [
    ...
    },
    {
      ...
      action: async (ctx: CommandContext): Promise<CommandResult> => {
        ...
        output.writeln(output.dim('  Use with: gemiflow workflow run -t ' + name));
  ...
  action: async (): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln('Usage: gemiflow workflow template <subcommand>');
    output.writeln();
...
export const workflowCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'gemiflow workflow run -t development --task "Build feature"', description: 'Run workflow' },
    { command: 'gemiflow workflow validate -f ./workflow.yaml', description: 'Validate workflow' },
    { command: 'gemiflow workflow list', description: 'List workflows' }
  ],
  action: async (): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('Workflow Commands'));
    output.writeln();
    output.writeln('Usage: gemiflow workflow <subcommand> [options]');
    output.writeln();
    output.writeln('Subcommands:');
    output.printList([
    ...
    output.writeln();
    output.writeln('Run "gemiflow workflow <subcommand> --help" for more info');
