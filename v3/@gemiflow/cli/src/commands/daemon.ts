...
const startCommand: Command = {
  ...
  options: [
    ...
    // keep dispatching headless worker sweeps. Default 12h (or GEMIFLOW_DAEMON_TTL_SECS); 0 = run until stopped.
  ...
  examples: [
    { command: 'gemiflow daemon start', description: 'Start daemon in background (default)' },
    { command: 'gemiflow daemon start --foreground', description: 'Start in foreground (blocks terminal)' },
    { command: 'gemiflow daemon start -w map,audit,optimize', description: 'Start with specific workers' },
    { command: 'gemiflow daemon start --headless --sandbox strict', description: 'Start with headless workers in strict sandbox' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    const isDaemonProcess = process.env.GEMIFLOW_DAEMON === '1';
    ...
    try {
      const stateDir = join(projectRoot, '.gemiflow');
...
function validatePath(path: string, label: string): void {
  ...
  if (!resolved.includes('.gemiflow') && !resolved.includes('bin')) {
...
async function startBackgroundDaemon(projectRoot: string, quiet: boolean, forwarded: ForwardedDaemonFlags = {}): Promise<CommandResult> {
  ...
  const stateDir = join(resolvedRoot, '.gemiflow');
  const pidFile = join(stateDir, 'daemon.pid');
  ...
  const forkOpts: Record<string, unknown> = {
    ...
    // daemon writes its own logs via appendFileSync to .gemiflow/logs/.
  ...
  if (!quiet) {
    ...
    output.printInfo(`Stop with: gemiflow daemon stop`);
  }
...
const stopCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow daemon stop', description: 'Stop the daemon' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const quiet = ctx.flags.quiet as boolean;
...
async function killBackgroundDaemon(projectRoot: string): Promise<boolean> {
  const pidFile = join(projectRoot, '.gemiflow', 'daemon.pid');
...
async function killStaleDaemonsPosix(projectRoot: string, quiet: boolean): Promise<void> {
  try {
    ...
    for (const line of lines) {
      ...
      if (!line.includes('gemiflow') && !line.includes('@gemiflow/cli')) continue;
...
/**
 * #2356: enumerate every running gemiflow daemon across ALL workspaces. Reuses
...
async function renderAllDaemonsStatus(): Promise<CommandResult> {
  ...
  if (daemons.length === 0) {
    output.printBox(
      'No gemiflow daemons are running in any workspace.',
      'GemiFlow Daemons (all workspaces)'
    );
  ...
  const rows = daemons.map(d => {
    ...
    if (d.workspace) {
      try {
        const statePath = join(d.workspace, '.gemiflow', 'daemon-state.json');
  ...
  if (staleCount > 0) {
    output.printWarning(
      `${staleCount} daemon(s) have outlived their TTL (or have run >12h). Stop one with: cd <workspace> && gemiflow daemon stop`
    );
  } else {
...
const statusCommand: Command = {
  ...
  options: [
    ...
    // leaked in another project is invisible. --all scans every running gemiflow
    // daemon across all workspaces (the global view that surfaces leaks).
    { name: 'all', short: 'a', type: 'boolean', description: 'List gemiflow daemons across ALL workspaces (global view — surfaces leaked daemons)' },
  ],
  examples: [
    { command: 'gemiflow daemon status', description: 'Show daemon status' },
    { command: 'gemiflow daemon status -v', description: 'Show detailed status' },
    { command: 'gemiflow daemon status --show-modes', description: 'Show worker execution modes' },
    { command: 'gemiflow daemon status --all', description: 'List daemons across all workspaces' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      output.printBox(
        [
          ...
          '',
          'Run "gemiflow daemon start" to start the daemon',
        ].join('\n'),
        'GemiFlow Daemon'
      );
...
const triggerCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow daemon trigger -w map', description: 'Trigger the map worker' },
    { command: 'gemiflow daemon trigger -w audit', description: 'Trigger security audit' },
    { command: 'gemiflow daemon trigger -w audit --headless', description: 'Trigger audit in headless sandbox' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
const enableCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow daemon enable -w predict', description: 'Enable predict worker' },
    { command: 'gemiflow daemon enable -w document --disable', description: 'Disable document worker' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const workerType = ctx.flags.worker as WorkerType;
...
const installSupervisorCommand: Command = {
  ...
  examples: [
    { command: 'gemiflow daemon install-supervisor', description: 'Install + load (auto-restart enabled)' },
    { command: 'gemiflow daemon install-supervisor --no-load', description: 'Write unit file but do not enable yet' },
    { command: 'gemiflow daemon install-supervisor --dry-run', description: 'Preview the unit file' },
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (platform === 'win32') {
      ...
      output.printInfo('Use Task Scheduler manually, or follow this issue: https://github.com/mrinalsunny/gemiflow/issues/1565');
      return { success: false, exitCode: 1 };
    }
    ...
    if (platform === 'darwin') {
      ...
      const plistPath = join(plistDir, 'io.ruv.gemiflow.daemon.plist');
      const logDir = join(projectRoot, '.gemiflow', 'logs');
      const plist = `<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0">
<dict>
    <key>Label</key><string>io.ruv.gemiflow.daemon</string>
    ...
    <dict>
        <key>GEMIFLOW_DAEMON</key><string>1</string>
    </dict>
</dict>
</plist>
`;
    ...
    const unitPath = join(unitDir, 'gemiflow-daemon.service');
    const unit = `[Unit]
Description=GemiFlow background worker daemon
After=default.target

[Service]
Type=simple
WorkingDirectory=${projectRoot}
Environment=GEMIFLOW_DAEMON=1
...
`;
    ...
    if (load) {
      try {
        ...
        execFileSync('systemctl', ['--user', 'enable', '--now', 'gemiflow-daemon.service'], { encoding: 'utf-8', timeout: 10000 });
        ...
        output.printInfo(`Run manually: systemctl --user daemon-reload && systemctl --user enable --now gemiflow-daemon.service`);
      }
    } else {
      output.printInfo(`Run when ready:  systemctl --user daemon-reload && systemctl --user enable --now gemiflow-daemon.service`);
    }
    return { success: true };
  },
};
...
const uninstallSupervisorCommand: Command = {
  ...
  action: async (): Promise<CommandResult> => {
    ...
    if (platform === 'darwin') {
      const plistPath = join(home, 'Library', 'LaunchAgents', 'io.ruv.gemiflow.daemon.plist');
      try {
        const { execFileSync } = await import('child_process');
        try { execFileSync('launchctl', ['unload', plistPath], { encoding: 'utf-8', timeout: 5000 }); } catch { /* ok */ }
      } catch { /* ignore */ }
    ...
    if (platform === 'linux') {
      const unitPath = join(home, '.config', 'systemd', 'user', 'gemiflow-daemon.service');
      try {
        const { execFileSync } = await import('child_process');
        try { execFileSync('systemctl', ['--user', 'disable', '--now', 'gemiflow-daemon.service'], { encoding: 'utf-8', timeout: 5000 }); } catch { /* ok */ }
      } catch { /* ignore */ }
...
export const daemonCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'gemiflow daemon start', description: 'Start the daemon' },
    { command: 'gemiflow daemon start --headless', description: 'Start with headless workers (E2B sandbox)' },
    { command: 'gemiflow daemon status', description: 'Check daemon status' },
    { command: 'gemiflow daemon stop', description: 'Stop the daemon' },
    { command: 'gemiflow daemon trigger -w audit', description: 'Run security audit' },
  ],
  action: async (): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('GemiFlow Daemon - Background Task Management'));
    output.writeln();
    ...
    output.writeln();
    output.writeln('Run "gemiflow daemon <subcommand> --help" for details');
