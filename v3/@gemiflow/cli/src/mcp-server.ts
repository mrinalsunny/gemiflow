 ...
 *
 * @module @gemiflow/cli/mcp-server
 * @version 3.0.0
 */
...
const DEFAULT_OPTIONS: Required<MCPServerOptions> = {
  ...
  host: 'localhost',
  port: 3000,
  pidFile: path.join(os.tmpdir(), 'gemiflow-mcp.pid'),
  logFile: path.join(os.tmpdir(), 'gemiflow-mcp.log'),
  tools: 'all',
  daemonize: false,
  timeout: 30000,
};
...
export class MCPServerManager extends EventEmitter {
  ...
  private async startStdioServer(): Promise<void> {
    ...
    console.error(
      `[${new Date().toISOString()}] INFO [gemiflow-mcp] (${sessionId}) Starting in stdio mode`
    );
    ...
    try {
      ...
      if (!status.initialized) {
        console.error(
          `[${new Date().toISOString()}] INFO [gemiflow-mcp] (${sessionId}) Auto-initializing memory database...`
        );
        const result = await initializeMemoryDatabase({ force: false, verbose: false });
        if (result.success) {
          console.error(
            `[${new Date().toISOString()}] INFO [gemiflow-mcp] (${sessionId}) Memory database initialized at ${result.dbPath}`
          );
        } else if (result.error && !result.error.includes('already exists')) {
          console.error(
            `[${new Date().toISOString()}] WARN [gemiflow-mcp] (${sessionId}) Memory database init returned: ${result.error}`
          );
        }
      } else {
        console.error(
          `[${new Date().toISOString()}] INFO [gemiflow-mcp] (${sessionId}) Memory database already initialized (v${status.version || 'unknown'})`
        );
      }
    } catch (memInitError) {
      ...
      console.error(
        `[${new Date().toISOString()}] WARN [gemiflow-mcp] (${sessionId}) Memory auto-init failed (tools will retry on first call): ${memInitError instanceof Error ? memInitError.message : String(memInitError)}`
      );
    }
    ...
    process.stdin.on('data', async (chunk) => {
      ...
      if (buffer.length > MAX_BUFFER_SIZE) {
        console.error(
          `[${new Date().toISOString()}] ERROR [gemiflow-mcp] Buffer exceeded ${MAX_BUFFER_SIZE} bytes, rejecting`
        );
        buffer = '';
        writeFrame({
          jsonrpc: '2.0',
      ...
      for (const line of lines) {
        if (line.trim()) {
          try {
            ...
            console.error(
              `[${new Date().toISOString()}] ERROR [gemiflow-mcp] Failed to parse message:`,
    ...
    process.stdin.on('end', () => {
      console.error(
        `[${new Date().toISOString()}] INFO [gemiflow-mcp] (${sessionId}) stdin closed, shutting down...`
      );
      process.exit(0);
    });
    ...
    process.on('SIGINT', () => {
      console.error(
        `[${new Date().toISOString()}] INFO [gemiflow-mcp] (${sessionId}) Received SIGINT, shutting down...`
      );
      process.exit(0);
    });
    ...
    process.on('SIGTERM', () => {
      console.error(
        `[${new Date().toISOString()}] INFO [gemiflow-mcp] (${sessionId}) Received SIGTERM, shutting down...`
      );
      process.exit(0);
    });
    ...
    try {
      switch (message.method) {
        case 'initialize':
          return {
            ...
            result: {
              ...
              serverInfo: { name: 'gemiflow', version: '3.0.0' },
              capabilities: {
                tools: { listChanged: true },
                resources: { subscribe: true, listChanged: true },
              },
        ...
        case 'notifications/initialized':
          ...
          console.error(
            `[${new Date().toISOString()}] INFO [gemiflow-mcp] (${sessionId}) Client initialized`
          );
          return null;
      ...
      console.error(
        `[${new Date().toISOString()}] ERROR [gemiflow-mcp] Error handling ${message.method}:`,
        error
      );
      return {
        jsonrpc: '2.0',
  ...
  private async startHttpServer(): Promise<void> {
    ...
    const { createMCPServer } = await import('@gemiflow/mcp');
  ...
  private async removePidFile(): Promise<void> {
    ...
    try {
      const legacyPath = path.join(process.env.GEMIFLOW_CWD || process.cwd(), '.gemiflow', 'mcp-server.pid');
  ...
  /**
   * Check if process is running AND is a node/gemiflow process.
  ...
  private isProcessRunning(pid: number): boolean {
    ...
    try {
      ...
      return cmdline.includes('node') || cmdline.includes('gemiflow') || cmdline.includes('npx');
    } catch {
