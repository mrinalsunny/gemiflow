...
// but were never registered, so the `gemiflow hooks coverage-*` CLI subcommands
...
/**
 * Lifecycle MCP tools for gemiflow-browser session-as-skill architecture
 * (ADR-0001 gemiflow-browser §7). Always registered: their handlers shell out
 * to ruvector + agent-browser + gemiflow memory and degrade gracefully
 ...
 * With `GEMIFLOW_STRICT_GUARDRAIL=true`, scans every string field of the
 ...
 * cost of `@gemiflow/security` does not hit every CLI invocation. Once
...
function applyContentBoundaryGuardrail(toolName: string, result: unknown): unknown {
  if (process.env.GEMIFLOW_STRICT_GUARDRAIL !== 'true') return result;
  ...
  if (_guardrailInstance === null) {
    try {
      ...
      const sec = require('@gemiflow/security') as {
