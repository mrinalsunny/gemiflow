/**
 * Helpers Generator
 * Creates utility scripts in .gemiflow/helpers/
 */
...
export function generatePreCommitHook(): string {
  return `#!/bin/bash
# GemiFlow Pre-Commit Hook
# Validates code quality before commit

set -e

echo "🔍 Running GemiFlow pre-commit checks..."
...
for FILE in $STAGED_FILES; do
  if [[ "$FILE" =~ \\.(ts|js|tsx|jsx)$ ]]; then
    ...
    npx @gemiflow/cli hooks pre-edit --file "$FILE" --validate-syntax 2>/dev/null || true
  fi
done
...
# Notify gemiflow of commit
npx gemiflow@latest hooks notify \\
...
const SESSION_DIR = path.join(process.cwd(), '.gemiflow', 'sessions');
...
const MEMORY_DIR = path.join(process.cwd(), '.gemiflow', 'data');
...
export function generateHookHandler(): string {
  ...
  const lines = [
    '#!/usr/bin/env node',
    '/**',
    ' * GemiFlow Hook Handler (Cross-Platform)',
    ...
    "    console.log('   - All agents available in .gemiflow/agents/ directory');",
...
export function generateIntelligenceStub(): string {
  const lines = [
    ...
    "const os = require('os');",
    '',
    "const DATA_DIR = path.join(process.cwd(), '.gemiflow', 'data');",
    ...
    '  var entries = [];',
    '  var candidates = [',
    '    path.join(os.homedir(), ".gemiflow", "projects"),',
    '    path.join(process.cwd(), ".gemiflow", "memory"),',
    '    path.join(process.cwd(), ".gemiflow", "memory"),',
    '  ];',
 ...
 * @gemiflow/memory is not installed. Gets overwritten when source copy succeeds.
 */
...
const DATA_DIR = join(PROJECT_ROOT, '.gemiflow', 'data');
...
async function loadMemoryPackage() {
  ...
  // when installed as a transitive dependency via npx gemiflow / npx gemiflow)
  try {
    const { createRequire } = await import('module');
    const require = createRequire(join(PROJECT_ROOT, 'package.json'));
    return require('@gemiflow/memory');
  } catch { /* fall through */ }

  // Strategy 2: ESM import (works when @gemiflow/memory is a direct dependency)
  try { return await import('@gemiflow/memory'); } catch { /* fall through */ }

  // Strategy 3: Walk up from PROJECT_ROOT looking for the package in any node_modules
  let searchDir = PROJECT_ROOT;
  const { parse } = await import('path');
  while (searchDir !== parse(searchDir).root) {
    const candidate = join(searchDir, 'node_modules', '@gemiflow', 'memory', 'dist', 'index.js');
...
export function generateWindowsDaemonManager(): string {
  return `# GemiFlow V3 Daemon Manager for Windows
...
function Start-SwarmMonitor {
    ...
    $process = Start-Process -FilePath 'node' -ArgumentList @(
        '-e',
        'setInterval(() => { require("fs").writeFileSync(".gemiflow/metrics/swarm-activity.json", JSON.stringify({swarm:{active:true,agent_count:0},timestamp:Date.now()})) }, 5000)'
...
function Show-Status {
    Write-Host ""
    Write-Host "GemiFlow V3 Daemon Status" -ForegroundColor Cyan
...
export function generateWindowsBatchWrapper(): string {
  return `@echo off
REM GemiFlow V3 - Windows Batch Wrapper
...
export function generateCrossPlatformSessionManager(): string {
  return `#!/usr/bin/env node
/**
 * GemiFlow Cross-Platform Session Manager
...
function getDataDir() {
  const localDir = path.join(process.cwd(), '.gemiflow', 'sessions');
  ...
  switch (platform) {
    case 'win32':
      return path.join(process.env.APPDATA || homeDir, 'gemiflow', 'sessions');
    case 'darwin':
      return path.join(homeDir, 'Library', 'Application Support', 'gemiflow', 'sessions');
    default:
      return path.join(homeDir, '.gemiflow', 'sessions');
  }
}
...
export function generateHelpers(options: InitOptions): Record<string, string> {
  ...
  if (options.components.helpers) {
    ...
    // string so init-generated PR templates can `cat .gemiflow/helpers/attribution`
...
/**
 * Generate cross-platform Node.js port of gemiflow-hook.sh (#2132).
 *
 * The bash shim works on Mac/Linux but fails on native Windows (exit 126).
 * This .cjs version is always deployed to .gemiflow/helpers/ so:
 ...
 *   - Both: .gemiflow/helpers/gemiflow-hook.cjs available as a canonical cross-platform shim
 */
export function generateGemiFlowHookCjs(): string {
  return `#!/usr/bin/env node
/**
 * gemiflow-hook.cjs — cross-platform Node.js port of gemiflow-hook.sh (#2132)
 *
 * Deployed to .gemiflow/helpers/ during gemiflow init. On Windows, the
 * generated .gemiflow/settings.json hooks point here instead of the
...
function main() {
  ...
  if (commandExists('gemiflow')) { invokeHook('gemiflow', [], hookArgs, stdinData); done(); }
  if (commandExists('gemiflow')) { invokeHook('gemiflow', [], hookArgs, stdinData); done(); }
