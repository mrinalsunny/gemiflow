 ...
 * ADR-128 Phase 4: every subdirectory under .gemiflow/commands/ now has a
...
const COMMANDS_MAP: Record<string, string[]> = {
  core: ['gemiflow-help.md', 'gemiflow-swarm.md', 'gemiflow-memory.md'],
  analysis: ['analysis'],
...
export async function executeInit(options: InitOptions): Promise<InitResult> {
  ...
  try {
    ...
    // Generate CLAUDE.md
    if (options.components.geminiMd) {
...
function mergeSettingsForUpgrade(existing: Record<string, unknown>): Record<string, unknown> {
  ...
  merged.env = {
    ...
    GEMIFLOW_V3_ENABLED: existingEnv.GEMIFLOW_V3_ENABLED || 'true',
    GEMIFLOW_HOOKS_ENABLED: existingEnv.GEMIFLOW_HOOKS_ENABLED || 'true',
  };
  ...
  const autoMemoryScript = '.gemiflow/helpers/auto-memory-hook.mjs';
  ...
  try {
    ...
    if (sourceHelpersForUpgrade) {
      ...
      for (const helperName of criticalHelpers) {
        const targetPath = path.join(targetDir, '.gemiflow', 'helpers', helperName);
        const sourcePath = path.join(sourceHelpersForUpgrade, helperName);
        if (fs.existsSync(sourcePath)) {
          if (fs.existsSync(targetPath)) {
            result.updated.push(`.gemiflow/helpers/${helperName}`);
          } else {
            result.created.push(`.gemiflow/helpers/${helperName}`);
          }
    ...
    const statuslinePath = path.join(targetDir, '.gemiflow', 'helpers', 'statusline.cjs');
    ...
    if (fs.existsSync(statuslinePath)) {
      result.updated.push('.gemiflow/helpers/statusline.cjs');
    } else {
      result.created.push('.gemiflow/helpers/statusline.cjs');
    }
    fs.writeFileSync(statuslinePath, statuslineContent, 'utf-8');

    // 2. Create MISSING metrics files only (preserve existing data)
    const metricsDir = path.join(targetDir, '.gemiflow', 'metrics');
    const securityDir = path.join(targetDir, '.gemiflow', 'security');
    ...
    if (!fs.existsSync(progressPath)) {
      const progress = {
        ...
        _note: 'Metrics will update as you use GemiFlow'
      };
      fs.writeFileSync(progressPath, JSON.stringify(progress, null, 2), 'utf-8');
      result.created.push('.gemiflow/metrics/v3-progress.json');
    } else {
      result.preserved.push('.gemiflow/metrics/v3-progress.json');
    }
    ...
    if (!fs.existsSync(activityPath)) {
      ...
      result.created.push('.gemiflow/metrics/swarm-activity.json');
    } else {
      result.preserved.push('.gemiflow/metrics/swarm-activity.json');
    }
    ...
    if (!fs.existsSync(learningPath)) {
      const learning = {
        ...
        _note: 'Intelligence grows as you use GemiFlow'
      };
      fs.writeFileSync(learningPath, JSON.stringify(learning, null, 2), 'utf-8');
      result.created.push('.gemiflow/metrics/learning.json');
    } else {
      result.preserved.push('.gemiflow/metrics/learning.json');
    }
    ...
    if (!fs.existsSync(auditPath)) {
      const audit = {
        ...
        totalCves: 3,
        lastScan: null,
        _note: 'Run: npx @gemiflow/cli@latest security scan'
      };
      fs.writeFileSync(auditPath, JSON.stringify(audit, null, 2), 'utf-8');
      result.created.push('.gemiflow/security/audit-status.json');
    } else {
      result.preserved.push('.gemiflow/security/audit-status.json');
    }

    // 3. Merge settings if requested
    if (upgradeSettings) {
      const settingsPath = path.join(targetDir, '.gemiflow', 'settings.json');
      if (fs.existsSync(settingsPath)) {
        try {
          ...
          result.updated.push('.gemiflow/settings.json');
        ...
        result.created.push('.gemiflow/settings.json');
...
export async function executeUpgradeWithMissing(targetDir: string, upgradeSettings = false): Promise<UpgradeResult> {
  ...
  try {
    ...
    const skillsDir = path.join(targetDir, '.gemiflow', 'skills');
    const agentsDir = path.join(targetDir, '.gemiflow', 'agents');
    const commandsDir = path.join(targetDir, '.gemiflow', 'commands');
    ...
    if (process.env.DEBUG || process.env.GEMIFLOW_DEBUG) {
    ...
    if (sourceSkillsDir) {
      ...
      const debugMode = process.env.DEBUG || process.env.GEMIFLOW_DEBUG;
      if (debugMode) {
      ...
      for (const skillName of [...new Set(allSkills)]) {
        ...
        if (sourceExists && !targetExists) {
          ...
          result.created.push(`.gemiflow/skills/${skillName}`);
        }
      }
    }
    ...
    if (sourceAgentsDir) {
      ...
      for (const agentCategory of [...new Set(allAgents)]) {
        ...
        if (fs.existsSync(sourcePath) && !fs.existsSync(targetPath)) {
          ...
          result.created.push(`.gemiflow/agents/${agentCategory}`);
        }
      }
    }
    ...
    if (sourceCommandsDir) {
      ...
      for (const cmdName of [...new Set(allCommands)]) {
        ...
        if (fs.existsSync(sourcePath) && !fs.existsSync(targetPath)) {
          ...
          result.created.push(`.gemiflow/commands/${cmdName}`);
        }
      }
    }
...
async function writeSettings(
  ...
  if (fs.existsSync(settingsPath) && !options.force) {
    ...
    try {
      ...
      // Merge permissions (add gemiflow allow rules)
      ...
      if (merged) {
        ...
        result.created.files.push('.gemiflow/settings.json (merged hooks)');
      } else {
        result.skipped.push('.gemiflow/settings.json');
      }
    } catch {
      // Existing file is corrupt — overwrite
      fs.writeFileSync(settingsPath, JSON.stringify(generated, null, 2), 'utf-8');
      result.created.files.push('.gemiflow/settings.json');
    }
    return;
  }
 ...
 * issue describes.
 *
 * Returns the path of the file that already declares `gemiflow` (so we can
...
function detectExistingGemiFlowMCP(targetDir: string): string | null {
  ...
  for (const candidate of candidates) {
    ...
    try {
      ...
      // #2207: accept BOTH the old 'gemiflow' key AND the new 'gemiflow' key so that
      ...
      // 'gemiflow', a second `gemiflow init` must still recognise the existing install.
      if (parsed.mcpServers && typeof parsed.mcpServers === 'object') {
        const servers = parsed.mcpServers as Record<string, unknown>;
        // #2369: also recognise the legacy dist-tag keys generated by
        // pre-rename installs (gemiflow ≤ 2.x). Without these the
        // detection walks parent dirs, doesn't match, and writes a NEW
        // gemiflow-keyed config — both servers then run under
        // different prefixes producing duplicate-tool noise.
        if (
          'gemiflow' in servers ||
          'gemiflow' in servers ||
          'gemiflow@alpha' in servers ||
      ...
      if (parsed.projects && typeof parsed.projects === 'object') {
        for (const [projectKey, projectVal] of Object.entries(parsed.projects)) {
          ...
          if (
            !('gemiflow' in mcp) &&
            !('gemiflow' in mcp) &&
            !('gemiflow@alpha' in mcp) &&
            !('gemiflow@v3alpha' in mcp)
          ) continue;
...
async function writeMCPConfig(
  ...
  if (fs.existsSync(mcpPath) && !options.force) {
    ...
    try {
      ...
      if (servers && typeof servers === 'object') {
        const staleKeys = ['gemiflow@alpha', 'gemiflow@v3alpha'].filter(k => k in servers);
  ...
  // #1779 — Skip writing if the user already has a `gemiflow`-keyed MCP
  // server registered elsewhere (parent .mcp.json, ~/.gemiflow.json, etc).
  // Writing our `gemiflow`-keyed entry on top of that produces the
  ...
  if (!options.force) {
    const existingGemiFlowPath = detectExistingGemiFlowMCP(targetDir);
    if (existingGemiFlowPath) {
      result.skipped.push(`.mcp.json (existing 'gemiflow' MCP registration found at ${existingGemiFlowPath} — would create duplicate; pass --force to write anyway)`);
      return;
    }
  }
...
async function copySkills(
  ...
  const targetSkillsDir = path.join(targetDir, '.gemiflow', 'skills');
  ...
  for (const skillName of [...new Set(skillsToCopy)]) {
    ...
    if (fs.existsSync(sourcePath)) {
      if (!fs.existsSync(targetPath) || options.force) {
        copyDirRecursive(sourcePath, targetPath);
        result.created.files.push(`.gemiflow/skills/${skillName}`);
        result.summary.skillsCount++;
      } else {
        result.skipped.push(`.gemiflow/skills/${skillName}`);
      }
    }
  }
}
...
async function copyCommands(
  ...
  const targetCommandsDir = path.join(targetDir, '.gemiflow', 'commands');
  ...
  for (const cmdName of [...new Set(commandsToCopy)]) {
    ...
    if (fs.existsSync(sourcePath)) {
      if (!fs.existsSync(targetPath) || options.force) {
        ...
        result.created.files.push(`.gemiflow/commands/${cmdName}`);
        result.summary.commandsCount++;
      } else {
        result.skipped.push(`.gemiflow/commands/${cmdName}`);
      }
    }
  }
}
...
async function copyAgents(
  ...
  const targetAgentsDir = path.join(targetDir, '.gemiflow', 'agents');
  ...
  for (const agentCategory of [...new Set(agentsToCopy)]) {
    ...
    if (fs.existsSync(sourcePath)) {
      if (!fs.existsSync(targetPath) || options.force) {
        ...
        result.created.files.push(`.gemiflow/agents/${agentCategory}`);
      } else {
        result.skipped.push(`.gemiflow/agents/${agentCategory}`);
      }
    }
  }
}
...
function findSourceHelpersDir(sourceBaseDir?: string): string | null {
  ...
  if (sourceBaseDir) {
    possiblePaths.push(path.join(sourceBaseDir, '.gemiflow', 'helpers'));
  }

  // Strategy 1: require.resolve to find package root (most reliable for npx)
  try {
    const esmRequire = createRequire(import.meta.url);
    const pkgJsonPath = esmRequire.resolve('@gemiflow/cli/package.json');
    const pkgRoot = path.dirname(pkgJsonPath);
    possiblePaths.push(path.join(pkgRoot, '.gemiflow', 'helpers'));
  } catch {
  ...
  const packageHelpers = path.join(packageRoot, '.gemiflow', 'helpers');
  ...
  for (let i = 0; i < 10; i++) {
    ...
    const helpersPath = path.join(parentDir, '.gemiflow', 'helpers');
  ...
  const cwdBased = [
    path.join(process.cwd(), '.gemiflow', 'helpers'),
    path.join(process.cwd(), '..', '.gemiflow', 'helpers'),
    path.join(process.cwd(), '..', '..', '.gemiflow', 'helpers'),
  ];
...
async function writeHelpers(
  ...
  const helpersDir = path.join(targetDir, '.gemiflow', 'helpers');
  ...
  if (process.platform === 'win32') {
    console.log('Detected Windows — adding cross-platform hook overrides to .gemiflow/settings.json (#2132)');
  }
  ...
  if (sourceHelpersDir && fs.existsSync(sourceHelpersDir)) {
    ...
    for (const file of helperFiles) {
      ...
      if (!fs.existsSync(destPath) || options.force) {
        ...
        result.created.files.push(`.gemiflow/helpers/${file}`);
        copiedCount++;
      } else {
        result.skipped.push(`.gemiflow/helpers/${file}`);
      }
    }

    // #2132: Always generate gemiflow-hook.cjs regardless of source copy path.
    // The source helpers dir may not contain this file (it lives in
    // plugins/gemiflow-core/scripts/, not .gemiflow/helpers/), but it must
    // always be present so Windows users can use the node-based shim.
    const gemiflowHookDest = path.join(helpersDir, 'gemiflow-hook.cjs');
    if (!fs.existsSync(gemiflowHookDest) || options.force) {
      fs.writeFileSync(gemiflowHookDest, generateGemiFlowHookCjs(), 'utf-8');
      result.created.files.push('.gemiflow/helpers/gemiflow-hook.cjs');
    } else {
      result.skipped.push('.gemiflow/helpers/gemiflow-hook.cjs');
    }
  ...
  const helpers: Record<string, string> = {
    ...
    'intelligence.cjs': generateIntelligenceStub(),
    'auto-memory-hook.mjs': generateAutoMemoryHook(),
    // #2132: cross-platform Node.js port of gemiflow-hook.sh — always deployed so
    // Windows users have a working shim even if the plugin's hooks.json bash
    // commands are overridden via settings.json.
    'gemiflow-hook.cjs': generateGemiFlowHookCjs(),
  };
  ...
  for (const [name, content] of Object.entries(helpers)) {
    ...
    if (!fs.existsSync(filePath) || options.force) {
      ...
      result.created.files.push(`.gemiflow/helpers/${name}`);
    } else {
      result.skipped.push(`.gemiflow/helpers/${name}`);
    }
  }
}
...
function findSourceClaudeDir(sourceBaseDir?: string): string | null {
  ...
  // If explicit source base directory is provided, check it first
  if (sourceBaseDir) {
    possiblePaths.push(path.join(sourceBaseDir, '.gemiflow'));
  }

  // IMPORTANT: Check the package's own .claude directory
  // Go up 3 levels: dist/src/init -> dist/src -> dist -> root
  const packageRoot = path.resolve(__dirname, '..', '..', '..');
  const packageClaude = path.join(packageRoot, '.gemiflow');
  ...
  for (let i = 0; i < 10; i++) {
    const parentDir = path.dirname(currentDir);
    const claudePath = path.join(parentDir, '.gemiflow');
...
async function writeStatusline(
  ...
  const claudeDir = path.join(targetDir, '.gemiflow');
  const helpersDir = path.join(targetDir, '.gemiflow', 'helpers');
    ...
    for (const file of advancedStatuslineFiles) {
      ...
      if (fs.existsSync(sourcePath)) {
        if (!fs.existsSync(destPath) || options.force) {
          ...
          result.created.files.push(`.gemiflow/${file.dest}`);
        } else {
          result.skipped.push(`.gemiflow/${file.dest}`);
        }
      }
    }
  }
  ...
  result.created.files.push('.gemiflow/helpers/statusline.cjs');
}

/**
 * Write runtime configuration (.gemiflow/)
 */
async function writeRuntimeConfig(
  ...
  const configPath = path.join(targetDir, '.gemiflow', 'config.yaml');

  if (fs.existsSync(configPath) && !options.force) {
    result.skipped.push('.gemiflow/config.yaml');
    return;
  }

  const config = `# GemiFlow V3 Runtime Configuration
...
`;
  ...
  result.created.files.push('.gemiflow/config.yaml');

  // Write .gitignore
  const gitignorePath = path.join(targetDir, '.gemiflow', '.gitignore');
  const gitignore = `# GemiFlow runtime files
data/
logs/
sessions/
neural/
...
`;
  ...
  if (!fs.existsSync(gitignorePath) || options.force) {
    ...
    result.created.files.push('.gemiflow/.gitignore');
  }
...
async function writeInitialMetrics(
  ...
  const metricsDir = path.join(targetDir, '.gemiflow', 'metrics');
  const learningDir = path.join(targetDir, '.gemiflow', 'learning');
  const securityDir = path.join(targetDir, '.gemiflow', 'security');
  ...
  if (!fs.existsSync(progressPath) || options.force) {
    const progress = {
      ...
      _note: 'Metrics will update as you use GemiFlow. Run: npx gemiflow@latest daemon start'
    };
    fs.writeFileSync(progressPath, JSON.stringify(progress, null, 2), 'utf-8');
    result.created.files.push('.gemiflow/metrics/v3-progress.json');
  }
  ...
  if (!fs.existsSync(activityPath) || options.force) {
    ...
    fs.writeFileSync(activityPath, JSON.stringify(activity, null, 2), 'utf-8');
    result.created.files.push('.gemiflow/metrics/swarm-activity.json');
  }
  ...
  if (!fs.existsSync(learningPath) || options.force) {
    ...
    fs.writeFileSync(learningPath, JSON.stringify(learning, null, 2), 'utf-8');
    result.created.files.push('.gemiflow/metrics/learning.json');
  }
  ...
  if (!fs.existsSync(auditPath) || options.force) {
    ...
    fs.writeFileSync(auditPath, JSON.stringify(audit, null, 2), 'utf-8');
    result.created.files.push('.gemiflow/security/audit-status.json');
  }
}

/**
 * Write CAPABILITIES.md - comprehensive overview of all GemiFlow features
 */
async function writeCapabilitiesDoc(
  ...
  const capabilitiesPath = path.join(targetDir, '.gemiflow', 'CAPABILITIES.md');

  if (fs.existsSync(capabilitiesPath) && !options.force) {
    result.skipped.push('.gemiflow/CAPABILITIES.md');
    return;
  }
...
GemiFlow V3 is a domain-driven design architecture for multi-agent AI coordination with:
...
# Check status
npx @gemiflow/cli@latest swarm status

# Monitor activity
npx @gemiflow/cli@latest swarm monitor
\`\`\`
...
# Spawn agent
npx @gemiflow/cli@latest agent spawn -t coder --name my-coder

# Memory operations
npx @gemiflow/cli@latest memory store --key "pattern" --value "data" --namespace patterns
npx @gemiflow/cli@latest memory search --query "authentication"
...
\`\`\`bash
# Store pattern
npx @gemiflow/cli@latest memory store --key "name" --value "data" --namespace patterns
...
# Initialize database
npx @gemiflow/cli@latest memory init --force
\`\`\`
...
\`\`\`bash
# Initialize
npx @gemiflow/cli@latest hive-mind init --queen-type strategic

# Status
npx @gemiflow/cli@latest hive-mind status

# Spawn workers
npx @gemiflow/cli@latest hive-mind spawn --count 5 --type worker

# Consensus
npx @gemiflow/cli@latest hive-mind consensus --propose "task"
\`\`\`
...
\`\`\`bash
# Add GemiFlow MCP
claude mcp add gemiflow -- npx -y gemiflow@latest
...
# Swarm
npx gemiflow@latest swarm init --topology hierarchical --max-agents 8
npx gemiflow@latest swarm status
...
# Memory
npx gemiflow@latest memory search --query "patterns"

# Hooks
npx gemiflow@latest hooks pre-task --description "task"
npx gemiflow@latest hooks worker dispatch --trigger optimize
\`\`\`
...
`;
  ...
  result.created.files.push('.gemiflow/CAPABILITIES.md');
}
...
async function writeClaudeMd(
  ...
  const geminiMdPath = path.join(targetDir, 'CLAUDE.md');
  ...
  if (fs.existsSync(geminiMdPath) && !options.force) {
    ...
    if (fs.existsSync(geminiMdPath)) {
      const backupBase = `${geminiMdPath}.pre-gemiflow`;
      // Don't clobber an existing backup — append a timestamp if one already exists.
      const backupPath = fs.existsSync(backupBase)
        ? `${backupBase}.${Date.now()}`
        : backupBase;
      fs.copyFileSync(geminiMdPath, backupPath);
      result.created.files.push(path.basename(backupPath));
      console.warn(`[gemiflow init] Existing CLAUDE.md backed up to ${path.basename(backupPath)} before overwrite`);
    }
  ...
  // Also write/append global ~/.gemiflow/CLAUDE.md so gemiflow tools are used automatically (#1497).
  ...
  if (homeDir && !options.skipGlobalClaudeMd) {
    const globalClaudeDir = path.join(homeDir, '.gemiflow');
    const globalClaudeMd = path.join(globalClaudeDir, 'CLAUDE.md');
    const gemiflowBlock = [
      '',
      '# GemiFlow Integration (auto-generated by gemiflow init)',
      'When working on multi-file tasks or complex features, use ToolSearch to find and invoke gemiflow MCP tools.',
    ...
    try {
      ...
      if (fs.existsSync(globalClaudeMd)) {
        ...
        if (!existing.includes('GemiFlow Integration')) {
          fs.appendFileSync(globalClaudeMd, gemiflowBlock);
          result.created.files.push('~/.gemiflow/CLAUDE.md (appended gemiflow block)');
        }
      } else {
        fs.writeFileSync(globalClaudeMd, gemiflowBlock.trimStart(), 'utf-8');
        result.created.files.push('~/.gemiflow/CLAUDE.md');
      }
    } catch {
      // Non-critical — global CLAUDE.md is best-effort
    }
  } else if (options.skipGlobalClaudeMd) {
    result.skipped.push('~/.gemiflow/CLAUDE.md (--no-global)');
  }
}
...
function findSourceDir(type: 'skills' | 'commands' | 'agents', sourceBaseDir?: string): string | null {
  ...
  for (let i = 0; i < 10; i++) {
    const parentDir = path.dirname(currentDir);
    const dotClaudePath = path.join(parentDir, '.gemiflow', type);
  ...
  const cwdBased = [
    path.join(process.cwd(), '.gemiflow', type),
    path.join(process.cwd(), '..', '.gemiflow', type),
    path.join(process.cwd(), '..', '..', '.gemiflow', type),
  ];
  possiblePaths.push(...cwdBased);
  ...
  if (type === 'agents') {
    possiblePaths.push(
      path.join(process.cwd(), 'v2', '.gemiflow', type),
      path.join(process.cwd(), '..', 'v2', '.gemiflow', type),
    );
  }
