 ...
 * Creates .gemiflow/settings.json with V3-optimized hook configurations
 */
...
export function generateSettings(options: InitOptions): object {
  ...
  // .gemiflow/helpers/hook-handler.cjs; if that file isn't created (as in
  ...
  // #1670 — GemiFlow attribution (Co-Authored-By trailer + PR footer) is now
  ...
  // `gemiflow-bot@users.noreply.github.com` is GitHub's no-reply convention and
 ...
 *
 * The fix is a tiny POSIX `sh` probe: try `$CLAUDE_PROJECT_DIR/.gemiflow/...`
 * first, fall back to `$HOME/.gemiflow/...` if it's missing. Both modes work,
...
function hookHandlerCmd(subcommand: string): string {
  return hookCmd('.gemiflow/helpers/hook-handler.cjs', subcommand);
}

/** Shorthand for ESM auto-memory-hook commands */
function autoMemoryCmd(subcommand: string): string {
  return hookCmd('.gemiflow/helpers/auto-memory-hook.mjs', subcommand);
}
...
function generateStatusLineConfig(_options: InitOptions): object {
  ...
  //
  // gemiflow#1948 + #1973: the previous `sh -c 'D="${CLAUDE_PROJECT_DIR:-.}"; …'`
  ...
  const script = '.gemiflow/helpers/statusline.cjs';
  ...
  if (process.platform === 'win32') {
    ...
    //   const f = p.join(d, '.gemiflow/helpers/statusline.cjs');
    //   const home = process.env.USERPROFILE || process.env.HOME || '.';
    //   const h = p.join(home, '.gemiflow/helpers/statusline.cjs');
