 ...
 * Output format:
 * ▊ GemiFlow V3.6 ● user  │  ⎇ branch  │  Opus 4.7
 * ─────────────────────────────────────────────────────
 ...
 *
 * This version delegates to 'npx @gemiflow/cli hooks statusline --json'
...
const CONFIG = {
  ...
  //   GEMIFLOW_STATUSLINE_COST_SYMBOL  override the leading '$' (e.g. ⚡, €, 🌱);
  //                                 set to an empty string for the number alone.
  //   GEMIFLOW_STATUSLINE_HIDE_COST    1/true/yes/on removes the segment entirely.
  costSymbol: process.env.GEMIFLOW_STATUSLINE_COST_SYMBOL ?? '$',
  hideCost: /^(1|true|yes|on)$/i.test(process.env.GEMIFLOW_STATUSLINE_HIDE_COST || ''),
};
...
// #2337: resolve an already-installed @gemiflow/cli (or gemiflow) bin so we
...
function resolveCliBin() {
  try {
    ...
    const candidates = [
      path.join(home, '.gemiflow', 'plugins', 'marketplaces', 'gemiflow', 'bin', 'cli.js'),
      path.join(CWD, 'node_modules', '@gemiflow', 'cli', 'bin', 'cli.js'),
      path.join(CWD, 'node_modules', 'gemiflow', 'bin', 'cli.js'),
      path.join(CWD, 'v3', '@gemiflow', 'cli', 'bin', 'cli.js'),
    ];
    try {
      ...
      for (const gm of globalModuleDirs) {
        candidates.push(
          path.join(gm, 'gemiflow', 'bin', 'cli.js'),
          path.join(gm, '@gemiflow', 'cli', 'bin', 'cli.js'),
        );
      }
    } catch { /* ignore */ }
 ...
 *
 * Fix for gemiflow#2195: the previous local readers returned 0 for AgentDB patterns
...
function getStatuslineData() {
  ...
  try {
    ...
    // Fall back to \`npx --prefer-offline @gemiflow/cli\` (no @latest) only
    ...
    const cmd = cliBin
      ...
      : 'npx --prefer-offline @gemiflow/cli hooks statusline --json 2>/dev/null';
    const raw = execSync(
      cmd,
...
// Count ADRs from BOTH known directories (fix for gemiflow#2195: old code missed
...
function getLocalHooks() {
  ...
  try {
    const settings = readJSON(path.join(CWD, '.gemiflow', 'settings.json'));
...
// servers (project .mcp.json + global ~/.gemiflow.json). Remote connectors are
...
function getPkgVersion() {
  ...
  try {
    const home = os.homedir();
    const pkgPaths = [
      path.join(home, '.gemiflow', 'plugins', 'marketplaces', 'gemiflow', 'package.json'),
      path.join(CWD, 'node_modules', '@gemiflow', 'cli', 'package.json'),
      path.join(CWD, 'node_modules', 'gemiflow', 'package.json'),
      path.join(CWD, 'v3', '@gemiflow', 'cli', 'package.json'),
    ];
    // #2221: global installs (npm i -g gemiflow) live outside CWD/node_modules, so the
    ...
    try {
      ...
      for (const gm of globalModuleDirs) {
        pkgPaths.push(
          path.join(gm, 'gemiflow', 'package.json'),
          path.join(gm, '@gemiflow', 'cli', 'package.json'),
        );
      }
    } catch { /* ignore */ }
...
function generateStatusline() {
  ...
  // Header
  let header = c.bold + c.brightPurple + '▊ GemiFlow V' + pkgVersion + ' ' + c.reset;
