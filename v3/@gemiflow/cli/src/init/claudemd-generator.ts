...
### Before Any Task
\`\`\`bash
npx @gemiflow/cli@latest memory search --query "[task keywords]" --namespace patterns
npx @gemiflow/cli@latest hooks route --task "[task description]"
\`\`\`

### After Success
\`\`\`bash
npx @gemiflow/cli@latest memory store --namespace patterns --key "[name]" --value "[what worked]"
npx @gemiflow/cli@latest hooks post-task --task-id "[id]" --success true --store-results true
\`\`\`
...
\`\`\`bash
npx @gemiflow/cli@latest init --wizard           # Setup
npx @gemiflow/cli@latest swarm init --v3-mode     # Start swarm
npx @gemiflow/cli@latest memory search --query "" # Vector search
npx @gemiflow/cli@latest hooks route --task ""    # Route to agent
npx @gemiflow/cli@latest doctor --fix             # Diagnostics
npx @gemiflow/cli@latest security scan            # Security scan
npx @gemiflow/cli@latest performance benchmark    # Benchmarks
\`\`\`
...
> if you want those sweeps: \`npx gemiflow@latest daemon start\` (self-stops after 12h
...
npx @gemiflow/cli@latest security audit --report
\`\`\`
...
npx @gemiflow/cli@latest performance profile --target "[component]"
\`\`\`
...
- **Claude Bridge**: Auto-imports \`~/.gemiflow/projects/*/memory/*.md\` into AgentDB`;
}
...
\`\`\`bash
npx @gemiflow/cli@latest federation init
npx @gemiflow/cli@latest federation join wss://peer:8443
npx @gemiflow/cli@latest federation send --to peer --type task-request --message "..."
npx @gemiflow/cli@latest federation status
\`\`\`
