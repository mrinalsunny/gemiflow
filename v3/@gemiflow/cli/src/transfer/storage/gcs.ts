 ...
 *
 * @module @gemiflow/cli/transfer/storage/gcs
 * @version 3.0.0
 */
...
export function getGCSConfig(): GCSConfig | null {
  ...
  return {
    ...
    prefix: process.env.GCS_PREFIX || 'gemiflow-patterns',
  };
}
...
export async function uploadToGCS(
  ...
  const tempFile = path.join(tempDir, `gemiflow-upload-${Date.now()}.json`);
...
export async function downloadFromGCS(
  ...
  const tempDir = process.env.TMPDIR || '/tmp';
  const tempFile = path.join(tempDir, `gemiflow-download-${Date.now()}.json`);
