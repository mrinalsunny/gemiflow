...
export class PatternDiscovery {
  ...
  async fetchRegistry(cid: string, gateway: string): Promise<PatternRegistry | null> {
    ...
    try {
      ...
      if (hasGCSCredentials()) {
        const gcsUri = `gs://gemiflow-patterns/registry/${cid}.json`;
  ...
  private getGenesisRegistry(cid: string): PatternRegistry {
    return {
      ...
      patterns: [
        {
          ...
          description: 'The foundational GemiFlow pattern model. Contains core routing patterns, complexity heuristics, and coordination trajectories for multi-agent swarms.',
          version: '1.0.0',
          ...
          publicKey: 'ed25519:gemiflow-team-key',
        },
      ],
      ...
      registryPublicKey: 'ed25519:gemiflow-registry-key',
    };
  }
