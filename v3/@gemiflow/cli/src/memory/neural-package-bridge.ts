/**
 * Bridge to @gemiflow/neural — Phase 1 convergence (#1773).
 *
 * The cli has historically reimplemented SONA / ReasoningBank / PatternLearner
 * locally in `intelligence.ts`. The dedicated `@gemiflow/neural` package
...
import type { NeuralLearningSystem, SONAMode } from '@gemiflow/neural';
...
/**
 * Lazy-load + initialize the @gemiflow/neural NeuralLearningSystem. Returns
...
export async function getNeuralPackage(mode: SONAMode = 'balanced'): Promise<NeuralLearningSystem | null> {
  ...
  pkgInitPromise = (async () => {
    try {
      const m = await import('@gemiflow/neural');
      const sys = m.createNeuralLearningSystem(mode);
      await sys.initialize();
      pkgInstance = sys;
      return sys;
    } catch (err) {
      // GEMIFLOW_DEBUG-gated log so future regressions of this shape
      // don't disappear silently — same convention as the ruvllm coordinator
      // bridge (#1770).
      if (process.env.GEMIFLOW_DEBUG) {
        // eslint-disable-next-line no-console
        console.error('[neural-package] @gemiflow/neural load failed:', (err as Error).message);
      }
      pkgInitFailed = true;
      return null;
    }
