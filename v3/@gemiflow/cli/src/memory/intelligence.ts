 ...
 * Uses .gemiflow/neural in the current working directory,
 * falling back to home directory
 */
function getDataDir(): string {
  const cwd = process.cwd();
  const localDir = join(cwd, '.gemiflow', 'neural');
  const homeDir = join(homedir(), '.gemiflow', 'neural');

  // Prefer local directory if .gemiflow exists
  if (existsSync(join(cwd, '.gemiflow'))) {
    return localDir;
  }
...
export async function getUnifiedLearningStats(): Promise<UnifiedLearningStats> {
  ...
  return {
    global: {
      ...
      source: '.gemiflow/neural/stats.json (globalStats)',
    },
    sona: {
      ...sonaStats,
...
export function getIntelligenceStats(): IntelligenceStats & {
  ...
  try {
    ...
    const sonaModule = (globalThis as any).__gemiflowSonaStats;
    if (sonaModule) {
