 ...
 * `GEMIFLOW_MEMORY_PATH` env var) all silently no-op'd. This helper
 * is the single source of truth — every `.swarm/memory.db` resolution in
 * this file flows through it.
 *
 * Precedence (highest → lowest):
 *   1. GEMIFLOW_MEMORY_PATH env var
 *   2. memory.persistPath / memory.path in gemiflow.config.json (cwd or
...
export function getMemoryRoot(): string {
  ...
  // 1. Env var
  const envPath = process.env.GEMIFLOW_MEMORY_PATH;
  if (envPath && envPath.trim().length > 0) {
    _memoryRootCache = path.resolve(envPath);
    return _memoryRootCache;
  }

  // 2. Config file (gemiflow.config.json)
  const configCandidates = [
    path.resolve(process.cwd(), 'gemiflow.config.json'),
    path.resolve(process.cwd(), '.gemiflow', 'config.json'),
  ];
 ...
 *   2. GEMIFLOW_DB_PATH - full file-path override (new in #2105)
 *   3. getMemoryRoot()/memory.db - directory from GEMIFLOW_MEMORY_PATH /
...
export function resolveDbPath(cliFlag?: string): string {
  ...
  const envDb = process.env.GEMIFLOW_DB_PATH;
...
async function getBridge(): Promise<typeof import('./memory-bridge.js') | null> {
  ...
  if (process.env.GEMIFLOW_DISABLE_BRIDGE === '1') return null;
 ...
 *
 * Failures are isolated: if @gemiflow/memory or agentdb is not installed,
...
export async function loadEmbeddingModel(options?: {
  ...
  try {
    ...
    // Inlined here rather than depending on @gemiflow/embeddings to
    // avoid a circular optional-dep at install time; the logic mirrors
    // @gemiflow/embeddings/src/transformers-loader.ts.
...
export async function storeEntry(options: {
  ...
  try {
    if (!fs.existsSync(dbPath)) {
      return { success: false, id: '', error: 'Database not initialized. Run: gemiflow memory init' };
    }
