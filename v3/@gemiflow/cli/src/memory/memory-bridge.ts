 ...
 * `GEMIFLOW_MEMORY_PATH` / `gemiflow.config.json#memory.persistPath`
...
function getDbPath(customPath?: string): string {
  ...
  try {
    // `getMemoryRoot()` honors $GEMIFLOW_MEMORY_PATH, then the
    // gemiflow.config.json `memory.persistPath`, then defaults to `.swarm`.
 ...
 * Returns null if @gemiflow/memory is not available.
 */
async function getRegistry(dbPath?: string): Promise<any | null> {
  ...
  if (!registryPromise) {
    registryPromise = (async () => {
      try {
        const { ControllerRegistry } = await import('@gemiflow/memory');
