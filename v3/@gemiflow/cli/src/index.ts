 ...
 * Modernized CLI for GemiFlow V3
 *
...
export class CLI {
  ...
  constructor(options: CLIOptions = {}) {
    this.name = options.name || 'gemiflow';
    this.description = options.description || 'GemiFlow V3 - AI Agent Orchestration Platform';
  ...
  private async loadConfig(configPath?: string): Promise<V3Config | undefined> {
    try {
      ...
      const { loadConfig: loadSystemConfig } = await import('@gemiflow/shared');
