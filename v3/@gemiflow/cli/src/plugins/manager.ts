...
export class PluginManager {
  ...
  constructor(baseDir: string = process.cwd()) {
    const pluginsDir = path.join(baseDir, '.gemiflow', 'plugins');
    this.config = {
      pluginsDir,
  ...
  async installFromNpm(
    ...
    try {
      ...
      if (fs.existsSync(packageJsonPath)) {
        ...
        // Check for gemiflow plugin metadata
        if (pkg['gemiflow']) {
          commands = pkg['gemiflow'].commands || [];
          hooks = pkg['gemiflow'].hooks || [];
        }
      }
  ...
  async upgrade(
    ...
    try {
      ...
      if (fs.existsSync(packageJsonPath)) {
        ...
        existing.commands = pkg['gemiflow']?.commands || existing.commands;
        existing.hooks = pkg['gemiflow']?.hooks || existing.hooks;
      }
...
export function getPluginManager(baseDir?: string): PluginManager {
  ...
  } else if (baseDir && defaultManager.getPluginsDir() !== path.join(baseDir, '.gemiflow', 'plugins')) {
