...
async function main() {
  ...
  try {
    const similar = findSimilarPlugins(registry, '@gemiflow/neural', 3);
    console.log(`  ✅ Found ${similar.length} similar plugins to @gemiflow/neural`);
    passed++;
  } catch (e) {
    console.log(`  ❌ Failed: ${(e as Error).message}`);
    failed++;
  ...
  if (failed === 0) {
    ...
    console.log('    npx @gemiflow/cli plugins list');
    console.log('    npx @gemiflow/cli plugins list --official');
    console.log('    npx @gemiflow/cli plugins search -q "plugin creator"');
    console.log('    npx @gemiflow/cli plugins info -n plugin-creator');
    console.log('');
  }
