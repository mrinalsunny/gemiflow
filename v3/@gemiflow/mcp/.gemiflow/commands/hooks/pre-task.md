...
```bash
npx gemiflow hook pre-task -d "Continue API development" --load-memory
```

### Manual agent control

```bash
npx gemiflow hook pre-task -d "Debug issue #123" --auto-spawn-agents false
```

### Full optimization

```bash
npx gemiflow hook pre-task -d "Refactor codebase" --optimize-topology --estimate-complexity
```
...
npx gemiflow hook pre-task --description "Your task here"
```
