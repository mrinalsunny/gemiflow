...
## Overview
Reviewed all command files in `.gemiflow/commands/analysis/` directory to ensure proper usage of:
- `mcp__gemiflow__*` tools (preferred)
- `npx gemiflow` commands (as fallback)
...
- Updated to: `Tool: mcp__gemiflow__token_usage` with appropriate parameters
...
**After**:
```
Tool: mcp__gemiflow__token_usage
Parameters: {"operation": "session", "timeframe": "24h"}
```

### 2. performance-bottlenecks.md
**Status**: ✅ Compliant (No changes needed)
**Reason**: Already uses proper `mcp__gemiflow__task_results` tool format
...
1. **MCP Tool Usage**: All direct tool calls now use `mcp__gemiflow__*` format
...
The analysis directory is now fully compliant with the GemiFlow command standards.