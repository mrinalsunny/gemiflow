...
# 🐝 GemiFlow Swarm Coordination

Advanced multi-agent coordination system with timeout-free execution, distributed memory sharing, and intelligent load balancing.

## Basic Usage
```bash
./gemiflow swarm "your complex task" --strategy <type> [options]
```
...
```bash
./gemiflow swarm "Build e-commerce REST API" \
...
```bash
./gemiflow swarm "Analyze AI market trends 2024-2025" \
  --strategy research \
  --background \
  --distributed \
  --max-agents 8
...
```bash
./gemiflow swarm "Optimize database queries and API performance" \
...
```bash
./gemiflow swarm "Implement secure payment processing system" \
  --strategy development \
  --mode distributed \
  --max-agents 10 \
  --parallel \
...
```bash
./gemiflow swarm "Comprehensive security audit and testing" \
  --strategy testing \
  --review \
  --verbose \
  --max-agents 6
...
./gemiflow memory store "swarm_objective" "Build scalable API" --namespace swarm

# Query swarm progress
./gemiflow memory query "swarm_progress" --namespace swarm

# Export swarm memory
./gemiflow memory export swarm-results.json --namespace swarm
```
...
### Dry run to preview:
```bash
./gemiflow swarm "Test task" --dry-run --strategy development
```

### Custom quality thresholds:
```bash
./gemiflow swarm "High quality API" \
  --strategy development \
