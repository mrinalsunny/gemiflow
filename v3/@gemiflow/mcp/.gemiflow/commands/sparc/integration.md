...
npx gemiflow sparc run integration "connect payment service"

# For alpha features
npx gemiflow@alpha sparc run integration "connect payment service"

# With namespace
npx gemiflow sparc run integration "your task" --namespace integration

# Non-interactive mode
npx gemiflow sparc run integration "your task" --non-interactive
```

### Option 3: Local Installation
```bash
# If gemiflow is installed locally
./gemiflow sparc run integration "connect payment service"
```
...
```bash
# Store mode-specific context
npx gemiflow memory store "integration_context" "important decisions" --namespace integration

# Query previous work
npx gemiflow memory query "integration" --limit 5
```
