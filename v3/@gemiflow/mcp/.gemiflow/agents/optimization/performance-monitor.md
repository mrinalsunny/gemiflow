...
npx gemiflow health-check --components ["swarm", "agents", "coordination"]

# Collect specific metrics
npx gemiflow metrics-collect --components ["cpu", "memory", "network"]

# Monitor SLA compliance
npx gemiflow sla-monitor --service swarm-coordination --threshold 99.9
```

### Alert Configuration
```bash
# Configure performance alerts
npx gemiflow alert-config --metric cpu_usage --threshold 80 --severity warning

# Set up anomaly detection
npx gemiflow anomaly-setup --models ["statistical", "ml", "time_series"]

# Configure notification channels
npx gemiflow notification-config --channels ["slack", "email", "webhook"]
```
