...
tools:
  ...
  - mcp__gemiflow__automation_setup
  ...
  swarm-analysis:
    ...
    steps:
      ...
      - name: Analyze Changes
        run: |
          npx gemiflow@v3alpha actions analyze \
            --commit ${{ github.sha }} \
            --suggest-tests \
            --optimize-pipeline
```
...
npx gemiflow@v3alpha actions generate-workflow \
  --analyze-codebase \
  --detect-languages \
...
- name: Swarm Test Selection
  run: |
    npx gemiflow@v3alpha actions smart-test \
  ...
  detect-and-build:
    ...
    steps:
      ...
      - name: Detect Languages
        id: detect
        run: |
          npx gemiflow@v3alpha actions detect-stack \
            --output json > stack.json
            
      - name: Dynamic Build Matrix
        run: |
          npx gemiflow@v3alpha actions create-matrix \
            --from stack.json \
            --parallel-builds
```
    ...
    steps:
      - name: Security Analysis Swarm
        run: |
          ...
          SECURITY_ISSUES=$(npx gemiflow@v3alpha actions security \
            --deep-scan \
            --format json)
...
npx gemiflow@v3alpha actions optimize \
...
gh run view ${{ github.run_id }} --json jobs,conclusion | \
  npx gemiflow@v3alpha actions analyze-failure \
    --suggest-fixes \
    --auto-retry-flaky
...
npx gemiflow@v3alpha actions resources \
  --analyze-usage \
  --suggest-runners \
  --cost-optimize
```
  ...
  heal-pipeline:
    ...
    steps:
      - name: Diagnose and Fix
        run: |
          npx gemiflow@v3alpha actions self-heal \
  ...
  progressive-deploy:
    ...
    steps:
      - name: Analyze Risk
        id: risk
        run: |
          npx gemiflow@v3alpha actions deploy-risk \
            --changes ${{ github.sha }} \
            --history 30d
            
      - name: Choose Strategy
        run: |
          npx gemiflow@v3alpha actions deploy-strategy \
  ...
  perf-swarm:
    ...
    steps:
      - name: Performance Analysis
        run: |
          npx gemiflow@v3alpha actions perf-test \
            --baseline main \
            --threshold 10% \
...
jobs:
  generate-matrix:
    ...
    steps:
      - id: set-matrix
        run: |
          MATRIX=$(npx gemiflow@v3alpha actions test-matrix \
            --detect-frameworks \
            --optimize-coverage)
...
npx gemiflow@v3alpha actions parallel-strategy \
...
npx gemiflow@v3alpha actions analytics \
  --workflow "ci.yml" \
  --period 30d \
...
npx gemiflow@v3alpha actions cost-optimize \
  --analyze-usage \
  --suggest-caching \
...
npx gemiflow@v3alpha actions failure-patterns \
  --period 90d \
  --classify-failures \
  --suggest-preventions
```
    ...
    steps:
      - name: Multi-Agent Validation
        run: |
          ...
          RESULTS=$(npx gemiflow@v3alpha actions pr-validate \
...
jobs:
  release:
    ...
    steps:
      - name: Release Swarm
        run: |
          npx gemiflow@v3alpha actions release \
            --analyze-changes \
            --generate-notes \
            --create-artifacts \
            --publish-smart
  ...
  docs:
    ...
    steps:
      - name: Documentation Swarm
        run: |
          npx gemiflow@v3alpha actions update-docs \
            --analyze-changes \
            --update-api-docs \
            --check-examples
```
...
npx gemiflow@v3alpha actions predict \
  --analyze-history \
  --identify-risks \
  --suggest-preventive
```
...
npx gemiflow@v3alpha actions recommend \
  --analyze-repo \
  --suggest-workflows \
...
npx gemiflow@v3alpha actions auto-optimize \
  --monitor-performance \
  --apply-improvements \
  --track-savings
```
...
- name: Debug Swarm
  run: |
    npx gemiflow@v3alpha actions debug \
      --verbose \
      --trace-agents \
      --export-logs
```
...
npx gemiflow@v3alpha actions profile \
  --workflow "ci.yml" \
  --identify-slow-steps \
...
mcp__gemiflow__swarm_init { topology: "mesh", maxAgents: 12 }
mcp__gemiflow__agent_spawn { type: "coordinator", name: "Workflow Coordinator" }
mcp__gemiflow__agent_spawn { type: "architect", name: "Pipeline Architect" }
mcp__gemiflow__agent_spawn { type: "coder", name: "Workflow Developer" }
mcp__gemiflow__agent_spawn { type: "tester", name: "CI/CD Tester" }
mcp__gemiflow__agent_spawn { type: "optimizer", name: "Performance Optimizer" }
mcp__gemiflow__agent_spawn { type: "monitor", name: "Automation Monitor" }
mcp__gemiflow__agent_spawn { type: "analyst", name: "Workflow Analyzer" }
...
const createIntelligentWorkflow = async (repoContext) => {
  ...
  await mcp__gemiflow__swarm_init({ topology: "hierarchical", maxAgents: 8 });
  
  // Spawn specialized workflow agents
  await mcp__gemiflow__agent_spawn({ type: "architect", name: "Workflow Architect" });
  await mcp__gemiflow__agent_spawn({ type: "coder", name: "YAML Generator" });
  await mcp__gemiflow__agent_spawn({ type: "optimizer", name: "Performance Optimizer" });
  await mcp__gemiflow__agent_spawn({ type: "tester", name: "Workflow Validator" });
  
  // Create adaptive workflow based on repository analysis
  const workflow = await mcp__gemiflow__workflow_create({
