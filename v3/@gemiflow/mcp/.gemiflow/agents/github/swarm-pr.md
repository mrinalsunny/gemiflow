...
gh pr view 123 --json body,title,labels,files | npx gemiflow@v3alpha swarm create-from-pr

# Auto-spawn agents based on PR labels
gh pr view 123 --json labels | npx gemiflow@v3alpha swarm auto-spawn

# Create swarm with PR context
gh pr view 123 --json body,labels,author,assignees | \
  npx gemiflow@v3alpha swarm init --from-pr-data
```
      ...
      - name: Handle Swarm Command
        run: |
          if [[ "${{ github.event.comment.body }}" == /swarm* ]]; then
            npx gemiflow@v3alpha github handle-comment \
...
npx gemiflow@v3alpha github pr-topology --pr 123
```
...
npx gemiflow@v3alpha github pr-init 123 \
  --auto-agents \
  --pr-data "$PR_INFO" \
  --diff "$PR_DIFF" \
  --analyze-impact
...
PROGRESS=$(npx gemiflow@v3alpha github pr-progress 123 --format markdown)
...
# Run swarm review
REVIEW_RESULTS=$(npx gemiflow@v3alpha github pr-review 123 \
...
npx gemiflow@v3alpha github multi-pr \
  --prs "123,124,125" \
  --strategy "parallel" \
  --share-memory
```
...
npx gemiflow@v3alpha github pr-deps 123 \
  --spawn-agents \
  --resolve-conflicts
```
...
```bash
# Auto-fix PR issues
npx gemiflow@v3alpha github pr-fix 123 \
...
SWARM_STATUS=$(npx gemiflow@v3alpha github pr-status 123)
...
createServer((req, res) => {
  if (req.url === '/github-webhook') {
    ...
    if (event.action === 'opened' && event.pull_request) {
      execSync(`npx gemiflow@v3alpha github pr-init ${event.pull_request.number}`);
    }
...
npx gemiflow@v3alpha github pr-init 456 \
...
npx gemiflow@v3alpha github pr-init 789 \
  --topology mesh \
...
npx gemiflow@v3alpha github pr-init 321 \
  --topology ring \
...
npx gemiflow@v3alpha github pr-report 123 \
...
npx gemiflow@v3alpha github export-metrics \
  --pr 123 \
  --to-insights
```
...
mcp__gemiflow__agent_spawn { type: "coordinator", name: "PR Coordinator" }
mcp__gemiflow__agent_spawn { type: "reviewer", name: "Code Reviewer" }
mcp__gemiflow__agent_spawn { type: "tester", name: "Test Engineer" }
mcp__gemiflow__agent_spawn { type: "analyst", name: "Impact Analyzer" }
...
const prPreHook = async (prData) => {
  ...
  await mcp__gemiflow__swarm_init({ topology, maxAgents: 8 });
  ...
  await mcp__gemiflow__coordination_sync({ swarmId: "current" });
};
...
mcp__gemiflow__coordination_sync { swarmId: "pr-review-swarm" }
