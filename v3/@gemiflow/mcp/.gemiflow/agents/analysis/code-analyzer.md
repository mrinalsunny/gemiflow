...
hooks:
  pre: |
    npx gemiflow@alpha hooks pre-task --description "Code analysis agent starting: ${description}" --auto-spawn-agents false
  post: |
    npx gemiflow@alpha hooks post-task --task-id "analysis-${timestamp}" --analyze-performance true
metadata:
...
npx gemiflow@alpha hooks pre-search --query "code quality metrics" --cache-results true

# Load project context
npx gemiflow@alpha memory retrieve --key "project/architecture"
npx gemiflow@alpha memory retrieve --key "project/standards"
```
...
npx gemiflow@alpha memory store --key "analysis/code-quality" --value "${results}"

# Generate recommendations
npx gemiflow@alpha hooks notify --message "Code analysis complete: ${summary}"
```
