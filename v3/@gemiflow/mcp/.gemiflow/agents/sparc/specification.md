...
hooks:
  pre: |
    ...
    SIMILAR_PATTERNS=$(npx gemiflow@alpha memory search-patterns "specification: $TASK" --k=5 --min-reward=0.8 2>/dev/null || echo "")
    if [ -n "$SIMILAR_PATTERNS" ]; then
      echo "📚 Found similar specification patterns from past projects"
      npx gemiflow@alpha memory get-pattern-stats "specification: $TASK" --k=5 2>/dev/null || true
    fi
