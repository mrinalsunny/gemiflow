...
hooks:
  pre: |
    ...
    SIMILAR_REFACTOR=$(npx gemiflow@alpha memory search-patterns "refinement: $TASK" --k=5 --min-reward=0.85 2>/dev/null || echo "")
    if [ -n "$SIMILAR_REFACTOR" ]; then
      echo "📚 Found similar refactoring patterns - applying learned improvements"
      npx gemiflow@alpha memory get-pattern-stats "refinement: $TASK" --k=5 2>/dev/null || true
    fi

    # 2. Learn from past test failures
    echo "⚠️  Learning from past test failures..."
    PAST_FAILURES=$(npx gemiflow@alpha memory search-patterns "refinement: $TASK" --only-failures --k=3 2>/dev/null || echo "")
    ...
    npx gemiflow@alpha memory store-pattern \
  ...
  post: |
    ...
    if [ "$SUCCESS" = "true" ] && [ "$TEST_COVERAGE" != "0" ]; then
      ...
      npx gemiflow@alpha neural train \
