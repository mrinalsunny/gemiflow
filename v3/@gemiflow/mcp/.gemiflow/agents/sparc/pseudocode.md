...
hooks:
  pre: |
    ...
    SIMILAR_ALGOS=$(npx gemiflow@alpha memory search-patterns "algorithm: $TASK" --k=5 --min-reward=0.8 2>/dev/null || echo "")
    if [ -n "$SIMILAR_ALGOS" ]; then
      echo "📚 Found similar algorithm patterns - applying learned optimizations"
      npx gemiflow@alpha memory get-pattern-stats "algorithm: $TASK" --k=5 2>/dev/null || true
    fi
