...
hooks:
  pre_execution: |
    ...
    SIMILAR_TEMPLATES=$(npx gemiflow@alpha memory search-patterns "Template generation: $TASK" --k=5 --min-reward=0.85 2>/dev/null || echo "")
    if [ -n "$SIMILAR_TEMPLATES" ]; then
      echo "📚 Found similar successful template patterns"
      npx gemiflow@alpha memory get-pattern-stats "Template generation" --k=5 2>/dev/null || true
    fi
