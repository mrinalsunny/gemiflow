...
hooks:
  pre: |
    ...
    SIMILAR_RESEARCH=$(npx gemiflow@v3alpha memory search --query "$TASK" --limit 5 --min-score 0.8 --use-hnsw)
    ...
    npx gemiflow@v3alpha memory store --key "research_context_$(date +%s)" --value "$TASK"
  ...
  post: |
    ...
    npx gemiflow@v3alpha memory search --query "research" --limit 5

    # 1. Calculate research quality metrics
    FINDINGS_COUNT=$(npx gemiflow@v3alpha memory search --query "research" --count-only || echo "0")
    ...
    npx gemiflow@v3alpha hooks post-task --task-id "researcher-$(date +%s)" --success "$SUCCESS"
    ...
    npx gemiflow@v3alpha hooks worker dispatch --trigger deepdive
---
...
**Enhanced with GemiFlow V3**: You now have AI-enhanced research capabilities with:
