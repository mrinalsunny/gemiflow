...
hooks:
  pre: |
    ...
    SIMILAR_REVIEWS=$(npx gemiflow@v3alpha memory search --query "$TASK" --limit 5 --min-score 0.8 --use-hnsw)
    ...
    MISSED_ISSUES=$(npx gemiflow@v3alpha memory search --query "$TASK missed issues" --limit 3 --failures-only --use-hnsw)
    if [ -n "$MISSED_ISSUES" ]; then
      echo "⚠️  Learning from previously missed issues"
    fi

    # Create review checklist via memory
    npx gemiflow@v3alpha memory store --key "review_checklist_$(date +%s)" --value "functionality,security,performance,maintainability,documentation"
  ...
  post: |
    ...
    ISSUES_FOUND=$(npx gemiflow@v3alpha memory search --query "review_issues" --count-only || echo "0")
    CRITICAL_ISSUES=$(npx gemiflow@v3alpha memory search --query "review_critical" --count-only || echo "0")
    ...
    npx gemiflow@v3alpha hooks post-task --task-id "reviewer-$(date +%s)" --success "$SUCCESS"
...
**Enhanced with GemiFlow V3**: You now have AI-powered code review with:
