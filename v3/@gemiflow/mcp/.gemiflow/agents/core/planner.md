...
hooks:
  pre: |
    ...
    SIMILAR_PLANS=$(npx gemiflow@v3alpha memory search --query "$TASK" --limit 5 --min-score 0.8 --use-hnsw)
    if [ -n "$SIMILAR_PLANS" ]; then
      echo "📚 Found similar successful planning patterns (HNSW-indexed)"
      npx gemiflow@v3alpha hooks intelligence --action pattern-search --query "$TASK" --k 5
    fi

    # 2. Learn from failed plans (EWC++ protected)
    FAILED_PLANS=$(npx gemiflow@v3alpha memory search --query "$TASK failures" --limit 3 --failures-only --use-hnsw)
    if [ -n "$FAILED_PLANS" ]; then
      echo "⚠️  Learning from past planning failures"
    fi

    npx gemiflow@v3alpha memory store --key "planner_start_$(date +%s)" --value "Started planning: $TASK"

    # 3. Store task start via hooks
    npx gemiflow@v3alpha hooks intelligence --action trajectory-start \
      --session-id "planner-$(date +%s)" \
      --task "$TASK"

  post: |
    echo "✅ Planning complete"
    npx gemiflow@v3alpha memory store --key "planner_end_$(date +%s)" --value "Completed planning: $TASK"

    # 1. Calculate planning quality metrics
    TASKS_COUNT=$(npx gemiflow@v3alpha memory search --query "planner_task" --count-only || echo "0")
    AGENTS_ALLOCATED=$(npx gemiflow@v3alpha memory search --query "planner_agent" --count-only || echo "0")
    ...
    npx gemiflow@v3alpha hooks intelligence --action pattern-store \
      --session-id "planner-$(date +%s)" \
      --task "$TASK" \
    ...
    npx gemiflow@v3alpha hooks post-task --task-id "planner-$(date +%s)" --success "$SUCCESS"
    ...
    npx gemiflow@v3alpha hooks worker dispatch --trigger map
---
...
**Enhanced with GemiFlow V3**: You now have AI-powered strategic planning with:
