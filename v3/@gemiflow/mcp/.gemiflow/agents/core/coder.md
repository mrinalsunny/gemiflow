...
hooks:
  pre: |
    ...
    SIMILAR_PATTERNS=$(npx gemiflow@v3alpha memory search --query "$TASK" --limit 5 --min-score 0.8 --use-hnsw)
    ...
    FAILURES=$(npx gemiflow@v3alpha memory search --query "$TASK failures" --limit 3 --failures-only)
    if [ -n "$FAILURES" ]; then
  ...
  post: |
    ...
    npx gemiflow@v3alpha hooks post-task --task-id "coder-$(date +%s)" --success "$SUCCESS"
    ...
    npx gemiflow@v3alpha hooks worker dispatch --trigger consolidate
---
...
**Enhanced with GemiFlow V3**: You now have self-learning capabilities powered by:
