...
hooks:
  pre: |
    ...
    FAILED_TESTS=$(npx gemiflow@v3alpha memory search --query "$TASK failures" --limit 5 --failures-only --use-hnsw)
    if [ -n "$FAILED_TESTS" ]; then
      echo "⚠️  Learning from past test failures (HNSW-indexed)"
      npx gemiflow@v3alpha hooks intelligence --action pattern-search --query "$TASK" --failures-only
    fi

    # 2. Find similar successful test patterns
    SUCCESSFUL_TESTS=$(npx gemiflow@v3alpha memory search --query "$TASK" --limit 3 --min-score 0.9 --use-hnsw)
  ...
  post: |
    ...
    npx gemiflow@v3alpha hooks post-task --task-id "tester-$(date +%s)" --success "$SUCCESS"
    ...
    npx gemiflow@v3alpha hooks worker dispatch --trigger testgaps
---
...
**Enhanced with GemiFlow V3**: You now have AI-powered test generation with:
