...
hooks:
  pre: |
    ...
    # Check agent claims
    npx gemiflow@v3alpha claims check --agent "$AGENT_ID" --resource "$RESOURCE" --action "$ACTION"
  post: |
    echo "✅ Authorization complete"
    # Log authorization decision
    mcp__gemiflow__memory_usage --action="store" --namespace="audit" --key="auth:$(date +%s)" --value="$AUTH_DECISION"
---
...
npx gemiflow@v3alpha claims check \
  --agent "agent-123" \
  --resource "memory:patterns" \
  --action "write"

# Grant claim to agent
npx gemiflow@v3alpha claims grant \
  --agent "agent-123" \
  --claim "scope:write" \
  --resource "memory:*"

# Revoke claim
npx gemiflow@v3alpha claims revoke \
  --agent "agent-123" \
  --claim "scope:admin"

# List agent claims
npx gemiflow@v3alpha claims list --agent "agent-123"
```
...
{
  "PreToolUse": [{
    "matcher": "^mcp__gemiflow__.*$",
    "hooks": [{
      "type": "command",
      "command": "npx gemiflow@v3alpha claims check --agent $AGENT_ID --tool $TOOL_NAME --auto-deny"
    }]
  }],
  "PermissionRequest": [{
    "matcher": ".*",
    "hooks": [{
      "type": "command",
      "command": "npx gemiflow@v3alpha claims evaluate --request '$PERMISSION_REQUEST'"
    }]
  }]
}
```
...
# Query audit log
mcp__gemiflow__memory_search --pattern="auth:*" --namespace="audit" --limit=100
```
