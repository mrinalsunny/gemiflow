...
hooks:
  pre: |
    ...
    mcp__gemiflow__memory_search --pattern="adr:*" --namespace="decisions" --limit=10
  ...
  post: |
    ...
    mcp__gemiflow__memory_usage --action="store" --namespace="decisions" --key="adr:$ADR_NUMBER" --value="$ADR_TITLE"
    # Train pattern on successful decision
    npx gemiflow@v3alpha hooks intelligence trajectory-step --operation="adr-created" --outcome="success"
---
...
The following ADRs define the GemiFlow V3 architecture:
...
```bash
# Create new ADR
npx gemiflow@v3alpha adr create "Decision Title"

# List all ADRs
npx gemiflow@v3alpha adr list

# Search ADRs
npx gemiflow@v3alpha adr search "memory backend"

# Check ADR status
npx gemiflow@v3alpha adr status ADR-006

# Supersede an ADR
npx gemiflow@v3alpha adr supersede ADR-005 ADR-012
```
...
# Search related ADRs
mcp__gemiflow__memory_search --pattern="adr:*memory*" --namespace="decisions"

# Get ADR details
mcp__gemiflow__memory_usage --action="retrieve" --namespace="decisions" --key="adr:006"
```
