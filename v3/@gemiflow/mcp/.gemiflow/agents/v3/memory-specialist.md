...
hooks:
  pre: |
    ...
    mcp__gemiflow__memory_namespace --namespace="${NAMESPACE:-default}" --action="init"
    # Check HNSW index status
    mcp__gemiflow__memory_analytics --timeframe="1h"
    # Store initialization event
    mcp__gemiflow__memory_usage --action="store" --namespace="swarm" --key="memory-specialist:init:${TASK_ID}" --value="$(date -Iseconds): Memory specialist session started"
  post: |
    echo "Memory optimization complete"
    # Persist memory state
    mcp__gemiflow__memory_persist --sessionId="${SESSION_ID}"
    # Compress and optimize namespaces
    mcp__gemiflow__memory_compress --namespace="${NAMESPACE:-default}"
    # Generate memory analytics report
    mcp__gemiflow__memory_analytics --timeframe="24h"
    # Store completion metrics
    mcp__gemiflow__memory_usage --action="store" --namespace="swarm" --key="memory-specialist:complete:${TASK_ID}" --value="$(date -Iseconds): Memory optimization completed"
---
...
class HybridMemoryBackend {
  constructor() {
    ...
    this.sqlite = new SQLiteBackend({
      path: process.env.GEMIFLOW_MEMORY_PATH || './data/memory',
      walMode: true,
      cacheSize: 10000,
      mmap: true
    });
...
mcp__gemiflow__memory_usage --action="store" --namespace="patterns" --key="auth:jwt-strategy" --value='{"pattern": "jwt-auth", "embedding": [...]}' --ttl=604800000

# Semantic search with HNSW
mcp__gemiflow__memory_search --pattern="authentication strategies" --namespace="patterns" --limit=10

# Namespace management
mcp__gemiflow__memory_namespace --namespace="project:myapp" --action="create"

# Memory analytics
mcp__gemiflow__memory_analytics --timeframe="7d"

# Memory compression
mcp__gemiflow__memory_compress --namespace="default"

# Cross-session persistence
mcp__gemiflow__memory_persist --sessionId="session-12345"

# Memory backup
mcp__gemiflow__memory_backup --path="./backups/memory-$(date +%Y%m%d).bak"

# Distributed sync
mcp__gemiflow__memory_sync --target="peer-agent-1"
```
...
npx gemiflow@v3alpha memory init --backend=hybrid --hnsw-enabled

# Memory health check
npx gemiflow@v3alpha memory health

# Search memories
npx gemiflow@v3alpha memory search -q "authentication patterns" --namespace="patterns"

# Consolidate memories
npx gemiflow@v3alpha memory consolidate --strategy=hybrid --retention=0.7

# Export/import namespaces
npx gemiflow@v3alpha memory export --namespace="project:myapp" --format=json
npx gemiflow@v3alpha memory import --file="backup.json" --namespace="project:myapp"

# Memory statistics
npx gemiflow@v3alpha memory stats --namespace="default"

# Quantization
npx gemiflow@v3alpha memory quantize --namespace="embeddings" --method=int8
```
