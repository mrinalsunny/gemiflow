...
hooks:
  pre: |
    ...
    mcp__gemiflow__memory_usage --action="store" --namespace="sparc" --key="session:$SESSION_ID" --value="$(date -Iseconds): SPARC workflow initiated for: $TASK"
    # Search for similar SPARC patterns
    mcp__gemiflow__memory_search --pattern="sparc:success:*" --namespace="patterns" --limit=5
    # Initialize trajectory tracking
    npx gemiflow@v3alpha hooks intelligence trajectory-start --session-id "$SESSION_ID" --agent-type "sparc-orchestrator" --task "$TASK"
  post: |
    echo "✅ SPARC workflow complete"
    # Store completion
    mcp__gemiflow__memory_usage --action="store" --namespace="sparc" --key="complete:$SESSION_ID" --value="$(date -Iseconds): SPARC workflow completed"
    # Train on successful pattern
    npx gemiflow@v3alpha hooks intelligence trajectory-end --session-id "$SESSION_ID" --verdict "success"
---
...
npx gemiflow@v3alpha sparc run full "$TASK"

# Run specific phase
npx gemiflow@v3alpha sparc run specification "$TASK"
npx gemiflow@v3alpha sparc run pseudocode "$TASK"
npx gemiflow@v3alpha sparc run architecture "$TASK"
npx gemiflow@v3alpha sparc run refinement "$TASK"
npx gemiflow@v3alpha sparc run completion "$TASK"

# TDD workflow
npx gemiflow@v3alpha sparc tdd "$FEATURE"

# Check phase status
npx gemiflow@v3alpha sparc status
```
...
mcp__gemiflow__memory_usage --action="store" --namespace="patterns" \
  --key="sparc:success:$(date +%s)" --value="$WORKFLOW_SUMMARY"

# Search for similar patterns
mcp__gemiflow__memory_search --pattern="sparc:*:$PROJECT_TYPE" --namespace="patterns"
```
