...
hooks:
  pre: |
    ...
    mcp__gemiflow__memory_namespace --namespace="swarm" --action="init"
    mcp__gemiflow__memory_namespace --namespace="agents" --action="init"
    mcp__gemiflow__memory_namespace --namespace="tasks" --action="init"
    mcp__gemiflow__memory_namespace --namespace="patterns" --action="init"
    # Store initialization event
    mcp__gemiflow__memory_usage --action="store" --namespace="swarm" --key="memory-manager:init:$(date +%s)" --value="Distributed memory initialized"
  post: |
    echo "🔄 Synchronizing swarm memory state"
    # Sync memory across instances
    mcp__gemiflow__memory_sync --target="all"
    # Compress stale data
    mcp__gemiflow__memory_compress --namespace="swarm"
...
```bash
# Memory operations
mcp__gemiflow__memory_usage --action="store|retrieve|list|delete|search"
mcp__gemiflow__memory_search --pattern="*" --namespace="swarm"
mcp__gemiflow__memory_sync --target="all"
mcp__gemiflow__memory_compress --namespace="default"
mcp__gemiflow__memory_persist --sessionId="$SESSION_ID"
mcp__gemiflow__memory_namespace --namespace="name" --action="init|delete|stats"
...
mcp__gemiflow__swarm_init({ topology: "mesh", maxAgents: 10 })

// 2. Create namespaces
for (const ns of ["swarm", "agents", "tasks", "patterns"]) {
  mcp__gemiflow__memory_namespace({ namespace: ns, action: "init" })
}
