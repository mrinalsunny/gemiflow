...
hooks:
  pre: |
    ...
    AIMDS_RESULT=$(npx gemiflow@v3alpha security defend --input "$TASK" --mode thorough --json 2>/dev/null)
    ...
    THREAT_PATTERNS=$(npx gemiflow@v3alpha memory search-patterns "$TASK" --k=10 --min-reward=0.85 --namespace=security_threats 2>/dev/null)
    if [ -n "$THREAT_PATTERNS" ]; then
      ...
      npx gemiflow@v3alpha memory get-pattern-stats "$TASK" --k=10 --namespace=security_threats 2>/dev/null
    fi

    # ═══════════════════════════════════════════════════════════════
    # PHASE 3: Learn from Past Security Failures
    # ═══════════════════════════════════════════════════════════════
    SECURITY_FAILURES=$(npx gemiflow@v3alpha memory search-patterns "$TASK" --only-failures --k=5 --namespace=security 2>/dev/null)
    ...
    if [[ "$TASK" == *"auth"* ]] || [[ "$TASK" == *"session"* ]] || [[ "$TASK" == *"inject"* ]] || \
      ...
      npx gemiflow@v3alpha security cve --check-relevant "$TASK" 2>/dev/null
    fi
  ...
  post: |
    ...
    if [ -n "$SECURITY_SESSION_ID" ]; then
      ...
      BEHAVIOR_RESULT=$(npx gemiflow@v3alpha security behavior \
      ...
      if [ -n "$BEHAVIOR_RESULT" ]; then
        ...
        if [ "$(echo "$ANOMALY_SCORE > 0.8" | bc 2>/dev/null)" = "1" ]; then
          ...
          npx gemiflow@v3alpha hooks notify --severity warning \
    ...
    if [ "$SUCCESS" = "true" ] && [ "$(echo "$REWARD > 0.85" | bc 2>/dev/null)" = "1" ]; then
      ...
      npx gemiflow@v3alpha security learn \
    ...
    if [ "$CRITICAL_COUNT" -gt 0 ]; then
      ...
      npx gemiflow@v3alpha hooks notify --severity critical \
...
npx gemiflow@v3alpha security defend --input "<suspicious input>" --mode thorough

# Analyze agent behavior
npx gemiflow@v3alpha security behavior --agent <agent-id> --window 1h

# Verify LTL security policy
npx gemiflow@v3alpha security policy --agent <agent-id> --formula "G(edit -> F(review))"

# Record successful mitigation for meta-learning
npx gemiflow@v3alpha security learn --threat-type prompt_injection --strategy sanitize --effectiveness 0.95
```
...
mcp__gemiflow__security_analyze({
  action: "behavior",
  agentId: agentId,
  timeWindow: "1h",
...
mcp__gemiflow__security_verify({
  action: "policy",
  agentId: agentId,
