...
function generateEnhancedMarkdown(report: FullValidationReport, options: ReportOptions): string {
  ...
  lines.push('|-----------|-----------|');
  lines.push('| gemiflow/hive-mind | @gemiflow/swarm |');
  lines.push('| gemiflow/swarm | @gemiflow/swarm |');
  lines.push('| gemiflow/memory | @gemiflow/memory |');
  lines.push('| gemiflow/agents | @gemiflow/agent-lifecycle |');
  lines.push('| gemiflow/tasks | @gemiflow/task-execution |');
  lines.push('');
...
function generateExamplesSection(): string[] {
  ...
  lines.push('npx gemiflow hive-mind init');
  lines.push('npx gemiflow hive-mind status');
  lines.push('');
  lines.push('# V3 (recommended)');
  lines.push('npx @gemiflow/cli swarm init');
  lines.push('npx @gemiflow/cli swarm status');
  lines.push('```');
  lines.push('');
  ...
  lines.push("import { HiveMind } from 'gemiflow/hive-mind';");
  lines.push("import { MemoryManager } from 'gemiflow/memory';");
  lines.push('');
  lines.push('// V3 imports with aliases');
  lines.push("import { UnifiedSwarmCoordinator as HiveMind } from '@gemiflow/swarm';");
  lines.push("import { UnifiedMemoryService as MemoryManager } from '@gemiflow/memory';");
  lines.push('');
...
function generateMigrationScriptsSection(): string[] {
  ...
  lines.push('npx @gemiflow/cli migrate --from v2 --to v3');
  lines.push('');
  lines.push('# Migrate configuration');
  lines.push('npx @gemiflow/cli migrate config --input .gemiflow/config.yaml');
  lines.push('');
  lines.push('# Migrate memory database');
  lines.push('npx @gemiflow/cli migrate memory --input .gemiflow/memory.db');
  lines.push('```');
  lines.push('');
