...
export const V2_CLI_COMMANDS: V2CLICommand[] = [
  // Core commands
  { name: 'init', aliases: ['i'], flags: ['--force', '--template'], description: 'Initialize gemiflow project', v3Equivalent: 'init' },
...
export class V2CompatibilityValidator {
  ...
  async validateAPI(): Promise<ValidationResult> {
    ...
    for (const iface of V2_API_INTERFACES) {
      ...
      checks.push({
        ...
        v3Behavior: isSupported
          ? `Import "${v3Equivalent}" from @gemiflow/*`
          : 'Not available',
  ...
  private generateRecommendations(
    ...
    if (cli.breakingChanges > 0) {
      ...
      recommendations.push('Run migration script: npx @gemiflow/cli migrate');
    }
    ...
    if (api.breakingChanges > 0) {
      recommendations.push('Update import statements to use @gemiflow/* packages');
...
export function generateCompatibilityReport(report: FullValidationReport): string {
  ...
  lines.push("import { SwarmCoordinator } from 'gemiflow/swarm';");
  lines.push('');
  lines.push("// V3 imports (using aliases)");
  lines.push("import { UnifiedSwarmCoordinator as HiveMind } from '@gemiflow/swarm';");
  lines.push("import { UnifiedSwarmCoordinator as SwarmCoordinator } from '@gemiflow/swarm';");
  lines.push('```');
  lines.push('');
  lines.push('---');
  lines.push('');
