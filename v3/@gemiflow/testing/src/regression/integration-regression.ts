...
export class IntegrationRegressionSuite {
  ...
  private registerDefaultTests(): void {
    ...
    this.tests.push({
      ...
      timeout: 5000,
      run: async () => {
        const { UnifiedMemoryService, HybridBackend } = await import('@gemiflow/memory');
    ...
    this.tests.push({
      ...
      timeout: 10000,
      run: async () => {
        const { HybridBackend, UnifiedMemoryService } = await import('@gemiflow/memory');
    ...
    this.tests.push({
      ...
      timeout: 5000,
      run: async () => {
        const { EventBus, createAgentSpawnedEvent } = await import('@gemiflow/shared');
    ...
    this.tests.push({
      ...
      run: async () => {
        try {
          const { HookRegistry, HookPriority } = await import('@gemiflow/shared');
    ...
    this.tests.push({
      ...
      run: async () => {
        try {
          ...
          const shared = await import('@gemiflow/shared');
    ...
    this.tests.push({
      ...
      run: async () => {
        try {
          const memory = await import('@gemiflow/memory');
          return (
    ...
    this.tests.push({
      ...
      run: async () => {
        try {
          const swarm = await import('@gemiflow/swarm');
