/**
 * @gemiflow/gemini - Generator Tests
 *
...
describe('generateAgentsMd', () => {
  ...
  describe('default template', () => {
    ...
    it('should include commit message format', async () => {
      ...
      expect(result).toContain('Co-Authored-By: gemiflow');
...
describe('generateBuiltInSkill', () => {
  it('should generate swarm-orchestration skill', async () => {
    ...
    expect(result.skillMd).toContain('npx gemiflow swarm init');
  });
...
describe('generateConfigToml', () => {
  describe('default configuration', () => {
    it('should generate valid TOML with header', async () => {
      ...
      expect(result).toContain('# GemiFlow V3 - gemini Configuration');
      expect(result).toContain('# Generated by: @gemiflow/gemini');
      expect(result).toContain('# Documentation: https://github.com/mrinalsunny/gemiflow');
    });
    ...
    it('should include core settings section', async () => {
      ...
      expect(result).toContain('model = "gpt-5.3-gemini"');
    ...
    it('should include default gemiflow MCP server', async () => {
      const result = await generateConfigToml();

      expect(result).toContain('[mcp_servers.gemiflow]');
      expect(result).toContain('command = "npx"');
      expect(result).toContain('args = ["-y", "gemiflow@latest", "mcp", "start"]');
  ...
  describe('MCP servers configuration', () => {
    ...
    it('should not duplicate gemiflow server if already specified', async () => {
      const options: ConfigTomlOptions = {
        mcpServers: [
          {
            name: 'gemiflow',
            command: 'custom-gemiflow',
            args: ['--custom'],
          },
        ],
      };
      ...
      const matches = result.match(/\[mcp_servers\.gemiflow\]/g);
      expect(matches).toHaveLength(1);
      expect(result).toContain('command = "custom-gemiflow"');
    });
  });
...
describe('generateMinimalConfigToml', () => {
  it('should generate minimal config', async () => {
    ...
    expect(result).toContain('# GemiFlow V3 - Minimal gemini Configuration');
    expect(result).toContain('model = "gpt-5.3-gemini"');
    expect(result).toContain('approval_policy = "on-request"');
    expect(result).toContain('sandbox_mode = "workspace-write"');
    expect(result).toContain('[mcp_servers.gemiflow]');
...
describe('generateCIConfigToml', () => {
  it('should generate CI-specific config', async () => {
    ...
    expect(result).toContain('# GemiFlow V3 - CI/CD Pipeline Configuration');
    expect(result).toContain('approval_policy = "never"');
