...
const SAMPLE_CLAUDE_MD = `# GemiFlow V3 Project
...
describe('convertSettingsToToml', () => {
  ...
  it('should convert MCP servers', () => {
    ...
    expect(result).toContain('args = ["-y", "@gemiflow/cli"]');
    expect(result).toContain('[mcp_servers.custom-server]');
    expect(result).toContain('command = "node"');
  });

  it('should add default gemiflow server when no mcpServers', () => {
    const settings = {
      model: 'gpt-4',
    };
    ...
    // The implementation adds a default gemiflow server when none specified
    expect(result).toContain('[mcp_servers.gemiflow]');
    expect(result).toContain('command = "npx"');
  });
