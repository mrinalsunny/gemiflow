/**
 * @gemiflow/gemini - SKILL.md Generator
 *
 * Generates SKILL.md files for OpenAI gemini CLI skills
      ...
      commands: [
        {
          ...
          command: 'npx gemiflow swarm init --topology hierarchical --max-agents 8 --strategy specialized',
          example: 'npx gemiflow swarm init --topology hierarchical --max-agents 6 --strategy specialized',
        },
        {
          name: 'Route Task',
          description: 'Route a task to the appropriate agents based on task type',
          command: 'npx @gemiflow/cli hooks route --task "[task description]"',
          example: 'npx @gemiflow/cli hooks route --task "implement OAuth2 authentication flow"',
        },
        {
          name: 'Spawn Agent',
          description: 'Spawn a specific agent type',
          command: 'npx @gemiflow/cli agent spawn --type [type] --name [name]',
          example: 'npx @gemiflow/cli agent spawn --type coder --name impl-auth',
        },
        {
          name: 'Monitor Status',
          description: 'Check the current swarm status',
          command: 'npx @gemiflow/cli swarm status --verbose',
        },
        {
          name: 'Orchestrate Task',
          description: 'Orchestrate a task across multiple agents',
          command: 'npx @gemiflow/cli task orchestrate --task "[task]" --strategy adaptive',
          example: 'npx @gemiflow/cli task orchestrate --task "refactor auth module" --strategy parallel --max-agents 4',
        },
        {
          name: 'List Agents',
          description: 'List all active agents',
          command: 'npx @gemiflow/cli agent list --filter active',
        },
      ],
      scripts: [
        {
      ...
      commands: [
        {
          ...
          command: 'npx @gemiflow/cli memory store --key "[key]" --value "[value]" --namespace patterns',
          example: 'npx @gemiflow/cli memory store --key "auth-jwt-pattern" --value "JWT validation with refresh tokens" --namespace patterns',
        },
        {
          name: 'Semantic Search',
          description: 'Search memory using semantic similarity',
          command: 'npx @gemiflow/cli memory search --query "[search terms]" --limit 10',
          example: 'npx @gemiflow/cli memory search --query "authentication best practices" --limit 5',
        },
        {
          name: 'Retrieve Entry',
          description: 'Retrieve a specific memory entry by key',
          command: 'npx @gemiflow/cli memory get --key "[key]" --namespace [namespace]',
          example: 'npx @gemiflow/cli memory get --key "auth-jwt-pattern" --namespace patterns',
        },
        {
          name: 'List Entries',
          description: 'List all entries in a namespace',
          command: 'npx @gemiflow/cli memory list --namespace [namespace]',
          example: 'npx @gemiflow/cli memory list --namespace patterns --limit 20',
        },
        {
          name: 'Delete Entry',
          description: 'Delete a memory entry',
          command: 'npx @gemiflow/cli memory delete --key "[key]" --namespace [namespace]',
        },
        {
          name: 'Initialize HNSW Index',
          description: 'Initialize HNSW vector search index',
          command: 'npx @gemiflow/cli memory init --enable-hnsw',
        },
        {
          name: 'Memory Stats',
          description: 'Show memory usage statistics',
          command: 'npx @gemiflow/cli memory stats',
        },
        {
          name: 'Export Memory',
          description: 'Export memory to JSON',
          command: 'npx @gemiflow/cli memory export --output memory-backup.json',
        },
      ],
      scripts: [
        {
      ...
      commands: [
        {
          ...
          command: 'npx @gemiflow/cli hooks route --task "specification: [requirements]"',
          example: 'npx @gemiflow/cli hooks route --task "specification: user authentication with OAuth2, MFA, and session management"',
        },
        {
          name: 'Pseudocode Phase',
          description: 'Write high-level pseudocode for the implementation',
          command: 'npx @gemiflow/cli hooks route --task "pseudocode: [feature]"',
          example: 'npx @gemiflow/cli hooks route --task "pseudocode: OAuth2 login flow with token refresh"',
        },
        {
          name: 'Architecture Phase',
          description: 'Design system structure, interfaces, and dependencies',
          command: 'npx @gemiflow/cli hooks route --task "architecture: [design]"',
          example: 'npx @gemiflow/cli hooks route --task "architecture: auth module with service layer, repository, and API endpoints"',
        },
        {
          name: 'Refinement Phase',
          description: 'Iterate on the design based on feedback',
          command: 'npx @gemiflow/cli hooks route --task "refinement: [feedback]"',
          example: 'npx @gemiflow/cli hooks route --task "refinement: add rate limiting and brute force protection"',
        },
        {
          name: 'Completion Phase',
          description: 'Finalize implementation with tests and documentation',
          command: 'npx @gemiflow/cli hooks route --task "completion: [final checks]"',
          example: 'npx @gemiflow/cli hooks route --task "completion: verify all tests pass, update API docs, security review"',
        },
        {
          name: 'SPARC Coordinator',
          description: 'Spawn SPARC coordinator agent',
          command: 'npx @gemiflow/cli agent spawn --type sparc-coord --name sparc-lead',
        },
      ],
      scripts: [
        {
      ...
      commands: [
        {
          ...
          command: 'npx @gemiflow/cli security scan --depth full',
          example: 'npx @gemiflow/cli security scan --depth full --output security-report.json',
        },
        {
          name: 'Input Validation Check',
          description: 'Check for input validation issues',
          command: 'npx @gemiflow/cli security scan --check input-validation',
          example: 'npx @gemiflow/cli security scan --check input-validation --path ./src/api',
        },
        {
          name: 'Path Traversal Check',
          description: 'Check for path traversal vulnerabilities',
          command: 'npx @gemiflow/cli security scan --check path-traversal',
        },
        {
          name: 'SQL Injection Check',
          description: 'Check for SQL injection vulnerabilities',
          command: 'npx @gemiflow/cli security scan --check sql-injection',
        },
        {
          name: 'XSS Check',
          description: 'Check for cross-site scripting vulnerabilities',
          command: 'npx @gemiflow/cli security scan --check xss',
        },
        {
          name: 'CVE Scan',
          description: 'Scan dependencies for known CVEs',
          command: 'npx @gemiflow/cli security cve --scan',
          example: 'npx @gemiflow/cli security cve --scan --severity high',
        },
        {
          name: 'Security Audit Report',
          description: 'Generate full security audit report',
          command: 'npx @gemiflow/cli security audit --report',
          example: 'npx @gemiflow/cli security audit --report --format markdown --output SECURITY.md',
        },
        {
          name: 'Threat Modeling',
          description: 'Run threat modeling analysis',
          command: 'npx @gemiflow/cli security threats --analyze',
        },
        {
          name: 'Validate Secrets',
          description: 'Check for hardcoded secrets',
          command: 'npx @gemiflow/cli security validate --check secrets',
        },
      ],
      scripts: [
        {
      ...
      commands: [
        {
          ...
          command: 'npx @gemiflow/cli performance benchmark --suite all',
          example: 'npx @gemiflow/cli performance benchmark --suite all --iterations 100 --output bench-results.json',
        },
        {
          name: 'Profile Code',
          description: 'Profile code execution for CPU and memory',
          command: 'npx @gemiflow/cli performance profile --target ./src',
          example: 'npx @gemiflow/cli performance profile --target ./src/api --duration 60s',
        },
        {
          name: 'Memory Analysis',
          description: 'Analyze memory usage patterns',
          command: 'npx @gemiflow/cli performance metrics --metric memory',
          example: 'npx @gemiflow/cli performance metrics --metric memory --threshold 100MB',
        },
        {
          name: 'Latency Analysis',
          description: 'Measure and analyze latency',
          command: 'npx @gemiflow/cli performance metrics --metric latency',
        },
        {
          name: 'Optimize Suggestions',
          description: 'Get automated optimization suggestions',
          command: 'npx @gemiflow/cli performance optimize --analyze',
          example: 'npx @gemiflow/cli performance optimize --analyze --apply-safe',
        },
        {
          name: 'Performance Report',
          description: 'Generate performance report',
          command: 'npx @gemiflow/cli performance report',
          example: 'npx @gemiflow/cli performance report --format html --output perf-report.html',
        },
        {
          name: 'Compare Benchmarks',
          description: 'Compare benchmark results',
          command: 'npx @gemiflow/cli performance benchmark --compare baseline.json current.json',
        },
        {
          name: 'WASM Benchmark',
          description: 'Run WASM-specific benchmarks',
          command: 'npx @gemiflow/cli performance benchmark --suite wasm',
        },
      ],
      scripts: [
        {
...
npx @gemiflow/cli performance benchmark \\
  --suite all \\
  --iterations 100 \\
