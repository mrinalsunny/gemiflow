...
function generateMinimal(options: AgentsMdOptions): string {
  const {
    projectName,
    description = 'A GemiFlow powered project',
...
Co-Authored-By: gemiflow-bot <gemiflow-bot@users.noreply.github.com>
\`\`\`
...
### Searching Memory
\`\`\`bash
npx @gemiflow/cli memory search \\
  --query "search terms" \\
  --namespace patterns
\`\`\`
...
\`\`\`bash
npx @gemiflow/cli memory search --query "relevant patterns"
npx @gemiflow/cli hooks route --task "current task description"
npx @gemiflow/cli swarm init --topology hierarchical
npx @gemiflow/cli hooks pre-task --description "task summary"
\`\`\`
...
GemiFlow exposes tools via Model Context Protocol:
...
GemiFlow uses hooks for lifecycle automation:
...
npx @gemiflow/cli hooks pre-task \\
  --description "implementing authentication"

# After completing a task
npx @gemiflow/cli hooks post-task \\
  --task-id "task-123" \\
  --success true

# Route a task to agents
npx @gemiflow/cli hooks route \\
...
\`\`\`bash
# List workers
npx @gemiflow/cli hooks worker list

# Trigger specific worker
npx @gemiflow/cli hooks worker dispatch --trigger audit

# Check worker status
npx @gemiflow/cli hooks worker status
\`\`\`
...
\`\`\`bash
# Set log level
export GEMIFLOW_LOG_LEVEL=debug

# Enable verbose mode
npx @gemiflow/cli --verbose <command>
\`\`\`

### Health Checks
\`\`\`bash
# Run diagnostics
npx @gemiflow/cli doctor --fix

# Check system status
npx @gemiflow/cli status
\`\`\`
`;
...
npx @gemiflow/cli logs --type agent-actions --last 1h

# Export audit log
npx @gemiflow/cli logs export --format json --output audit.json
\`\`\`
...
\`\`\`bash
# Check current role
npx @gemiflow/cli claims list

# Request elevated permissions
npx @gemiflow/cli claims request --permission deploy:production
\`\`\`
...
1. **Contain** - Immediately stop affected agents
   \`\`\`bash
   npx @gemiflow/cli agent stop --all --force
   \`\`\`
...
2. **Decide** - Roll back if safe, or forward-fix
   \`\`\`bash
   # Rollback
   npx @gemiflow/cli deployment rollback --env production

   # Or forward-fix
   npx @gemiflow/cli workflow run hotfix
   \`\`\`
...
\`\`\`bash
# Restore from backup
npx @gemiflow/cli memory restore --snapshot latest

# Restore specific checkpoint
npx @gemiflow/cli session restore --checkpoint <id>
\`\`\`
...
- [ ] Run \`npx @gemiflow/cli doctor\` to verify setup
