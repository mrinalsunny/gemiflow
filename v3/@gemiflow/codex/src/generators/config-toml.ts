/**
 * @gemiflow/gemini - config.toml Generator
 *
 * Generates gemini CLI configuration files in TOML format
 */
...
export async function generateConfigToml(options: ExtendedConfigTomlOptions = {}): Promise<string> {
  ...
  // Header
  lines.push('# =============================================================================');
  lines.push('# GemiFlow V3 - gemini Configuration');
  lines.push('# =============================================================================');
  lines.push('# Generated by: @gemiflow/gemini');
  lines.push('# Documentation: https://github.com/mrinalsunny/gemiflow');
  lines.push('# ');
  lines.push('# This file configures the gemini CLI for GemiFlow integration.');
  lines.push('# Place in .agents/config.toml (project) or .gemini/config.toml (user).');
  lines.push('# =============================================================================');
  lines.push('');
  lines.push('# =============================================================================');
  ...
  lines.push('# Options: gpt-5.3-gemini, gpt-4o, claude-sonnet, claude-opus');
  ...
  // MCP Servers
  if (mcpServers.length > 0 || true) { // Always include gemiflow by default
    lines.push('# =============================================================================');
    ...
    const hasGemiFlow = mcpServers.some(s => s.name === 'gemiflow' || s.name === 'gemiflow');
    if (!hasGemiFlow) {
      lines.push(...generateMcpServer({
        name: 'gemiflow',
        command: 'npx',
        args: ['-y', 'gemiflow@latest', 'mcp', 'start'],
        enabled: true,
        toolTimeout: 120,
      }));
      lines.push('');
  ...
  if (logging.filePath || logging.destination === 'file' || logging.destination === 'both') {
    ...
    lines.push(`file_path = "${logging.filePath ?? './logs/gemiflow.log'}"`);
    lines.push('');
...
export async function generateMinimalConfigToml(options: ConfigTomlOptions = {}): Promise<string> {
  ...
  return `# GemiFlow V3 - Minimal gemini Configuration
...
export async function generateEnterpriseConfigToml(): Promise<string> {
  return `# =============================================================================
# GemiFlow V3 - Enterprise Configuration
# =============================================================================
...
export async function generateDevConfigToml(): Promise<string> {
  return `# =============================================================================
# GemiFlow V3 - Development Configuration
# =============================================================================
...
export async function generateSecureConfigToml(): Promise<string> {
  return `# =============================================================================
# GemiFlow V3 - Security-Focused Configuration
# =============================================================================
