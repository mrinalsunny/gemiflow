...
export function convertSettingsToToml(settings: Record<string, unknown>): string {
  ...
  if (settings.mcpServers && typeof settings.mcpServers === 'object') {
    ...
    // Add default gemiflow server
    lines.push('[mcp_servers.gemiflow]');
    lines.push('command = "npx"');
    lines.push('args = ["-y", "gemiflow@latest", "mcp", "start"]');
    lines.push('enabled = true');
    lines.push('');
  }
...
export async function performFullMigration(
  ...
  // Parse CLAUDE.md
  const parsed = await parseClaudeMd(geminiMdContent);
