...
export class DualModeOrchestrator extends EventEmitter {
  ...
  async initializeSharedMemory(taskContext: string): Promise<void> {
    ...
    await this.runCommand(
      'npx',
      ['gemiflow@alpha', 'memory', 'init', '--force'],
      projectPath
    );
...
1. Search shared memory for context: npx gemiflow@alpha memory search --query "<relevant terms>" --namespace ${sharedNamespace}
2. Complete your assigned task
3. Store your results: npx gemiflow@alpha memory store --key "${config.id}-result" --value "<your summary>" --namespace ${sharedNamespace}
  ...
  private async collectSharedMemory(): Promise<Record<string, any>> {
    ...
    try {
      const output = await this.runCommand(
        'npx',
        ['gemiflow@alpha', 'memory', 'list', '--namespace', sharedNamespace, '--format', 'json'],
        projectPath
      );
