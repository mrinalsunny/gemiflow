...
[![npm version](https://img.shields.io/npm/v/@gemiflow/memory.svg)](https://www.npmjs.com/package/@gemiflow/memory)
[![Ecosystem downloads](https://img.shields.io/badge/ecosystem%20downloads-22.2M%2B-blue.svg)](https://github.com/mrinalsunny/gemiflow/blob/main/data/clone-data.proof.json)
[![Git clones (14d)](https://img.shields.io/badge/git%20clones%2014d-115k-blueviolet.svg)](https://github.com/mrinalsunny/gemiflow/blob/main/data/clone-data.ledger.json)
...
- **Graceful retrieval.** `service.search('query')` no longer throws when `@gemiflow/embeddings` is unavailable — it degrades to FTS5 keyword search and emits `health.embedder = 'degraded'`. The full hybrid path also adds real Reciprocal Rank Fusion + MMR diversity rerank.
...
- **Node 26 install support** ([#1867](https://github.com/mrinalsunny/gemiflow/issues/1867)) — `better-sqlite3` is fully optional; installs succeed on Node 24/26 and the package falls back to sql.js when the native build isn't available.
...
```bash
npm install @gemiflow/memory
```

## Standalone use (without the GemiFlow CLI)

This package works on its own — you don't need `@gemiflow/cli` or the
full GemiFlow install. Use it any time you want HNSW vector search, an
...
```typescript
import { ControllerRegistry } from '@gemiflow/memory';
...
```typescript
import { createHybridService } from '@gemiflow/memory';

async function embedder(text: string): Promise<Float32Array> {
  // Use any embedding provider — OpenAI, @gemiflow/embeddings, your own model.
...
import { MemoryService } from '@gemiflow/memory';
...
```typescript
import { MemoryService, HNSWIndex } from '@gemiflow/memory';
...
```typescript
import { MemoryConsolidator } from '@gemiflow/memory';
...
`MemoryService.search()` and `semanticSearch()` no longer hard-fail when `@gemiflow/embeddings` (or whatever embedder you injected) is unavailable. They:
...
```typescript
import { AgentDBAdapter } from '@gemiflow/memory';
...
```typescript
import { CacheManager } from '@gemiflow/memory';
...
```typescript
import { QueryBuilder } from '@gemiflow/memory';
...
```typescript
import { MemoryMigrator, createMigrator } from '@gemiflow/memory';
...
```typescript
import { AutoMemoryBridge } from '@gemiflow/memory';
...
Connects insights to the `@gemiflow/neural` learning pipeline. When neural is unavailable, all operations degrade to no-ops.

### Quick Start

```typescript
import { AutoMemoryBridge, LearningBridge } from '@gemiflow/memory';
...
```typescript
import { LearningBridge } from '@gemiflow/memory';

const lb = new LearningBridge(backend, {
  // Optional: inject neural loader for custom setups
  neuralLoader: async () => {
    const { NeuralLearningSystem } = await import('@gemiflow/neural');
...
```typescript
import { AutoMemoryBridge, MemoryGraph } from '@gemiflow/memory';
...
```typescript
import { MemoryGraph } from '@gemiflow/memory';
...
```typescript
import { createAgentBridge, transferKnowledge } from '@gemiflow/memory';
...
|-------|-----------|----------|
| `project` | `<gitRoot>/.gemiflow/agent-memory/<agent>/` | Project-specific learnings |
| `local` | `<gitRoot>/.gemiflow/agent-memory-local/<agent>/` | Machine-local data |
| `user` | `~/.gemiflow/agent-memory/<agent>/` | Cross-project user knowledge |
...
// → /workspaces/my-project/.gemiflow/agent-memory/my-agent/
...
- `better-sqlite3` — **Optional** native SQLite driver for higher throughput. The package works without it (sql.js fallback), so installs succeed on Node 24/26 even when the native build can't compile ([#1867](https://github.com/mrinalsunny/gemiflow/issues/1867))
- `@gemiflow/embeddings` — **Optional** for semantic search. When absent or failing, `search()` automatically falls back to FTS5 keyword (see [Graceful retrieval](#graceful-retrieval-fts5-fallback))
- `@gemiflow/neural` — **Optional** peer dependency for self-learning (graceful fallback when unavailable)

## Related Packages

- [@gemiflow/neural](../neural) - Neural learning integration (SONA, ReasoningBank, EWC++)
- [@gemiflow/shared](../shared) - Shared types and utilities
- [@gemiflow/hooks](../hooks) - Session lifecycle hooks for auto memory sync
