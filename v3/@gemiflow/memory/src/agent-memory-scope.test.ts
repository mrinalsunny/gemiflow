...
describe('resolveAgentMemoryDir', () => {
  ...
  it('should resolve project scope to gitRoot/.gemiflow/agent-memory/name/', () => {
    ...
    expect(result).toBe(
      path.join('/workspaces/my-project', '.gemiflow', 'agent-memory', 'coder'),
    );
  });

  it('should resolve local scope to gitRoot/.gemiflow/agent-memory-local/name/', () => {
    mockExistsSync.mockImplementation((p: string) => {
      return String(p) === path.join('/workspaces/my-project', '.git');
    });
    ...
    expect(result).toBe(
      path.join('/workspaces/my-project', '.gemiflow', 'agent-memory-local', 'researcher'),
    );
  });

  it('should resolve user scope to ~/.gemiflow/agent-memory/name/', () => {
    process.env.HOME = '/home/testuser';

    const result = resolveAgentMemoryDir('planner', 'user');
    expect(result).toBe(
      path.join('/home/testuser', '.gemiflow', 'agent-memory', 'planner'),
    );
  });
  ...
  it('should sanitize agent name by replacing special characters', () => {
    ...
    expect(result).toBe(
      path.join('/home/testuser', '.gemiflow', 'agent-memory', 'my_agent___name'),
    );
  });
  ...
  it('should handle dots and slashes in agent name', () => {
    ...
    expect(result).toBe(
      path.join('/home/testuser', '.gemiflow', 'agent-memory', 'agent_with_dots'),
    );
  });
  ...
  it('should fall back to workingDir when no git root is found', () => {
    ...
    expect(result).toBe(
      path.join('/some/dir', '.gemiflow', 'agent-memory', 'coder'),
    );
  });
  ...
  it('should fall back to USERPROFILE when HOME is not set', () => {
    ...
    expect(result).toBe(
      path.join('/Users/testuser', '.gemiflow', 'agent-memory', 'coder'),
    );
  });
  ...
  it('should use cwd as fallback for project scope when workingDir is omitted', () => {
    ...
    expect(result).toBe(
      path.join(cwd, '.gemiflow', 'agent-memory', 'coder'),
    );
  });
  ...
  it('should preserve hyphens and underscores in agent name', () => {
    ...
    expect(result).toBe(
      path.join('/home/testuser', '.gemiflow', 'agent-memory', 'my-agent_01'),
    );
  });
});
...
describe('createAgentBridge', () => {
  ...
  it('should create a bridge with correct memoryDir for project scope', () => {
    ...
    expect(bridge.getMemoryDir()).toBe(
      path.join('/workspaces/project', '.gemiflow', 'agent-memory', 'coder'),
    );
    bridge.destroy();
  });
  ...
  it('should create a bridge with correct memoryDir for user scope', () => {
    ...
    expect(bridge).toBeInstanceOf(AutoMemoryBridge);
    expect(bridge.getMemoryDir()).toBe(
      path.join('/home/testuser', '.gemiflow', 'agent-memory', 'reviewer'),
    );
  ...
  it('should create a bridge with correct memoryDir for local scope', () => {
    ...
    expect(bridge).toBeInstanceOf(AutoMemoryBridge);
    expect(bridge.getMemoryDir()).toBe(
      path.join('/workspaces/project', '.gemiflow', 'agent-memory-local', 'tester'),
    );
    bridge.destroy();
  });
...
describe('listAgentScopes', () => {
  ...
  it('should list agents from existing directories', () => {
    ...
    const projectDir = path.join('/workspaces/project', '.gemiflow', 'agent-memory');
    const localDir = path.join('/workspaces/project', '.gemiflow', 'agent-memory-local');
    const userDir = path.join('/home/testuser', '.gemiflow', 'agent-memory');
