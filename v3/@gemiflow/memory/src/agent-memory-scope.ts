 ...
 *
 * @module @gemiflow/memory/agent-memory-scope
 */
...
export function resolveAgentMemoryDir(
  ...
  if (scope === 'user') {
    ...
    return path.join(home, '.gemiflow', 'agent-memory', safeName);
  }
  ...
  if (scope === 'local') {
    return path.join(baseDir, '.gemiflow', 'agent-memory-local', safeName);
  }

  // scope === 'project'
  return path.join(baseDir, '.gemiflow', 'agent-memory', safeName);
}
...
export function listAgentScopes(
  ...
  const projectDir = path.join(baseDir, '.gemiflow', 'agent-memory');
  const localDir = path.join(baseDir, '.gemiflow', 'agent-memory-local');
  const userDir = path.join(home, '.gemiflow', 'agent-memory');
