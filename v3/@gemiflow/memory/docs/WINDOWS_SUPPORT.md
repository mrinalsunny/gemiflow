...
This implementation adds **complete Windows cross-platform support** to the `@gemiflow/memory` module using sql.js as a WASM-based SQLite fallback when native compilation fails.
...
```typescript
import { getPlatformInfo, getAvailableProviders } from '@gemiflow/memory';
...
RUN npm install @gemiflow/memory
