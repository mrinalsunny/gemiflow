...
The `@gemiflow/memory` module provides **universal cross-platform support** through intelligent database provider selection. This ensures your application works seamlessly on Windows, macOS, and Linux without code changes.
...
```typescript
import { createDatabase, getPlatformInfo } from '@gemiflow/memory';
...
```typescript
import { createDatabase } from '@gemiflow/memory';
...
```typescript
import { createDatabase, getAvailableProviders } from '@gemiflow/memory';
...
npm install @gemiflow/memory --no-optional
...
GEMIFLOW_DB_PROVIDER=sql.js

# Enable verbose logging
GEMIFLOW_DB_VERBOSE=true

# Set auto-persist interval (ms)
GEMIFLOW_DB_PERSIST_INTERVAL=10000
```
