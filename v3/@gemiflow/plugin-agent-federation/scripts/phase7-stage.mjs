...
// Persist local private key (mode 0600) for operator to move to .gemiflow/federation/
writeFileSync(
...
writeFileSync(join(stageDir, 'gemiflow-fed.conf'), interfaceConfig);

// Firewall projection per OS
const fw = new wgFirewall.WgFirewallService();
const fwFile = process.platform === 'linux' ? 'gemiflow-fed.nft' : 'gemiflow-fed.pf';
...
console.log(`  ${stageDir}/gemiflow-fed.conf               (wg-quick config)`);
...
console.log('  [ ] Review gemiflow-fed.conf for unexpected [Peer] blocks (defense vs compromised manifest)');
...
console.log(`  sudo install -m 0600 ${stageDir}/gemiflow-fed.conf /etc/wireguard/gemiflow-fed.conf`);
if (process.platform === 'linux') {
  console.log(`  sudo nft -f ${stageDir}/${fwFile}`);
} else {
  console.log(`  sudo pfctl -a gemiflow-fed -f ${stageDir}/${fwFile}`);
}
console.log('  sudo wg-quick up gemiflow-fed');
console.log();
console.log('=== Verification ===');
console.log('  sudo wg show gemiflow-fed');
console.log(`  ping ${peerMeshIP.replace('/32', '')}    # mesh reachability`);
console.log();
console.log('=== Rollback ===');
console.log('  sudo wg-quick down gemiflow-fed');
if (process.platform === 'linux') {
  console.log('  sudo nft delete table inet gemiflow_fed');
} else {
  console.log('  sudo pfctl -a gemiflow-fed -F all');
}
