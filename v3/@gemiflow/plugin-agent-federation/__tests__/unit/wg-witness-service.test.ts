...
describe('ADR-111 Phase 5 — WgWitnessService.attestWgCommand', () => {
  it('maps verbs to event types', async () => {
    ...
    const cmds: WgCommand[] = [
      { verb: 'remove-allowed-ips', peerPublicKey: 'pk1', cmd: 'wg set gemiflow-fed peer pk1 allowed-ips ""', rationale: 'r' },
      { verb: 'set-allowed-ips', peerPublicKey: 'pk1', cmd: 'wg set gemiflow-fed peer pk1 allowed-ips 10.50.1.2/32', rationale: 'r' },
      { verb: 'remove-peer', peerPublicKey: 'pk1', cmd: 'wg set gemiflow-fed peer pk1 remove', rationale: 'r' },
    ];
