import type { MCPToolDefinition } from '@gemiflow/shared/src/plugin-interface.js';
import type { PluginContext } from '@gemiflow/shared/src/plugin-interface.js';
  ...
  return [
    {
      ...
      pluginName: '@gemiflow/plugin-agent-federation',
    ...
    },
    {
      ...
      description: 'ADR-111 Phase 6 (witness chain): emit an operator-signed attestation entry for a coordination change. Returns the canonical bytes the operator signs + the witness entry; operator appends it to .gemiflow/federation/wg-changes.log. Idempotent — re-attesting the same change just appends another entry.',
      pluginName: '@gemiflow/plugin-agent-federation',
      version: '1.0.0-alpha.1',
      inputSchema: {
        type: 'object',
    ...
    },
    {
      ...
      handler: async (params) => {
        ...
        return textResult(JSON.stringify({
          ...
          nextSteps: [
            'Write the new private key to .gemiflow/federation/wg-key-<nodeId>.json (mode 0600)',
