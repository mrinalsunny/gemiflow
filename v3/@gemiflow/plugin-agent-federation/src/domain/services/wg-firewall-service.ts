 ...
 *   - `pf` for macOS — `pfctl -a gemiflow-fed -f <file>`, anchor-scoped so
 *     gemiflow's rules don't collide with the operator's main pf ruleset.
 *
...
export interface WgFirewallServiceConfig {
  ...
  /** WG interface name to scope rules to. Defaults to `gemiflow-fed`. */
  readonly interfaceName?: string;
  /** Where the operator should write the rule file. Used for the load-command string. */
  readonly rulePath?: string;
  /** pf anchor (macOS only). Defaults to `gemiflow-fed`. */
...
const DEFAULT_INTERFACE = 'gemiflow-fed';
const DEFAULT_PF_ANCHOR = 'gemiflow-fed';
...
export class WgFirewallService {
  ...
  private renderNftables(projections: ReadonlyArray<{
    ...
    lines.push('# Generated by gemiflow federation plugin — ADR-111 Phase 4.');
    ...
    lines.push('');
    lines.push('table inet gemiflow_fed {');
  ...
  private defaultRulePath(): string {
    return this.platform === 'linux-nftables'
      ? '/etc/nftables.d/gemiflow-fed.nft'
