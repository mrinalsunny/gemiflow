#!/usr/bin/env node
/**
 * Standalone CLI shim for @gemiflow/plugin-agent-federation.
 * Maps `gemiflow-federation <subcommand> [args]` -> CLICommandDefinition handlers.
 *
...
function printHelp(commandNames: string[]): void {
  console.log('gemiflow-federation — Cross-installation agent federation CLI');
  console.log('');
  console.log('Usage: gemiflow-federation <subcommand> [options]');
  console.log('');
  ...
  console.log('');
  console.log('Run "gemiflow-federation <subcommand> --help" for command-specific options.');
}
...
async function main(): Promise<void> {
  ...
  if (!resolved) {
    ...
    console.error('Run "gemiflow-federation --help" to list available subcommands.');
    process.exitCode = 1;
    return;
  }
