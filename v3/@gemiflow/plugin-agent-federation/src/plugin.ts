import type {
  ...
  AgentTypeDefinition,
} from '@gemiflow/shared/src/plugin-interface.js';
...
export class AgentFederationPlugin implements gemiflowPlugin {
  ...
  readonly dependencies = ['@gemiflow/security', '@gemiflow/aidefence'];
  ...
  async initialize(context: PluginContext): Promise<void> {
    ...
    // signatures. Persist to .gemiflow/federation/key-<nodeId>.json so
    ...
    const keyDir = join(process.cwd(), '.gemiflow', 'federation');
    ...
    return [
      {
        ...
        defaultConfig: {
          ...
          timeout: 300_000,
          metadata: {
            pluginSource: '@gemiflow/plugin-agent-federation',
          },
        },
