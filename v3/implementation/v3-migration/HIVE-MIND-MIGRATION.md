...
V2's Hive-Mind system is a comprehensive multi-agent coordination framework. V3 consolidates this into the `@gemiflow/swarm` module with a unified `SwarmCoordinator` per ADR-003.
...
| `Memory.ts` | `@gemiflow/memory` | ✅ Enhanced |
| `Communication.ts` | `@gemiflow/shared/events` | ✅ Complete |
...
**Migration Path:**
```typescript
// V3 Implementation needed in @gemiflow/swarm/src/queen-coordinator.ts
...
```typescript
// V3: v3/@gemiflow/swarm/src/topology-manager.ts
const topologyConfigs = {
  mesh: { /* similar */ },
...
```typescript
// V3: v3/@gemiflow/swarm/src/consensus/consensus-engine.ts
...
### V3 Agent Types
```typescript
// v3/@gemiflow/swarm/src/types.ts
...
npx gemiflow hive-mind task --description "Implement feature"
npx gemiflow hive-mind wizard
npx gemiflow hive-mind pause
npx gemiflow hive-mind resume
npx gemiflow hive-mind stop
npx gemiflow hive-mind ps
npx gemiflow hive-mind optimize-memory
```
...
npx gemiflow agent spawn --type queen-coordinator
...
import { Queen } from 'gemiflow/hive-mind/core/Queen';
