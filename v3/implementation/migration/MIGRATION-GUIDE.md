# GemiFlow v2 to v3 Migration Guide

## Overview

This guide walks you through migrating from GemiFlow v2.x to v3.0. The migration is designed to be **zero-breaking-changes** - all v2 code continues to work while you gradually adopt v3 features.

## Prerequisites

- Node.js >= 18.0.0
- GemiFlow v2.7.x installed
...
This command:
- Backs up your current config to `.gemiflow/config.json.v2.backup`
...
npx gemiflow status --check-migration
```
...
cp .gemiflow/settings-enhanced.json .gemiflow/settings.prod.json
...
```bash
rm .gemiflow/settings-complete.json
rm .gemiflow/settings-checkpoint-*.json
rm .gemiflow/settings.reasoningbank-*.json
rm .gemiflow/settings-npx-hooks.json
```
...
// v3 enhanced (opt-in)
import { HybridMemorySystem } from 'gemiflow/v3';
...
GemiFlow v3 automatically detects Windows and uses sql.js.
...
```bash
# Restore v2 config
cp .gemiflow/config.json.v2.backup .gemiflow/config.json

# Downgrade package
npm install gemiflow@2.7.47

# Restore v2 settings (if needed)
git checkout HEAD~1 -- .gemiflow/settings-*.json
```
...
1. Open `.gemiflow/config.json.v2.backup`
2. Copy hook definitions
3. Paste into `.gemiflow/config.json` under `hooks` key
