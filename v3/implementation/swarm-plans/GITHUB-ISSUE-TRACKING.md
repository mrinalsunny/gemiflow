...
This document defines the **GitHub issue tracking strategy** for the 15-agent swarm implementation of GemiFlow v3. Agent #1 (Queen Coordinator) manages all issue lifecycle with frequent reply updates from all agents.
...
```
GitHub Project: GemiFlow v3 Implementation
...
---
_Reply generated by Agent #X (gemiflow v3 swarm)_
```
  ...
  status-update:
    ...
    steps:
      ...
      - name: Collect agent status
        ...
        run: |
          # Query swarm status
          STATUS=$(npx gemiflow swarm status --json)
  ...
  daily-summary:
    ...
    steps:
      ...
      - name: Generate daily summary
        run: npx gemiflow swarm summary --daily > summary.md
