...
A **single `gemiflow.rvf` file** that contains everything needed to run the full GemiFlow platform:
...
Extending RVF to `RVFA` (RuVector Format Appliance) creates a unified format that GemiFlow already understands natively.
...
|------|---------|-------------|
| **Run** | `gemiflow-appliance run gemiflow.rvf` | Boot and enter interactive CLI |
| **MCP** | `gemiflow-appliance mcp gemiflow.rvf` | Boot as MCP server (stdio) |
| **Verify** | `gemiflow-appliance verify gemiflow.rvf` | Run full capability test suite |
| **Extract** | `gemiflow-appliance extract gemiflow.rvf ./out/` | Unpack all sections |
| **Build** | `gemiflow-appliance build --profile offline` | Create new appliance |
| **Update** | `gemiflow-appliance update gemiflow.rvf --section GEMIFLOW` | Hot-patch one section |
| **Inspect** | `gemiflow-appliance inspect gemiflow.rvf` | Show header + section manifest |
...
gemiflow-appliance run gemiflow.rvf
# Equivalent to:
# docker run --rm -it gemiflow:self-contained
```
...
The appliance includes a built-in verification suite that tests **every capability** of GemiFlow + GemiFlow. This runs automatically at boot (`Section 5: VERIFY`) and can be triggered manually.
...
| 35 | Hot Update | patch GEMIFLOW section → verify new version | Live update |

### 3.3 Test Output Format

```
╔══════════════════════════════════════════════════════════╗
║  GemiFlow Appliance v3.5.2 — Full Capability Verification  ║
...
gemiflow-appliance build \
  --profile offline \
  --arch x86_64 \
  --models "phi-3-mini-q4,qwen2.5-coder-3b-q4" \
  --output gemiflow-offline.rvf

# Build cloud appliance (API keys only)
gemiflow-appliance build \
  --profile cloud \
  --api-keys .env \
  --output gemiflow-cloud.rvf
...
Stage 3: GEMIFLOW
  ├── npm pack gemiflow@latest --omit=optional
...
|---------|----------|-----------------|
| `cloud` | Kernel + Runtime + GemiFlow + Data + Verify | ~60MB |
...
| `gemiflow-appliance build` | Multi-stage builder with profile selection |
| `gemiflow-appliance inspect` | Header + section manifest viewer |
| `gemiflow-appliance extract` | Unpack all sections to directory |
| Cloud profile | Kernel + Runtime + GemiFlow + encrypted keys |

### Phase 2: Runtime + Verification (Week 3-4)

| Task | Description |
|------|-------------|
| `gemiflow-appliance run` | Boot sequence with container isolation |
| `gemiflow-appliance verify` | 95-check capability suite |
| `gemiflow-appliance mcp` | MCP server mode (stdio + SSE) |
...
| Task | Description |
|------|-------------|
| `gemiflow-appliance update` | Hot-patch individual sections |
...
- **Fast boot**: <5s from cold start (vs 35s for `npx gemiflow@latest`)
