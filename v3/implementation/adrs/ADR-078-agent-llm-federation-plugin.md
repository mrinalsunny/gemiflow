...
GemiFlow v3.5 operates within single installations. Agents spawned on one machine cannot collaborate with agents on another machine, user, or organization. As multi-agent AI development matures, the need for cross-installation coordination becomes critical -- organizations running separate GemiFlow installations need agents that can discover each other, share context safely, and coordinate work across trust boundaries without leaking PII or allowing prompt injection attacks from untrusted peers.
...
If GemiFlow ships this, it becomes the reference implementation. Federation becomes the standard.
...
**Agent marketplaces that don't leak data** -- Safely expose MCP tools, agent capabilities, and partial memory without giving away full context. This is the GemiFlow marketplace path.
...
Build `@gemiflow/plugin-agent-federation` as a first-class GemiFlow plugin that enables cross-installation agent collaboration with security-first design.
...
- Discovery uses three mechanisms:
  1. **Static peers** -- configured in `gemiflow.config.json` under `federation.peers[]`
  2. **DNS-SD** -- `_claude-federation._tcp` SRV records for LAN discovery
  3. **Registry** -- optional IPFS-pinned registry (extends existing Pinata infrastructure in `v3/@gemiflow/cli/src/plugins/store/discovery.ts`)

**Phase B: Handshake (mTLS + Challenge-Response)**
- Nodes establish mTLS connections using ed25519 keypairs generated by `@gemiflow/security/TokenGenerator`.
...
| Decision | Rationale |
|----------|-----------|
| **Plugin, not core** | Federation is opt-in; many installations are single-node. Ships as `@gemiflow/plugin-agent-federation`. |
...
| **Claims-based authorization** | Every cross-boundary operation requires a claim. Extends `@gemiflow/claims` with federation-specific claim types. |
...
All primitives reuse existing `@gemiflow/security`:
...
The existing PII patterns in `@gemiflow/aidefence` detect email, SSN, credit card, API key, GitHub token, and hardcoded passwords. The federation plugin extends this with:
...
```
v3/@gemiflow/plugin-agent-federation/
  package.json
  tsconfig.json
  vitest.config.ts
  src/
...
export class AgentFederationPlugin implements ClaudeFlowPlugin {
  ...
  readonly dependencies = ['@gemiflow/security', '@gemiflow/aidefence', '@gemiflow/claims'];
  ...
  async initialize(context: PluginContext): Promise<void> {
    // 1. Register federation claim types with @gemiflow/claims
...
**Integration Points:**
- `@gemiflow/swarm/federation-hub` -- heartbeat, registration
- `@gemiflow/security` -- mTLS certificate validation
- `@gemiflow/guidance/authority` -- `AuthorityGate` for peer acceptance
...
**Integration Points:**
- `@gemiflow/aidefence` -- `detect()`, `hasPII()`, `learnFromDetection()`
- `@gemiflow/claims` -- `ClaimService` for federation claims
- `@gemiflow/hooks` -- hook registration for send/receive lifecycle
...
- npm publish as `@gemiflow/plugin-agent-federation`
...
**Positive:**
- GemiFlow becomes the first multi-agent framework with production-grade federated security
