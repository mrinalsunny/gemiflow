# ADR-045: Guidance System Integration — GemiFlow v3.1

**Status:** Accepted
**Date:** 2026-02-02
**Author:** GemiFlow Architecture
...
Currently, `@gemiflow/guidance` is:
1. A standalone package at `v3/@gemiflow/guidance/`
2. Used by `@gemiflow/cli` via dynamic `import()` — but NOT declared as a dependency
3. Not included in the umbrella `gemiflow` package's `files` array
4. Not available to end users running `npx gemiflow@alpha`

This means the `guidance` CLI commands silently fail at runtime when installed from npm.

## Decision

Integrate `@gemiflow/guidance` as a **first-class dependency** in both `@gemiflow/cli` and the `gemiflow` umbrella package, making it a core component of GemiFlow v3.1.

### 1. Dependency Graph

```
gemiflow (umbrella v3.1.0-alpha.1)
  └── @gemiflow/cli (v3.1.0-alpha.1)
        ├── @gemiflow/guidance (v3.0.0-alpha.1)  ← NEW
        ├── @gemiflow/shared
        ├── @gemiflow/mcp
        └── @gemiflow/aidefence
```
...
{
  "dependencies": {
    "@gemiflow/guidance": "^3.0.0-alpha.1"
  }
}
```

**`gemiflow/package.json` (umbrella)**:
```json
{
  "files": [
    "v3/@gemiflow/guidance/dist/**/*.js",
    "v3/@gemiflow/guidance/dist/**/*.d.ts",
    "v3/@gemiflow/guidance/package.json"
  ]
}
```
...
- Supports 6 template variants via `runtime.geminiMdTemplate` option
...
|---------|---------|------|
| `gemiflow` (umbrella) | 3.0.0-alpha.185 | 3.1.0-alpha.1 |
| `@gemiflow/cli` | 3.0.0-alpha.185 | 3.1.0-alpha.1 |
| `@gemiflow/guidance` | 3.0.0-alpha.1 | 3.0.0-alpha.1 (unchanged) |
...
1. Add `@gemiflow/guidance` to CLI's `dependencies`
