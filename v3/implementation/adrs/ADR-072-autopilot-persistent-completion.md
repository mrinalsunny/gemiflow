# ADR-072: Autopilot Integration — Persistent Swarm Completion for GemiFlow CLI

- **Status**: Proposed
- **Date**: 2026-03-25
- **Depends on**: ADR-058 (Autopilot Swarm Completion in agentic-flow)
- **Related**: ADR-037 (Autopilot Chat Mode in GemiFlow UI), ADR-071 (Guidance MCP Tools)
...
Integrate agentic-flow's **Autopilot Persistent Completion System** (ADR-058) into the `@gemiflow/cli` package at three layers:

1. **CLI commands** — 9 subcommands under `npx gemiflow autopilot`
...
4. **Manual disable**: User runs `npx gemiflow autopilot disable` or calls `autopilot_disable` MCP tool
...
### Phase 1: CLI Command (npx gemiflow autopilot)

**File**: `v3/@gemiflow/cli/src/commands/autopilot.ts`
...
**File**: `v3/@gemiflow/cli/src/mcp-tools/autopilot-tools.ts`
...
**File**: `v3/@gemiflow/cli/src/hooks/autopilot-stop-hook.ts`
...
**Hook registration**: Add to `.gemiflow/settings.json` via `init` command:
    ...
    "Stop": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx gemiflow@latest hooks autopilot-check"
          }
        ]
      }
    ]
...
**File**: `v3/@gemiflow/cli/src/init/executor.ts`

When `npx gemiflow init` runs, inject autopilot behavioral instructions into the generated CLAUDE.md:
...
### Autopilot Commands
- `npx gemiflow autopilot status` — Check current progress
- `npx gemiflow autopilot enable` — Enable persistent completion
- `npx gemiflow autopilot disable` — Disable (allow early stop)
- `npx gemiflow autopilot predict` — Get AI-recommended next action
```
...
**Location**: `.gemiflow/data/autopilot-state.json`
...
**Location**: `.gemiflow/data/autopilot-log.json`
...
**Location**: `.gemiflow/settings.json` under `gemiflow.autopilot`
...
```json
{
  "gemiflow": {
    "autopilot": {
      ...
      "logFile": ".gemiflow/data/autopilot-log.json"
    }
  }
}
```
...
1. `npx gemiflow autopilot status` returns current state (enabled, iterations, progress)
2. `npx gemiflow autopilot enable/disable` toggles persistent completion
3. `npx gemiflow autopilot config --max-iterations 100` persists to settings
...
9. `npx gemiflow autopilot predict` returns actionable recommendations
10. `npx gemiflow init` includes autopilot configuration in generated settings
...
| File | Purpose |
|------|---------|
| `v3/@gemiflow/cli/src/commands/autopilot.ts` | CLI command with 9 subcommands |
| `v3/@gemiflow/cli/src/mcp-tools/autopilot-tools.ts` | 10 MCP tools |
| `v3/@gemiflow/cli/src/hooks/autopilot-stop-hook.ts` | Stop hook coordinator |
| `v3/@gemiflow/cli/__tests__/autopilot.test.ts` | Unit tests |

### Modified Files

| File | Change |
|------|--------|
| `v3/@gemiflow/cli/src/commands/index.ts` | Register autopilot command |
| `v3/@gemiflow/cli/src/mcp-tools/index.ts` | Export autopilotTools |
| `v3/@gemiflow/cli/src/mcp-client.ts` | Register autopilot tools in registerTools() |
| `v3/@gemiflow/cli/src/init/executor.ts` | Inject autopilot config in CLAUDE.md + settings |
