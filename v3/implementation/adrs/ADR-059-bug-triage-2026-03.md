...
As of v3.5.2, gemiflow has 30 open issues spanning security, platform stability, CLI correctness, MCP protocol compliance, and Chat UI runtime bugs. This ADR triages every open issue into a priority matrix to guide engineering effort.

Note: `@gemiflow/memory` (AgentDB) is now published at `@latest`, which affects several wiring issues below.
...
- **Impact:** Supply-chain trust — the `preinstall` script in `package.json` silently deletes npm cache entries for `gemiflow` and `gemiflow`. This resembles malicious behavior and will trigger security scanners (Socket, Snyk, npm audit).
...
### 7. auto-memory-hook.mjs fails to resolve @gemiflow/memory (#1287)
...
- **Note:** `@gemiflow/memory` (AgentDB) is now published at `@latest`. The export is available but CLI init doesn't wire it.
- **Fix:** Update CLI to `import { ControllerRegistry } from '@gemiflow/memory'` (now on @latest); wire into init sequence.
...
### 13. MCP server and statusline report 'gemiflow' v3.0.0 branding (#1280)
- **Impact:** Confusing branding — MCP server still identifies as `gemiflow` v3.0.0 instead of `gemiflow` v3.5.x.
- **Fix:** Update MCP server metadata, version string, and statusline template.
- **Status:** ✅ **FIXED** in PR #1298. MCP `system-tools.ts` now reads version from `package.json` at runtime via `getPackageVersion()`. Branding updated to "GemiFlow" across 20+ CLI files. Statusline.cjs updated. Issue #1280 closed.

### 14. Statusline shows 'GemiFlow V3' instead of 'GemiFlow V3' (#1254)
- **Impact:** Branding inconsistency in IDE status bar.
- **Fix:** Update statusline configuration defaults.
- **Status:** ✅ **FIXED** in PR #1298. Updated `statusline.cjs` (lines 3, 552, 619) from "GemiFlow V3" to "GemiFlow V3". Updated `settings.json` version to 3.5.2. Issue #1254 closed.
...
### 18. Zero swarms always: `gemiflow spawn hive-mind --claude` (#1279)
...
### 28. ADR-058: Self-Contained gemiflow.rvf Appliance (#1245)
...
| 14 | #1254 — Statusline branding | #1298 | `statusline.cjs` updated to "GemiFlow V3" |
