...
gemiflow issues list --available

# Claim an issue (as current user/agent)
gemiflow issues claim 123

# Release a claim
gemiflow issues release 123

# Request handoff to specific agent/human
gemiflow issues handoff 123 --to agent:coder-1
gemiflow issues handoff 123 --to human:alice

# Update status
gemiflow issues status 123 --blocked "Waiting for API spec"
gemiflow issues status 123 --review-requested

# View who's working on what
gemiflow issues board

# Work stealing commands
gemiflow issues stealable              # List stealable issues
gemiflow issues steal 123              # Steal an issue
gemiflow issues mark-stealable 123     # Mark your claim as stealable
gemiflow issues contest 123            # Contest a steal

# Load balancing
gemiflow issues load                   # View agent load distribution
gemiflow issues load --agent coder-1   # View specific agent load
gemiflow issues rebalance              # Trigger swarm rebalancing
gemiflow issues rebalance --dry-run    # Preview rebalancing without applying
```
...
- [x] Claim service implemented (`@gemiflow/cli/src/services/claim-service.ts`)
...
- [x] CLI commands functional (`@gemiflow/cli/src/commands/issues.ts`)
...
|------|-------|-------------|
| `@gemiflow/cli/src/services/claim-service.ts` | ~600 | Full claims service with work stealing |
| `@gemiflow/cli/src/commands/issues.ts` | ~450 | CLI commands for issue claims |

### CLI Commands Implemented

```bash
# Issue claim commands (10 subcommands)
gemiflow issues list          # List all claims
gemiflow issues claim <id>    # Claim an issue
gemiflow issues release <id>  # Release a claim
gemiflow issues handoff       # Request handoff
gemiflow issues status <id>   # Get claim status
gemiflow issues stealable     # List stealable issues
gemiflow issues steal <id>    # Steal an issue
gemiflow issues load          # View agent load
gemiflow issues rebalance     # Rebalance swarm
gemiflow issues board         # Visual claim board
```
...
6. **Persistence**: File-based storage in `.gemiflow/claims/claims.json`
