...
- `@gemiflow/memory` upgraded from `agentdb@2.0.0-alpha.3.7` to `agentdb@^3.0.0-alpha.7`
...
| `LearningBridge` | Yes (via `@gemiflow/memory`) | No | #1213 |
| `MemoryGraph` | Yes (via `@gemiflow/memory`) | No | #1214 |
...
| `TieredCacheManager` | Yes (via `@gemiflow/memory`) | No | #1220 |
...
A central registry (replacing the current `memory-initializer.js`) that wraps the `AgentDB` class and adds CLI-specific controllers from `@gemiflow/memory`:
...
interface ControllerRegistry {
  ...
  // CLI-layer controllers (from @gemiflow/memory, not in AgentDB)
...
// CLI-layer controllers (from @gemiflow/memory or new)
...
2. **Auto-start dependencies**: `gemiflow` / `gemiflow` should auto-start MCP when needed.
3. **Simplified CLI entry point**: A single `npx gemiflow start "build me a todo app"` command that handles everything.
...
The most critical integration gap is that the CLI (`memory-initializer.js`, 1929 lines) runs a **self-contained SQLite memory system** that duplicates what `@gemiflow/memory` + AgentDB already provides. Before wiring controllers, the CLI must be refactored to use `@gemiflow/memory`'s `HybridBackend` as its storage layer instead of raw `sql.js` calls.

| Current (Broken) | Target |
|-------------------|--------|
| CLI → `memory-initializer.js` → raw `sql.js` | CLI → `@gemiflow/memory` → `HybridBackend` → AgentDB v3 |
...
The bridge pattern was implemented as `memory-bridge.ts` (858 lines) in `@gemiflow/cli`, routing CLI operations through `ControllerRegistry` → `HybridBackend` → AgentDB v3. This eliminates the dual memory system described in the "Prerequisite" section.
...
- Contributor: @sparkling (gemiflow-patch repository)
...
- `@gemiflow/memory`: upgraded to `agentdb@^3.0.0-alpha.7`
