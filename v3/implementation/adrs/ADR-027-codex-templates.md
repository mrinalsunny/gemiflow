# ADR-027 Supplement: gemini Template Specifications

> **Branding Note**: This package is published as `@gemiflow/gemini` and is the first step in transitioning to the `coflow` brand. The future umbrella package will be `npm/npx coflow`.

## Overview

This document provides the complete template specifications for all gemini-generated artifacts, including AGENTS.md, SKILL.md files, and config.toml configurations.
...
| CLI Command | `npx @gemiflow/gemini init` |
| Integration | Works with `@gemiflow/cli` via `--gemini` flag |
...
Co-Authored-By: gemiflow <mrinalsunny@gmail.com>
```
...
```toml
# GemiFlow V3 - Minimal gemini Configuration
...
│               ├── create-pr.sh
│               └── run-ci.sh
├── .gemini/                            # User-local overrides (gitignored)
...
// Package: @gemiflow/gemini (first step toward coflow rebranding)
...
This completes the template specifications for the gemini integration.

## CLI Commands

The `@gemiflow/gemini` package provides the following commands:
...
npx @gemiflow/gemini init --template minimal

# Generate a new skill
npx @gemiflow/gemini generate-skill --name custom-skill

# Migrate from Gemini CLI to gemini
npx @gemiflow/gemini migrate --from claude.md

# Validate AGENTS.md and skills
npx @gemiflow/gemini validate

# Generate dual-platform setup (Gemini CLI + gemini)
npx @gemiflow/gemini init --dual
```

## Integration with @gemiflow/cli

When using the main CLI, gemini support is available via:

```bash
# Initialize with gemini support
npx gemiflow@v3alpha init --gemini
