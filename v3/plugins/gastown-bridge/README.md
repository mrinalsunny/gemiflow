...
[![npm version](https://img.shields.io/npm/v/@gemiflow/plugin-gastown-bridge.svg)](https://www.npmjs.com/package/@gemiflow/plugin-gastown-bridge)
[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://opensource.org/licenses/MIT)

## Introduction

The **Gas Town Bridge Plugin** brings Steve Yegge's powerful [Gas Town](https://github.com/steveyegge/gastown) multi-agent orchestrator to GemiFlow V3. Gas Town introduces battle-tested concepts for durable workflow execution that complement GemiFlow's swarm intelligence.
...
Seamlessly sync between Gas Town's Beads and GemiFlow's AgentDB:
...
### Gas Town vs GemiFlow V3

| Feature | Gas Town | GemiFlow V3 | With This Plugin |
|---------|----------|----------------|------------------|
...
```bash
# Install via GemiFlow CLI (recommended)
npx gemiflow@latest plugins install -n @gemiflow/plugin-gastown-bridge

# Or install directly via npm
npm install @gemiflow/plugin-gastown-bridge
...
```typescript
import { GasTownBridgePlugin } from '@gemiflow/plugin-gastown-bridge';
...
// Register with GemiFlow
await gemiflow.registerPlugin(plugin);
```
...
```typescript
// gemiflow.config.ts
import { defineConfig } from 'gemiflow';
import { GasTownBridgePlugin } from '@gemiflow/plugin-gastown-bridge';
...
```typescript
const bead = await gemiflow.mcp.call('gt_beads_create', {
  title: 'Hello Gas Town',
  description: 'My first bead from GemiFlow!',
  priority: 3,
  labels: ['tutorial'],
});
...
```typescript
const ready = await gemiflow.mcp.call('gt_beads_ready', {
  limit: 5,
});
...
await gemiflow.mcp.call('gt_formula_create', {
...
const cooked = await gemiflow.mcp.call('gt_formula_cook', {
...
const convoy = await gemiflow.mcp.call('gt_convoy_create', {
  name: 'v2.0-release',
...
```typescript
// Check convoy status
const status = await gemiflow.mcp.call('gt_convoy_status', {
  convoy_id: convoy.id,
  detailed: true,
});
...
const optimized = await gemiflow.mcp.call('gt_wasm_optimize_convoy', {
  convoy_id: convoy.id,
...
await gemiflow.mcp.call('gt_sling', {
  bead_id: 'gt-abc123',
  target: 'polecat',
  formula: 'implement-feature',
});
...
```typescript
const agents = await gemiflow.mcp.call('gt_agents', {
  rig: 'main',
  role: 'polecat',
...
await gemiflow.mcp.call('gt_beads_sync', {
  direction: 'pull',
  rig: 'production',
  conflictStrategy: 'newest-wins',
});
...
const result = await gemiflow.mcp.call('gt_beads_sync', {
  direction: 'both',
  conflictStrategy: 'merge',
});
...
for (const formula of formulas) {
  await gemiflow.mcp.call('gt_formula_cook', {
...
// Do this (352x faster):
const results = await gemiflow.mcp.call('gt_wasm_cook_batch', {
...
const result = await gemiflow.mcp.call('gt_wasm_parse_formula', {
  content: formulaToml,
});
...
**Built with ❤️ by the GemiFlow Team**
