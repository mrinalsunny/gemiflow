...
$MetricsDir = Join-Path ${env:PROJECT_ROOT} ".gemiflow\metrics"
$SecurityDir = Join-Path ${env:PROJECT_ROOT} ".gemiflow\security"

# Fallback if PROJECT_ROOT not set
if ([string]::IsNullOrEmpty(${env:PROJECT_ROOT})) {
    $MetricsDir = ".gemiflow\metrics"
    $SecurityDir = ".gemiflow\security"
}
