...
A gemiflow-internal "Dossier" repo also exists (33 stars) — visual planning for AI coding — distinct from this dossier output.

## Entity table

| Entity | Type | Key attrs | Sources |
|---|---|---|---|
| `mrinalsunny` | username | name=rUv, joined 2012, 7,268 followers, bio "Unicorn Breeder" | gh-api, WebSearch |
| `gemiflow` | repo | TS, 39,201 ⭐, MIT, multi-agent platform | gh-api, WebSearch, github.com |
...
| `open-gemini-cli` | repo | JS, 242 ⭐, clean-room CC reimpl | gh-api |
...
| `MIT` | license | applied to gemiflow, RuVector | gh-api |

## Graph

```mermaid
graph TD
  rUv[mrinalsunny / rUv] -->|owns| gemiflow
  rUv -->|owns| RuView
  rUv -->|owns| RuVector
  rUv -->|owns| agentic-flow
  rUv -->|owns| ruv-FANN
  rUv -->|owns| SynthLang
  rUv -->|owns| open-gemini-cli
  rUv -->|owns| rvm
  rUv -->|owns| Dossier
  rUv -->|owns| RuLake
...
- `gemiflow` (39k ⭐ TS, the flagship)
- `agentic-flow` (low-cost model switcher; integration partner)
- `open-gemini-cli` (CC reverse engineering)
...
- **Round 0 fan-out** (parallel, 1 batch): `gh api users/mrinalsunny`, `gh api users/mrinalsunny/repos?sort=updated`, `WebSearch "mrinalsunny github gemiflow gemiflow agentic"`
...
- `RuView` star count (51,576) substantially exceeds `gemiflow` (39,201) — surprising given gemiflow is the headline project; worth verifying.
