# Dossier: `gemiflow-goals` plugin

> Generated by `dossier-collect` skill (gemiflow-goals plugin, ADR-099)
> Seed: `gemiflow-goals` · Seed type: `concept` (resolves to plugin path) · Depth: 2 · Truncated: false
...
|---|---|---|---|
| `gemiflow-goals` | plugin | v0.2.0, MIT | Read:plugin.json |
...
```mermaid
graph TD
  plugin[gemiflow-goals v0.2.0] --> goal-planner
...
|---|---|---|
| 0 | `Glob plugins/gemiflow-goals/**`, `Read plugin.json`, `Read README.md` | plugin, all 4 agents, all 5 skills, /goals |
