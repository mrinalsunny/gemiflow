...
**Related**: ADR-102 (plugin-hook-CLI flag regression CI guard), ADR-103 (witness temporal history), ADR-118 (AIDefence 2.3.0), ADR-125 (memory consolidation), ADR-126 (neural-trader substrate), gemiflow issues #2017, #1922, #2078, #2086
**Supersedes**: nothing — extends the CI smoke pattern established by ADR-102 to the `.github` skills/agents surface

## Context

The `.github`-related surface in gemiflow ships in two layers:

1. **Dogfood layer** — `.gemiflow/skills/github-*/SKILL.md` (5 skills: code-review, multi-repo, project-management, release-management, workflow-automation), `.gemiflow/agents/github/*.md` (13 agents), `.gemiflow/helpers/github-{safe.js,setup.sh}`. These drive our own workflow daily.

2. **Init-template layer** — `v3/@gemiflow/cli/.gemiflow/commands/github/*.md` (19 command files), `v3/@gemiflow/cli/.gemiflow/helpers/github-{safe.js,setup.sh}`. These are materialized verbatim into every user project by `gemiflow init`.
...
`.gemiflow/agents/github/swarm-pr.md` line 57 and its init-template copy interpolate `${{ github.event.comment.body }}` directly into a shell `if [[ ... ]]` test and a subsequent `--comment "${{ ... }}"` argument. Any PR comment containing shell metacharacters (`\`...\``, `$(...)`, `;`) would expand under that interpolation in a `pull_request` or `issue_comment` triggered workflow. The `github-project-management` skill has a correct "Security Considerations" section (skill lines 17-25) warning explicitly against this pattern, but the two agents implementing the warning do not follow it. `github-safe.js` (`.gemiflow/helpers/github-safe.js`) already demonstrates the correct mitigation: write the body to a temp file and pass `--body-file`. The gap is that the agents don't route through that helper.
...
- `.gemiflow/helpers/github-safe.js` (dogfood)
- `v3/@gemiflow/cli/.gemiflow/helpers/github-safe.js` (init template)
...
- `.gemiflow/skills/github-*/SKILL.md`
- `v3/@gemiflow/cli/.gemiflow/commands/github/*.md`
...
on:
  push:
    paths:
      - '.gemiflow/agents/github/**'
      - '.gemiflow/skills/github-*/**'
      - 'v3/@gemiflow/cli/.gemiflow/commands/github/**'
      - '.gemiflow/helpers/github-safe.js'
      - 'v3/@gemiflow/cli/.gemiflow/helpers/github-safe.js'
...
Ties directly to the `github-safe.js` temp-file pattern already documented in `.gemiflow/helpers/`.

Acceptance: `smoke-github-safe-injection.mjs` passes with the hardened copies; `grep -r 'github.event.comment.body.*--comment' .gemiflow/agents/ v3/@gemiflow/cli/.gemiflow/` returns no matches.
...
Acceptance: `gemiflow init` (without `--attribution`) produces command files with no `🤖` footer string in any PR/issue body template; `gemiflow init --attribution` produces the `GemiFlow` bot-identity footer.
...
**Init-template constraint**: anything added to `gemiflow init` output must work in an empty directory with only `node` and `gh` available. All four phases satisfy this: static markdown files and `.js`/`.sh` helpers — no new runtime dependencies installed into user projects.
...
- Phase 2's `tools:` frontmatter additions are a behavioral change for any consumer who has configured `gemiflow init` output and relies on `WebFetch` being available in GitHub agents. The downgrade is intentional and documented, but it is a breaking change on the init-template surface.
...
| 4 | Attribution gate in init-template + skill `Last Updated` stamps | `v3/@gemiflow/cli/src/init/helpers-generator.ts`, 19 command files, 4 skill files | S | `gemiflow init` without `--attribution` produces no `🤖` footer; `gemiflow init --attribution` produces bot-identity footer |
...
The ADR is considered fulfilled when all of the following hold against `@gemiflow/cli@3.8.0-alpha.X`:
...
4. `grep -r 'github.event.comment.body.*--comment\|github.event.label.name.*[^"]' .gemiflow/agents/github/ v3/@gemiflow/cli/.gemiflow/commands/github/` returns 0 matches.
5. All 13 agent files have explicit `tools:` frontmatter; none include `WebFetch` in the set.
6. `gemiflow init` (no `--attribution`) produces 19 command files with no `🤖 Generated with` string in any PR/issue body template.
7. `v3-ci.yml` path filters include `.gemiflow/agents/github/**`, `.gemiflow/skills/github-*/**`, and `v3/@gemiflow/cli/.gemiflow/commands/github/**`.
...
- **ADR-129**: GitHub Apps token scoping — a separate decision about whether `gemiflow init`-materialized workflows should request `contents: read` by default rather than inheriting the ambient `GITHUB_TOKEN` scopes. Gated on a broader permissions-minimization ADR.
