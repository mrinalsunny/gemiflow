# ADR-123 — GemiFlow Graph Intelligence Engine: real-time relationship intelligence with complexity-aware execution
...
GemiFlow isn't shipping "a faster solver". It is committing to a new architectural stance:

> **Intelligence that understands its own computational cost.**

Traditional AI systems recompute everything. GemiFlow only computes what changed enough to matter, only at the depth the runtime can afford, and only over the relationships that are still load-bearing.

### The GemiFlow Intelligence Stack (after ADR-123)
...
> GemiFlow continuously reasons across agents, memory, infrastructure, workflows, and distributed systems while automatically adapting computation to available runtime budgets.
...
The rest of this document is the technical commitment — five SOTA axes surveyed, the eleven GemiFlow graphs catalogued, the architecture diagrammed, an eight-phase rollout, and seven open questions. Lead from the layered story above; the layers are the architecture, not just the marketing.
...
GemiFlow is the only agent platform in the field that already ships **Ed25519-signed witness chains** (ADR-103), **portable RVF cognitive containers** (`@ruvector/rvf@0.2.1`), and a **federated peer mesh with budgeted transport** (ADR-104/105/111). Layer `sublinear-time-solver@1.7.0` on top of that substrate and GemiFlow gains a capability that LangGraph, AutoGen, Letta, MemGPT, Mem0, HippoRAG, and every browser-agent vendor structurally cannot produce: **signed, replayable, federatable, complexity-budgeted, coherence-gated personalized-PageRank artifacts**.
...
The rest of the ADR enumerates the eleven graphs GemiFlow already runs (from the research gist), the integration mechanics that turn each into a sublinear primitive, and the eight-phase rollout that lands the substrate without regressing ADR-122.
...
**Beyond-SOTA insight committed:** the asymmetric-DD result is load-bearing for GemiFlow. Most of the eleven GemiFlow graphs are *not* symmetric (federation trust is one-way, span causality is one-way, file imports are one-way, cost attribution is one-way). Many integrations would have stalled on "your matrix is not SDD". The 2025 RDD/CDD result makes them all in-scope.
...
**Beyond-SOTA insight committed:** GemiFlow's substrate has *eleven* live graphs (research gist). Every one of them admits a single-entry PPR formulation. The cost is not "build a graph" — the graphs already exist; the cost is just plumbing the PPR query through. That's a 100× to 2000× speedup per query against the current baseline implementations (full PR walks), achieved by *substitution*, not by inventing new memory primitives.
...
| GemiFlow (today) | RVF + AgentDB + HNSW | GOAP + MCTS (ADR-122 Phase 4) | partial (HNSW vector) | **Ed25519 + RVF** | **ADR-104/105/111** |
| **GemiFlow (after ADR-123)** | + signed PR artifacts | + microsecond feasibility | **single-entry PPR over 11 graphs** | **signed PR vectors** | **federation-distributable PR cache** |
...
**Beyond-SOTA insight committed:** the comparative gap is not "GemiFlow has a sublinear solver while others don't" (that's a library swap any competitor can make in a sprint). The gap is "GemiFlow has eleven live graphs *already wired into the substrate* AND it has the cryptographic primitives to sign their reductions AND it has the federation transport to distribute them". The combination is the moat.

## Context — the eleven graphs GemiFlow already runs
...
|---|---|---|---|---|---|---|
| 1 | `@gemiflow/browser` (ADR-122 Phase 2) | Selector-break ↔ DOM-mutation causal graph | `(I − αP^T)π = (1−α)e` | Single-entry forward-push | O(N) walk per snapshot | O(log N) per element-ref |
| 2 | `@gemiflow/browser` (ADR-122 Phases 4 + 7) | MCTS tree | `v = r + γPv` (Bellman) | Single-entry PR (value approx) | depth-O(d) UCT-only | O(log branches) global value augment |
| 3 | `gemiflow-federation` | Peer trust mesh | `(I − αT)τ = e` (α=0.7) | Single-entry forward-push | O(N²) closure walk | O(log peers) |
| 4 | `gemiflow-knowledge-graph` | Entity-relation graph | Standard PR (α=0.85) | Single-entry PR | O(nnz) per query (≥100 ms @ 10k) | sub-ms |
| 5 | `gemiflow-rag-memory` | Graph-RAG chunk connectivity | Personalized PR seeded by query embedding | Single-entry PPR (top-K) | flat-MMR rerank | O(log chunks) per candidate |
| 6 | `gemiflow-cost-tracker` | Prompt → agent → MCP → model causation | `(I − αP^T)blame = e` | Single-entry PR | O(traces) | O(log traces) |
| 7 | `gemiflow-observability` | Span dependency graph | Standard PR on spans | Single-entry forward-push | O(spans) | O(log spans) |
| 8 | `gemiflow-neural-trader` | n/a (covariance matrix Σ — SPD) | `Σx = μ` | CG full solve | Neumann ~50 µs @ n=256 | CG **816 ns** (40–60×) |
| 9 | `gemiflow-goals` | n/a (precondition LP) | Kyng–Sachdeva packing/covering LP | ε-feasibility check | O(states) A\* enumeration | microsecond infeasibility detection |
| 10 | `gemiflow-aidefence` | Syscall call graph | `(I − αP^T)suspicion = e` (α=0.95) | Single-entry PR from flagged syscall | full trace walk | O(log calls) |
| 11 | `gemiflow-jujutsu` (`diff-analyze`) | File-import graph | `(I − αP^T)impact = e` (α=0.8) | Single-entry PR from changed file | O(LOC × imports) per push | O(log files) PR-time |
...
- Autonomous agents (`@gemiflow/browser` substrate, ADR-122)
- Browser automation (the ADR-122 stack: Stagehand / Browser Use / Playwright targets)
- AI infrastructure (`@gemiflow/*` monorepo)
- Observability platforms (`gemiflow-observability`)
- Security systems (`gemiflow-aidefence` + `gemiflow-security-audit`)
- Distributed memory (`gemiflow-agentdb` + `gemiflow-rag-memory` + `@gemiflow/memory`)
- Edge AI (`is_edge_safe()` gate on every MCP call)
- AIoT systems (`gemiflow-iot-cognitum` device-coordinator + telemetry-analyzer)
...
`gemiflow-sublinear` sits between the eleven graph-owner plugins and the solver. It does not own any graphs of its own — every graph stays in its owning plugin's storage (AgentDB, HNSW index, span store, etc.). The adapter contract is the load-bearing boundary.

### Plugin module layout

```
plugins/gemiflow-sublinear/
├── plugin.json
├── package.json
├── src/
│   ├── index.ts
...
The plugin ships as `@gemiflow/plugin-sublinear` on IPFS (Pinata) and follows the existing plugin registry mechanics in `v3/@gemiflow/cli/src/plugins/store/discovery.ts`.
...
```ts
// In every owning plugin, e.g. plugins/gemiflow-federation/src/sublinear-adapter.ts
export interface SparseMatrixExport {
  ...
  graphId: string;        // "gemiflow-federation:trust-mesh"
...
Adapters live in the owning plugins (not centralised in `gemiflow-sublinear`) — see Open Question 1. The registry in `gemiflow-sublinear` is just a thin `Map<graphId, SublinearAdapter>` populated at plugin-load time via the existing plugin manifest's `exports` field.
...
pub struct SignedPageRankArtifact {
    ...
    pub graph_id: GraphId,                    // e.g. "gemiflow-federation:trust-mesh"
...
**Initial state**: `{adr_122_merged: true, sublinear_npm_published: true, plugin_gemiflow_sublinear_exists: false, eleven_adapters_exist: false, signed_pr_artifact_schema: undefined, witness_signer_available: true, federation_transport_available: true, browser_tests_passing: 230}`.

**Goal state**: `{plugin_gemiflow_sublinear_published: true, all_eleven_adapters_shipped: true, signed_pr_artifact_schema_defined: true, federation_can_request_signed_pr: true, browser_tests_passing: 230, no_adr_122_regression: true}`.
...
**Critical path** (shortest chain to the highest-leverage outcome — "GemiFlow browser MCTS branch selection uses sublinear globally-aware value scoring"):
...
- ADR-103 witness signer must be invokable from a new plugin (it already is — `mcp__gemiflow__witness` is shipped)
...
### Phase 1 — Foundation: `gemiflow-sublinear` plugin scaffold + adapter contract + solver bridge + five MCP tools

Lay the substrate. No wedges go live yet — this is the platform.

- Scaffold `plugins/gemiflow-sublinear/` with `plugin.json`, package manifest, MCP tool entry points
- Add `sublinear-time-solver@1.6.0` as a runtime dep
- Freeze the `SublinearAdapter` and `SparseMatrixExport` types in a new `@gemiflow/shared` export so all eleven owning plugins can import them
...
- Ship to npm + IPFS registry as `@gemiflow/plugin-sublinear@0.1.0-alpha.1`

**Acceptance:**
- `gemiflow plugins list` shows `@gemiflow/plugin-sublinear`
...
Replace the O(N) break-count ratio in `@gemiflow/browser`'s Phase-2 causal-recovery code path with O(log N) single-entry PR.

- Ship `plugins/gemiflow-browser/src/sublinear-adapter.ts` exposing `exportAsSparseMatrix({purpose: "page-rank", seedNodes: <breaking-selector-ids>})` over the AgentDB causal graph for the current domain
- Wire `mcp__gemiflow__browser_explain_recovery` to call `sublinear/page-rank-entry` instead of walking the full break-event history
...
- Feature flag `GEMIFLOW_BROWSER_PR_CAUSAL` defaults to false in 0.1; flips to true in 0.2 after benchmark confirms
...
- `gemiflow-federation`: trust-mesh adapter with α=0.7; replaces transitive-trust closure walks
- `gemiflow-cost-tracker`: prompt→agent→MCP→model causation adapter; α=0.85; powers real-time "blame this prompt" UI
- `gemiflow-observability`: span dependency adapter; α=0.85; powers "most-influential span" debugging queries
...
- `gemiflow-knowledge-graph`: entity-relation graph adapter; α=0.85; replaces full-graph PR on every entity-importance query
- `gemiflow-rag-memory`: graph-RAG chunk connectivity adapter; α=0.85; PPR seeded by query embedding; replaces flat MMR rerank in `memory_search_unified`'s graph-RAG mode
...
- `gemiflow-neural-trader`: portfolio mean-variance adapter; `Σx = μ` via CG (no graph involved — direct SPD solve)
...
- `gemiflow-goals`: precondition-LP adapter; `sublinear/feasibility` short-circuits A\* on infeasible goals
- `gemiflow-aidefence`: syscall-call-graph adapter; α=0.95 (high decay so suspicion can travel far); single-entry PR from flagged syscall back to root agent
- `gemiflow-jujutsu`: file-import-graph adapter; α=0.8; single-entry PR from changed file → "blast radius" score in PR review
- `@gemiflow/embeddings`: replace hand-rolled JL with `sublinear/jl-embed`; this is a correctness fix not a performance fix, but it closes ADR-121 Phase 4
...
- Implement `witness-signer.ts` in `gemiflow-sublinear`; reuse the existing ADR-103 witness machinery via `mcp__gemiflow__witness`
- Add the `SignedPageRankArtifact` schema as a versioned export in `@gemiflow/shared`
...
|---|---|---|---|
| 1 | foundation | `gemiflow-sublinear@0.1.0-alpha.1` + 6 MCP tools (incl. `solve-on-change`) + `maxComplexityClass`/`coherenceThreshold` budget surface | alpha.1 |
...
Wedge 2 (browser MCTS global value augment) lands as a separate `@gemiflow/browser@3.0.0-alpha.{N+1}` release tied to ADR-122's Phase 4/7 — it's an upgrade to the substrate consumer, not a new phase of `gemiflow-sublinear` itself.
...
1. **Adapter ownership: plugin-local vs centralised in `gemiflow-sublinear`.** This ADR commits to **plugin-local adapters** (e.g. `plugins/gemiflow-federation/src/sublinear-adapter.ts`) because the owning plugin already understands its own storage layout. The cost is that `gemiflow-sublinear` cannot ship adapter logic on its own — it depends on adapter installation by the owning plugin. The alternative (centralised adapters in `gemiflow-sublinear`) reduces moving parts but tightly couples `gemiflow-sublinear` to every owning plugin's internal schema. Decided: plugin-local. Reversible if adapter drift becomes a maintenance burden.
...
5. **Failure modes — what happens when the source matrix is NOT diagonally-dominant?** ~~Open as of original draft.~~ **Closed upstream by `sublinear-time-solver@1.7.0`'s coherence gate** (2026-05-19 revision): `coherence_score(matrix)` returns the per-row DD margin in [−∞, 1]; `check_coherence_or_reject(matrix, threshold)` produces `SolverError::Incoherent { coherence, threshold }` with `is_recoverable() = true` and severity `Low`. Plugins pass `coherenceThreshold` on the MCP call; the structured rejection lets adapters fall back gracefully (clamp negative weights, renormalise rows, or switch to a dense solver). GemiFlow no longer hand-rolls a DD check. Original guidance still applies for non-DD branches (CG needs SPD; LP has its own preconditions; the owning plugin still chooses preprocessing semantics because only it knows the weight meaning) — the only change is that the failure now arrives as a clean structured error instead of silent divergence.

6. **Node-Rust binding strategy — WASM, native node addon, or both?** `sublinear-time-solver@1.7.0` ships both. The plugin defaults to **native node addon when available, WASM as a fallback**. The detection is at plugin-load time, mirroring how `@gemiflow/embeddings` handles ONNX runtime detection. The WASM path serves Edge runtimes (Cloudflare Workers, Deno Deploy) where native modules are unavailable; the native path is the hot path in the v3 monorepo. Latency budgets above assume native; WASM has ~3× overhead. **Edge detection now uses the upstream 1.7.0 `is_edge_safe()` helper** rather than an ad-hoc check.
