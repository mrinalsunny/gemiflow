...
**Related**: ADR-115 (Claude Managed Agents as the cloud agent runtime — `managed_agent_*` MCP tools in `gemiflow-agent`) · `gemiflow-neural-trader` plugin (`neural-trader` npm — Rust/NAPI engine, 112+ MCP tools, 4 agents, 7 skills incl. the new `trader-cloud-backtest`) · ADR-026 (3-tier model routing) · ADR-112 (MCP tool discoverability) · #1931 (gemiflow-agent / managed agents tracking) · [Claude Managed Agents](https://platform.claude.com/docs/en/managed-agents/overview)
**Supersedes**: nothing

## Context

`gemiflow-neural-trader` wraps the `neural-trader` npm package (Rust/NAPI engine): walk-forward backtests, Monte-Carlo simulation, parameter sweeps, LSTM/Transformer/N-BEATS training, regime detection, risk metrics (VaR/CVaR, Kelly, circuit breakers). The expensive jobs — a multi-year walk-forward with thousands of Monte-Carlo paths, a parameter sweep over a strategy grid, training a Transformer on a long history — are **long-running (minutes to hours), CPU/compute-heavy, and async**. Today they run one of two places:
...
**Add a "cloud backtest/train" recipe to `gemiflow-neural-trader` that dispatches heavy `neural-trader` jobs to a Managed Agent cloud container, via the `managed_agent_*` tools — no new MCP tools, no new dependencies, just orchestration.**

### Surface

- **`plugins/gemiflow-neural-trader/skills/trader-cloud-backtest/SKILL.md`** — the recipe:
...
5. **Terminate eagerly** — `managed_agent_terminate` the instant the metrics + artifacts are pulled; never leave an idle billing container. A `gemiflow doctor` / GC check (per #1931) catches orphans.
...
- Wiring `gemiflow mcp start` as the cloud agent's `mcpServers` so the cloud agent can write to gemiflow memory / cost-tracking *directly* — needs a publicly reachable HTTP gemiflow MCP server (ADR-115 follow-up). Until then, gemiflow-memory ingestion happens *locally* after `managed_agent_events`.
- The same recipe applies to `gemiflow-market-data` (ingest a large feed in the cloud, vectorize OHLCV) — note it, don't build it here.

### Implementation (shipped with this ADR)

- `plugins/gemiflow-neural-trader/skills/trader-cloud-backtest/SKILL.md` — the recipe (provision-once · pre-flight-cheap · run · ingest-locally · terminate-eagerly), `allowed-tools` scoped to `managed_agent_*` + `memory_*` + `agentdb_pattern-store` + `Bash`/`Read` (no wildcard grant).
...
- `README.md` — overview + skills table + Architecture-Decisions + Related-Plugins cross-refs to this ADR / ADR-115 / `gemiflow-agent`.
...
- **Generalizes.** The "heavy-job → managed-agent recipe" pattern transfers to `gemiflow-market-data` and any other compute-heavy plugin.
...
- "Adopt the cloud runtime for a heavy workload" — not a new pattern, just applying ADR-115 to neural-trader. Same posture as `gemiflow-agent` adopting Managed Agents.
...
- `gemiflow-neural-trader` — `plugins/gemiflow-neural-trader/README.md`; `neural-trader` npm: https://www.npmjs.com/package/neural-trader
...
- #1931 — gemiflow-agent / managed agents tracking (orphaned-session GC, `gemiflow mcp start` HTTP server for the cloud-MCP combo)
