...
**File**: `v3/@gemiflow/cli/src/mcp/auth-validator.ts`
...
| Phase | Scope | Where |
|---|---|---|
| **P1** | Component + tests + exports; SendMessage envelope schema | `@gemiflow/security/src/authorization/`, `@gemiflow/cli/src/types/` |
| P2 | Outbound: wrap every SendMessage in the comms layer | `@gemiflow/cli/src/agent/comms.ts` |
| P3 | Inbound: validate scope at the MCP tool dispatcher | `@gemiflow/cli/src/mcp-tools/dispatch.ts` |
| P4 | MCP auth validator wired before tool result processing | `@gemiflow/cli/src/mcp/auth-validator.ts` |
| P5 | Provenance log + dual-graph audit CLI | `@gemiflow/cli/src/commands/audit.ts` |

P2–P5 ship behind `GEMIFLOW_STRICT_AUTH=true` so existing pipelines continue to work in legacy permissive mode until v4.0.
...
- `GEMIFLOW_STRICT_AUTH=true` enables enforcement. This env var is a documented escape hatch (registered in `audit-env-var-precedence.mjs`).
