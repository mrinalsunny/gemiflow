...
ADR-131 closed the content boundary and ADR-144 closes the runtime action boundary. Both assume the *code that's about to run* is trustworthy in the first place. Two Grade A papers published April–May 2026 demonstrate that assumption is broken under current GemiFlow defenses.
...
GemiFlow's current plugin install path (`v3/@gemiflow/cli/src/plugins/store/discovery.ts`) fetches manifests from IPFS via Pinata. It carries **no signature verification** and **no semantic intent analysis**.
...
**Mnemonic Sovereignty survey** (arXiv:2604.16548, Apr 2026, Grade A): Catalogs nine governance primitives required for secure long-term agent memory. No existing published architecture satisfies all nine. The GemiFlow gap: the shared `collaboration` namespace (and all AgentDB namespaces) accepts writes from any agent with no per-namespace authorization. ADR-131 catches *what gets read out*; this ADR catches *who is allowed to write in*.
...
Add `PluginIntegrityVerifier` to `@gemiflow/security` with two verification stages run at `plugins install` time.
...
- `discovery.ts` MUST refuse to install unsigned plugins when `GEMIFLOW_STRICT_PLUGINS=true` (default: warn-only for backwards compatibility).
- Trust anchors (publisher keys we vouch for) live in `v3/@gemiflow/cli/src/plugins/trust/trust-anchors.json`. Edits are gated on CODEOWNERS review.
...
- Install blocks if confidence exceeds `GEMIFLOW_PLUGIN_SCH_THRESHOLD` (default `0.8`).
...
- `v3/@gemiflow/security/src/plugins/integrity-verifier.ts` (new)
- `v3/@gemiflow/cli/src/plugins/store/discovery.ts` — verification hook on install
- `v3/@gemiflow/cli/src/plugins/trust/trust-anchors.json` (new)
...
**Implementation targets**:
- `v3/@gemiflow/memory/src/namespaces/authorization.ts` (new)
- `v3/@gemiflow/memory/src/agent-db.ts` — grant enforcement
- `v3/@gemiflow/cli/src/agent/spawn.ts` — `writeNamespaces` parameter

### Integration plan (phased — P1 is the first PR)

| Phase | Scope | Where |
|---|---|---|
| **P1** | `PluginIntegrityVerifier` skeleton + Stage-1 signature path; trust-anchors file with the existing official-plugin keys | `@gemiflow/security/src/plugins/`, `@gemiflow/cli/src/plugins/trust/` |
| P2 | Stage-2 semantic scan (pattern fallback first, classifier opt-in) | same files |
| P3 | Memory namespace ACL primitives 1–3 in AgentDB | `@gemiflow/memory/src/namespaces/` |
| P4 | `agent spawn --write-namespaces` plumbing through every spawn callsite | `@gemiflow/cli/src/agent/spawn.ts`, hooks |
...
- Plugin verification defaults to **warn-only** (`GEMIFLOW_STRICT_PLUGINS=false`). Existing unsigned plugins continue to install with a warning.
- Memory namespace ACLs are **additive**: agents spawned without `writeNamespaces` retain legacy full-access until `GEMIFLOW_STRICT_MEMORY=true` is set.
...
- Positions GemiFlow's memory governance ahead of every 2026 competitor surveyed — none satisfy more than 4 of the 9 Mnemonic Sovereignty primitives.

**Negative / risks**:
- Plugin publishers must generate Ed25519 keypairs and sign manifests (new workflow). The gemiflow-plugin-creator skill MUST be updated to scaffold the signing step.
...
- Smoke test: `plugins install ./unsigned-plugin` warns by default, errors under `GEMIFLOW_STRICT_PLUGINS=true`.
