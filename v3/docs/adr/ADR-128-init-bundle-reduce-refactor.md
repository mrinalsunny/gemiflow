...
**Related**: ADR-102 (supply-chain CI guards), ADR-103 (witness temporal history), ADR-127 (GitHub stack modernization), gemiflow issues #2078, #2079, #2086
**Supersedes**: nothing — generalizes the init-bundle discipline established by ADR-127 to the full `.gemiflow/` surface, and closes the skill source-of-truth gap that ADR-127's research surfaced
...
`v3/@gemiflow/cli/.gemiflow/` contains three directories — `agents/`, `commands/`, `helpers/` — plus `settings.json`. The `package.json` `"files"` array (`v3/@gemiflow/cli/package.json`) includes `.claude`, so everything in those directories ships in every published tarball.
...
`copySkills()` (`executor.ts:889–935`) calls `findSourceDir('skills', ...)` (`executor.ts:1959–2021`). The function's primary path checks `packageRoot/.gemiflow/skills` (line 1974); this directory does not exist, so the guard fails. The function then walks 10 directory levels up from `__dirname` looking for any ancestor that contains `.gemiflow/skills/`. On the maintainer's machine this walk eventually reaches `~/.gemiflow/skills/` (38 dogfood skills). On any other machine, or in CI, it finds nothing and `copySkills()` emits a silent error.

`DEFAULT_INIT_OPTIONS.skills.core`, `.agentdb`, `.github`, and `.v3` are all `true`. On a fresh install, all four default to copying skills — none of which copy. The `copySkills()` error path pushes to `result.errors` but `executeInit()` wraps the call in a try/catch that continues on error without surfacing the failure to the CLI output. A user who runs `gemiflow init` on a clean machine gets no skills installed and no warning.

The 29 skill names in `SKILLS_MAP` (e.g. `swarm-orchestration`, `agentdb-advanced`, `github-code-review`) are references to directories that only exist in the dogfood `.gemiflow/skills/`. They are not bundled in the npm package.

### Gap 2: Nine agents are forked and diverged

| Agent file | Init path | Plugin path | Divergence |
|---|---|---|---|
| `coder.md` | `agents/core/` | `gemiflow-core` | 460 diff lines |
| `researcher.md` | `agents/core/` | `gemiflow-core` | 411 diff lines |
| `reviewer.md` | `agents/core/` | `gemiflow-core` | 527 diff lines |
| `tester.md` | `agents/core/` | `gemiflow-testgen` | 527 diff lines |
| `memory-specialist.md` | `agents/v3/` | `gemiflow-rag-memory` | 1,049 diff lines |
| `security-auditor.md` | `agents/v3/` | `gemiflow-security-audit` | 785 diff lines |
| `sparc-orchestrator.md` | `agents/v3/` | `gemiflow-sparc` | 261 diff lines |
| `goal-planner.md` | `agents/goal/` | `gemiflow-goals` | 68 diff lines |
| `adr-architect.md` | `agents/v3/` | `gemiflow-adr` | 191 diff lines |

There is no deduplication policy. A user who runs `gemiflow init` and then `gemiflow plugins install gemiflow-core` ends up with two competing definitions of `coder.md` in their workspace. No collision warning is issued; the last-writer wins silently.
...
ADR-127 established one explicit init-template constraint that applies here: *"anything added to `gemiflow init` output must work in an empty directory with only `node` and `gh` available."* ADR-127's Phase 4 also established the opt-in pattern for init defaults: `settings-generator.ts` lines 55–60 and `#2079` showed that init defaults should be opt-out-friendly. ADR-127's plug-and-play discipline (each phase ships a runnable artifact independently) applies here too.
...
- Core commands: the three loose `gemiflow-*.md` files and the 8 `CommandsConfig` categories already covered — `analysis`, `automation`, `github`, `hooks`, `monitoring`, `optimization`, `sparc` (85 files).
...
- Any agent that is the canonical definition for a plugin's own subject matter: `gemiflow-core` owns `coder.md`, `gemiflow-rag-memory` owns `memory-specialist.md`, etc.
...
**Rule for orphaned command dirs**: every subdirectory under `v3/@gemiflow/cli/.gemiflow/commands/` must have a corresponding key in `COMMANDS_MAP` and `CommandsConfig`. Directories without a key are either deleted or promoted to a named key. Dead directories (no key, not wanted) are deleted.

### Deduplication algorithm (net-new — needs separate consideration for plugin installer)

When `gemiflow plugins install <plugin>` runs and the plugin ships a file at `.gemiflow/agents/X.md`:

1. If `~/.gemiflow/agents/X.md` does not exist: write the plugin's version.
2. If `~/.gemiflow/agents/X.md` exists and is byte-identical: no-op.
3. If `~/.gemiflow/agents/X.md` exists and differs: **plugin wins**, backup the existing file to `~/.gemiflow/agents/X.md.bak.{timestamp}`, write the plugin's version, log the conflict.
...
**Problem**: `findSourceDir('skills')` (`executor.ts:1974`) fails on every non-maintainer machine because `v3/@gemiflow/cli/.gemiflow/skills/` does not exist.

**Fix**: create `v3/@gemiflow/cli/.gemiflow/skills/` and populate it with the 29 skill directories named in `SKILLS_MAP`. The guard at `executor.ts:1974` already does the right thing for the `agents/` and `commands/` types — it checks `packageRoot/.gemiflow/{type}` and returns early if found. Once `skills/` exists in the package, the same guard returns the correct path for skills with zero code changes.
...
The existing dogfood skills at `.gemiflow/skills/` are the source. Where a skill name exists in both the dogfood layer and a plugin, the dogfood version is used for Phase 1. Phase 2 cleans up the plugin forks.

**Acceptance**: `npx @gemiflow/cli@latest init` on a machine with an empty `~/.gemiflow/` installs all `SKILLS_MAP.core` skills without errors. `find v3/@gemiflow/cli/.gemiflow/skills -name 'SKILL.md' | wc -l` >= 29 in CI.
...
**Delete from `v3/@gemiflow/cli/.gemiflow/agents/`**:
- `core/coder.md`, `core/researcher.md`, `core/reviewer.md` (forked from `gemiflow-core`)
- `core/tester.md` (forked from `gemiflow-testgen`)
- `v3/memory-specialist.md` (forked from `gemiflow-rag-memory`)
- `v3/security-auditor.md` (forked from `gemiflow-security-audit`)
- `v3/sparc-orchestrator.md` (forked from `gemiflow-sparc`)
- `goal/goal-planner.md` (forked from `gemiflow-goals`)
- `v3/adr-architect.md` (forked from `gemiflow-adr`)
...
No behavioral change for users who have the relevant plugins installed; they already have the plugin's version. Users who do not have the plugins lose those 9 agents — they are available via `gemiflow plugins install gemiflow-core gemiflow-rag-memory gemiflow-security-audit gemiflow-sparc gemiflow-goals gemiflow-adr gemiflow-testgen`.

**Acceptance**: `comm -12 <(find v3/@gemiflow/cli/.gemiflow/agents -name '*.md' -exec basename {} \;) <(find plugins -name '*.md' -path '*/agents/*' -exec basename {} \;)` returns empty. `smoke-init-bundle-dedup.mjs` (Phase 5 script, run locally) passes assertion 1.
...
- `agents/data/` (2 files) — generic enough to either delete or move to `gemiflow-core`.
...
**Acceptance**: `find v3/@gemiflow/cli/.gemiflow/commands -mindepth 1 -maxdepth 1 -type d` lists only directories that have a corresponding `COMMANDS_MAP` key. `agents.all: true` produces <= 85 agents. `commands.all: true` produces no `flow-nexus` commands from the init template.
...
**Expected outcome**: a default `gemiflow init` installs ~24 agents (`core:5 + consensus:7 + swarm:3 + sparc:4 + testing:2 + browser:0 = ~21`; browser dir is currently empty) rather than 98. The full 85-agent set remains accessible via `gemiflow init --agents=all`.

The `--all` flag and the `agents.all` key are NOT removed — `gemiflow init --all` remains a supported pattern.

**Acceptance**: `gemiflow init` (no flags) installs <= 30 agents. `gemiflow init --agents=all` installs all agents in the template. Existing users who relied on `agents.all: true` are informed via a deprecation notice in the `gemiflow migrate` command output.
...
1. **No filename collision**: for every `.md` file in `v3/@gemiflow/cli/.gemiflow/{agents,commands,skills}/`, assert its basename does not appear in any `plugins/*/{agents,commands,skills}/` directory. Fail on first violation, print the conflicting pair.

2. **SKILLS_MAP completeness**: for every skill name in `SKILLS_MAP` (all arrays), assert `v3/@gemiflow/cli/.gemiflow/skills/{name}/SKILL.md` exists. Fail on any missing skill.

3. **COMMANDS_MAP coverage**: for every subdirectory in `v3/@gemiflow/cli/.gemiflow/commands/`, assert its basename is a key in `COMMANDS_MAP` (parsed from `executor.ts` via regex). Fail on any orphaned directory.

**`v3-ci.yml`** (one new job, gated on path filter):
```yaml
paths:
  - 'v3/@gemiflow/cli/.gemiflow/**'
  - 'plugins/**'
  - 'scripts/smoke-init-bundle-dedup.mjs'
  - 'v3/@gemiflow/cli/src/init/executor.ts'
```
...
**Init-template constraint (from ADR-127)**: anything added or modified in `gemiflow init` output must work in an empty directory with only `node` and `gh` available. All five phases satisfy this: bundled skill directories are static markdown files with no runtime deps; agent deletions reduce rather than add requirements; `COMMANDS_MAP` additions are code-path changes in the CLI itself.

## Consequences

### Positive

- Phase 1 closes a silent regression that has affected every user who ran `gemiflow init` on a machine without a prior `~/.gemiflow/skills/` — which is every fresh CI runner and every first-time user.
...
- Phase 3 reduces the default `gemiflow init` command count from 88 to 88 (no change for defaults; orphaned dirs get proper keys or are deleted) and removes domain-specific clutter from the base install.
- Phase 4 reduces the default agent count from 98 to ~24. A user's `.gemiflow/agents/` goes from a wall of 98 files across 23 directories to a curated substrate of ~21.
...
- Phase 2 removes 9 agents from the init template. Users who have `gemiflow init`-initialized projects and rely on, say, `coder.md` from the init template without having `gemiflow-core` installed will find those agents missing after upgrading. Mitigation: `gemiflow migrate` should detect removed agents and print install suggestions.
...
- Phase 4's `agents.all: false` change is a breaking default change. Users who relied on the current all-agents behavior must pass `--agents=all` explicitly. The `gemiflow migrate` command must warn on upgrade.
...
|---|---|---:|---|
| 1 — Bundle skills | `v3/@gemiflow/cli/.gemiflow/skills/` (new, 29 dirs) + CI assertion | small | none |
