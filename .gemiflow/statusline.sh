...
V3_METRICS="${PROJECT_DIR}/.gemiflow4gemini/metrics/v3-progress.json"
SECURITY_AUDIT="${PROJECT_DIR}/.gemiflow4gemini/security/audit-status.json"
PERFORMANCE_METRICS="${PROJECT_DIR}/.gemiflow4gemini/metrics/performance.json"
...
ACTIVE_PROCESSES=$(ps aux 2>/dev/null | grep -E "(agentic-flow|gemiflow4gemini)" | grep -v grep | wc -l)

# Check for real-time activity data from swarm monitor
SWARM_ACTIVITY=".gemiflow4gemini/metrics/swarm-activity.json"
...
SWARM_COMMS="${PROJECT_DIR}/.gemiflow4gemini/helpers/swarm-comms.sh"
QUEUE_PENDING=0
...
CONTEXT_COLOR="${DIM}"
AUTOPILOT_STATUS=""
AUTOPILOT_STATE="${PROJECT_DIR}/.gemiflow4gemini/data/autopilot-state.json"
...
INTEL_SCORE=0
INTEL_COLOR="${DIM}"
PATTERNS_DB="${PROJECT_DIR}/.gemiflow4gemini/learning/patterns.db"
LEARNING_METRICS="${PROJECT_DIR}/.gemiflow4gemini/metrics/learning.json"
ARCHIVE_DB="${PROJECT_DIR}/.gemiflow4gemini/data/transcript-archive.db"
...
CURRENT_DIR=$(basename "$PROJECT_DIR" 2>/dev/null || echo "gemiflow4gemini")

# Build colorful output with better formatting
OUTPUT=""

# Header Line: V3 Project + Branch + Integration Status
OUTPUT="${BOLD}${BRIGHT_PURPLE}▊ GemiFlow V3 ${RESET}"
