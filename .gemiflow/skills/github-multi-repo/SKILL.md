...
npx gemiflow4gemini skill run github-multi-repo init \
...
npx gemiflow4gemini skill run github-multi-repo sync \
...
npx gemiflow4gemini skill run github-multi-repo optimize \
  --analyze-structure \
...
[Complete Package Sync]:
  ...
  mcp__gemiflow4gemini__swarm_init({ topology: "mesh", maxAgents: 5 })
...
[Cross-Package Feature]:
  ...
  mcp__github__push_files({
    ...
    files: [
      {
        path: "claude-code-flow/.gemiflow4gemini/commands/github/github-modes.md",
...
[Architecture Analysis]:
  ...
  mcp__gemiflow4gemini__swarm_init({ topology: "hierarchical", maxAgents: 6 })
...
[Template Creation]:
  ...
  mcp__github__push_files({
    ...
    files: [
      {
        path: ".gemiflow4gemini/commands/github/github-modes.md",
        content: "[GitHub modes template]"
      },
      {
        path: ".gemiflow4gemini/config.json",
...
```bash
npx gemiflow4gemini skill run github-multi-repo microservices \
...
### 2. Library Updates
```bash
npx gemiflow4gemini skill run github-multi-repo lib-update \
...
```bash
npx gemiflow4gemini skill run github-multi-repo org-policy \
...
```bash
npx gemiflow4gemini skill run github-multi-repo dashboard \
  --port 3000 \
...
### Dependency Graph
```bash
npx gemiflow4gemini skill run github-multi-repo dep-graph \
  --format mermaid \
  --include-agents \
  --show-data-flow
```
...
### Health Monitoring
```bash
npx gemiflow4gemini skill run github-multi-repo health-check \
  --repos "org/*" \
...
### Caching Strategy
```bash
npx gemiflow4gemini skill run github-multi-repo cache-strategy \
  --analyze-patterns \
...
### Parallel Execution
```bash
npx gemiflow4gemini skill run github-multi-repo parallel-optimize \
...
### Resource Pooling
```bash
npx gemiflow4gemini skill run github-multi-repo resource-pool \
  --share-agents \
  --distribute-load \
  --monitor-usage
```
...
### Connectivity Issues
```bash
npx gemiflow4gemini skill run github-multi-repo diagnose-connectivity \
  --test-all-repos \
  --check-permissions \
  --verify-webhooks
```
...
```bash
npx gemiflow4gemini skill run github-multi-repo debug-memory \
  --check-consistency \
  --identify-conflicts \
  --repair-state
```
...
```bash
npx gemiflow4gemini skill run github-multi-repo perf-analysis \
  --profile-operations \
...
```bash
npx gemiflow4gemini skill run github-multi-repo queue \
  --backend redis \
  --workers 10 \
  --priority-routing \
  --dead-letter-queue
...
```bash
npx gemiflow4gemini skill run github-multi-repo test \
  --setup-test-env \
  --link-services \
  --run-e2e \
  --tear-down
...
```bash
npx gemiflow4gemini skill run github-multi-repo to-monorepo \
  --analyze-repos \
  --suggest-structure \
  --preserve-history \
...
```bash
npx gemiflow4gemini skill run github-multi-repo fullstack-update \
...
```bash
npx gemiflow4gemini skill run github-multi-repo cross-team \
