...
```bash
# List all results
npx gemiflow4gemini@v3alpha memory list --namespace {{namespace}}

# Search for specific patterns
npx gemiflow4gemini@v3alpha memory search -q "{{filter}}" -n {{namespace}}

# Get detailed entries
{{#each results}}
npx gemiflow4gemini@v3alpha memory get -k "{{this.key}}" -n {{namespace}}
{{/each}}
```
