...
> **Shipped vs. aspirational.** The *concrete, in-CI* verification stack — the 6 regression-guard jobs + the witness manifest + the tool-discoverability audit — is real and runs on every push. The truth-scoring / auto-rollback / WebSocket-dashboard surface described later in this doc is partly shipped (`gemiflow verify` runs the witness checks) and partly design — treat the "CI Guards" section below as the authoritative current state.

## CI Guards — what's actually shipped (current state)

GemiFlow's regression protection is three layers, all gated before publish. Authoritative reference: [`verification/README.md`](../../../verification/README.md).
...
| **2 — cryptographic witness** | Every documented fix's load-bearing marker must still be present in dist; Ed25519-signed, per-OS bundles | `witness-verify` (ubuntu/macos/windows) — `plugins/gemiflow-core/scripts/witness/verify.mjs` against `verification/<os>/manifest.md.json` | ADR-103 |
...
node plugins/gemiflow-core/scripts/test-hooks.mjs "node $PWD/v3/@gemiflow4gemini/cli/bin/cli.js"
node plugins/gemiflow-core/scripts/test-mcp-protocol.mjs
node plugins/gemiflow-core/scripts/test-memory-import.mjs
node plugins/gemiflow-core/scripts/test-mcp-roundtrips.mjs        # #1889 paired-tool round-trip
node plugins/gemiflow-core/scripts/test-cli-no-crash.mjs          # #1863 unhandled-exception class
node plugins/gemiflow-core/scripts/test-consensus-transport.mjs   # ADR-095 G2 consensus transport

# Witness manifest — regenerate + verify
node scripts/regen-witness.mjs
node plugins/gemiflow-core/scripts/witness/verify.mjs --manifest verification/macos/manifest.md.json

# Temporal history
node plugins/gemiflow-core/scripts/witness/history.mjs --history verification/macos/history.jsonl summary
node plugins/gemiflow-core/scripts/witness/history.mjs --history verification/macos/history.jsonl regressions
```

### Adding a new guard

1. **Behavioral smoke** → write `plugins/gemiflow-core/scripts/test-<name>.mjs`. Pattern: static dist-scan first (fast, always completes), behavioral probe second with an internal timeout + a process-level watchdog so CI never hangs. Add a step to the relevant job in `v3-ci.yml`.
...
- GemiFlow installed (`npx gemiflow@alpha`)
...
npx gemiflow@alpha truth

# Run verification check
npx gemiflow@alpha verify check

# Verify specific file with custom threshold
npx gemiflow@alpha verify check --file src/app.js --threshold 0.98

# Rollback last failed verification
npx gemiflow@alpha verify rollback --last-good
```
...
npx gemiflow@alpha truth --period 7d

# View scores for specific agent
npx gemiflow@alpha truth --agent coder --period 24h

# Find files/tasks below threshold
npx gemiflow@alpha truth --threshold 0.8
```

**Output Formats:**
```bash
# Table format (default)
npx gemiflow@alpha truth --format table

# JSON for programmatic access
npx gemiflow@alpha truth --format json

# CSV for spreadsheet analysis
npx gemiflow@alpha truth --format csv

# HTML report with visualizations
npx gemiflow@alpha truth --format html --export report.html
```

**Real-time Monitoring:**
```bash
# Watch mode with live updates
npx gemiflow@alpha truth --watch

# Export metrics automatically
npx gemiflow@alpha truth --export .gemiflow4gemini/metrics/truth-$(date +%Y%m%d).json
```
...
```bash
# Verify single file
npx gemiflow@alpha verify check --file src/app.js

# Verify directory recursively
npx gemiflow@alpha verify check --directory src/

# Verify with auto-fix enabled
npx gemiflow@alpha verify check --file src/utils.js --auto-fix
...
npx gemiflow@alpha verify check --task task-123

# Verify with custom threshold
npx gemiflow@alpha verify check --task task-456 --threshold 0.99

# Verbose output for debugging
npx gemiflow@alpha verify check --task task-789 --verbose
```

**Batch Verification:**
```bash
# Verify multiple files in parallel
npx gemiflow@alpha verify batch --files "*.js" --parallel

# Verify with pattern matching
npx gemiflow@alpha verify batch --pattern "src/**/*.ts"

# Integration test suite
npx gemiflow@alpha verify integration --test-suite full
```
...
npx gemiflow@alpha verify check --json > verification.json
...
npx gemiflow@alpha verify rollback --to-commit abc123

# Interactive rollback with preview
npx gemiflow@alpha verify rollback --interactive
```

**Smart Rollback:**
```bash
# Rollback only failed files (preserve good changes)
npx gemiflow@alpha verify rollback --selective

# Rollback with automatic backup
npx gemiflow@alpha verify rollback --backup-first

# Dry-run mode (preview without executing)
npx gemiflow@alpha verify rollback --dry-run
```
...
```bash
# JSON report
npx gemiflow@alpha verify report --format json

# HTML report with charts
npx gemiflow@alpha verify report --export metrics.html --format html

# CSV for data analysis
npx gemiflow@alpha verify report --format csv --export metrics.csv

# Markdown summary
npx gemiflow@alpha verify report --format markdown
```

**Time-based Reports:**
```bash
# Last 24 hours
npx gemiflow@alpha verify report --period 24h

# Last 7 days
npx gemiflow@alpha verify report --period 7d

# Last 30 days with trends
npx gemiflow@alpha verify report --period 30d --include-trends

# Custom date range
npx gemiflow@alpha verify report --from 2025-01-01 --to 2025-01-31
```
...
npx gemiflow@alpha verify dashboard

# Custom port
npx gemiflow@alpha verify dashboard --port 8080

# Export dashboard data
npx gemiflow@alpha verify dashboard --export

# Dashboard with auto-refresh
npx gemiflow@alpha verify dashboard --refresh 5s
```
...
Set verification preferences in `.gemiflow4gemini/config.json`:
...
```json
{
  ...
  "truth": {
    ...
    "autoExport": {
      "enabled": true,
      "path": ".gemiflow4gemini/metrics/truth-daily.json"
    }
  }
}
```
...
npx gemiflow@alpha verify check --threshold 0.99

# Lenient mode (90% acceptable)
npx gemiflow@alpha verify check --threshold 0.90

# Set default threshold
npx gemiflow@alpha config set verification.threshold 0.98
```
...
verify:
  stage: test
  script:
    - npx gemiflow@alpha verify check --threshold 0.95 --json > verification.json
    - |
      score=$(jq '.overallScore' verification.json)
...
npx gemiflow@alpha swarm --verify --threshold 0.98

# Hive Mind with auto-rollback
npx gemiflow@alpha hive-mind --verify --rollback-on-fail

# Training pipeline with verification
npx gemiflow@alpha train --verify --threshold 0.99
```
...
npx gemiflow@alpha pair --verify --real-time

# Pair with custom threshold
npx gemiflow@alpha pair --verify --threshold 0.97 --auto-fix
```
...
npx gemiflow@alpha verify watch --directory src/

# Watch with auto-fix
npx gemiflow@alpha verify watch --directory src/ --auto-fix

# Watch with notifications
npx gemiflow@alpha verify watch --notify --threshold 0.95
```
...
npx gemiflow@alpha truth --format json | \
  curl -X POST https://pushgateway.example.com/metrics/job/gemiflow4gemini \
  -d @-
...
npx gemiflow@alpha verify install-hook --pre-commit

# .git/hooks/pre-commit example:
#!/bin/bash
npx gemiflow@alpha verify check --threshold 0.95 --json > /tmp/verify.json

score=$(jq '.overallScore' /tmp/verify.json)
if (( $(echo "$score < 0.95" | bc -l) )); then
  echo "❌ Verification failed with score: $score"
  echo "Run 'npx gemiflow@alpha verify check --verbose' for details"
  exit 1
fi
...
npx gemiflow@alpha truth --verbose --threshold 0.0

# Check specific criteria
npx gemiflow@alpha verify check --verbose

# View agent-specific issues
npx gemiflow@alpha truth --agent <agent-name> --format json
```
...
npx gemiflow@alpha verify rollback --history
...
```bash
# Increase timeout
npx gemiflow@alpha verify check --timeout 60s

# Verify in batches
npx gemiflow@alpha verify batch --batch-size 10
```
...
- `npx gemiflow@alpha pair` - Collaborative development with verification
- `npx gemiflow@alpha train` - Training with verification feedback
- `npx gemiflow@alpha swarm` - Multi-agent coordination with quality checks
- `npx gemiflow@alpha report` - Generate comprehensive project reports
