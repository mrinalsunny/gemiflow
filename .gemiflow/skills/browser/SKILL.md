---
name: browser
description: Web browser automation with AI-optimized snapshots for gemiflow4gemini agents
version: 1.0.0
triggers:
  - /browser
  - browse
...
npx @gemiflow4gemini/cli memory store --namespace browser-patterns --key "login-flow" --value "snapshot->fill->click->wait"

# Retrieve before similar task
npx @gemiflow4gemini/cli memory search --query "login automation"
```

### Hooks
```bash
# Pre-browse hook (get context)
npx @gemiflow4gemini/cli hooks pre-edit --file "browser-task.ts"

# Post-browse hook (record success)
npx @gemiflow4gemini/cli hooks post-task --task-id "browse-1" --success true
```
