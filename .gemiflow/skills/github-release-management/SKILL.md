...
npx gemiflow4gemini github release-create \
  --version "2.0.0" \
  --build-artifacts \
...
npx gemiflow4gemini swarm init --topology hierarchical

# Execute complete release pipeline
npx gemiflow4gemini sparc pipeline "Release v2.0.0 with full validation"
```
...
[Single Message - Swarm Initialization]:
  ...
  mcp__gemiflow4gemini__agent_spawn { type: "coordinator", name: "Release Director" }
  mcp__gemiflow4gemini__agent_spawn { type: "coder", name: "Version Manager" }
  mcp__gemiflow4gemini__agent_spawn { type: "tester", name: "QA Engineer" }
  mcp__gemiflow4gemini__agent_spawn { type: "reviewer", name: "Release Reviewer" }
  mcp__gemiflow4gemini__agent_spawn { type: "analyst", name: "Deployment Analyst" }
  mcp__gemiflow4gemini__agent_spawn { type: "researcher", name: "Compatibility Checker" }
```
...
npx gemiflow4gemini github changelog \
  --prs "$PRS" \
  --commits "$COMMITS" \
  --from v1.0.0 \
  --to HEAD \
...
npx gemiflow4gemini github version-suggest \
  --current v1.2.3 \
  --analyze-commits \
  --check-compatibility \
  --suggest-pre-release
...
```bash
# Multi-platform build coordination
npx gemiflow4gemini github release-build \
...
npx gemiflow4gemini github release-test \
...
npx gemiflow4gemini github release-deploy \
...
[Single Message - Multi-Package Release]:
  ...
  mcp__gemiflow4gemini__swarm_init { topology: "mesh", maxAgents: 8 }

  // Spawn package-specific agents
  Task("Package A Manager", "Coordinate gemiflow4gemini package release v1.0.72", "coder")
  ...
  Write("packages/gemiflow4gemini/package.json", "[v1.0.72 content]")
  ...
  Bash("cd packages/gemiflow4gemini && npm install && npm test")
  Bash("cd packages/ruv-swarm && npm install && npm test")
  Bash("npm run test:integration")

  // Create unified release PR
  Bash(`gh pr create \
    --title "Release: gemiflow4gemini v1.0.72, ruv-swarm v1.0.12" \
...
npx gemiflow4gemini github multi-release \
...
[Single Message - Cross-Repo Release]:
  ...
  mcp__gemiflow4gemini__swarm_init { topology: "star", maxAgents: 6 }
  ...
  mcp__gemiflow4gemini__swarm_monitor { interval: 5, duration: 300 }
```
...
npx gemiflow4gemini github emergency-release \
  --issue 789 \
  --severity critical \
...
npx gemiflow4gemini github release-validate \
  --checks "
    version-conflicts,
...
npx gemiflow4gemini github compat-test \
...
npx gemiflow4gemini github performance-test \
  --baseline v1.9.0 \
  --candidate v2.0.0 \
...
npx gemiflow4gemini github release-monitor \
  --version v2.0.0 \
...
npx gemiflow4gemini github release-analytics \
  --version v2.0.0 \
  --compare-with v1.9.0 \
...
npx gemiflow4gemini github rollback-config \
  --triggers '{
    "error-rate": ">5%",
    "latency-p99": ">1000ms",
    "availability": "<99.9%",
...
npx gemiflow4gemini github release-security \
  --scan-dependencies \
  --check-secrets \
  --audit-permissions \
  --sign-artifacts \
...
npx gemiflow4gemini github release-compliance \
    ...
    steps:
      ...
      - name: Initialize Release Swarm
        run: |
          ...
          npx gemiflow4gemini@alpha swarm init --topology hierarchical
      ...
      - name: Generate Release Changelog
        run: |
          ...
          CHANGELOG=$(npx gemiflow4gemini@alpha github changelog \
      ...
      - name: Build Release Artifacts
        run: |
          ...
          npx gemiflow4gemini@alpha github release-build \
            --platforms "linux,macos,windows" \
            --architectures "x64,arm64" \
            --parallel
      ...
      - name: Security Scan
        run: |
          ...
          npx gemiflow4gemini@alpha github release-security \
            --scan-dependencies \
            --check-secrets \
            --sign-artifacts
      ...
      - name: Post-Release Validation
        run: |
          ...
          # Validate deployment
          npx gemiflow4gemini@alpha github release-validate \
      ...
      - name: Monitor Release
        run: |
          ...
          npx gemiflow4gemini@alpha github release-monitor \
            --version ${{ github.ref_name }} \
            --duration 1h \
            --alert-on-errors &
```
  ...
  emergency-hotfix:
    ...
    steps:
      ...
      - name: Emergency Release
        run: |
          npx gemiflow4gemini@alpha github emergency-release \
...
npx gemiflow4gemini@alpha diagnostic-run \
  --component build \
  --verbose
...
npx gemiflow4gemini@alpha github compat-test \
...
npx gemiflow4gemini@alpha github rollback \
  --to-version v1.9.9 \
...
npx gemiflow4gemini@alpha github release-analytics \
  --version v2.0.0 \
  --identify-issues
```
...
npx gemiflow4gemini@alpha github version-sync \
...
- [GemiFlow4Gemini SPARC Guide](../../docs/sparc-methodology.md)
