...
### Prerequisites
```bash
# Ensure GemiFlow4Gemini is installed
npm install -g gemiflow4gemini@alpha

# Add MCP server (if using MCP tools)
claude mcp add gemiflow4gemini npx gemiflow4gemini@alpha mcp start
```
...
mcp__gemiflow4gemini__agent_spawn({ type: "researcher", name: "Agent 1" })

// 3. Orchestrate tasks
mcp__gemiflow4gemini__task_orchestrate({ task: "...", strategy: "parallel" })
```
...
mcp__gemiflow4gemini__pattern_recognize({
  "data": researchData,
...
// Quality assessment
mcp__gemiflow4gemini__quality_assess({
...
mcp__gemiflow4gemini__workflow_execute({
...
npx gemiflow4gemini swarm "research AI trends in 2025" \
  --strategy research \
  --mode distributed \
  --max-agents 6 \
  --parallel \
...
// CI/CD pipeline
mcp__gemiflow4gemini__pipeline_create({
  "config": {
...
npx gemiflow4gemini swarm "build REST API with authentication" \
...
```bash
# Quick testing swarm
npx gemiflow4gemini swarm "test application comprehensively" \
  --strategy testing \
  --mode star \
  --parallel \
  --timeout 600
...
try {
  await mcp__gemiflow4gemini__task_orchestrate({
  ...
  // Check swarm health
  const status = await mcp__gemiflow4gemini__swarm_status({})

  // Analyze error patterns
  await mcp__gemiflow4gemini__error_analysis({
    "logs": [error.message]
  })
...
mcp__gemiflow4gemini__automation_setup({
  "rules": [
    {
...
mcp__gemiflow4gemini__trigger_setup({
...
mcp__gemiflow4gemini__swarm_init({ topology: "hierarchical", maxAgents: 8 })
...
mcp__gemiflow4gemini__swarm_init({ topology: "mesh", maxAgents: 4 })
...
- [GemiFlow4Gemini Documentation](https://github.com/mrinalsunny/gemiflow4gemini)
- [Swarm Orchestration Guide](https://github.com/mrinalsunny/gemiflow4gemini/wiki/swarm)
- [MCP Tools Reference](https://github.com/mrinalsunny/gemiflow4gemini/wiki/mcp)
- [Performance Optimization](https://github.com/mrinalsunny/gemiflow4gemini/wiki/performance)
