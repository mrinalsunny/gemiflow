...
```bash
# Check status
gemiflow4gemini pair --status

# View history
gemiflow4gemini pair --history
...
# End session
gemiflow4gemini pair --end [--save] [--report]
```
...
```bash
gemiflow4gemini pair --start --mode driver
```
...
```bash
gemiflow4gemini pair --start --mode navigator
```
...
gemiflow4gemini pair --start --mode switch

# 5-minute intervals (rapid)
gemiflow4gemini pair --start --mode switch --interval 5m

# 15-minute intervals (deep focus)
gemiflow4gemini pair --start --mode switch --interval 15m
```
...
#### Basic Configuration
Create `.gemiflow4gemini/pair-config.json`:
...
```bash
# Set configuration
gemiflow4gemini pair config set defaultMode switch
gemiflow4gemini pair config set verification.threshold 0.98

# Get configuration
gemiflow4gemini pair config get
gemiflow4gemini pair config get defaultMode

# Export/Import
gemiflow4gemini pair config export > config.json
gemiflow4gemini pair config import config.json

# Reset
gemiflow4gemini pair config reset
```
...
```bash
# Create profile
gemiflow4gemini pair profile create refactoring \
  --mode driver \
  --verify true \
  --threshold 0.98 \
  --focus refactor
...
# Use profile
gemiflow4gemini pair --start --profile refactoring

# List profiles
gemiflow4gemini pair profile list
```
...
gemiflow4gemini pair --template refactor
...
# Feature template
gemiflow4gemini pair --template feature
...
# Debug template
gemiflow4gemini pair --template debug
...
# Learning template
gemiflow4gemini pair --template learn
# Mode: Mentor
# Pace: Slow
# Explanations: Detailed
# Examples: Many
...
```bash
# Save session
gemiflow4gemini pair --save [--name <name>]

# Load session
gemiflow4gemini pair --load <session-id>

# Export session
gemiflow4gemini pair --export <session-id> [--format json|md]

# Generate report
gemiflow4gemini pair --report <session-id>
```
...
```bash
# Start in background
gemiflow4gemini pair --start --background

# Monitor background session
gemiflow4gemini pair --monitor

# Attach to background session
gemiflow4gemini pair --attach <session-id>

# End background session
gemiflow4gemini pair --end <session-id>
```
...
gemiflow4gemini pair --start --record

# Replay session
gemiflow4gemini pair --replay <session-id>

# Session analytics
gemiflow4gemini pair --analytics <session-id>
```
...
**With Git:**
```bash
gemiflow4gemini pair --start --git --auto-commit
```

**With CI/CD:**
```bash
gemiflow4gemini pair --start --ci --non-interactive
```

**With IDE:**
```bash
gemiflow4gemini pair --start --ide vscode
```
...
- Run `gemiflow4gemini pair config validate`
...
- `gemiflow4gemini pair --help` - Show help
- `gemiflow4gemini pair config` - Manage configuration
- `gemiflow4gemini pair profile` - Manage profiles
- `gemiflow4gemini pair templates` - List templates
- `gemiflow4gemini pair agents` - List available agents
