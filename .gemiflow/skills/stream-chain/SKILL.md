...
```bash
gemiflow4gemini stream-chain run \
...
```bash
gemiflow4gemini stream-chain pipeline analysis
```
...
```bash
gemiflow4gemini stream-chain run <prompt1> <prompt2> [...] [options]
```
...
```bash
gemiflow4gemini stream-chain pipeline <type> [options]
```
...
```bash
gemiflow4gemini stream-chain pipeline refactor
```
...
```bash
gemiflow4gemini stream-chain pipeline test
```
...
```bash
gemiflow4gemini stream-chain pipeline optimize
```
...
```bash
gemiflow4gemini stream-chain pipeline refactor --timeout 60 --verbose
```

#### Debug Test Generation

```bash
gemiflow4gemini stream-chain pipeline test --debug
```

#### Comprehensive Optimization

```bash
gemiflow4gemini stream-chain pipeline optimize --timeout 90 --verbose
```
...
Define reusable pipelines in `.gemiflow4gemini/config.json`:
...
```bash
gemiflow4gemini stream-chain pipeline security
gemiflow4gemini stream-chain pipeline documentation
```
...
## Integration with GemiFlow4Gemini

### Combine with Swarm Coordination

```bash
# Initialize swarm for coordination
gemiflow4gemini swarm init --topology mesh
...
# Results stored in .gemiflow4gemini/memory/stream-chain/
```
...
gemiflow4gemini stream-chain pipeline optimize --debug
...
```bash
gemiflow4gemini stream-chain run "complex task" --timeout 120
```
...
```bash
gemiflow4gemini stream-chain run "step 1" "step 2" --debug
```
...
cat .gemiflow4gemini/config.json | grep -A 10 "streamChain"
```
