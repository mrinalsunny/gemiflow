...
The Hive Mind system represents the pinnacle of multi-agent coordination in GemiFlow4Gemini, implementing a queen-led hierarchical architecture where a strategic queen coordinator directs specialized worker agents through collective decision-making and shared memory.
...
npx gemiflow4gemini hive-mind init

# Force reinitialize
npx gemiflow4gemini hive-mind init --force

# Custom configuration
npx gemiflow4gemini hive-mind init --config hive-config.json
```
...
npx gemiflow4gemini hive-mind spawn "Build microservices architecture"

# Strategic queen type
npx gemiflow4gemini hive-mind spawn "Research AI patterns" --queen-type strategic

# Tactical queen with max workers
npx gemiflow4gemini hive-mind spawn "Implement API" --queen-type tactical --max-workers 12

# Adaptive queen with consensus
npx gemiflow4gemini hive-mind spawn "Optimize system" --queen-type adaptive --consensus byzantine

# Generate Gemini CLI commands
npx gemiflow4gemini hive-mind spawn "Build full-stack app" --claude
```
...
npx gemiflow4gemini hive-mind status

# Get detailed metrics
npx gemiflow4gemini hive-mind metrics

# Monitor collective memory
npx gemiflow4gemini hive-mind memory
```
...
npx gemiflow4gemini hive-mind sessions

# Pause a session
npx gemiflow4gemini hive-mind pause <session-id>

# Resume a paused session
npx gemiflow4gemini hive-mind resume <session-id>

# Stop a running session
npx gemiflow4gemini hive-mind stop <session-id>
```
...
```bash
npx gemiflow4gemini hive-mind spawn "Build REST API" --claude
```
...
npx gemiflow4gemini sparc tdd "User authentication" --hive-mind
...
npx gemiflow4gemini hive-mind spawn "Analyze repo quality" --objective "owner/repo"

# PR review coordination
npx gemiflow4gemini hive-mind spawn "Review PR #123" --queen-type tactical
```
...
Hive Mind integrates with GemiFlow4Gemini hooks for automation:
...
```bash
npx gemiflow4gemini hive-mind spawn "Research ML frameworks" --queen-type strategic
```

**Tactical Queens** - For implementation and execution
```bash
npx gemiflow4gemini hive-mind spawn "Build authentication" --queen-type tactical
```

**Adaptive Queens** - For optimization and dynamic tasks
```bash
npx gemiflow4gemini hive-mind spawn "Optimize performance" --queen-type adaptive
```
...
npx gemiflow4gemini hive-mind memory --gc

# Optimize database
npx gemiflow4gemini hive-mind memory --optimize

# Export and clear
npx gemiflow4gemini hive-mind memory --export --clear
```
...
npx gemiflow4gemini hive-mind spawn "..." --consensus weighted

# Or use simple majority
npx gemiflow4gemini hive-mind spawn "..." --consensus majority
```
...
Define specialized workers in `.gemiflow4gemini/agents/`:
...
```bash
# Frontend hive
npx gemiflow4gemini hive-mind spawn "Build UI" --name frontend-hive

# Backend hive
npx gemiflow4gemini hive-mind spawn "Build API" --name backend-hive
...
npx gemiflow4gemini hive-mind export <session-id> --output backup.json

# Import session
npx gemiflow4gemini hive-mind import backup.json
```
...
npx gemiflow4gemini hive-mind spawn "Build e-commerce platform" \
...
```bash
# Spawn research hive
npx gemiflow4gemini hive-mind spawn "Research GraphQL vs REST" \
  --queen-type adaptive \
  --consensus byzantine
...
```bash
# Review coordination
npx gemiflow4gemini hive-mind spawn "Review PR #456" \
  --queen-type tactical \
  --max-workers 6
...
**Maintained By**: GemiFlow4Gemini Team
**License**: MIT
