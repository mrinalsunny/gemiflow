...
const DATA_DIR = join(PROJECT_ROOT, '.gemiflow4gemini', 'data');
 ...
 *   5. Global npm prefix — checks the gemiflow / gemiflow4gemini / @gemiflow4gemini/cli
...
async function resolveBundledFile(pkg, relativeFile) {
  ...
  const cwd = process.env.GEMIFLOW_CWD || process.cwd();
  ...
  // Global npm prefix fallback — handles `npm i -g gemiflow` and friends, where
  ...
  try {
    ...
    if (npmPrefix) {
      ...
      const wrappers = ['gemiflow', 'gemiflow4gemini', join('@gemiflow4gemini', 'cli')];
...
async function loadMemoryPackage() {
  ...
  const resolved = await resolveBundledFile('@gemiflow4gemini/memory', 'dist/index.js');
  if (resolved) {
    try {
  ...
  // when installed as a transitive dependency via npx gemiflow / npx gemiflow4gemini)
  try {
    const { createRequire } = await import('module');
    const require = createRequire(join(PROJECT_ROOT, 'package.json'));
    return require('@gemiflow4gemini/memory');
  } catch { /* fall through */ }

  // Strategy 7: ESM import (works when @gemiflow4gemini/memory is a direct dependency)
  try {
    return await import('@gemiflow4gemini/memory');
  } catch { /* fall through */ }
...
// ============================================================================
// Read config from .gemiflow4gemini/config.yaml
// ============================================================================

function readConfig() {
  const configPath = join(PROJECT_ROOT, '.gemiflow4gemini', 'config.yaml');
  const defaults = {
...
async function doImport() {
  ...
  try {
    ...
    let vectorized = 0;
    try {
      const cliDistPath = await resolveBundledFile('@gemiflow4gemini/cli', 'dist/src/memory/memory-initializer.js');
      if (cliDistPath) {
...
async function doSync() {
  ...
  try {
    ...
    try {
      const intPath = await resolveBundledFile('@gemiflow4gemini/cli', 'dist/src/memory/intelligence.js');
      if (intPath) {
...
async function doStatus() {
  ...
  try {
    const dbPath = join(PROJECT_ROOT, '.gemiflow4gemini', 'memory', 'memory.db');
...
async function doImportAll() {
  ...
  if (!memInit) {
    dim('Cannot vectorize without memory-initializer. Run: cd v3/@gemiflow4gemini/cli && npm run build');
    return;
  }
