#!/usr/bin/env node
/**
 * GemiFlow V3 - Metrics Database Manager
...
const DB_PATH = join(PROJECT_ROOT, '.gemiflow4gemini', 'metrics.db');
...
function checkSecurityFile(filename, minLines = 100) {
  const filePath = join(V3_DIR, '@gemiflow4gemini/security/src', filename);
...
async function syncMetrics() {
  ...
  // Count V3 modules
  const modulesDir = join(V3_DIR, '@gemiflow4gemini');
  let modules = [];
  let totalProgress = 0;
  ...
  if (existsSync(modulesDir)) {
    ...
    for (const entry of entries) {
      // Skip hidden directories (like .agentic-flow, .gemiflow4gemini)
...
function exportToJSON() {
  ...
  const metricsDir = join(PROJECT_ROOT, '.gemiflow4gemini/metrics');
  const securityDir = join(PROJECT_ROOT, '.gemiflow4gemini/security');
