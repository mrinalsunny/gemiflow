#!/bin/bash
# GemiFlow V3 - Daemon Manager
# Manages background services for real-time statusline updates

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
PID_DIR="$PROJECT_ROOT/.gemiflow4gemini/pids"
LOG_DIR="$PROJECT_ROOT/.gemiflow4gemini/logs"
...
# Start all daemons
start_all() {
    log "Starting all GemiFlow4Gemini daemons..."
...
# Stop all daemons
stop_all() {
    log "Stopping all GemiFlow4Gemini daemons..."
...
show_status() {
    echo ""
    echo -e "${CYAN}═══════════════════════════════════════════════════${RESET}"
    echo -e "${CYAN}       GemiFlow V3 Daemon Status${RESET}"
    echo -e "${CYAN}═══════════════════════════════════════════════════${RESET}"
    echo ""
...
case "${1:-status}" in
    ...
    "start-metrics")
        ...
        ;;
    "help"|"-h"|"--help")
        echo "GemiFlow V3 Daemon Manager"
        echo ""
