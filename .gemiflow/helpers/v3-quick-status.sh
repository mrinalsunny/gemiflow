...
echo -e "${PURPLE}⚡ GemiFlow V3 Quick Status${RESET}"
...
if [ -f ".gemiflow4gemini/metrics/v3-progress.json" ]; then
  DOMAINS=$(jq -r '.domains.completed // 0' ".gemiflow4gemini/metrics/v3-progress.json" 2>/dev/null || echo "0")
  AGENTS=$(jq -r '.swarm.activeAgents // 0' ".gemiflow4gemini/metrics/v3-progress.json" 2>/dev/null || echo "0")
  DDD_PROGRESS=$(jq -r '.ddd.progress // 0' ".gemiflow4gemini/metrics/v3-progress.json" 2>/dev/null || echo "0")
fi

if [ -f ".gemiflow4gemini/security/audit-status.json" ]; then
  CVES_FIXED=$(jq -r '.cvesFixed // 0' ".gemiflow4gemini/security/audit-status.json" 2>/dev/null || echo "0")
fi

if [ -f ".gemiflow4gemini/metrics/performance.json" ]; then
  SPEEDUP=$(jq -r '.flashAttention.speedup // "1.0x"' ".gemiflow4gemini/metrics/performance.json" 2>/dev/null || echo "1.0x")
  MEMORY=$(jq -r '.memory.reduction // "0%"' ".gemiflow4gemini/metrics/performance.json" 2>/dev/null || echo "0%")
fi
