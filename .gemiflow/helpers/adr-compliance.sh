#!/bin/bash
# GemiFlow V3 - ADR Compliance Checker Worker
...
check_adr_005() {
  ...
  [ -d "$PROJECT_ROOT/v3/@gemiflow4gemini/mcp" ] && score=$((score + 40))
  ...
  grep -rq "schema\|jsonSchema" "$PROJECT_ROOT/v3/@gemiflow4gemini/mcp" 2>/dev/null && score=$((score + 30))
