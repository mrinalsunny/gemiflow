 ...
 *   3. AgentDB from @gemiflow4gemini/memory (HNSW vector search)
...
const RESTORE_BUDGET = parseInt(process.env.GEMIFLOW_COMPACT_RESTORE_BUDGET || '4000', 10);
const MAX_MESSAGES = 500;
const BLOCK_COMPACTION = process.env.GEMIFLOW_BLOCK_COMPACTION === 'true';
const COMPACT_INSTRUCTION_BUDGET = parseInt(process.env.GEMIFLOW_COMPACT_INSTRUCTION_BUDGET || '2000', 10);
const RETENTION_DAYS = parseInt(process.env.GEMIFLOW_RETENTION_DAYS || '30', 10);
const AUTO_OPTIMIZE = process.env.GEMIFLOW_AUTO_OPTIMIZE !== 'false'; // on by default

// ============================================================================
// Context Autopilot — prevent compaction by managing context size in real-time
// ============================================================================
const AUTOPILOT_ENABLED = process.env.GEMIFLOW_CONTEXT_AUTOPILOT !== 'false'; // on by default
const CONTEXT_WINDOW_TOKENS = parseInt(process.env.GEMIFLOW_CONTEXT_WINDOW || '200000', 10);
const AUTOPILOT_WARN_PCT = parseFloat(process.env.GEMIFLOW_AUTOPILOT_WARN || '0.70');
const AUTOPILOT_PRUNE_PCT = parseFloat(process.env.GEMIFLOW_AUTOPILOT_PRUNE || '0.85');
...
class RuVectorBackend {
  ...
  async initialize() {
    ...
    this.pool = new Pool({
      ...
      application_name: 'gemiflow4gemini-context-persistence',
    });
...
async function resolveBackend() {
  ...
  // Tier 3: AgentDB from @gemiflow4gemini/memory (HNSW)
  try {
    const localDist = join(PROJECT_ROOT, 'v3/@gemiflow4gemini/memory/dist/index.js');
    let memPkg = null;
    if (existsSync(localDist)) {
      memPkg = await import(`file://${localDist}`);
    } else {
      memPkg = await import('@gemiflow4gemini/memory');
    }
