...
.gemiflow4gemini/helpers/v3.sh init

# Quick status check
.gemiflow4gemini/helpers/v3.sh status

# Update progress metrics
.gemiflow4gemini/helpers/v3.sh update domain 3
.gemiflow4gemini/helpers/v3.sh update agent 8
.gemiflow4gemini/helpers/v3.sh update security 2
```
...
- **`v3.sh`** - Main command-line interface for all V3 operations
  ```bash
  .gemiflow4gemini/helpers/v3.sh help           # Show all commands
  .gemiflow4gemini/helpers/v3.sh status         # Quick development status
  .gemiflow4gemini/helpers/v3.sh update domain 3 # Update specific metrics
  .gemiflow4gemini/helpers/v3.sh validate       # Validate configuration
  .gemiflow4gemini/helpers/v3.sh full-status    # Complete status overview
  ```
...
- **`update-v3-progress.sh`** - Update V3 development metrics
  ```bash
  # Usage examples:
  .gemiflow4gemini/helpers/update-v3-progress.sh domain 3      # Mark 3 domains complete
  .gemiflow4gemini/helpers/update-v3-progress.sh agent 8       # 8 agents active
  .gemiflow4gemini/helpers/update-v3-progress.sh security 2    # 2 CVEs fixed
  .gemiflow4gemini/helpers/update-v3-progress.sh performance 2.5x # Performance boost
  .gemiflow4gemini/helpers/update-v3-progress.sh status        # Show current status
  ```
...
Helpers are configured in `.gemiflow4gemini/settings.json`:
```json
{
  "helpers": {
    "directory": ".gemiflow4gemini/helpers",
    "enabled": true,
    "v3ProgressUpdater": ".gemiflow4gemini/helpers/update-v3-progress.sh"
  }
}
```
...
1. Create script in `.gemiflow4gemini/helpers/`
