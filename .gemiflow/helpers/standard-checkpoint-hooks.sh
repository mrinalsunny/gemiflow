...
pre_edit_checkpoint() {
    ...
    if [ -n "$file" ]; then
        ...
        # Store metadata
        mkdir -p .gemiflow4gemini/checkpoints
        cat > ".gemiflow4gemini/checkpoints/$(date +%s).json" <<EOF
{
...
task_checkpoint() {
    ...
    if [ -n "$task" ]; then
        ...
        # Store metadata
        mkdir -p .gemiflow4gemini/checkpoints
        cat > ".gemiflow4gemini/checkpoints/task-$(date +%s).json" <<EOF
{
...
session_end_checkpoint() {
    ...
    local summary_file=".gemiflow4gemini/checkpoints/summary-$session_id.md"
...
## Checkpoints Created
$(find .gemiflow4gemini/checkpoints -name '*.json' -mtime -1 -exec basename {} \; | sort)
