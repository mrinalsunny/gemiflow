...
gemiflow4gemini truth

# View scores for last 7 days
gemiflow4gemini truth --period 7d

# Export to HTML report
gemiflow4gemini truth --export report.html --format html
```

### Advanced Analysis
```bash
# Monitor real-time scores
gemiflow4gemini truth --watch

# Find problematic files
gemiflow4gemini truth --threshold 0.8

# Agent-specific metrics
gemiflow4gemini truth --agent coder --period 24h

# JSON for processing
gemiflow4gemini truth --format json | jq '.overall_score'
```
...
- name: Check Truth Scores
  run: |
    gemiflow4gemini truth --format json > truth.json
...
gemiflow4gemini truth --format json | \
...
Set truth display preferences in `.gemiflow4gemini/config.json`:
