...
```bash
npx gemiflow4gemini github swarm [options]
```
...
```bash
npx gemiflow4gemini github swarm --repository owner/repo
```

### Maintenance-focused swarm

```bash
npx gemiflow4gemini github swarm -r owner/repo -f maintenance --issue-labels
```

### Development swarm with PR automation

```bash
npx gemiflow4gemini github swarm -r owner/repo -f development --auto-pr --code-review
```

### Full-featured triage swarm

```bash
npx gemiflow4gemini github swarm -r owner/repo -a 8 -f triage --issue-labels --auto-pr
```
