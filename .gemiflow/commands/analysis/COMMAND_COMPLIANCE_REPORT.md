...
## Overview
Reviewed all command files in `.gemiflow4gemini/commands/analysis/` directory to ensure proper usage of:
- `mcp__gemiflow4gemini__*` tools (preferred)
- `npx gemiflow4gemini` commands (as fallback)
...
- Updated to: `Tool: mcp__gemiflow4gemini__token_usage` with appropriate parameters
...
**After**:
```
Tool: mcp__gemiflow4gemini__token_usage
Parameters: {"operation": "session", "timeframe": "24h"}
```

### 2. performance-bottlenecks.md
**Status**: ✅ Compliant (No changes needed)
**Reason**: Already uses proper `mcp__gemiflow4gemini__task_results` tool format
...
1. **MCP Tool Usage**: All direct tool calls now use `mcp__gemiflow4gemini__*` format
...
The analysis directory is now fully compliant with the GemiFlow4Gemini command standards.