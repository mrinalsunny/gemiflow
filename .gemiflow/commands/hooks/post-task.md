...
```bash
npx gemiflow4gemini hook post-task -t "api-refactor" --analyze-performance --generate-report
```

### Memory storage

```bash
npx gemiflow4gemini hook post-task -t "bug-fix-123" --store-decisions --export-learnings
```

### Quick cleanup

```bash
npx gemiflow4gemini hook post-task -t "minor-update" --analyze-performance false
```
...
npx gemiflow4gemini hook post-task --task-id "your-task-id" --analyze-performance true
```
