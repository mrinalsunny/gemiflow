...
```bash
npx gemiflow4gemini hook post-edit -f "api/auth.js" --memory-key "auth/login-implementation"
```

### Format and validate

```bash
npx gemiflow4gemini hook post-edit -f "config/webpack.js" --auto-format --validate-output
```

### Neural training

```bash
npx gemiflow4gemini hook post-edit -f "utils/helpers.ts" --train-patterns --memory-key "utils/refactor"
```
...
```bash
# After editing files
npx gemiflow4gemini hook post-edit --file "path/to/edited.js" --memory-key "feature/step1"
```
