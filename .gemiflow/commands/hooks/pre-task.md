...
```bash
npx gemiflow4gemini hook pre-task -d "Continue API development" --load-memory
```

### Manual agent control

```bash
npx gemiflow4gemini hook pre-task -d "Debug issue #123" --auto-spawn-agents false
```

### Full optimization

```bash
npx gemiflow4gemini hook pre-task -d "Refactor codebase" --optimize-topology --estimate-complexity
```
...
npx gemiflow4gemini hook pre-task --description "Your task here"
```
