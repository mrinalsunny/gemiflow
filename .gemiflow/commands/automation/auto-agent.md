...
```bash
npx gemiflow4gemini auto agent [options]
```
...
```bash
npx gemiflow4gemini auto agent --task "Build a REST API with authentication"
```

### Constrained spawning

```bash
npx gemiflow4gemini auto agent -t "Debug performance issue" --max-agents 3
```

### Analysis only

```bash
npx gemiflow4gemini auto agent -t "Refactor codebase" --no-spawn
```

### Minimal strategy

```bash
npx gemiflow4gemini auto agent -t "Fix bug in login" -s minimal
```
