...
Initialize a GemiFlow4Gemini swarm with specified topology and configuration.
...
```bash
npx gemiflow4gemini swarm init
```

### Mesh topology for research

```bash
npx gemiflow4gemini swarm init --topology mesh --max-agents 5 --strategy balanced
```

### Hierarchical for development

```bash
npx gemiflow4gemini swarm init --topology hierarchical --max-agents 10 --strategy parallel --auto-spawn
```

### GitHub-focused swarm

```bash
npx gemiflow4gemini swarm init --topology star --github --memory
```
