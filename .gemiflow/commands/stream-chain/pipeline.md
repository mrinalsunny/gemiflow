...
```bash
gemiflow4gemini stream-chain pipeline refactor --timeout 60
```

### Verbose Test Generation
```bash
gemiflow4gemini stream-chain pipeline test --verbose
```
...
Define custom pipelines in `.gemiflow4gemini/config.json`:
